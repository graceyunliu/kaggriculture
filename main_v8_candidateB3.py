...
# =====================================================================
# MAIN V8 CANDIDATE B -- demand-headroom crop reweight, ISOLATED. Base:
# v7.9, byte-identical except for two `want[...]` seed-target constants
# in economy(). NOT stacked on Candidate A (main_v8_candidateA.py): that
# fertilize-pipeline experiment was empirically tested (base + 2
# throttling iterations) and lost decisively every time (v1 -$50,711
# mean/11 seeds unthrottled, v2 -$32,137 with 1-hand/turn + protected-
# hand exclusion, v3 -$23,504 with an added "only when farm is not
# already behind on watering/weeds" gate -- shrinking each round but
# never close to break-even). Since it never worked, stacking further
# changes on top of it would confound any result, so this candidate
# builds directly on v7.9 instead, exactly like the project's established
# one-variable-at-a-time discipline (see e.g. v7.6a vs v7.6b).
#
# --- The change (two constants, want[] targets in economy()) ---
#
#  want["WHEAT"]:      8  -> min(space, 25)   (was a flat, phase-blind 8
#                       against a 635-unit town demand pool -- the
#                       largest pool in the game, and WHEAT is also the
#                       animal feed input, so growing more organically
#                       cuts BUY_PRODUCT feed spend too. WHEAT is last in
#                       PLANT_PRIORITY so the extra seed stock only fills
#                       tiles STRAWBERRY/MELON/TOMATO don't claim first --
#                       this targets the 36-50/100 idle tiles the
#                       project's own replay analysis already measured
#                       sitting empty every day from day 10-29.)
#  want["STRAWBERRY"]: min(space,6/10) -> min(space,8/14)  (v7.9 already
#                       raised this from a flat 3; still leaves headroom
#                       against the 534-unit pool and the project's own
#                       measured real-opponent volume of 200+/match.)
#
# MELON, TOMATO, CARROT targets are untouched -- this stays a two-crop,
# two-constant demand-headroom change, not a general seed-budget
# increase, so a result can be attributed to it cleanly.
# =====================================================================
#
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        # Candidate B: raised further from v7.9's 6/10 -- still well
        # under the 534-unit town demand pool headroom.
    ...
    if day <= CUTOFF["CARROT"]:
        want["CARROT"] = 4
    if day <= CUTOFF["WHEAT"]:
        # Candidate B3 (tuned up from B's 25 after B2's tune-DOWN to 16
        # measured a decisive loss, -$30,757 mean/11 seeds, while B itself
        # was a near-wash, +$5,384/11 seeds not significant -- since
        # `want[crop] = min(space, X)` is usually bound by real empty-tile
        # count (36-50 idle tiles measured most days), a LOWER X directly
        # means more tiles left empty when space > X, which is pure
        # forgone revenue against a 635-unit pool. B3 tests pushing X
        # higher still, since B's 25 was often still below real idle
        # tile count.
