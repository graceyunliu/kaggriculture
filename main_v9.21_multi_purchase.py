...
# =====================================================================
# MAIN V9.21 -- multi-order animal purchases (fill all MAX_CONCURRENT_
# BUILDS build slots in ONE turn instead of one new order per turn), on
# top of v9.12 (melon fertilize-in-window on v9.10's hire fix).
#
...
#
# RESULT -- TESTED, REJECTED (real regression, not just no-gain). 5-seed
# smoke, both-seats: vs v9.5, mean -$8,706 both-seats-sum (-$4,353/game),
# t=-1.66, 95% CI [-$19,005,+$1,592] -- crosses zero narrowly but a
# consistent 1/5 loss record. Vs opp_scenario_v14: mean -$98,203
# both-seats-sum (-$49,102/game), t=-13.21 -- WORSE than v9.10/v9.12's
# own -$46,860/game baseline (a small but consistent -$2,242/game
# regression, not an improvement). Single-seed census (seed 1) explains
# why: buying 3 animals up front in one turn only bought ONE extra
# animal's worth of head start (3 vs the baseline's 2 on day1) before
# hitting the SAME plateau, and the fleet then got permanently stuck at
# 6 animals for the rest of the match (day13 onward), worse than
# baseline's eventual 10. Hypothesis: the real per-animal bottleneck
# isn't order-submission rate, it's the multi-turn WALK-to-shed / pick-
# up / walk-to-site / PLACE cycle time each pre-pinned worker slot
# takes to complete -- compressing 3 orders into turn 1 doesn't shorten
# that cycle at all (same MAX_CONCURRENT_BUILDS=3 ceiling, same walk
# distances), it just front-loads 3x the cash outlay into one turn,
# which apparently starved wheat/seed purchases in that exact window
# and compounded into a worse crop economy overall. The clone avoids
# this because it doesn't pre-pin a worker to an animal AT PURCHASE
# TIME at all -- animals bought into the shed are claimed by whichever
# worker is free THAT TURN via ordinary job-scoring dispatch, so
# multiple purchases don't queue behind the same 3 walk-cycles. A real
# fix would need to decouple placement from purchase-time worker
# pinning (route "carry a shed animal to its site" through assign_
# tasks()'s existing joint dispatch instead of animal_plans' per-slot
# state machine) -- a genuinely large rewrite of the mature animal-
# acquisition pipeline, bigger than anything attempted this session,
# NOT attempted here. The diagnostic finding itself (opponent's bulk-
# buy + decoupled-placement architecture as the real tempo driver) is
# real and well-evidenced (see fleet_census.py, kept in project root);
# only THIS PARTICULAR fix for it failed. NOT submitted.
# =====================================================================
