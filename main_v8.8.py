...
# =====================================================================
# MAIN V8.8 -- crop-share controller for planting, ISOLATED. Base:
# main_v8.3.py (the validated champion -- NOT main_v8.4.py/v8.5.py's
# rejected sheep-gate variants or main_v8.6.py's GOOSE addition or
# main_v8.7.py's OPTIMAL task assignment). Byte-identical except for
# HOW a "plant" task tile picks its crop.
#
# --- Why (external analysis, Aug 5, cross-checked against this file) ---
#
# v8.3's planting logic is "first available crop in PLANT_PRIORITY
# order": plantable_crops() returns
# ["STRAWBERRY","MELON","TOMATO","WHEAT","CARROT"] filtered by seed
# stock and cutoff day, and both unit_action() and assign_tasks() always
# take that list's FIRST entry. Verified by reading the code (not taking
# the external analysis's word for it): PLANT_PRIORITY at line 523,
# consumed at lines 1319-1322 and 1423-1427 pre-edit. Because
# want["STRAWBERRY"] (8/14 seed target) is bought before MELON/WHEAT
# seeds run low, this mechanically concentrates the farm at ~75-76%
# strawberry -- a side effect of list order and seed-buy sizing, not a
# deliberate allocation decision. An external ChatGPT/GLM analysis
# (compared against a hypothesized top-ladder mix of ~61% strawberry /
# 20% melon / 20% wheat) proposed replacing "first available" with "plant
# whichever plantable crop is currently furthest below its target land
# share" -- a pure planting-order change, no new engine actions,
# structures, or pathfinding, which keeps this a clean single-variable
# test against v8.3.
#
# --- The change ---
#
# TARGET_CROP_SHARE (below) sets target shares of standing PLANT tiles:
# STRAWBERRY 60%, MELON 15%, WHEAT 15%, TOMATO 5%, CARROT 5% (Grace's
# split -- tomato/carrot get a small explicit floor instead of being
# excluded, since v8.3 already treats them as small opportunistic
# crops via flat seed targets of 3/4). _crop_tile_counts() counts
# current standing PLANT tiles by crop from me["tiles"] once per turn;
# _choose_crop_to_plant() picks, among this turn's still-plantable
# crops, whichever has the largest (target_share - current_share)
# deficit, mutating a running crop_state so several plants in the same
# turn keep spreading instead of all picking the same furthest-behind
# crop. Wired into BOTH unit_action() and assign_tasks() (the live path,
# GLOBAL_TASK_ASSIGNMENT=True) so the two dispatch paths can't plant
# differently. Nothing else changes: seed-buy targets (want[...]),
# CUTOFF, PLANT_PRIORITY itself (kept only for plantable_crops()'s
# seed/cutoff filter), and every animal/task-order/priority-score system
# are untouched.
#
# --- What this deliberately does NOT do ---
#
# No wheat feed-floor tied to fleet size (ChatGPT's optional refinement)
# -- kept out of this first challenger so a win/loss is attributable to
# the share controller alone, not a second simultaneous change. No
# fertilizer-application action, no GOOSE, no animal-gate changes --
# separate, already-tried-or-planned variants (v8.4-v8.7).
#
...
# v8.8: target shares of standing PLANT tiles, replacing PLANT_PRIORITY's
# "first available" pick for which crop actually gets planted (seed-buy
# targets in economy()'s want[...] block are untouched -- this only
# governs which of the currently-plantable crops a given empty tile
# gets, once seeds for more than one crop are already in hand).
TARGET_CROP_SHARE = {
    "STRAWBERRY": 0.60,
    "MELON": 0.15,
    "WHEAT": 0.15,
...
def _crop_tile_counts(me):
    """Current standing PLANT-tile counts by crop, over the whole farm --
    the denominator TARGET_CROP_SHARE is measured against. Counts every
    owned PLANT tile regardless of age/growth stage, matching how
    perceive() identifies PLANT tiles elsewhere in this file."""
    counts = {crop: 0 for crop in TARGET_CROP_SHARE}
    total = 0
    for row in me["tiles"]:
        for t in row:
...
def _choose_crop_to_plant(plantable, crop_state):
    """v8.8: pick whichever currently-plantable crop is furthest below its
    TARGET_CROP_SHARE, instead of always taking plantable_crops()'s first
    (PLANT_PRIORITY-ordered) entry. crop_state is {"counts": {...},
    "total": n}, mutated in place on every call so a run of several
    plant actions within the same turn keeps spreading across crops
    instead of every unit independently picking the same single
    furthest-behind one. Crops outside TARGET_CROP_SHARE (none currently)
    are ignored for ranking but still fall back to plantable[0] if
    nothing in TARGET_CROP_SHARE is plantable this turn."""
    if not plantable:
        return None
    counts, total = crop_state["counts"], crop_state["total"]
    best_crop, best_deficit = None, float("-inf")
    for crop in plantable:
        if crop not in TARGET_CROP_SHARE:
            continue
        current_share = (counts.get(crop, 0) / total) if total > 0 else 0.0
        deficit = TARGET_CROP_SHARE[crop] - current_share
        if deficit > best_deficit:
            best_deficit = deficit
            best_crop = crop
    if best_crop is None:
        best_crop = plantable[0]
    crop_state["counts"][best_crop] = crop_state["counts"].get(best_crop, 0) + 1
    crop_state["total"] = total + 1
    return best_crop
...
def unit_action(pos, tasks, seeds, day, task_order, crop_state):
    ...
    Kept byte-identical (aside from v8.8's crop-choice swap below) and
    still used when GLOBAL_TASK_ASSIGNMENT=False, so that flag-off path
    is a true no-op vs main_v7.11.py otherwise. See assign_tasks() below
    for the v8 replacement."""
    x, y = pos
    for key in task_order:
        if (x, y) in tasks[key]:
            tasks[key].remove((x, y))
            if key == "plant":
                crop = _choose_crop_to_plant(plantable_crops(seeds, day), crop_state)
                if crop:
                    seeds[crop] -= 1
...
def assign_tasks(unit_positions, tasks, seeds, day, task_order, crop_state):
...
def _agent(obs):
    ...
    # v8.8: current standing-crop mix, recomputed once per turn -- the
    # shared, mutated-in-place state _choose_crop_to_plant() ranks
    # against for every plant action this turn (farmer, hands, and the
    # joint assign_tasks() pass all draw from the same crop_state so a
    # turn with several plants keeps spreading, not repeating the same
    # single furthest-behind crop).
    _counts, _total = _crop_tile_counts(me)
    crop_state = {"counts": _counts, "total": _total}
    ...
    if farmer_op is None:
        if GLOBAL_TASK_ASSIGNMENT:
            ...
            farmer_op = unit_action(farmer_pos, tasks, seeds, day, task_order, crop_state)
    ...
    for i, h in enumerate(me["hands"]):
        ...
        if op is not None:
            ...
            if GLOBAL_TASK_ASSIGNMENT:
                ...
                hand_ops[i] = unit_action(hand_pos, tasks, seeds, day, task_order, crop_state)
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        ...
        crop_ops = assign_tasks(crop_unit_positions, tasks, seeds, day, task_order, crop_state)
        k = 0
