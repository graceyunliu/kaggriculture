...
# =====================================================================
# MAIN V9.6 -- retest EXPERIMENT_MAX_FLEET past its v9.1-iter-C ceiling
# (10 -> 14), on top of v9.5. Base: main_v9.5_statehygiene.py.
#
# Why retest a number that was already rejected: iter C's -- fleet 12+
# saturates the hand pool (weeds 22 vs 2, strawberry tiles die late) --
# was measured under the OLD v9.1 support system, BEFORE parallel build
# (v9.2), fertilize cycle-compression (v9.3), and the v9.3fix/v9.5
# pipeline-correctness passes existed. Two of those are directly
# relevant to the failure mode iter C hit:
#   - Fertilize compression roughly doubles yield-per-tile on ongoing
#     crops, so the same strawberry acreage needs less replanting labor
#     to produce the same revenue -- the crop-labor budget iter C was
#     competing against animal-service labor for is not the same size
#     it was when iter C ran.
#   - Parallel build (MAX_CONCURRENT_BUILDS=3) plus the v9.3fix/v9.5
#     confirmation-and-reset-hygiene passes removed several sources of
#     wasted hand-turns (phantom BOUGHT slots, stale claimed sites,
#     double-claims) that would have eaten into the same labor margin
#     iter C found itself short on.
#
# Live-traced (fleet_trace2.py, seed 1 vs Opponents/opp_scenario_v14.py)
# before this change: our fleet plateaus at 9 animals / 9-10 hands by
# day 12-13 and never approaches the old EXPERIMENT_MAX_FLEET=10 cap on
# its own -- the cap was already non-binding at seed 1's actual growth
# rate, so raising it costs nothing if the fleet still can't reach it.
# The opponent runs a hand FLOOR of 10 (not a target) from day ~7 and
# caps at MAX_HANDS=12, reaching 15 animals; our own hire formula
# (target = work/5, capped at HAND_TARGET_MAX=18) already scales
# hires with animal_work, so there is headroom for it to respond if
# the fleet is allowed to grow past 9-10 -- this is not expected to
# require a separate hand-formula change to test first.
#
# Isolated change, nothing else: EXPERIMENT_MAX_FLEET 10 -> 14 (matches
# the herd clone's observed final fleet size). If this reproduces
# iter C's failure mode (weed spike, late strawberry die-off, net
# money loss vs v9.5), the modernized support system doesn't change
# the verdict and the direction is closed again, more conclusively.
#
# RESULT -- TESTED, REJECTED, iter C's verdict reproduced on the
# modernized base. Gate vs v9.5, both-seats, 15 seeds (1, 7, 42, 99,
# 123, 202, 303, 555, 2024, 8080, 17, 2025, 3001, 3002, 3003):
# v9.5 wins 15/15, +$12,956/game, t=+4.26, 95% CI [+$6,993,+$18,919]
# -- entirely against this build.
#
# Diagnostic detail (fleet_trace2.py + an instrumented copy of
# _animal_expansion_feasible/_sheep_expansion_feasible logging each
# gate's pass/fail reason, not kept in this file): behavior is
# seed-dependent in an informative way.
#   - Seed 1 vs Opponents/opp_scenario_v14.py: byte-identical to v9.5.
#     Our fleet plateaus at 9 animals, never even reaching the OLD
#     cap of 10 -- EXPERIMENT_MAX_FLEET was not the binding constraint
#     here at all. From day 13 on, every purchase attempt fails the
#     HAND_CAPACITY check (projected_work ~111-121 > 5*HAND_TARGET_MAX
#     =90), driven almost entirely by ~40-55 unplanted "empty" tiles
#     + ~30-40 pending waters -- animal_work itself is a small ~9. And
#     the hire formula's own target (min(18, ceil(work/5))) should
#     already be requesting 18 hands once work exceeds 90, yet the
#     observed hand count plateaus at 9-10 -- hiring itself is
#     capital-constrained (cash on hand sits at $100-$5,000 through
#     days 13-20), too thin to fund the Fibonacci hire-cost ladder up
#     to the formula's own target. Fleet-scale and the previously-
#     flagged day-8/day-10 cash-flow stall are the SAME root cause
#     (capital scarcity in the days-8-to-20 window), not two separate
#     levers -- raising any gate constant here can't fix it alone if
#     the money to act on a looser gate isn't there yet.
#   - Seed 7 (and the general 15-seed gate vs v9.5): fleet DOES reach
#     14 by day 16 (vs v9.5's 10), and money is lower at every
#     checkpoint through day 29 ($68,197 vs $74,150 final) -- the
#     larger fleet costs more in crop-labor cannibalization than it
#     earns in extra milk/wool, exactly iter C's original mechanism,
#     now confirmed to still hold after fertilize compression +
#     parallel build + hygiene fixes changed the crop/animal labor
#     balance. Modernizing the support system did not change the
#     verdict on a flat cap-raise.
#
# NOT submitted. Direction closed more conclusively than iter C left
# it. The real next lever is capital allocation/timing in the
# day-8-to-20 window (funds hiring AND animal purchases AND crop
# throughput simultaneously), not any gate threshold in this file.
# =====================================================================
...
# gates run unbounded."
EXPERIMENT_MAX_FLEET = 14  # v9.6: retest past v9.1 iter C's 10-animal ceiling now that
                            # fertilize compression (v9.3) + parallel build (v9.2) +
                            # confirmation/reset hygiene (v9.3fix/v9.5) have changed the
                            # crop-vs-animal labor tradeoff iter C hit its failure mode
                            # under. 14 matches the herd clone's observed final fleet
                            # size (Opponents/opp_scenario_v14.py). See file header.
