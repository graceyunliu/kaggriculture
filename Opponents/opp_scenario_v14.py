...
def _melon_target(obs):
    ...
    price = int(prices.get("MELON", 250) or 250)
    shops = set(((obs.get("town", {}) or {}).get("unlocked_shops", []) or []))
    milk_support = bool(shops & {"PIZZA_SHOP", "ICE_CREAM_SHOP", "SMOOTHIE_SHOP"})
    if opponent_tiles >= 12 and "BRUNCH_SPOT" in shops and not milk_support:
        return 10
    if opponent_tiles <= 5 and price >= 250:
        return 12
    if opponent_tiles <= 8 and price >= 230:
        return 10
    return 8
...
def _herd_targets(obs, farm, private, capacity):
    ...
    if day < HERD_EXPANSION_DAY:
        ...
        if day == HERD_FINAL_DAY:
            shops = set(
                ((obs.get("town", {}) or {}).get("unlocked_shops", []) or [])
            )
            animal_demand_known = bool(
                shops
                & {
                    "PIZZA_SHOP",
                    "ICE_CREAM_SHOP",
            ...
            opponent_counts = _opponent_animal_counts(obs)
            mirror_distance = (
                abs(owned["COW"] - opponent_counts["COW"])
                + abs(owned["SHEEP"] - opponent_counts["SHEEP"])
            )
            mirror_pressure = (
                sum(opponent_counts.values()) >= MID_HERD - 1
                and mirror_distance <= 2
            )
            if not animal_demand_known or mirror_pressure:
                stage_target = MID_HERD
...
def _role_plan(obs, farm):
    ...
    for quadrant in ("NW", "NE", "SW", "SE"):
        ...
        sequence = []
        if quadrant == "NW":
            if int(obs.get("day", 0) or 0) < 3:
                sequence.extend(["MELON"] * fixed.get("MELON", 0))
                sequence.extend(["CARROT"] * 2)
                sequence.extend(["STRAWBERRY"] * max(0, strawberry_count - 2))
