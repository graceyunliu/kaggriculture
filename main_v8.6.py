...
# =====================================================================
# MAIN V8.6 -- GOOSE as a third fallback species, ADDITIVE. Base:
# main_v8.3.py (the validated champion -- NOT main_v8.4.py's rejected
# last_day change or main_v8.5.py's rejected crop-space relaxation).
#
# --- Why (Grace's question after v8.5's rejection) ---
#
# v8.3's SHEEP addition works but is capacity-starved: diagnosis (project
# memory) found crop-space, not hand-capacity, is the real blocker of
# sheep's gate almost every game, including winning ones. main_v8.4.py
# (tighten SHEEP's last_day from real breakeven math) and main_v8.5.py
# (relax SHEEP's own crop-space check) were both tried and rejected --
# the crop-space relaxation in particular worked mechanically (ties
# 5->0) but was a financial wash with high variance (-$1,358/game,
# t=-0.78): SHEEP is $500 and produces only every 3rd day (interval=3),
# so a marginal purchase into genuinely tight crop-space is an expensive
# bet that often doesn't pay back before day 29.
#
# GOOSE (engine ANIMALS dict: cost=300, first_yield_day=4, interval=1,
# structure=COOP) is a structurally different bet, not a bigger swing at
# the same one: 40% cheaper, and interval=1 means every day of runway
# produces (base+bonus) 1-2 EGG instead of sheep's one tick per 3 days --
# a much smaller, smoother downside if the purchase turns out to be
# marginal, instead of sheep's harsh 3-day step function (this is also
# why GOOSE's last_day, below, matters far less than SHEEP's did -- one
# day earlier or later only moves value by ~1 tick, not 3). EGG is also
# genuinely unclaimed: real ladder data shows ~77 EGG sold across 5
# opponent games total against a 341-unit/game town demand pool.
#
# --- Why this needed real pipeline changes, not just a new gate ---
#
# GOOSE uses structure="COOP", not "PASTURE". Every existing function
# was hardcoded to PASTURE: perceive() only scanned for PASTURE tiles,
# animal_setup_action() always emitted "BUILD_PASTURE", and
# animal_maintenance_action()/the survival checks only ever read
# view["my_pastures"]. This build generalizes all of that:
#   - perceive() now also scans COOP tiles into my_coops/empty_coops,
#     and returns a combined view["my_animals"] = my_pastures + my_coops
#     for every structure-agnostic consumer (maintenance, survival
#     checks, n_animals count, wheat-feed math).
#   - ANIMAL_SPECS gained a "structure" field per species (COW/SHEEP =
#     PASTURE, GOOSE = COOP). animal_setup_action() now looks up the
#     right structure's (my_key, empty_key) list and BUILD_{structure}
#     verb instead of hardcoding PASTURE -- COW/SHEEP's own behavior is
#     unchanged since they still resolve to the same PASTURE lists.
#   - Every other view["my_pastures"] read that was actually meant to be
#     "any active animal, regardless of species" (survival checks in
#     both expansion gates, _pending_animal_work, the farmer's urgent-
#     feed preempt, animal_reconcile, animal_maintenance_action's
#     candidate list) now reads view["my_animals"] instead. FEED/CARE/
#     HARVEST/COLLECT_FERTILIZER were already structure-agnostic engine
#     actions (they only check "animal" in tile), so no change needed
#     there -- this was purely a perception/dispatch gap, not a missing
#     engine capability.
#
# --- The purchase gate ---
#
# COW's gate (unchanged) tried first every turn; SHEEP's gate (unchanged
# from v8.3 -- hand-capacity relaxed by SHEEP_HAND_HEADROOM, crop-space
# untouched) tried second; GOOSE's new gate tried third, only once both
# of the above say no this turn. GOOSE's gate reuses the same hand-
# capacity relaxation as SHEEP (GOOSE_HAND_HEADROOM=4, hand-capacity was
# never shown to be the dominant blocker) but ALSO relaxes crop-space,
# by GOOSE_CROP_SPACE_HEADROOM=6 -- deliberately smaller than v8.5's
# rejected SHEEP_CROP_SPACE_HEADROOM=10, reasoning that GOOSE's lower
# cost and gentler interval-1 degradation make a purchase under tighter
# crop-space conditions a smaller, more forgivable bet than the same
# relaxation was for sheep. This is a real, informed choice, not a
# proven-safe one -- it needs its own seeded result like everything else
# in this file's history.
#
# GOOSE's last_day: engine math (first_yield_day=4, interval=1, no
# lifespan cap) gives ticks(P) = max(0, 26-P) production ticks before
# day-29 match end. last_day=24 guarantees >= 2 ticks. Unlike SHEEP,
# getting this exact number slightly wrong costs ~1 tick (~$50-90) per
# day of error, not a whole 3-day cycle, so this is a much lower-stakes
# guess than main_v8.4.py's was -- flagged, not assumed safe.
#
# --- No-op check (REQUIRED) ---
#   GOOSE_ENABLED=False must reproduce main_v8.3.py exactly, since GOOSE
#   never gets a chance to buy and every other change here is either a
#   pure rename (my_pastures -> my_animals, same underlying tiles for
#   COW/SHEEP) or additive (new COOP-scanning code that only matters
#   once a COOP tile exists, which never happens with GOOSE disabled).
#
# --- Test protocol ---
#   1. No-op: GOOSE_ENABLED=False vs main_v8.3.py, --both-seats, must be
#      exactly 0.0 on every seed.
#   2. Real test: GOOSE_ENABLED=True vs main_v8.3.py, --both-seats, same
#      15-seed set as every prior v8.x round.
#   3. Smoke tests: starter/pass 720-turn, starter 48-turn truncated,
#      all DONE/DONE.
# =====================================================================
#
# =====================================================================
...
ANIMAL_SPECS = {
    ...
    # it); SHEEP stays day 3. "structure" (v8.6) is the engine's
    # BUILD_/PLACE structure kind for that species -- COW and SHEEP both
    # use PASTURE, GOOSE uses COOP. See file header for last_day math.
    "COW":   {"cost": 400, "min_money": 700, "start_day": 0, "last_day": 22, "structure": "PASTURE"},
    "SHEEP": {"cost": 500, "min_money": 800, "start_day": 3, "last_day": 22, "structure": "PASTURE"},
    # v8.6: cost/min_money follow the same $300 buffer pattern as COW
    # (400->700) and SHEEP (500->800). last_day=24 guarantees >=2 of the
    # ticks(P)=max(0,26-P) production ticks the engine's first_yield_day=
    # 4/interval=1 gives before day-29 match end -- see file header for
    # why this is much lower-stakes than SHEEP's last_day was.
    "GOOSE": {"cost": 300, "min_money": 600, "start_day": 0, "last_day": 24, "structure": "COOP"},
}
ANIMAL_STRUCTURE_LISTS = {   # species' "structure" -> (occupied-key, empty-key) in perceive()'s view
    "PASTURE": ("my_pastures", "empty_pastures"),
    "COOP":    ("my_coops", "empty_coops"),
}
...
# v8.6: GOOSE tried third, only once both COW's and SHEEP's gates say no
# this turn. Reuses SHEEP's hand-capacity relaxation as-is (never shown
# to be the dominant blocker) but ALSO relaxes crop-space -- deliberately
# smaller than main_v8.5.py's rejected SHEEP_CROP_SPACE_HEADROOM=10. See
# file header for the reasoning (lower cost, gentler interval-1 downside
# than SHEEP's interval-3).
GOOSE_ENABLED = True
GOOSE_HAND_HEADROOM = 4
GOOSE_CROP_SPACE_HEADROOM = 6
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    my_coops = []              # v8.6: same shape as my_pastures/
    empty_coops = []           # empty_pastures, for GOOSE's COOP structure
    ...
    return {
        ...
        "my_coops": my_coops, "empty_coops": empty_coops,
        # v8.6: structure-agnostic "any active animal" view for every
        # consumer that doesn't care whether it's a PASTURE or COOP --
        # survival checks, n_animals count, wheat-feed math, maintenance
        # dispatch. For COW/SHEEP-only play (GOOSE_ENABLED=False) this is
        # always identical to my_pastures alone, since my_coops stays empty.
...
def _goose_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads):
    """v8.6: GOOSE's own gate, tried only after both _animal_expansion_
    feasible() (COW) and _sheep_expansion_feasible() (SHEEP) have already
    returned False this turn. Checks 1 (survival) and 2 (wheat budget)
    are identical/unrelaxed, same as SHEEP's. Check 3 (hand-capacity)
    reuses SHEEP's own relaxation amount (GOOSE_HAND_HEADROOM ==
    SHEEP_HAND_HEADROOM) since hand-capacity was never shown to be the
    dominant blocker. Check 5 (crop-space) is ALSO relaxed here, by
    GOOSE_CROP_SPACE_HEADROOM -- deliberately smaller than main_v8.5.py's
    rejected SHEEP_CROP_SPACE_HEADROOM=10, on the reasoning that GOOSE's
    lower cost ($300) and gentler interval=1 (vs SHEEP's interval=3)
    make a purchase under tighter crop-space conditions a smaller bet.
    See file header for the full comparison."""
    if any(not t.get("fed_today", True) and t.get("consecutive_unfed", 0) >= 1
           for _pos, t in view["my_animals"]):
        return False
    ...
    available_for_crops = len(view["empty"]) - len(reserved_sites) - 1
    if available_for_crops < EXPANSION_MIN_CROP_TILES_PER_QUAD * quads - GOOSE_CROP_SPACE_HEADROOM:
        return False
        ...
        if target_plan is None:
            ...
            if _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads):
                ...
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            # v8.6: GOOSE tried third, only once COW's and SHEEP's gates
            # have both already said no this turn -- see file header.
            elif GOOSE_ENABLED and _goose_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads):
...
# the nearest tile in view["my_animals"] (v8.6: my_pastures + my_coops),
# which naturally includes every ACTIVE plan's site regardless of species,
# structure, or how many are active.
# =====================================================================
...
def animal_setup_action(pos, view, shed, farmer_carry, plan, day, hour, reserved_sites=()):
    ...
    v8.6: structure-aware -- looks up the plan's species' structure
    (PASTURE or COOP) via ANIMAL_STRUCTURE_LISTS to pick the right
    occupied/empty view keys and BUILD_{structure} verb, instead of the
    old hardcoded PASTURE/BUILD_PASTURE. COW/SHEEP both resolve to the
    same PASTURE lists as before, so their behavior is unchanged."""
    species = plan["species"]
    stage = plan["stage"]
    ...
    structure = ANIMAL_SPECS[species]["structure"]
    _my_key, empty_key = ANIMAL_STRUCTURE_LISTS[structure]
    empty_structures = view[empty_key]
