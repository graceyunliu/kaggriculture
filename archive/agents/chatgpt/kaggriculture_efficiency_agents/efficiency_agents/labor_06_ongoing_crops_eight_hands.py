# LABOR EFFICIENCY AGENT 06: ongoing_crops_eight_hands
# Base: main_v8.3.py
# Objective: Reduce replant burden so each worker supports more occupied tiles.
# Isolated policy changes: Eight-hand cap plus strawberry/tomato-heavy ongoing-crop portfolio.
...
PLANT_PRIORITY = ["STRAWBERRY", "TOMATO", "MELON", "WHEAT", "CARROT"]
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        want["STRAWBERRY"] = min(space, 12 if ph == 0 else 20)
    if day <= CUTOFF["TOMATO"] and budget > 600:
        want["TOMATO"] = 8   # ongoing crop, same small-standing-patch approach
