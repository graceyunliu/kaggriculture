# =====================================================================
# MAIN V11A -- COMPACT LIVESTOCK ENGINE
#
# Economy reset on the v10.6 execution chassis.
# Goal: test whether the validated 10-animal ceiling is really a crop-estate ceiling.
# The agent deliberately keeps a small crop estate, buys all feed, and allows a larger
# cow/sheep fleet to consume labor that would otherwise service distant low-value crops.
# Empty land is not a hiring signal.
# =====================================================================
...
#   4. Hard EXPERIMENT_MAX_FLEET = 15 cap. Unchanged from v8.10.
...
EXPANSION_MIN_CROP_TILES_PER_QUAD = 2   # crop-land reserve, scales with
...
ANIMAL_SERVICE_MIN_HANDS = 5  # v9.1 iter A: 5 starved crop planting during the ramp; back to v8.11's validated 3
...
SHEEP_FLOOR_BY_FLEET_SIZE = {4: 1, 6: 2, 8: 3, 10: 4, 12: 5, 14: 6, 15: 7}  # v9.1
...
FERT_SHED_RESERVE = 10       # v9.3 iter F: working stock for fertilize dispatch
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion: only buy when the current footprint is genuinely tight. ---
    if 1 <= day <= 18 and len(view["empty"]) < 6:
        land_score = PRIORITY_LAND_SCARCE
        if quads == 1 and budget > 1400:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2600 and day <= 15:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 5000 and day <= 12:
    ...
    # v11A: deliberately small crop estate. Strawberries are the compact,
    # fertilizable crop core; melon is only a low-touch outer-ring filler.
    want = {}
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        want["STRAWBERRY"] = min(space, 6 if ph == 0 else 10)
    if day <= CUTOFF["MELON"] and budget > 900:
        want["MELON"] = min(space, 2 if ph == 0 else 4)
    # No tomato/carrot/wheat production contract in this economy.
