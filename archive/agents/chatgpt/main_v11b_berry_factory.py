# =====================================================================
# MAIN V11B -- BERRY FACTORY
#
# Economy reset on the v10.6 execution chassis.
# Goal: maximize fertilized strawberry throughput with a deliberately bounded pasture
# fleet whose secondary purpose is fertilizer generation. Almost all crop capital goes
# to near-shed strawberries; low-touch filler crops are minimized. Empty land does not
# create hiring demand.
# =====================================================================
...
#   4. Hard EXPERIMENT_MAX_FLEET = 8 cap. Unchanged from v8.10.
...
EXPANSION_MIN_CROP_TILES_PER_QUAD = 4   # crop-land reserve, scales with
...
SHEEP_FLOOR_BY_FLEET_SIZE = {2: 1, 4: 2, 6: 3, 8: 4}  # v9.1
...
FERT_SHED_RESERVE = 14       # v9.3 iter F: working stock for fertilize dispatch
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion: strawberries need compact capacity, not blanket acreage. ---
    if 1 <= day <= 16 and len(view["empty"]) < 4:
        land_score = PRIORITY_LAND_SCARCE
        if quads == 1 and budget > 1400:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2800 and day <= 13:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 5200 and day <= 10:
    ...
    # v11B: berry factory. The crop estate is intentionally concentrated.
    want = {}
    if day <= CUTOFF["STRAWBERRY"] and budget > 800:
        want["STRAWBERRY"] = min(space, 16 if ph == 0 else 30)
    if day <= CUTOFF["MELON"] and budget > 1800:
        want["MELON"] = min(space, 2)
    # No routine tomato/carrot/wheat contract; preserve labor for berry service.
