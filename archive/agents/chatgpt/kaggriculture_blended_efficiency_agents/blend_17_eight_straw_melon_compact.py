# BLENDED EFFICIENCY AGENT 17: eight_straw_melon_compact
# Objective: jointly improve turn, land, and labor efficiency.
# Policy: 8 hands, compact premium crop blend.
# Base: main_v8.3.py
...
ANIMAL_SPECS = {
    ...
    "COW":   {"cost": 400, "min_money": 700, "start_day": 0,  "last_day": 14},
    "SHEEP": {"cost": 500, "min_money": 800, "start_day": 3,  "last_day": 14},
}
...
SHEEP_ENABLED = True
SHEEP_HAND_HEADROOM = 2   # extra hand-target ceiling allowed specifically
...
PLANT_PRIORITY = ['STRAWBERRY', 'MELON', 'WHEAT', 'TOMATO', 'CARROT']
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["MELON"] and budget > 700:
        melon_target = min(space, 9 if ph == 0 else 15)
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        want["STRAWBERRY"] = min(space, 9 if ph == 0 else 15)
    if day <= CUTOFF["TOMATO"] and budget > 600:
        want["TOMATO"] = 2   # ongoing crop, same small-standing-patch approach
    ...
    if hour == 0 and day < 29:
        ...
        target = max(HAND_TARGET_MIN, min(HAND_TARGET_MAX, math.ceil(work / 7)))
