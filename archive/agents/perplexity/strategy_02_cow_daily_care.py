# strategy_02_cow_daily_care.py
# Strategy #2: Full daily-care cow ranch — $169.67/tile/day
# Day 0: BUILD_PASTURE, BUY_ANIMAL COW, PLACE
# Every day after: FEED + CARE, no exceptions
# Harvest on each 2-day production tick starting day 8
#
# This agent focuses on a single cow tile with maximum care discipline.
...
SHEEP_ENABLED = False  # Disable sheep for this strategy
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # Minimal land buys; assume starting land is enough for 1 pasture + small crop buffer.

    # Minimal wheat seeds for early feed.
    ph = phase(day)
    ...
    if not in_progress:
        ...
        if target_plan is None:
            if _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads):
                target_plan = {"species": "COW", "stage": "NONE", "site": None, "stage_turn": None, "retries": 0}
