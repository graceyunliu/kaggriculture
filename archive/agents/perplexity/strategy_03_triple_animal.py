# strategy_03_triple_animal.py
# Strategy #3: Triple-animal ranch (goose+cow+sheep) — $146.56/tile/day
# Build a coop and two pastures. Buy and place one of each animal on day 0.
# Feed and care for all three, every day, for the whole season.
#
# This agent:
# - prioritizes early land buys to secure 3 tiles (1 coop + 2 pastures),
# - buys goose, cow, sheep as early as feasible,
# - focuses on daily FEED + CARE for all three animals,
# - minimal crop activity (only tiny wheat buffer for feed if needed).
    ...
    for y, row in enumerate(me["tiles"]):
        for x, t in enumerate(row):
            ...
            if kind == "WEED":
                ...
                # Treat coop similarly for perception; engine handles coop tiles separately.
                pass
            elif kind == "PLANT":
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, species):
    # Allow exactly 3 animals: one goose, one cow, one sheep.
    goose_count = sum(1 for p in animal_plans if p.get("species") == "GOOSE" and p["stage"] in ("BOUGHT", "CARRYING", "ACTIVE")) + sum(
        1 for _pos, t in view["my_pastures"] if t.get("animal") == "GOOSE"
    )
    cow_count = sum(1 for p in animal_plans if p.get("species") == "COW" and p["stage"] in ("BOUGHT", "CARRYING", "ACTIVE")) + sum(
        1 for _pos, t in view["my_pastures"] if t.get("animal") == "COW"
    )
    sheep_count = sum(1 for p in animal_plans if p.get("species") == "SHEEP" and p["stage"] in ("BOUGHT", "CARRYING", "ACTIVE")) + sum(
        1 for _pos, t in view["my_pastures"] if t.get("animal") == "SHEEP"
    )
    if species == "GOOSE" and goose_count >= 1:
        return False
    if species == "COW" and cow_count >= 1:
        return False
    if species == "SHEEP" and sheep_count >= 1:
        return False
    if n_animals >= 3:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # Land expansion: aim for 3 tiles (1 coop + 2 pastures).
    if 1 <= day <= 22:
        land_score = _land_priority(view)
        if quads == 1 and budget > 1000:
            priced_orders.append((land_score, ["BUY_LAND"]))
            budget -= 1000
        elif quads == 2 and budget > 2000 and day <= 18:
            priced_orders.append((land_score, ["BUY_LAND"]))
            budget -= 2000
        # Skip third land buy; assume starting land + 2 buys is enough for 3 tiles.

    # Minimal wheat seeds for feed cash flow.
    ph = phase(day)
    space = len(view["empty"]) + 2
    want = {}
    if day <= CUTOFF["WHEAT"] and day <= 5:
        want["WHEAT"] = min(space, 6)
    ...
    # Animal fleet: one goose, one cow, one sheep.
    ...
    if not in_progress:
        ...
        if target_plan is None:
            # Try goose, then cow, then sheep.
            for species in ["GOOSE", "COW", "SHEEP"]:
                if _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, species):
                    spec = ANIMAL_SPECS[species]
                    if spec["start_day"] <= day <= spec["last_day"] and shed.get(species, 0) == 0 and budget > spec["min_money"]:
                        target_plan = {"species": species, "stage": "NONE", "site": None, "stage_turn": None, "retries": 0}
