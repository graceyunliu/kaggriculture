...
# =====================================================================
# MAIN V_CLONEREPLICA1 -- real-ladder capital-allocation clone, layered on
# v10.6's dispatch engine. Base: main_v10.6_radius3.py. assign_tasks(),
# unit_action(), BFS/movement helpers, and the animal worker-slot pipeline
# state machine (NONE->ORDERED->BOUGHT->CARRYING->ACTIVE) are UNTOUCHED --
# only the capital-allocation layer (land timing, herd staging/species
# choice, crop mix, hire cap, feed stock, sell-price floors) is replaced,
# using constants extracted directly from Opponents/opp_scenario_v14.py
# (a real top-of-ladder clone). See project memory / task brief for the
# full extracted spec. Every prior gap-closing attempt on this project
# grafted ONE piece of a real opponent's policy onto v10.6's own timing
# and lost; this build replaces the WHOLE capital-allocation shape at
# once so the pieces aren't fighting machinery tuned for a different
# strategy.
#
# Changes vs v10.6 (each independent, all in economy()/plantable_crops()/
# _grow_reserved_sites(), none in the dispatch engine):
#   1. LAND: buy NE on day>=5, SW on day>=9 (engine's LAND_ORDER is fixed
#      ["NE","SW","SE"], so "cap at 3 quadrants, never buy SE" is simply
#      "never issue the 3rd BUY_LAND" -- verified against the engine
#      source). CASH_RESERVE=250 added to the affordability check.
#   2. HERD: replaces the old survival/wheat/hand-capacity/crop-space
#      GATE logic with a staged TARGET (CORE_HERD=4 while day<7,
#      MID_HERD=11 while day<11, TARGET_HERD=15 from day 11, frozen after
#      ANIMAL_PURCHASE_LAST_DAY=18), capacity-limited by
#      ANIMAL_SLOTS={"NW":4,"NE":7,"SW":4,"SE":0} summed over currently
#      unlocked quadrants (note: 4/11/15 line up exactly with NW-alone /
#      NW+NE / NW+NE+SW capacity -- the staging is really a capacity
#      schedule tied to the land-unlock days above). Species choice: COW
#      floor 3, SHEEP floor 1, remaining slots go to whichever species
#      scores higher on price/demand/crowding (each capped at 9). Two
#      safety checks are KEPT from v10.6's old gate (not in the opponent
#      spec, but load-bearing to avoid animal starvation/cash crashes):
#      no purchase while an existing pasture has already missed a
#      feeding, and a purchase's feed shortfall can't exceed
#      EXPANSION_WHEAT_BUDGET_FRAC of current cash.
#   3. ANIMAL SITING: _grow_reserved_sites() now respects the per-quadrant
#      ANIMAL_SLOTS cap (was: nearest-to-shed only, no quadrant limit).
#   4. CROP MIX: STRAWBERRY and TOMATO dropped entirely (want[] targets
#      removed, PLANT_PRIORITY shortened) -- this also makes the old
#      near-shed-radius ONGOING_CROPS siting mechanism inert (nothing
#      ongoing left to site), so plantable_crops()'s per-tile ranking is
#      repurposed instead for quadrant-aware crop-mix siting: MELON
#      preferred on tiles in the NW quadrant (target ~10 tiles, flat, not
#      dynamically scaled per the task brief's "don't over-engineer"
#      note), WHEAT/CARROT preferred elsewhere. want[] targets for
#      WHEAT/CARROT sum CROP_MIX's per-quadrant allocation over whichever
#      quadrants are actually unlocked (NW 4/2, NE 4/1, SW 4/1).
#   5. HIRE CAP: HAND_TARGET_MAX 18 -> 12 (MAX_HANDS from the spec);
#      fibonacci-cost hire loop machinery itself is unchanged.
#   6. FEED STOCK: target_wheat n_animals*2+2 -> max(8, n_animals*3)
#      (FEED_STOCK_DAYS=3), applied both to the buy trigger and to the
#      sell-side feed reserve so paced WHEAT selling doesn't eat the
#      buffer this now implies.
#   7. SELL FLOORS: WHEAT/CARROT/MELON/MILK/WOOL get the spec's exact
#      reserve fractions (0.68/0.55/0.58/0.42/0.40) as their price-floor
#      fraction of base price, replacing the old flat 0.85-premium/
#      0.70-other split for those five items. (Reading opp_scenario_v14.py
#      directly: its own RESERVE_FRACTION dict is used as `base_price *
#      frac` -> a MINIMUM SELL PRICE threshold, not a stock-quantity
#      reserve -- despite the "hold back this fraction of current stock"
#      phrasing in the task brief. Implemented to match the verified
#      ground-truth mechanism, which also matches v10.6's own existing
#      frac*base floor machinery, since that's a more faithful port than
#      inventing a new stock-reserve mechanism from a paraphrase.)
# =====================================================================
...
ANIMAL_SPECS = {
    # v_clonereplica1: last_day extended to ANIMAL_PURCHASE_LAST_DAY (18)
    # for both species -- the staged herd target (below) can call for
    # purchases up to that day, so the old day-12/15 tails would have
    # silently clipped the late stage of the 11->15 ramp.
    "COW":   {"cost": 400, "min_money": 550, "start_day": 0,  "last_day": 18},
    "SHEEP": {"cost": 500, "min_money": 650, "start_day": 3,  "last_day": 18},
}
SHEEP_ENABLED = True

# --- v_clonereplica1: staged herd target + per-quadrant siting caps,
# ported directly from opp_scenario_v14.py's exact constants (see file
# header). Replaces v10.6's _animal_expansion_feasible/
# _sheep_expansion_feasible gate pair and SHEEP_FLOOR_BY_FLEET_SIZE
# composition guard entirely (those functions/constants are left defined
# below, unused, rather than deleted, to keep this diff smaller).
CORE_HERD = 4
MID_HERD = 11
TARGET_HERD = 15
...
CASH_RESERVE = 250
LAND_OPEN_DAYS = (5, 9)   # day NE (extra_land=0) / SW (extra_land=1) open

# Crop-mix targets, per unlocked quadrant (SE omitted -- never bought,
# see LAND section). MELON stays NW-only; WHEAT/CARROT get planted in
# any unlocked quadrant per this table, summed live off
# me["unlocked_quadrants"] in economy().
CROP_MIX = {
    "NW": {"MELON": 10, "WHEAT": 4, "CARROT": 2},
    "NE": {"WHEAT": 4, "CARROT": 1},
    "SW": {"WHEAT": 4, "CARROT": 1},
}
FEED_STOCK_DAYS = 3   # feed target = max(8, n_animals * FEED_STOCK_DAYS)
SELL_FLOOR_FRAC = {   # price-floor fraction of base price -- see file header
    "WHEAT": 0.68, "CARROT": 0.55, "MELON": 0.58, "MILK": 0.42, "WOOL": 0.40,
}
SHOPS = {
    "BAKERY":         ("EGG", "WHEAT"),
...
SHEEP_HAND_HEADROOM = 4   # unused by v_clonereplica1's new herd logic;
                           # kept only because _sheep_expansion_feasible
                           # (dead code below) still references it.
...
HAND_TARGET_MIN = 4
HAND_TARGET_MAX = 12   # v_clonereplica1: 18 -> 12 (opp_scenario_v14's MAX_HANDS).
                         # Hire loop machinery itself (fibonacci cost, the
                         # while-loop below) is unchanged.

# Market-order category slot reservations
MAX_HIRE_SLOTS = HAND_TARGET_MAX
MAX_SEED_SLOTS = 5   # fits MELON/WHEAT/CARROT (STRAWBERRY/TOMATO no longer bought)
...
# v_clonereplica1: STRAWBERRY/TOMATO dropped from the crop mix entirely
# (see file header) -- their want[] targets are gone in economy(), so
# seeds.get() for them stays 0 all game and they never appear in
# plantable_crops()'s `base` list regardless of this order or CUTOFF
# still listing them. MELON kept ahead of WHEAT/CARROT as the base order;
# plantable_crops(tile=...) re-ranks per quadrant below (v_clonereplica1
# repurposes the old near-shed-radius mechanism for quadrant-aware
# crop-mix siting instead of the now-inert ongoing/one-time distinction).
PLANT_PRIORITY = ["MELON", "WHEAT", "CARROT"]
...
# v_clonereplica1: ONGOING_CROPS/NEAR_SHED_RADIUS/_dist_to_shed (v10.5/
# v10.6's near-shed ongoing-crop siting mechanism) are now INERT -- with
# STRAWBERRY/TOMATO removed from the crop mix (see file header), no crop
# in ONGOING_CROPS can ever appear in plantable_crops()'s `base` list
# (their seed counts stay 0 all game), so that near/far split never has
# anything to rank. Left defined for anyone diffing against v10.6; the
# per-tile ranking below is repurposed for quadrant-aware crop-mix siting
# instead (MELON preferred in NW, WHEAT/CARROT preferred elsewhere, per
# CROP_MIX / the extracted opp_scenario_v14.py spec).
...
def _quadrant_of_pos(pos):
    """Matches the engine's own _quadrant_of exactly (kaggriculture.py):
    half = BOARD // 2; N/S by y, W/E by x."""
    x, y = pos
    half = BOARD // 2
...
def plantable_crops(seeds, day, tile=None):
    """v_clonereplica1: base filtering unchanged from v10.6 (available
    seed stock + not past CUTOFF). Per-tile ranking replaced: instead of
    v10.6's near-shed-distance/ongoing-crop split (now inert, see above),
    rank by CROP_MIX's quadrant assignment -- MELON is NW's crop (target
    ~10 tiles there), WHEAT/CARROT are the other unlocked quadrants' crop
    (and NW's remainder). `tile=None` preserves the old position-agnostic
    behavior for call sites without a specific tile (e.g. the
    reserved-crop-hand-count pressure estimate)."""
    base = [c for c in plant_priority_current if seeds.get(c, 0) > 0 and day <= CUTOFF.get(c, -1)]
    if tile is None or len(base) <= 1:
        return base
    if _quadrant_of_pos(tile) == "NW":
        return sorted(base, key=lambda c: 0 if c == "MELON" else 1)
    return sorted(base, key=lambda c: 1 if c == "MELON" else 0)
...
# =====================================================================
# v_clonereplica1: STAGED HERD TARGET + SPECIES SCORING
# Replaces _animal_expansion_feasible/_sheep_expansion_feasible (still
# defined above, now unused) as the decision of whether/what to buy next.
# Constants ported directly from opp_scenario_v14.py (see file header).
# =====================================================================

def _available_animal_capacity(unlocked_quadrants):
    return sum(ANIMAL_SLOTS.get(q, 0) for q in unlocked_quadrants)


def _herd_stage_target(day, n_owned_total, capacity):
    """Staged target: CORE_HERD while day<7, MID_HERD while day<11,
    TARGET_HERD from day 11 on, frozen at current ownership past
    ANIMAL_PURCHASE_LAST_DAY. Always capped by real per-quadrant capacity
    (never asks for more animals than there is pasture room for)."""
    ...
    if day > ANIMAL_PURCHASE_LAST_DAY:
        stage_target = n_owned_total
    return min(capacity, max(n_owned_total, stage_target))


def _town_demand_per_day(obs, item, day):
    center = 2 * (4 if day >= 20 else 2 if day >= 10 else 1)
    shop = 0
    for name in (obs.get("town", {}) or {}).get("unlocked_shops", []) or []:
...
def _livestock_score(obs, animal, own_count, opponent_count, day):
    """normalized_price x (1 + 0.012*town_demand_per_day) / (1 + 0.18*opp
    + 0.08*own) -- ported verbatim from opp_scenario_v14.py's
    _livestock_score. Prefers the species with the better relative
    price/demand, penalizes over-concentrating in one species."""
    product = "MILK" if animal == "COW" else "WOOL"
    base_price = MARKET_PARAMS[product]["base"]
    normalized_price = live_price(obs, product) / float(base_price)
    demand_support = 1.0 + 0.012 * _town_demand_per_day(obs, product, day)
    crowding = 1.0 + 0.18 * opponent_count + 0.08 * own_count
    return normalized_price * demand_support / crowding


def _herd_species_targets(obs, opp, day, owned_cow, owned_sheep, target_total):
    """COW floor 3, SHEEP floor 1 (once target_total>=CORE_HERD), then
    remaining slots go one at a time to whichever species currently
    scores higher, each capped at 9 before the other absorbs the rest."""
    targets = {"COW": max(3 if target_total >= CORE_HERD else 0, owned_cow),
               "SHEEP": max(1 if target_total >= CORE_HERD else 0, owned_sheep)}
    opp_counts = _opp_animal_counts(opp)
    while sum(targets.values()) < target_total:
        eligible = [n for n in ("COW", "SHEEP") if targets[n] < 9]
        if not eligible:
            eligible = ["COW", "SHEEP"]
        animal = max(eligible, key=lambda name: (
            _livestock_score(obs, name, targets[name], opp_counts[name], day),
            -targets[name],
            name == "COW"))
        targets[animal] += 1
    return targets
...
def _herd_purchase_safe(obs, view, budget, shed, n_animals):
    """Two safety checks kept from v10.6's old gate (not in the opponent
    spec, but load-bearing): no purchase while an existing pasture has
    already missed a feeding, and a purchase's feed shortfall can't cost
    more than EXPANSION_WHEAT_BUDGET_FRAC of current cash."""
    if any(not t.get("fed_today", True) and t.get("consecutive_unfed", 0) >= 1
           for _pos, t in view["my_pastures"]):
        return False
    next_target_wheat = max(8, (n_animals + 1) * FEED_STOCK_DAYS)
    have = shed.get("WHEAT", 0)
    shortfall = max(0, next_target_wheat - have)
    shortfall_cost = shortfall * live_price(obs, "WHEAT")
    if shortfall_cost > EXPANSION_WHEAT_BUDGET_FRAC * max(budget, 1):
...
def _grow_reserved_sites(view):
    ...
    a new animal purchase actually fires. v_clonereplica1: respects the
    per-quadrant ANIMAL_SLOTS cap (NW 4 / NE 7 / SW 4 / SE 0) -- picks the
    nearest still-unclaimed empty tile to the shed among quadrants that
    still have room under their cap, falling back to the old
    nearest-only behavior only if every quadrant's cap is already full
    (shouldn't happen: TARGET_HERD=15 == sum of the caps over the 3
    quadrants this build ever unlocks)."""
    candidates = [t for t in view["empty"]
                  if t not in CENTER_TILES and t not in reserved_sites]
    if not candidates:
        return
    counts = {"NW": 0, "NE": 0, "SW": 0, "SE": 0}
    for t in reserved_sites:
        counts[_quadrant_of_pos(t)] += 1
    eligible = [t for t in candidates if counts[_quadrant_of_pos(t)] < ANIMAL_SLOTS.get(_quadrant_of_pos(t), 0)]
    pool = eligible or candidates
    pool.sort(key=lambda t: _dist(t, _nearest_center(t)))
    reserved_sites.append(pool[0])
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion ---
    # v_clonereplica1: day-gated to LAND_OPEN_DAYS=(5,9) instead of v10.6's
    # budget-only gate, and the quads==3 branch is REMOVED entirely -- the
    # engine's own LAND_ORDER is the fixed sequence ["NE","SW","SE"]
    # (verified in vendor/kaggle_environments_engine/kaggriculture.py), so
    # "cap at 3 quadrants total (NW+NE+SW), never buy SE" is exactly
    # "never issue the 3rd BUY_LAND order" -- no quadrant-identity tracking
    # needed. CASH_RESERVE=250 added on top of the raw sticker price.
    if quads == 1 and day >= LAND_OPEN_DAYS[0] and budget > 1000 + CASH_RESERVE:
        priced_orders.append((_land_priority(view), ["BUY_LAND"])); budget -= 1000
    elif quads == 2 and day >= LAND_OPEN_DAYS[1] and budget > 2000 + CASH_RESERVE:
        priced_orders.append((_land_priority(view), ["BUY_LAND"])); budget -= 2000
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        # v_clonereplica1 bug fix: a NONE-stage plan whose species purchase
        # window (ANIMAL_SPECS[species]["last_day"]) has already closed can
        # NEVER confirm (the day-range check below is monotonic) -- left
        # in place, it is found FIRST by the line above every subsequent
        # turn and permanently blocks re-evaluation, silently freezing herd
        # growth for the rest of the match even with ample cash (found via
        # diagnostic trace: herd stuck at 3 COW+4 SHEEP from day 9 onward
        # in the vs-v10.6 seed-1 game, budget growing to $68k unused).
        # Discard it here so a fresh species decision can be made below.
        if target_plan is not None and day > ANIMAL_SPECS[target_plan["species"]]["last_day"]:
            animal_plans.remove(target_plan)
            target_plan = None
        if target_plan is None and day <= ANIMAL_PURCHASE_LAST_DAY:
            # v_clonereplica1: staged-target herd sizing replaces v10.6's
            # gate pair (_animal_expansion_feasible/_sheep_expansion_
            # feasible) -- see file header + _herd_stage_target/
            # _herd_species_targets below. Two safety checks are kept
            # inline via _herd_purchase_safe (survival + wheat-budget).
            capacity = _available_animal_capacity(me["unlocked_quadrants"])
            owned_cow = sum(1 for p in animal_plans if p["species"] == "COW"
                             and p["stage"] in ("ACTIVE", "ORDERED", "BOUGHT", "CARRYING"))
            owned_sheep = sum(1 for p in animal_plans if p["species"] == "SHEEP"
                               and p["stage"] in ("ACTIVE", "ORDERED", "BOUGHT", "CARRYING"))
            n_owned_total = owned_cow + owned_sheep
            stage_target = _herd_stage_target(day, n_owned_total, capacity)
            if stage_target > n_owned_total and _herd_purchase_safe(obs, view, budget, shed, n_animals):
                targets = _herd_species_targets(obs, opp, day, owned_cow, owned_sheep, stage_target)
                owned = {"COW": owned_cow, "SHEEP": owned_sheep}
                deficits = [(targets[s] - owned[s], s) for s in ("COW", "SHEEP") if targets[s] > owned[s]]
                deficits.sort(key=lambda ds: (-ds[0], ds[1] != "COW"))
                # v_clonereplica1 bug fix: don't just take deficits[0] --
                # found via diagnostic trace that a single stranded/
                # unclaimed animal already sitting in the shed (its build
                # plan lost or never progressed to BOUGHT) permanently
                # trips the existing unclaimed_in_shed<=0 confirmation gate
                # below for THAT species, and picking only the top-scoring
                # deficit meant the whole herd froze at 8/15 animals for
                # the back half of a real seed-1 match (budget grew to
                # $29k+ unused) because the code kept re-choosing the same
                # blocked species every hour instead of falling back.
                # Loop the (already score-ranked) deficits and take the
                # first species whose shed stock isn't already stranded.
                for _d, species_choice in deficits:
                    claimed_precheck = sum(1 for p in animal_plans
                                            if p["species"] == species_choice and p["stage"] == "BOUGHT")
                    if shed.get(species_choice, 0) - claimed_precheck <= 0:
                        target_plan = {"species": species_choice, "stage": "NONE", "site": None,
    ...
    # --- Seed pipeline ---
    # v_clonereplica1: STRAWBERRY/TOMATO dropped; MELON/WHEAT/CARROT
    # targets now sum CROP_MIX over whichever quadrants are actually
    # unlocked (NW MELON 10 / WHEAT 4 / CARROT 2, NE+SW WHEAT 4/CARROT 1
    # each) instead of v10.6's flat space-scaled targets. Wheat is now
    # PLANTED again (unlike v9.1-v10.6's buy-only-feed design) since it
    # is both this build's feed source and its NE/SW cash crop.
    ph = phase(day)
    space = len(view["empty"]) + 2
    unlocked = me["unlocked_quadrants"]
    want = {}
    if day <= CUTOFF["MELON"] and budget > 700 and "NW" in unlocked:
        melon_target = min(space, CROP_MIX["NW"]["MELON"])
    ...
    if day <= CUTOFF["WHEAT"]:
        wheat_target = sum(CROP_MIX.get(q, {}).get("WHEAT", 0) for q in unlocked)
        if wheat_target > 0:
            want["WHEAT"] = min(space, wheat_target)
    if day <= CUTOFF["CARROT"]:
        carrot_target = sum(CROP_MIX.get(q, {}).get("CARROT", 0) for q in unlocked)
        if carrot_target > 0:
            want["CARROT"] = min(space, carrot_target)
    ...
    # v_clonereplica1: FEED_STOCK_DAYS=3 target, min 8, per spec.
    if n_animals > 0:
        target_wheat = max(8, n_animals * FEED_STOCK_DAYS)
        have = shed.get("WHEAT", 0)
    ...
    feed_reserve = max(8, n_animals * FEED_STOCK_DAYS) if n_animals else 0
    sellable = []
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        if is_attack and item in PREMIUM_CROPS:
            ...
            frac = SELL_FLOOR_FRAC.get(item, 0.85 if item in PREMIUM_CROPS else 0.70)
...
def _agent(obs):
    ...
    def _taken_sites():
        # v_clonereplica1 bug fix: recomputed fresh on every call (was a
        # single snapshot taken once per turn, before ANY of this turn's
        # setup dispatches ran) -- found via diagnostic trace: two
        # site-less BOUGHT plans dispatched in the SAME turn (farmer's
        # slot 0 and a hand's slot 1) both read the identical stale
        # snapshot (neither's about-to-be-chosen site was in it yet) and
        # independently picked the same nearest empty tile as their site.
        # animal_setup_action() then built exactly one physical pasture
        # there; reconcile's site_occupied check (which only tests
        # whether *a* pasture exists at plan["site"], not that it's
        # uniquely this plan's) marked BOTH plans ACTIVE off that one
        # pasture. The second animal, still riding in its worker's
        # carry with no plan left driving it anywhere, got silently
        # auto-dropped into the shed by the engine's overnight
        # _drop_inventories_to_shed with no plan left to claim it --
        # permanently tripping the unclaimed_in_shed<=0 purchase gate for
        # that species (shed stock that can never be claimed or spent)
        # and freezing herd growth for the rest of the match even with
        # ample cash and stage_target still above n_owned_total. Calling
        # this fresh (instead of reusing one set for the whole turn)
        # means each successive assignment within the same turn is
        # visible to the next, since plan["site"] mutations land directly
        # in animal_plans (shared, mutated in place).
        return {p.get("site") for p in animal_plans
                if p["stage"] in ("BOUGHT", "CARRYING") and p.get("site")}
    ...
    if active_setup_plan is not None and not urgent_existing_feed:
        farmer_op = animal_setup_action(farmer_pos, view, shed, farmer_carry, active_setup_plan, day, hour, sites,
                                        exclude_sites=_taken_sites() - {active_setup_plan.get("site")})
    ...
    for i, h in enumerate(me["hands"]):
        ...
        if pl is not None and not urgent_existing_feed:
            op = animal_setup_action(hand_pos, view, shed, hand_carry, pl, day, hour, sites,
                                     exclude_sites=_taken_sites() - {pl.get("site")})
            if op is not None:
                hand_ops[i] = op
                continue
