...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["TOMATO"] and budget > 600:
        # v10.4 hypothesis: this flat "3" is the exact same shape as the
        # pre-v7.9 STRAWBERRY bug (small-standing-patch premise, wrong
        # because ongoing crops have a fixed lifetime yield, not a
        # perpetual one) -- confirmed via full-history grep that TOMATO's
        # target has been an unscaled 3 in literally every shipped
        # version since v7.5, never revisited when STRAWBERRY's own
        # identical bug was found and fixed. TOMATO reaches its max_yield
        # cap in only 4 daily ticks (interval=1 vs STRAWBERRY's interval=2)
        # so its total labor commitment per plant is SHORTER (~12 days
        # incl. decay vs STRAWBERRY's ~17), its market absorbs volume far
        # more gently (above_target 0.60 vs STRAWBERRY's 1.60 -- realized
        # price should hold much better at scale), and it ranks behind
        # STRAWBERRY/MELON in PLANT_PRIORITY so it can only claim tiles
        # they don't already want -- capped by `space` like they are, so
        # this can't over-plant beyond real available land. Different risk
        # profile than v9.13's rejected STRAWBERRY/MELON coverage bump
        # (that one added MORE of two already-heavily-farmed, higher-
        # maintenance premium crops; this fills a near-zero-output crop's
        # already-idle leftover land instead).
        want["TOMATO"] = min(space, 8 if ph == 0 else 16)
