...
# =====================================================================
# MAIN V12.6 -- EV-GATED LAND EXPANSION + PREDICTIVE SHED THROTTLE, on
# top of v10.6 (radius 3). Base: main_v10.6_radius3.py, unchanged except
# for the additions tagged "v12.6" below. Round-8 hypothesis
# (shed-integrated combos), sibling of main_v12.5_predictive_throttle.py
# -- SELF-CONTAINED here too (this file does not assume v12.5's code
# exists; the forecast machinery is rebuilt from scratch below, byte-for-
# byte the same formula, so this file stands alone).
#
# --- Part A: predictive shed-inflow forecast (rebuilt, same as v12.5) ---
#
# For every currently-growing ongoing-crop tile (STRAWBERRY/TOMATO past
# its first production age) and every currently-active pasture, convert
# the engine's own per-crop/per-species accrual interval into an expected
# units/day rate (1.0/interval, base rate only, no fertilizer/care bonus
# assumed -- conservative). Sum into projected_daily_inflow. Net it
# against a rough flat sell-through estimate (SELL_CAPACITY_PER_DAY=18
# units/day -- same rationale as v12.5: roughly half of the 10 per-turn
# order slots typically go to non-SELL categories, and each SELL order
# usually clears a whole item's shed stock in one shot, so throughput is
# bounded by distinct-items-clearable-per-day more than per-order size).
# Compound net_daily_inflow over FORECAST_HORIZON_DAYS=5 days on top of
# the current shed_load; THROTTLE fires if that projection exceeds
# THROTTLE_TRIGGER_FRAC (0.85) of SHED_CAP.
#
# --- Part B: simplified EV-gated land purchase (NEW mechanism) ---
#
# v10.6's land purchase is unconditional once quads/budget/day thresholds
# are met (buy quad 2 at budget>1000, day<=22; quad 3 at budget>2000,
# day<=18; quad 4 at budget>4000, day<=16) -- no estimate of whether the
# NEXT quadrant will actually pay for itself is ever computed. This build
# adds one: a rough expected-value estimate for the next quadrant,
# self-contained (no dependency on any other file):
#
#   EV_PER_TILE_DAY = $20        (rough constant, mid of the $10-30/tile/
#                                  day range once a tile is ramped -- this
#                                  project's own siting work has measured
#                                  strawberry/melon marginal tile-days in
#                                  the $60-80 range at good prices and
#                                  much less at glut/marginal-unit prices,
#                                  so $20 is a deliberately conservative
#                                  blended estimate across the whole mix
#                                  including one-time/low-value crops and
#                                  idle days, not a best-case number)
#   RAMP_DAYS = 6                 (days before a newly unlocked quadrant's
#                                  tiles are realistically staffed/watered/
#                                  producing at the assumed rate -- hands
#                                  don't reach the new quadrant and get
#                                  crops established on day 1 of unlock)
#   EFFECTIVE_DAYS = clamp(0, 30, (LIQUIDATE_DAY - day) - RAMP_DAYS)
#                                  (days of production left in the match
#                                  once ramp-up is subtracted, capped at a
#                                  30-day season per the spec)
#   EV = EV_PER_TILE_DAY * EFFECTIVE_DAYS * TILES_PER_QUAD - land_cost
#   HURDLE = EV_HURDLE_MULT (0.5) * land_cost
#            -- EV must clear land_cost by at least 50% margin, not just
#            break even, before a purchase is allowed. A pure break-even
#            EV is not "clearly beats a hurdle" -- it's a coin flip given
#            how rough EV_PER_TILE_DAY is.
#   Buy only if EV > HURDLE (in addition to the original day/budget
#   gates, which stay as a floor/ceiling, not replaced).
#
# --- Part C: the same purchase ALSO gated on the throttle forecast ---
#
# Even when EV clears its hurdle, land is skipped this turn if Part A's
# throttle is active -- more land under a footprint that's already about
# to overrun sell-through capacity would only let MORE new plantings
# start once the throttle lifts, working against the point of both
# mechanisms. This tests whether tying capital-allocation TIMING to a
# shed-capacity forecast (not just an isolated EV number) avoids growing
# into a bottleneck that a static EV estimate alone can't see (EV has no
# notion of what's ALREADY committed and about to land).
#
# Nothing else changes vs v10.6: animal purchases, seed targets, crop
# scheduling, dispatch, selling logic, hiring, and NEAR_SHED_RADIUS
# siting are all byte-identical.
#
# --- What could make this fail ---
#
# Two independent guesses stack here (EV_PER_TILE_DAY and the throttle
# forecast's own SELL_CAPACITY_PER_DAY) -- if either is miscalibrated,
# land purchases could be blocked too often (idle-tile forgone revenue,
# the exact failure mode v7.11's cost-of-delay land-scoring was built to
# avoid) or too rarely (a near-no-op vs v10.6, uninformative either way).
# =====================================================================
#
# =====================================================================
...
# --- v12.6 Part A: predictive shed-inflow forecast (rebuilt self-
# contained, identical formula to main_v12.5_predictive_throttle.py --
...
def _projected_daily_inflow(me, day, view):
    ...
    bonus assumed -- conservative). See file header."""
    inflow = 0.0
    for row in me["tiles"]:
        for t in row:
            if not (isinstance(t, dict) and t.get("kind") == "PLANT"):
...
# --- v12.6 Part B: simplified, self-contained EV land-purchase gate ---
EV_PER_TILE_DAY = 20.0
RAMP_DAYS = 6
MAX_EFFECTIVE_DAYS = 30
EV_HURDLE_MULT = 0.5   # EV must exceed 1.5x land_cost, not just break even


def _land_ev_clears_hurdle(day, land_cost):
    """Rough EV estimate for the next quadrant: EV_PER_TILE_DAY x
    (season days remaining after ramp-up) x TILES_PER_QUAD, minus the
    land cost itself, must clear a 50%-margin hurdle over land_cost. See
    file header for the full rationale on each constant."""
    effective_days = max(0, min(MAX_EFFECTIVE_DAYS, (LIQUIDATE_DAY - day) - RAMP_DAYS))
    ev = EV_PER_TILE_DAY * effective_days * TILES_PER_QUAD - land_cost
    return ev > EV_HURDLE_MULT * land_cost
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v12.6 Part A: predictive throttle, computed once per turn off the
    # FORECAST, not the live shed_load number.
    ...
    # --- Land expansion ---
    # v12.6 Parts B+C: original day/budget/quad gates stay as a floor, but
    # a purchase additionally needs (a) its own EV to clear a 50%-margin
    # hurdle over land_cost, AND (b) the predictive throttle to be OFF
    # this turn -- more land under an already-overproducing footprint
    # would only enable more new plantings once the throttle lifts.
    if 1 <= day <= 22 and not throttle:
        land_score = _land_priority(view)
        if quads == 1 and budget > 1000 and _land_ev_clears_hurdle(day, 1000):
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2000 and day <= 18 and _land_ev_clears_hurdle(day, 2000):
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 4000 and day <= 16 and _land_ev_clears_hurdle(day, 4000):
