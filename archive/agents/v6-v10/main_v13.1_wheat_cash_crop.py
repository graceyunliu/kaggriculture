...
# =====================================================================
# v13.1 (Aug 7) -- Round 9 glut-resistance hypothesis, built on champion
# v10.6. Market-math finding: WHEAT's MARKET_PARAMS above_target=0.20
# (log shape, T=400) means price barely moves even at heavy combined-
# market oversupply ($25 -> ~$19 at 2x T) -- unlike STRAWBERRY/MELON
# (above_target 1.60/3.60, linear/sq shape, T=100/300) which crash to the
# $1 floor past just 1x T of combined (both players') oversupply. Since
# v9.1, WHEAT planting has been fully removed from this codebase ("feed
# is market-bought") -- the `want` dict in economy() has no want["WHEAT"]
# entry, so WHEAT is never grown as a cash crop, only bought at spot
# price for feed. This reintroduces WHEAT as an actively-planted crop
# with a real sell-surplus target (same want[...]=min(space, ph0/ph1)
# pattern as MELON/STRAWBERRY), on the theory that WHEAT's glut-immunity
# makes it a low-risk way to convert otherwise-idle land/labor into
# steady, ~non-decaying revenue -- especially against a real opponent
# whose own production is also drawing down the shared market inventory
# number, which self-play vs v10.6 (which doesn't glut the market) can't
# surface.
#
# Siting: plantable_crops() already treats WHEAT correctly for this
# purpose without any change. WHEAT is a one-time crop (not in
# ONGOING_CROPS = {STRAWBERRY, TOMATO}), so plantable_crops() already
# sorts one-time crops to the FRONT of the priority list on FAR tiles and
# to the BACK on NEAR tiles (see its v10.5 docstring) -- i.e. WHEAT
# already gets far-from-shed siting priority, same as CARROT/MELON,
# because it only needs a handful of total visits (plant + occasional
# water during its short bonus window + one harvest) rather than the
# daily revisits STRAWBERRY/TOMATO need. That is still the right
# tradeoff even at meaningfully higher WHEAT volume: siting cost scales
# with (distance x visit count), and reintroducing WHEAT doesn't change
# its visit count, only how many tiles get planted with it. Decision:
# leave plantable_crops()/ONGOING_CROPS unchanged.
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v13.1: reintroduced as an actively-planted cash crop -- see file
    # header for the glut-resistance rationale. Surplus beyond feed needs
    # (JIT feed wheat block below, further down) sells at spot; WHEAT's
    # flat above_target=0.20 log-shape price curve makes that surplus
    # much less risky to grow at volume than STRAWBERRY/MELON.
