...
# =====================================================================
# MAIN V9.11 -- EXPERIMENT_MAX_FLEET 10 -> 12, on top of v9.10's
# economically-calibrated hiring fix. Base: main_v9.10_hire_calibrated.py.
# See EXPERIMENT_MAX_FLEET's own comment below for the rationale: this
# retests v9.6's rejected fleet-cap raise now that the real cause of
# labor scarcity (the hour==0 hiring bug) is fixed, using a fleet cap
# that matches the new, genuinely-reachable hand target (12) instead of
# jumping straight to the herd clone's 14-15.
#
# RESULT -- TESTED, REJECTED, decisively. 5-seed smoke, both-seats: v9.5
# wins 5/5, +$8,894/game (paired-sum/2), t=-8.40 -- not worth a 15-seed
# confirmation, this is unambiguous. Vs opp_scenario_v14: -$47,402/game,
# essentially identical to v9.10's own -$46,860 (no further compression
# at all). So raising the animal cap is harmful independent of whether
# labor capacity is the bottleneck -- even with hands genuinely able to
# reach 12/day (v9.10's fix, confirmed working), adding a 12th animal
# still costs more than it earns. Combined with v9.6's identical
# verdict on a different labor base, this closes EXPERIMENT_MAX_FLEET
# as a lever twice over, from two different angles (labor-constrained
# and labor-unconstrained) -- the fleet-size ceiling itself, not any
# support-system deficiency around it, is the real limit. Land/crop-
# space competition (each pasture displaces a crop tile permanently,
# regardless of hand count) is the more likely remaining explanation,
# not yet tested directly. NOT submitted.
# =====================================================================
...
# gates run unbounded."
EXPERIMENT_MAX_FLEET = 12  # v9.11: was 10. v9.1 iter C found 12+ animals
                            # saturated the hand pool, and v9.6 (same 10->14
                            # raise on the v9.5 base) reproduced that even on
                            # a modernized crop/labor system -- but v9.6's
                            # diagnosis showed real hand count never actually
                            # exceeded ~9-10 regardless of the animal cap
                            # (the hour==0 hiring bug fixed in v9.10 capped
                            # it there). With that fixed and HAND_TARGET_MAX
                            # raised to a genuinely reachable 12 (v9.10),
                            # retesting the fleet cap at the matching ratio
                            # (12 hands : 12 animals, same ~1:1 ratio v8.11
                            # validated at 10:10) rather than jumping
                            # straight to 14 to isolate this one variable.
