...
# =====================================================================
# v11.18 -- PINNED-QUADRANT CREWS (shot in the dark, on top of v10.6).
# Base chassis: main_v10.6_radius3.py, unmodified except for this one
# structural bet. Round-7 "shots in the dark" theme -- more speculative
# than the parallel shed-capacity/geometric-siting/capital-allocation
# hypothesis rounds.
#
# Mechanism: today, every hand that falls through to ordinary crop duty
# (farmer + hired hands alike) draws from ONE shared, globally-dispatched
# task pool via assign_tasks() -- a joint-nearest-pair matcher across the
# WHOLE board every turn. This hypothesis instead PERMANENTLY pins each
# worker SLOT (farmer = slot 0, hand i = slot i+1) to exactly one
# currently-unlocked quadrant, via slot_index % len(unlocked_quadrants)
# -- deterministic and stable for as long as the quadrant-unlock count
# doesn't change, and naturally reassigned on re-hire (a fired/rehired
# hand gets a new roster index -> new quadrant) per the hypothesis
# spec's leniency. Each turn, the crop-dispatch fallback (the block that
# used to call assign_tasks() ONCE across every idle unit + the full
# task-pool dict) instead groups idle units by their pinned quadrant,
# filters each of urgent_water/harvest/water/plant/weeds down to that
# quadrant's own tiles (via _quad_of_tile(), mirroring the engine's own
# _quadrant_of()), and calls assign_tasks() once PER quadrant group. No
# cross-quadrant crop dispatch. Animal maintenance/setup dispatch
# (animal_maintenance_action / animal_setup_action) is untouched -- it
# already targets nearest-pasture directly rather than drawing from a
# shared assign_tasks() pool, so quadrant-pinning it would be a second,
# separate change outside this hypothesis's scope.
#
# Rationale: mimics how a human player might compartmentalize attention
# per-quadrant instead of globally optimizing -- worth testing given the
# movement-time-share (~58-60%) is documented as the project's one
# remaining unclaimed structural lever, and per-quadrant pinning could
# either reduce cross-board walking (crews stay local) or hurt badly
# (losing the global joint-nearest-pair optimality that made v8's
# assign_tasks() a validated win over v7.11's per-unit dispatch).
# =====================================================================
...
def _quad_of_tile(pos, board_size=10):
    """v11.18: mirrors the engine's own _quadrant_of(x, y, board_size) --
    half = board_size // 2, N/S by y, W/E by x. Used to filter each
    quadrant-pinned crew's candidate task pool to its own tiles."""
    x, y = pos
    half = board_size // 2
...
def _agent(obs):
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        # v11.18 PINNED CREWS: instead of one joint assign_tasks() call
        # across the whole board, split idle units + the task pool by
        # each unit's PERMANENTLY pinned quadrant (slot index mod
        # unlocked-quadrant count -- farmer is slot 0, hand i is slot
        # i+1) and run assign_tasks() once per quadrant group, each
        # filtered to that quadrant's own tiles. No cross-quadrant
        # dispatch.
        unlocked_q = me["unlocked_quadrants"]
        nq = max(1, len(unlocked_q))
        units = []  # (pinned_quad, pos, is_farmer, hand_i)
        if farmer_needs_crop:
            units.append((unlocked_q[0 % nq], farmer_pos, True, None))
        for (i, pos) in hand_needs_crop:
            units.append((unlocked_q[(i + 1) % nq], pos, False, i))
        groups = {}
        for q, pos, is_farmer, hand_i in units:
            groups.setdefault(q, []).append((pos, is_farmer, hand_i))
        for q, members in groups.items():
            q_tasks = {key: [t for t in pool if _quad_of_tile(t) == q]
                       for key, pool in tasks.items()}
            q_positions = [m[0] for m in members]
            q_ops = assign_tasks(q_positions, q_tasks, seeds, day, task_order)
            # Write consumed tiles back into the shared pool (assign_tasks
            # mutated q_tasks in place, not the outer `tasks` dict, since
            # q_tasks holds fresh filtered lists).
            for key in tasks:
                remaining = set(q_tasks.get(key) or [])
                tasks[key] = [t for t in tasks[key]
                              if _quad_of_tile(t) != q or t in remaining]
            for op, (pos, is_farmer, hand_i) in zip(q_ops, members):
                if is_farmer:
                    farmer_op = op
                else:
                    hand_ops[hand_i] = op
