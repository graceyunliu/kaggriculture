...
# =====================================================================
# MAIN V8 CANDIDATE A -- FERTILIZE pipeline, ISOLATED. Base: v7.9,
# byte-identical except for: (1) the fertilizer sell block now reserves
# a buffer for application instead of selling 100%, (2) a new
# `fertilizable` list in perceive()'s view, (3) a new fertilize_action()
# that carries FERTILIZER from the shed to a STRAWBERRY/TOMATO plant and
# applies it, wired into the farmer/hand dispatch loop right after
# animal maintenance and before ordinary crop tasks. Every other v7.9
# rule (crop mix, hiring, land timing, market-order buckets, task
# priority) is untouched.
#
# --- Why ---
#
# v7.9 sells 100% of collected FERTILIZER (ANIMAL_PRODUCTS pipeline
# already collects it via COLLECT_FERTILIZER) into FERTILIZER, a product
# excluded from TOWN_CENTER_PRODUCTS with a measured ZERO town demand
# pool -- every unit sold permanently depresses its own price with no
# demand ever refilling it. Confirmed in engine source: FERTILIZE
# (op=="FERTILIZE" in _apply_unit_action) consumes 1 carried FERTILIZER
# and sets fertilized_until_day=day+2; a watered day within that window
# doubles yield accrual (_apply_unit_action WATER handler and
# _daily_refresh_plants both check `fertilized_until_day >= day`).
#
# For ongoing crops (TOMATO interval=1, STRAWBERRY interval=2), doubling
# accrual per watered/fertilized event doesn't raise the per-plant cap
# (both cap at max_yield=4) but it reaches that cap in FEWER interval
# steps, which sets max_lifespan_step earlier -- the tile frees up
# sooner for a replant. Over a fixed number of remaining match days this
# converts into MORE total units per tile-slot (a second planting cycle
# fits where only one did before), not more units per single plant.
#
# MELON is deliberately excluded (engine's window-math already lets it
# cap out unfertilized: watering window age 6-12 = 7 waterings vs
# max_yield=6). CARROT and WHEAT are also excluded here to keep this
# candidate isolated to the two ongoing premium crops the project's own
# demand-pool analysis flagged as having real headroom (STRAWBERRY 534,
# TOMATO 335) -- see Candidate B for a crop-mix change on top of this.
#
# --- The change ---
#
#  1. FERTILIZE_TARGETS = {STRAWBERRY, TOMATO}.
#  2. Fertilizer sell block: was unconditional `sell_orders.append(...)`
#     for the full shed amount; now only the amount above
#     FERT_SELL_RESERVE (a small buffer sized for in-transit demand) is
#     sold, so fertilizer accumulated beyond what hands can carry/apply
#     doesn't just rot in the shed either.
#  3. New fertilize_action(): mirrors animal_maintenance_action's
#     pickup/deliver pattern -- if carrying FERTILIZER, walk to the
#     nearest fertilizable tile and FERTILIZE; if not carrying but the
#     shed has some and a target exists, walk to a shed-adjacent tile
#     and PICKUP.
#  4. Wired into _agent(): checked for the farmer and every hand right
#     after animal-maintenance dispatch (which still takes priority --
#     animal survival before fertilizer optimization) and before the
#     ordinary crop task_order fallback.
#
# =====================================================================
#
# =====================================================================
...
# --- Candidate A: FERTILIZE pipeline ---
FERTILIZE_TARGETS = {"STRAWBERRY", "TOMATO"}
FERT_CARRY_TARGET = 3     # units picked up per shed trip
FERT_SELL_RESERVE = 12    # keep this much in shed for application backlog;
                            # only true overflow beyond it gets sold
FERT_MAX_HANDS_PER_TURN = 1  # v2: throttle -- see note at call site
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    fertilizable = []         # Candidate A: STRAWBERRY/TOMATO tiles worth a FERTILIZE
        ...
        for x, t in enumerate(row):
            ...
            if kind == "WEED":
                ...
                if c:
                    ...
                    if yu > 0 and ready:
                        harvest.append((x, y))
                    if (t.get("crop") in FERTILIZE_TARGETS
    ...
    return {
        ...
        "opp_imminent": imminent, "fertilizable": fertilizable,
    }
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Fertilizer: reserve for FERTILIZE application, sell only true
    # overflow beyond FERT_SELL_RESERVE (Candidate A -- was unconditional
    # full-shed sell into a zero-demand-pool product in v7.9). ---
    fert = shed.get("FERTILIZER", 0)
    if fert > FERT_SELL_RESERVE:
        sell_orders.append(["SELL", "FERTILIZER", fert - FERT_SELL_RESERVE])
...
# =====================================================================
# FERTILIZE PIPELINE (Candidate A)
# =====================================================================

def fertilize_action(pos, view, shed, carry, exclude=()):
    """Carry FERTILIZER from the shed to a STRAWBERRY/TOMATO plant and
    apply it. Mirrors animal_maintenance_action's pickup/deliver pattern.
    Returns (op_or_None, claimed_pos_or_None) -- callers should add the
    claimed position to their exclude set before calling this for the
    next worker, so two workers don't converge on the same tile."""
    targets = [t for t in view["fertilizable"] if t not in exclude]
    carry_fert = carry.get("FERTILIZER", 0)
    if carry_fert > 0 and targets:
        target = _nearest(pos, targets)
        if pos == target:
            return ["FERTILIZE"], target
        return [_step_toward(pos, target)], None
    if carry_fert == 0 and targets and shed.get("FERTILIZER", 0) > 0:
        if pos in CENTER_TILES:
            qty = min(FERT_CARRY_TARGET, shed.get("FERTILIZER", 0))
            return ["PICKUP", "FERTILIZER", qty], None
        return [_step_toward(pos, _nearest_center(pos))], None
    return None, None
...
def _agent(obs):
    ...
    claimed_fert = set()   # Candidate A: fertilize-target tiles claimed this turn
    farmer_op = None
    ...
    hand_ops = []
    fert_dispatched = [0]  # mutable counter, Candidate A v2/v3 throttle
    # v3: only allow fertilize dispatch when the farm is not already
    # showing watering/weed backlog -- v2 (unconditional 1-hand/turn cap)
    # still lost decisively (-$32,137 mean over 11 seeds) because a
    # steady trickle of one hand/turn diverted to shed-and-back trips
    # adds up to a real fraction of total labor over ~700 turns,
    # regardless of instantaneous cap. Gating on real farm health means
    # the diversion only happens when there is genuinely slack capacity.
    fert_safe = (len(view["urgent_water"]) == 0 and len(view["weeds"]) < WEED_PRIORITY_THRESHOLD)
    for i, h in enumerate(me["hands"]):
        ...
        if op is not None:
            ...
            # Candidate A v2: fertilize duty is deliberately throttled --
            # v1 (checked for every farmer/hand, unlimited) diverted
            # enough labor into PICKUP/walk/FERTILIZE logistics to let
            # weeds climb unchecked from day 14 on (measured: weeds 0->20
            # and never recovering, shed_load pinned near cap) and lost
            # -$50,711 mean over 11 seeds -- the same crop-neglect
            # failure class already documented for over-hiring and
            # tile-compaction experiments. v2 restricts it to: (a) never
            # the farmer, (b) never a protected/reserved crop hand, (c)
            # at most FERT_MAX_HANDS_PER_TURN hands total per turn.
            fop = None
            if fert_safe and (not is_reserved) and fert_dispatched[0] < FERT_MAX_HANDS_PER_TURN:
                fop, fclaim = fertilize_action(hand_pos, view, shed, hand_carry, claimed_fert)
                if fop is not None:
                    fert_dispatched[0] += 1
