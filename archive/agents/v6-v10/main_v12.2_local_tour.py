...
# =====================================================================
# MAIN V12.2 -- QUADRANT-LOCALITY CO-LOCATED DISPATCH, "the local tour"
# (Aug 7, round 8 on top of the undefeated main_v10.6_radius3.py
# champion). Combines the TWO pieces round 7 tested in isolation and
# both lost: v11.6 (pastures at quadrant-own-center, -$76,132/game) moved
# pasture siting away from the shed but left dispatch fully global, so
# the moved pasture just became a farther target for the same
# assign_tasks() joint-nearest-pair matching -- no compensating benefit,
# pure extra distance. v11.18 (permanently pinned per-quadrant crews,
# unchanged pasture siting, -$13,960/game) pinned workers away from
# global dispatch but the pastures those crews had to service were still
# sited near the shared shed, not inside their own quadrant -- pure loss
# of flexibility, no locality gain. Neither piece alone can pay for
# itself; this hypothesis is that they only pay off TOGETHER, since the
# whole point of a "local tour" is that a crew's existing in-quadrant
# crop walk passes directly by ITS OWN pasture at near-zero marginal
# distance, per the file's f x lifetime x distance framing (a visit
# folded into a route the worker is already running costs ~0 marginal
# distance vs. a dedicated round-trip costing full distance x2).
#
# MECHANISM (a) -- fixed hand crews, farmer stays flexible/overflow:
# `_hand_quadrant(hand_idx, unlocked)` maps hand ROSTER SLOT i to
# quadrant `unlocked[i % len(unlocked)]`, cycling through whichever
# quadrants are currently unlocked (engine order NW, then NE/SW/SE as
# they unlock -- see LAND_ORDER in kaggriculture.py). Hands are
# day-laborers: the engine wipes farm["hands"] to [] and hires_today to
# 0 every single night (`_end_of_day` in kaggriculture.py), so no
# literal worker OBJECT can persist overnight -- "permanent crew" here
# means the ASSIGNMENT RULE (slot i -> quadrant) is fixed for the whole
# game, giving each day's hand at slot i a consistent quadrant identity
# even though the physical hand is freshly hired each morning. The
# FARMER is deliberately left globally dispatched (unrestricted quadrant
# = None in the new `unit_quads` param) rather than also pinned, as a
# flexible/overflow unit -- chosen because v11.18's decisive loss was
# attributed specifically to losing assign_tasks()'s global joint-
# nearest-pair visibility for EVERY unit; keeping one globally-visible
# unit preserves at least a fraction of that benefit while still testing
# the locality hypothesis on the hand pool.
#
# MECHANISM (b) -- co-located pasture siting: `animal_setup_action()`
# gains an `owner_quad` parameter. When the assigned build-worker is a
# HAND (plan["worker"] >= 1), its owning crew's quadrant (via
# `_hand_quadrant`) restricts the pasture-site candidate pool to that
# quadrant first (falling back to the old global candidate pool only if
# the quadrant genuinely has no empty tile available, e.g. not yet
# unlocked or full) -- so a hand-built pasture lands inside the same
# quadrant that crew's ordinary crop tasks are already restricted to by
# mechanism (a), folding daily feed/care trips into the crew's existing
# local tour instead of requiring a separate cross-map walk. Within the
# quadrant-filtered pool, the existing near-shed-first tie-break
# (`min(..., key=_dist to nearest center)`) is kept, so the site is still
# as close to the shed as that quadrant allows. Farmer-built pastures
# (plan["worker"] == 0) keep the OLD unrestricted global near-shed
# siting (owner_quad=None) -- consistent with the farmer staying the
# flexible/overflow unit in mechanism (a).
# =====================================================================
...
def _quad_of_tile(pos):
    """v12.2: matches the engine's own quadrant split exactly
    (kaggriculture.py `_quadrant_of`: half = board_size // 2, board is
    10x10 -- half=5, "N"/"S" by y<5, "W"/"E" by x<5). CENTER_TILES sit
    right on this boundary, which is why they touch all four quadrants."""
    x, y = pos
    return ("N" if y < 5 else "S") + ("W" if x < 5 else "E")


def _hand_quadrant(hand_idx, unlocked):
    """v12.2: fixed crew-assignment RULE -- hand roster slot `hand_idx`
    always answers for the same quadrant identity, cycling through
    currently-unlocked quadrants in engine unlock order (unlocked[0] is
    always "NW"; later entries follow LAND_ORDER as they unlock). Hands
    are day-laborers (engine wipes farm["hands"]=[] every night -- see
    file header), so this is a fixed RULE across the whole game, not a
    literal worker identity persisting overnight. Returns None if no
    quadrant is unlocked yet (shouldn't happen in practice -- NW is
    always unlocked from turn 0)."""
    if not unlocked:
        return None
    return unlocked[hand_idx % len(unlocked)]
...
def animal_setup_action(pos, view, shed, farmer_carry, plan, day, hour, reserved_sites=(), exclude_sites=(),
    ...
    v12.2: `owner_quad` (new) -- when given (a hand-owned build; farmer
    builds always pass None, see file header mechanism (b)), restricts
    the pasture-site candidate pool to that quadrant first, so a hand's
    new pasture lands inside the same quadrant its ordinary crop tasks
    are already restricted to by `_hand_quadrant()` -- folding daily
    feed/care trips into that crew's existing local tour. Falls back to
    the unrestricted candidate pool if the quadrant has no empty tile
    available (not yet unlocked, or genuinely full) so a build can never
    hard-deadlock for want of quadrant-local land."""
    species = plan["species"]
    stage = plan["stage"]
    ...
    if site is None or (site not in view["empty"] and site not in view["empty_pastures"]):
        ...
        if not candidates:
            return None
        # v12.2: co-locate a hand-owned pasture within its crew's quadrant.
        if owner_quad is not None:
            local = [t for t in candidates if _quad_of_tile(t) == owner_quad]
            if local:
                candidates = local
...
def assign_tasks(unit_positions, tasks, seeds, day, task_order, unit_quads=None):
    ...
    rank" rule. Unrestricted by `unit_quads` -- a unit already standing
    on a task costs zero extra movement, so there's no locality reason to
    refuse the freebie.
    ...
    v12.2: `unit_quads` (new, optional) -- same length as unit_positions,
    each entry either a quadrant id string (restrict that unit's Pass-2
    candidate tiles to its own quadrant, mechanism (a) in the file
    header) or None (unrestricted/global -- the farmer). Default None
    (or an all-None list) reproduces v10.6's exact unrestricted behavior.
    A quadrant-restricted unit that finds no matching tile in a tier
    simply stays unassigned for that tier and tries the next one, same
    as any unit that loses a Pass-2 race today -- falls through to the
    existing idle-drift-toward-shed fallback if never matched at all.

    Returns a list of ops, same length and order as unit_positions.
    """
    n = len(unit_positions)
    if unit_quads is None:
        unit_quads = [None] * n
    ...
    for key in task_order:
        ...
        for i in unassigned:
            ...
            q = unit_quads[i]
            for t in pool:
                if q is not None and _quad_of_tile(t) != q:
                    continue
...
def _agent(obs):
    ...
    for i, h in enumerate(me["hands"]):
        ...
        if pl is not None and not urgent_existing_feed:
            # v12.2: this build's owning crew is hand slot i -- site its
            # pasture inside that crew's assigned quadrant (mechanism (b)).
            op = animal_setup_action(hand_pos, view, shed, hand_carry, pl, day, hour, sites,
                                     exclude_sites=taken_sites - {pl.get("site")},
                                     owner_quad=_hand_quadrant(i, me["unlocked_quadrants"]))
            if op is not None:
                hand_ops[i] = op
                continue
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        ...
        # v12.2: farmer (if present) stays unrestricted/global (None);
        # each hand is restricted to its own fixed crew quadrant.
        crop_unit_quads = (([None] if farmer_needs_crop else []) +
                            [_hand_quadrant(i, me["unlocked_quadrants"]) for i, _pos in hand_needs_crop])
        crop_ops = assign_tasks(crop_unit_positions, tasks, seeds, day, task_order, unit_quads=crop_unit_quads)
        k = 0
