...
def _shed_dist(pos):
    """Manhattan distance to the nearest shed-access tile (hand spawn point)."""
    return min(_dist(pos, c) for c in CENTER_TILES)
...
def _agent(obs):
    ...
    tasks = {
        ...
        # v7.10d: plant COMPACTLY. Hands are wiped nightly and respawn at the
        # shed, so every scattered tile costs a fresh commute each morning.
        # 66% of all unit-turns in a v7.9 match are movement. Offer the
        # planter the empties CLOSEST to the shed first, so the farm stays a
        # tight blob instead of spreading to the map corners.
        "plant": (sorted(view["empty"], key=_shed_dist)[:n_plantable]
                  if crops_now and hour < 21 else []),
