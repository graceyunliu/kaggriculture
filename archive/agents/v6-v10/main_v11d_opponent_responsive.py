# =====================================================================
# MAIN V11D -- OPPONENT-RESPONSIVE ECONOMY
#
# Economy reset on the v10.6 execution chassis.
# Goal: allocate crops and animal species against the shared market state instead of a
# fixed portfolio. Scarce/high-priced products receive more capacity; glutted products
# lose capacity. Land and hiring follow committed work rather than empty acreage.
# =====================================================================
...
#   4. Hard EXPERIMENT_MAX_FLEET = 12 cap. Unchanged from v8.10.
...
def _market_desirability(obs, item):
    inv = obs.get("market", {}).get("inventory", {}).get(item, I0)
    price = live_price(obs, item)
    base = MARKET_PARAMS[item]["base"]
    scarcity = max(-1.0, min(1.0, (I0 - inv) / max(1.0, MARKET_PARAMS[item]["T"])))
    return (price / max(1.0, base)) + 0.35 * scarcity

def _responsive_species(obs):
    milk = _market_desirability(obs, "MILK")
    wool = _market_desirability(obs, "WOOL")
    return "SHEEP" if wool > milk * 1.05 else "COW"
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion: shared-market planner buys capacity, not acreage. ---
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is None:
            ...
            preferred = "SHEEP" if composition_wants_sheep else _responsive_species(obs)
            if preferred == "SHEEP" and SHEEP_ENABLED and _sheep_expansion_feasible(
                    obs, view, budget, shed, n_animals, len(me["hands"]), quads, day):
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            elif preferred == "COW" and _animal_expansion_feasible(
                    obs, view, budget, shed, n_animals, len(me["hands"]), quads, day):
    ...
    # v11D: shared-market crop allocation. Targets move with current
    # scarcity/price rather than staying fixed all season.
    want = {}
    candidates = ["STRAWBERRY", "MELON", "TOMATO", "CARROT"]
    scores = {c: _market_desirability(obs, c) for c in candidates if day <= CUTOFF[c]}
    for c, score in sorted(scores.items(), key=lambda kv: -kv[1])[:3]:
        if score < 0.85:
            continue
        if c == "STRAWBERRY":
            cap = 16
            ...
            cap = 5
        want[c] = min(space, max(2, int(cap * min(1.5, score))))
