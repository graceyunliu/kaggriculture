...
# =====================================================================
# V11.17 -- TOWN-SHOP-DEMAND-AWARE SELL PRIORITY on top of v10.6
# (round-7 selling-rhythm theme). Aggregate replay data flagged
# SHED_AT_CAP as the dominant loss cause (31/100 losses). Mechanism
# (engine source: kaggriculture.py's SHOPS dict + _town_consume): every
# `townShopSellInterval` steps, each unlocked town shop passively drains
# fixed units from specific products' market inventory (raising their
# price via the same inventory-vs-price curve a real sale would move).
# Hypothesis: products with an active shop drain are under elevated
# near-term demand/price pressure -- selling into that pressure (and
# proactively clearing shed space for that product before its next
# drain cycle) should beat the champion's pure live-price ranking. This
# variant reads obs["town"]["unlocked_shops"], builds the set of
# products any unlocked shop currently drains (via a hardcoded copy of
# the engine's SHOPS mapping), and re-sorts the sellable loop to put
# shop-drained products first, breaking ties by the existing
# -live_price ranking. All the existing per-item quantity/floor/
# force-dump logic is unchanged -- only the iteration order changes.
# =====================================================================
...
I0 = 10_000
SHED_CAP = 100
# v11.17: hardcoded copy of the engine's SHOPS mapping (kaggriculture.py)
# -- which products each unlocked town shop passively drains.
SHOPS = {
    "BAKERY":         ["EGG", "WHEAT"],
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v11.17: products any currently-unlocked town shop is actively
    # draining -- prioritized ahead of pure live-price ranking below.
    shop_drained = set()
    for _shop in (obs.get("town", {}) or {}).get("unlocked_shops", []):
        shop_drained.update(SHOPS.get(_shop, []))
    sellable = []
    ...
    for item, n in sorted(sellable, key=lambda kv: (0 if kv[0] in shop_drained else 1, -live_price(obs, kv[0]))):
