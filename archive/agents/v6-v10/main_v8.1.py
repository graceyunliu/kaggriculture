...
# =====================================================================
# MAIN V8.1 -- FERTILIZE on WHEAT only, ISOLATED. Base: main_v8.py,
# unchanged except for: (1) a FERTILIZER_RESERVE kept in the shed instead
# of selling 100% of it, (2) a fertilize-then-water pipeline for WHEAT
# tiles (fertilize_action(), plus perceive()'s new fert_wheat/
# fert_wheat_age tracking), and (3) _agent()'s dispatch loop giving each
# unit a chance at fertilize_action() before it falls through to ordinary
# crop duty. want["WHEAT"] is left at v8's value of 8 -- this isolates
# the fertilize effect alone, per the single-variable discipline in
# kaggriculture-v8-architecture.md section 7 (never stack an untested
# economic lever on top of another untested one).
#
# Ported from main_v8_candidateD.py (which tested this same mechanism,
# isolated, on the v7.9 base -- but was built with instrumentation and
# never run to a verdict, per kaggriculture-v8-architecture.md section
# 5.2/8 step 4) onto the current, better-validated main_v8.py base
# (v7.11 + global task assignment, 15/15 seeded wins, see main_v8.py's
# own header). GLOBAL_TASK_ASSIGNMENT is untouched by this file: the
# fertilize check runs BEFORE a unit is routed into assign_tasks()'s
# batched crop-duty pool, exactly the same "priority over ordinary crop
# work when otherwise free" placement Candidate D used ahead of
# unit_action() on the v7.9/v7.11 dispatch loop.
#
# --- Why WHEAT and not STRAWBERRY (a prior, separately-tested round) ---
#
# WHEAT is `"ongoing": False`. Non-ongoing crops accrue yield through the
# WATER action handler, which on each watering day inside the window
# `[(max_yield_day+1)//2, max_yield_day]` (by age in days since
# planting) adds `2 if fertilized_until_day >= day else 1` to
# yield_units, capped at max_yield. For WHEAT: max_yield_day=4, so the
...
# town demand pool (the largest single demand pool in the game).
# STRAWBERRY/TOMATO are `"ongoing": True` and accrue yield via a
# different, calendar-only path capped by `min(max_yield, ...)`
# regardless of fertilizer -- fertilizing an ongoing crop only reaches
# the same ceiling faster, never higher, and this was already tested and
# rejected (Candidate A, -$32k to -$50k mean/game). Not repeated here.
#
# --- The catch: timing ---
#
...
# one FERTILIZER unit -- the efficient case. This candidate targets
# WHEAT tiles at age <= 2 specifically (WHEAT_FERT_MAX_AGE below).
#
# --- Mechanics, not races ---
#
# `_daily_refresh_plants` turns a PLANT tile into a WEED if
# `consecutive_unwatered` reaches 2 -- skipping watering entirely for a
# tile to "wait" for fertilizer would risk killing it. This file does
# NOT remove fertilize-pending WHEAT tiles from the normal water task
# pool; fertilize_action() only gets first crack at a free unit ahead of
# the batched crop-duty assignment, so on a day it doesn't win the race
# the tile still gets watered on schedule via assign_tasks() (just
# without that day's bonus) -- a same-as-v8 fallback, never worse.
#
# --- Test protocol ---
#
#   Seeded --both-seats vs main_v8.py, >=8 seeds, one variable only
#   (this file vs its exact parent). Mechanism check: FERTTRACE-style
#   fertilize_ops / age-hit distribution, plus wheat units sold and
#   average realized WHEAT price, alongside the money result.
# =====================================================================
#
# =====================================================================
...
# --- v8.1: WHEAT fertilize pipeline ---
FERTILIZER_RESERVE = 8      # keep this many FERTILIZER in shed for wheat use;
                              # the rest still gets sold every turn like v8
...
# --- instrumentation (v8.1) -- mechanism trace, not behavior ---
FERT_STATS = {
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    fert_wheat = []            # v8.1: WHEAT tiles eligible for FERTILIZE
    fert_wheat_age = {}        # v8.1: pos -> age, for instrumentation
        ...
        for x, t in enumerate(row):
            ...
            if kind == "WEED":
                ...
                if c:
                    ...
                    if yu > 0 and ready:
                        harvest.append((x, y))
                # v8.1: WHEAT fertilize eligibility -- age <= 2, not already
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["CARROT"]:
        want["CARROT"] = 4
    if day <= CUTOFF["WHEAT"]:
        want["WHEAT"] = 8   # v8.1: UNCHANGED from v8 -- isolate fertilize effect alone
    ...
    # v8.1: was "sell everything"; now keep FERTILIZER_RESERVE units in the
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        if qty > 0:
            ...
            if item == "WHEAT":  # v8.1 instrumentation
    ...
    # v8.1: mechanism trace, printed every turn from day 29 onward -- the
    # engine's final input observation ever fed back in tops out at
    # day=29, hour=22 (empirically confirmed by main_v8_candidateD.py),
    # so there's no single reliable "last call" to gate a one-shot print
    # on. Take the LAST such stderr line per game (grep) for the true
    # final, fully-accumulated counters.
    if day >= 29:
...
# =====================================================================
# FERTILIZE PIPELINE (v8.1) -- proactively fetches FERTILIZER from the
# shed and applies it to WHEAT tiles in view["fert_wheat"] before their
# yield-bonus window closes. Mirrors the PICKUP-then-carry pattern
...
def _agent(obs):
    ...
    if farmer_op is None and active_setup_plan is not None:
        ...
        farmer_op = animal_setup_action(farmer_pos, view, shed, farmer_carry, active_setup_plan, day, hour, sites)
    if farmer_op is None:
        # v8.1: fertilize-wheat gets priority over ordinary crop work
        # (ahead of the batched global crop-task assignment) when the
        # farmer has nothing more urgent to do.
    ...
    for i, h in enumerate(me["hands"]):
        ...
        if op is not None:
            ...
            if claim is not None:
                claimed_pastures.add(claim)
            continue
        # v8.1: fertilize-wheat before falling to ordinary crop tasks.
