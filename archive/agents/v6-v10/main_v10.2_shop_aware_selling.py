...
# =====================================================================
# MAIN V9.22 -- more concurrent animal-build slots (3->6) + v9.21's
# multi-order-per-turn purchase loop, on top of v9.12 (melon fertilize-
# in-window on v9.10's hire fix).
#
# Hypothesis (opening-sequence-tempo investigation, per Grace's
# selection after three tuning categories -- volume/scale, task-
# dispatch efficiency, capital-timing -- all closed out at v9.6-v9.20
# without beating v9.10/v9.12's +$4,860/game): built a one-off census
# script (fleet_census.py, per-day animal/hand/money trace, kept in
# project root) and ran our v9.12 vs Opponents/opp_scenario_v14.py on
# seed 1. Found the real driver of the day8-20 stall directly, not
# guessed: our fleet sat at 2 animals for the ENTIRE first 8 days (day1
# through day8, flat), then crawled 3(d9)->4(d11)->6(d12)->9(d14)->
# 10(d16). The clone went 4(d1)->6(d8)->10(d9)->11(d10)->15(d12) -- it
# gains 4 animals between day8 and day9 ALONE. Reading opp_scenario_v14
# directly explains why: its BUY_ANIMAL orders carry qty up to 2 (buys
# 2 COW + 2 SHEEP = 4 in one turn if budget allows), and purchase is
# completely DECOUPLED from placement -- animals sit in the shed as
# stock, claimed by whichever worker is free that turn via the same
# job-scoring dispatch used for crops, no worker pre-pinned at
# purchase time. Our v9.x architecture instead pins each animal
# purchase to a specific worker BUILD SLOT (MAX_CONCURRENT_BUILDS=3,
# farmer + 2 hands) at the moment of ordering, and previously submitted
# only ONE new order per turn regardless of how many slots were free --
# so filling all 3 slots took 3 turns minimum even with money sitting
# idle.
#
# This build does NOT adopt the opponent's decoupled architecture (too
# large a rewrite this round) -- it keeps every existing safety property
# (worker-slot pinning, the same feasibility gates, MAX_CONCURRENT_
# BUILDS=3 unchanged) and only removes the ARTIFICIAL one-order-per-turn
# throttle: the purchase block now loops, submitting additional orders
# in the same turn as long as build slots remain AND the same
# _animal_expansion_feasible/_sheep_expansion_feasible gates as before
# still pass. Both gates gained a `pending` parameter (animals already
# approved this same turn, including the one being evaluated) so the
# hand-capacity projection and wheat-affordability checks account for
# the cumulative same-turn load instead of silently under-counting a
# burst of approvals with a flat "+1".
#
# v9.21's OWN result (kept for reference, not re-derived here): letting
# purchase ORDERS fill v9.21's still-3-slot ceiling in one turn instead
# of one order/turn was a real regression (-$49,102/game vs the clone,
# worse than v9.10/v9.12's -$46,860 baseline) -- it front-loaded 3x the
# cash outlay into turn one without shortening the walk-cycle at all
# (same 3 physical slots), starving wheat/seed spending for no gain.
#
# v9.22 targets the walk-cycle THROUGHPUT directly instead: raises
# MAX_CONCURRENT_BUILDS from 3 to 6. That constant was tuned back when
# hand count was thin (pre-v9.10, "farmer + first two hands") and caps
# WHICH slots can ever be pinned to an animal build regardless of how
# many hands exist -- since v9.10's hire fix reliably reaches
# HAND_TARGET_MAX=12 hands, slots 3-5 (hands 3-5) were sitting
# permanently ineligible to help fetch/place animals no matter how idle
# they were. More parallel build slots means more workers can be
# walking/carrying animals to their sites AT ONCE, which is the actual
# lever the census pointed at (cycle time, not order rate) -- unlike
# v9.11/v9.14 (which raised the FINAL fleet target and reproduced crop-
# labor cannibalization), this doesn't touch EXPERIMENT_MAX_FLEET=10 at
# all, only how many workers can be mid-build simultaneously during the
# ramp. v9.21's multi-order-per-turn purchase loop is kept (now
# meaningful with more slots to fill, where it was previously capped at
# the same 3-slot ceiling regardless).
#
# RESULT -- TESTED, REJECTED, AND THE UNDERLYING HYPOTHESIS DISPROVEN.
# 5-seed smoke, both-seats: vs v9.5, wash-negative (-$4,598/game, t=-1.77,
# 1/5 wins). Vs opp_scenario_v14: -$51,534/game, WORSE than both v9.10/
# v9.12's -$46,860 baseline AND v9.21's own -$49,102 (raising build slots
# made it slightly worse still, not better). Fleet census on the same
# seed used for v9.21 came back essentially BYTE-IDENTICAL for the
# critical days 1-8 (fleet still 3 animals flat through day8) -- raising
# MAX_CONCURRENT_BUILDS from 3 to 6 changed NOTHING in the early window.
#
# Instrumented `_animal_expansion_feasible` directly (temporary debug
# build, not kept) to log which of its 5 checks actually fails each
# call during days 0-9. Result: worker-slot/build-concurrency was NEVER
# the binding constraint -- the dominant failures are `cropspace` (92
# occurrences, `available_for_crops` frequently negative: with only one
# quadrant unlocked early, near-shed land is genuinely scarce) and
# `wheat` (74 occurrences, feeding one more animal costs too large a
# share of the still-thin early budget). `handcap` (worker-slot-
# adjacent) fired exactly ONCE in the entire days-0-9 window. So the
# real early-game constraint was never about how many workers can walk
# an animal to its site -- it's that buying more animals genuinely
# competes for the same scarce early land and cash that crop
# establishment needs, and the existing gates are correctly refusing
# once that tradeoff turns negative.
#
# This exact lever -- relaxing/bypassing those gates for the first few
# animals to buy a faster herd -- turns out to be DOCUMENTED PRIOR ART
# in this same file's own header (see the FERT_SHED_RESERVE comment,
# "iter E tried bypassing the crop-space/feed-frac gates for the first
# 6 animals through day 8: fleet completed day 11 but money collapsed
# -$11.7k on seed 1 -- early pastures+capital cannibalize early crop
# establishment. The gates are correct; REVERTED.") -- an earlier round
# of this same project already tried exactly this and got the same
# verdict this instrumentation independently reconfirms.
#
# CONCLUSION: the census's original finding (opponent buys/places
# animals dramatically faster) is real, but the CAUSE isn't a fixable
# dispatch/architecture inefficiency on our side -- it's that the clone
# accepts a fundamentally more aggressive early-game risk posture
# (bigger herd sooner, tolerating more crop-establishment sacrifice)
# that our agent's gates correctly refuse because, on our own crop/
# economy tuning, that tradeoff has been directly tested and loses
# money. Copying the clone's purchase RATE without also copying
# whatever makes ITS crop economy tolerate the sacrifice (different
# crop mix, different land allocation, different overall strategy) is
# not a safe or available lever via any change scoped this session.
# NOT submitted. Two failed sub-hypotheses (v9.21, v9.22) plus this
# gate diagnostic now close out the opening-sequence-tempo category.
# =====================================================================

# =====================================================================
# MAIN V9.10 -- economically-calibrated hiring, on top of v9.5. Base:
# main_v9.5_statehygiene.py.
#
# v9.9 established two things by instrumenting the hire block directly
# against Opponents/opp_scenario_v14.py: (1) hands reset to empty every
# engine midnight, and our hire block only fired at hour==0, so it was
# hard-capped at MAX_MARKET_ORDERS=10 new hires/day no matter how high
# the formula's target climbed -- extending to hour<=2 (matching the
# opponent's own gating exactly) lets the fleet actually reach the
# formula's target; (2) but HAND_TARGET_MAX=18 turned out to be
# economically ruinous once reachable, because reaching it costs the
# FULL Fibonacci sum fresh every single day (sum(fib(0..17))=$6,764/day
# -- vs $376/day for 12, $143/day for 10). HAND_TARGET_MAX=18 was never
# actually calibrated against this cost curve; it was simply unreachable
# before, so nothing ever tested whether it was worth its real price.
#
# This build keeps v9.9's hour<=2 + real-hires_today-cost-index fix
# (both needed for hiring to behave correctly at all past a single
# hour) but lowers HAND_TARGET_MAX 18->12, matching the herd clone's
# own MAX_HANDS=12 -- landing at the cheap, clearly-profitable end of
# the cost curve instead of the expensive end. Hypothesis: this should
# let real hand count grow from our previous ~9-10 plateau to a
# sustainable ~12, which should also relieve the animal-expansion
# gate's HAND_CAPACITY check (v9.6's diagnosis) enough to let the fleet
# grow past 9-10 toward the herd clone's 14-15, without v9.6's failure
# mode (that build raised the animal cap directly with no matching
# labor increase) or v9.9's failure mode (labor increase with no
# economic ceiling).
#
# RESULT -- TESTED, WASH vs v9.5, REAL BUT PARTIAL COMPRESSION vs the
# herd clone. Gate vs v9.5, both-seats, 15 seeds (same set as v9.6/7):
# v9.5 wins 9/15, +$1,392/game (paired-sum/2), t=+1.27, 95% CI
# [-$764,+$3,549] -- includes zero, NOT a statistically real regression
# (the 5-seed smoke's -$4,032/game was noise, not signal). Vs
# opp_scenario_v14, 15 seeds: -$46,860/game, vs v9.5's own established
# -$51.7k baseline -- a real +$4,860/game compression of the top-tier
# gap, but short of a promotion-worthy margin and well short of the
# $10k bar. NOT submitted, but NOT rejected either -- kept as a base to
# build on (see v9.11): EXPERIMENT_MAX_FLEET is still hardcoded at 10
# in this file (unchanged from v9.5), so the animal fleet still can't
# grow past 10 even though real hand capacity is now genuinely higher
# (up to 12/day, actually reachable, unlike v9.6's attempt where the
# hour==0 bug meant hands never actually exceeded ~9-10 regardless of
# any animal-cap setting). Re-testing the fleet-cap raise now that the
# real labor constraint is fixed is the obvious next step.
# =====================================================================

# =====================================================================
# MAIN V9.5 -- STATE-HYGIENE pass on v9.3fix2 (review round 3; no
# strategy change). Base: main_v9.3_fertilize.py @ fa30a54.
#   1. ORDERED no longer inflates feed buying: economy() splits the
#      fleet count into n_animals (gate/composition ceilings, ORDERED-
#      inclusive -- an in-flight order must hold its place against the
#      fleet cap) and n_support (ACTIVE+BOUGHT+CARRYING only) used for
#      the wheat purchase target and the feed sale reserve. Aligns the
#      code with what the v9.3fix comment already claimed.
#   2. Plan-state reset hygiene: expired ORDERED plans are fully reset
#      (_reset_plan: site/worker/stage_turn/retries cleared); reusing a
#      NONE plan as a new order also clears stale site/worker; and
#      _fail_stage falling back to NONE clears site/worker too.
#      EXPLICIT DECISION on sites: clearing a stale site does not orphan
#      an already-built pasture, because setup_action's site selection
#      globally prefers view["empty_pastures"] -- any future plan finds
#      the built pasture first anyway. What clearing FIXES: a NONE plan
#      with a stale (unbuilt) site kept a plantable crop tile excluded
#      via claimed_sites forever once purchase windows closed.
#   3. Reviewer's "CARRYING one-turn orphan delay" point: verified
#      ALREADY FIXED by v9.3fix2's pass reorder (reconcile runs first,
#      so a stranded CARRYING plan falls to BOUGHT and is re-pinned in
#      the SAME turn's hygiene pass). No change needed.
#
...
RAMP_DAY = 10   # v10: days < RAMP_DAY are pure-infrastructure (land +
                # animals only, no crop seed spend at all -- see seed
                # pipeline comment in economy()). During the ramp, the
                # wheat-feed-budget gate is loosened since it's no longer
                # competing with seed purchases for the same cash.
RAMP_WHEAT_BUDGET_FRAC = 0.80
...
MAX_CONCURRENT_BUILDS = 6   # v9.2: was 3 ("farmer + first two hands"),
                              # tuned back when hand count was thin. v9.22:
                              # raised now that v9.10's hire fix reliably
                              # gets hand count to HAND_TARGET_MAX=12 --
                              # the old cap of 3 meant only slots 0-2 (farmer
                              # + first 2 hands) could EVER be pinned to an
                              # animal build, no matter how many more hands
                              # existed. v9.21's census found the real
                              # bottleneck is walk-cycle THROUGHPUT (how many
                              # workers can be walking/carrying animals in
                              # parallel), not purchase-order rate -- this
                              # directly targets that by letting more of the
                              # existing hand pool help.
...
HAND_TARGET_MIN = 4
HAND_TARGET_MAX = 12  # v9.10: was 18 -- see file header. Hands reset to empty
                       # every engine midnight, so reaching a daily target
                       # costs sum(fib(0..target-1)) fresh EVERY day:
                       # 10->$143, 12->$376, 14->$986, 16->$2,583, 18->$6,764.
                       # v9.9 proved 18 is reachable once hiring isn't capped
                       # to a single hour, but ruinously expensive -- 12
                       # matches Opponents/opp_scenario_v14.py's own
                       # MAX_HANDS=12 and sits at the cheap end of the curve.
...
# v10.2 hypothesis: town shops are a PERMANENT extra buyer once unlocked
# (drains inventory of their linked products every townShopSellInterval
# turns, forever after) -- real absorption capacity for a shop-backed
# item is higher than the static MARKET_PARAMS["T"] models, but no
# version in this lineage reads obs["town"]["unlocked_shops"] anywhere
# (confirmed by grep -- genuinely untested lever, flagged but never
# built per project notes). Sell floor should relax a bit per backing
# shop actually unlocked right now, so we capture more of a real,
# currently-active demand pool instead of holding back at a floor tuned
# for the average/static case. Verbatim from vendor/kaggriculture.py's
# SHOPS dict so shop names match obs["town"]["unlocked_shops"] exactly.
SHOP_PRODUCTS = {
    "BAKERY":         ["EGG", "WHEAT"],
...
SHOP_FRAC_RELIEF = 0.05   # floor relief per unlocked shop backing this item
SHOP_FRAC_RELIEF_CAP = 0.20  # never relax more than 4 shops' worth


def _shop_backed_relief(item, unlocked_shops):
    n = sum(1 for shop in unlocked_shops if item in SHOP_PRODUCTS.get(shop, ()))
    return min(SHOP_FRAC_RELIEF_CAP, SHOP_FRAC_RELIEF * n)
...
def perceive(me, opp, day):
    ...
    # v9.12: MELON added -- a DIFFERENT real-engine mechanic (WATER
    # handler, kaggriculture.py ~374-386, not _daily_refresh_plants):
    # non-ongoing crops get a yield_units bonus on each watering during
    # the back half of their growth window (age in [(max_day+1)//2,
    # max_day]) -- +1 normally, +2 if the tile is fertilized that day,
    # capped at max_yield. Previously flagged as the next small lever
    # ("they do it, we don't") after v9.3's fertilize compression; only
    # MELON gets this (the other non-ongoing crops -- WHEAT/CARROT --
    # are too low-value to justify diverting fertilizer-carrying labor).
    fert_targets = []
    for y, row in enumerate(me["tiles"]):
        for x, t in enumerate(row):
            if not (isinstance(t, dict) and t.get("kind") == "PLANT"):
                continue
            c = CROPS.get(t.get("crop"))
            if not c or t.get("fertilized_until_day", -1) >= day:
                continue
            age = day - t.get("planted_day", day)
            if c["ongoing"]:
                ...
                if (window_start <= age <= c["max_day"]
                        and t.get("yield_units", 0) < c["max_yield"]):
                    fert_targets.append((x, y))
...
def plantable_crops(seeds, day):
    return [c for c in plant_priority_current if seeds.get(c, 0) > 0 and day <= CUTOFF.get(c, -1)]
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99, pending=1):
    ...
    v9.21: `pending` is how many animals (including this one) have already
    been approved THIS SAME TURN -- callers evaluating a 2nd/3rd purchase
    in one turn (see the multi-order purchase loop below) pass a running
    count so the hand-capacity/wheat-budget projections account for the
    cumulative load, not a flat "+1" that would silently under-count a
    burst of same-turn approvals.
    """
    ...
    projected_work = crop_work + (animal_work + pending) * ANIMAL_WORK_WEIGHT
    ...
    available_for_crops = len(view["empty"]) - len(reserved_sites) - pending
...
def _sheep_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99, pending=1):
    ...
    the same EXPERIMENT_MAX_FLEET ceiling as COW's gate. v9.21: `pending`
    -- see _animal_expansion_feasible's docstring, same cumulative-load
    accounting for same-turn bursts."""
    if n_animals >= EXPERIMENT_MAX_FLEET:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # NOT toward support sizing -- an unconfirmed animal needs no feed.
    # v9.5: that exclusion is now actually implemented via n_support
    # (the v9.3fix comment claimed it while n_animals still fed the
    # wheat target and feed reserve below). Wheat floor / service hands
    # in _agent() already used their own BOUGHT/CARRYING-only counts.
    n_animals = (len(view["my_pastures"]) +
                 sum(1 for p in animal_plans if p["stage"] in ("ORDERED", "BOUGHT", "CARRYING")))
    n_support = (len(view["my_pastures"]) +
    ...
    # v9.21: fill ALL remaining build slots in one turn instead of
    # submitting at most one new order per turn. Diagnostic census (single
    # seed, our v9.12 vs Opponents/opp_scenario_v14.py) found the real
    # driver of the day8-20 tempo gap: our fleet sat at 2 animals for the
    # entire first 8 days (one new order per turn, itself gated on a free
    # build slot -- 3 turns just to fill 3 concurrent slots), while the
    # clone went 4 (day1) -> 6 (day8) -> 10 (day9) -> 15 (day12), because
    # its BUY_ANIMAL orders carry qty up to 2 and are never gated on
    # worker-slot availability at all (placement is a separate, decoupled
    # dispatch). This keeps our worker-slot-pinned architecture (each
    # in-flight plan still needs its own build slot to walk+place, still
    # capped at MAX_CONCURRENT_BUILDS=3, unchanged) but stops throttling
    # the PURCHASE side to one order/turn -- loops until either the slot
    # ceiling or the SAME feasibility gates as before (now cumulative-load
    # aware via `pending`, see the gate docstrings) says no.
    pending = 0
    while n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1; v9.21: loop
        pending += 1
        ...
        if target_plan is None:
            ...
            if (composition_wants_sheep and
                    _sheep_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day, pending=pending)):
            ...
            elif _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day, pending=pending):
                target_plan = {"species": "COW", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            elif SHEEP_ENABLED and _sheep_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day, pending=pending):
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
        if target_plan is None:
            break  # v9.21: neither gate passed this round -- stop the loop
        ...
        if not (spec["start_day"] <= day <= spec["last_day"]
                and unclaimed_in_shed <= 0
                and budget > spec["min_money"]):
            break  # v9.21: this round's candidate isn't buyable -- stop
        ...
        # v9.3fix: do NOT advance to BOUGHT / pin a worker slot / grow the
        # site reserve here -- all of that now happens in _agent's
        # confirmation pass, only once the purchase verifiably executed.
        # BUY_ANIMAL sits at PRIORITY_MEDIUM (below every seed buy and
        # hire), so truncation past the 10-order cap is most likely
        # exactly in the busy early build window; treating a submitted
        # order as a completed purchase created phantom plans that held a
        # build slot, counted into n_animals, and (via negative
        # unclaimed_in_shed) permitted same-species double-buys.
        # v9.5: clear stale site/worker in case this is a reused NONE plan
        # from an earlier failed cycle -- a fresh order owns nothing until
        # confirmation.
        _advance(target_plan, "ORDERED", day, hour, site=None, worker=None)
        budget -= spec["cost"]
        # v9.21: this order now counts as in-flight for the NEXT loop
        # iteration's slot check and n_animals for the NEXT gate call --
        # matches what would happen next turn anyway, just compressed
        # into this one.
        n_in_progress += 1
        n_animals += 1

    # --- Seed pipeline ---
    # v10 RAMP HYPOTHESIS: the shared-clone tape's own money curve is
    # flat/negative through day 10 (pure infrastructure spend: land +
    # animal fleet), then compounds hard afterward. Our v9.x line always
    # tried to run a parallel crop economy DURING the buildout window,
    # which per the v9.3 "iter E" experiment cannibalizes both (fleet
    # completed day 11 but money collapsed -$11.7k -- early pastures +
    # capital compete with early crop establishment for the same cash).
    # v10 tests the un-mixed version of that idea: during days 0-9,
    # suppress ALL seed spend (no MELON/STRAWBERRY/TOMATO/CARROT), so
    # every dollar goes to land + animals + their feed, matching the
    # clone's observed sequencing instead of splitting the difference.
    # Seed pipeline turns fully on at RAMP_DAY once the fleet/land
    # buildout is essentially done (COW last_day=12, SHEEP last_day=15).
    # v10.1: RAMP HYPOTHESIS (v10.0) EMPIRICALLY REJECTED -- suppressing all
    # crop seed spend through day 10 cost ~50% of final money (-$96k to
    # -$116k vs v9.3, 3/3 seeds, both-seats). Root cause diagnosed, not
    # just observed: STRAWBERRY/MELON/TOMATO are ongoing/one-time crops
    # whose total lifetime yield is fixed by planting day, not just by
    # cash available -- delaying STRAWBERRY's first buy from day 0 to
    # day 10 roughly HALVES its productive window (19 days of 10-29
    # production vs 9 days of 20-29), and MELON's buy window (day<=16)
    # shrinks from 16 days to 6. Land/animal purchases already run BEFORE
    # the seed pipeline in priority order (see comment below), so they
    # were already getting first claim on cash -- suppressing crops
    # entirely just forfeited growing season for no offsetting gain.
    # Reverted to v9.22's always-on seed pipeline unchanged.
    ph = phase(day)
    space = len(view["empty"]) + 2
    want = {}
    if day <= CUTOFF["MELON"] and budget > 700:
        melon_target = min(space, 6 if ph == 0 else 12)  # v9.1 iter A: ph1 was 10
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        want["STRAWBERRY"] = min(space, 10 if ph == 0 else 20)  # v9.1 iter A: keep the seed pipe full through the day-18 cutoff
    if day <= CUTOFF["TOMATO"] and budget > 600:
        want["TOMATO"] = 3   # ongoing crop, same small-standing-patch approach
    if day <= CUTOFF["CARROT"] and budget > 400:
        # v10.1 hypothesis: CARROT is the most under-exploited pool on
        # record (431-unit demand pool, Pet Cafe shop doubles its draw)
        # yet every prior version (v7-v9) caps it at a flat 4 -- a
        # leftover placeholder, not a tuned value. Scale it like the
        # other crops instead of leaving it flat.
        want["CARROT"] = min(space, 8 if ph == 0 else 16)
    ...
    # v9.5: sized by n_support, not n_animals -- an ORDERED plan awaiting
    # confirmation needs no feed yet (2-hour window, but 3 concurrent
    # orders could overbuy 6 wheat in the tight early-capital window).
    if n_support > 0:
        target_wheat = n_support * 2 + FEED_CARRY_TARGET
        have = shed.get("WHEAT", 0)
    ...
    feed_reserve = n_support * 2 + (FEED_CARRY_TARGET if n_support else 0)  # v9.5: was n_animals
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        if is_attack and item in PREMIUM_CROPS:
            ...
            frac -= _shop_backed_relief(item, unlocked_shops)  # v10.2
    ...
    # alongside crops so the hand pool scales with the fleet too. v9.10:
    # hour==0 -> hour<=2 (matches opp_scenario_v14's own gating; hands
    # reset to empty every engine midnight and a single hour is capped
    # at MAX_MARKET_ORDERS=10 new hires, so one hour alone can never
    # reach a target above 10) + Fibonacci cost index starts from the
    # real me["hires_today"] (persists across hours, only resets at end
    # of day -- assuming a fresh-0 index at hour 1/2 wildly underprices
    # continuing to hire). See file header and HAND_TARGET_MAX below.
    if hour <= 2 and day < 29:
        ...
        n = int(me.get("hires_today", 0) or 0)
        hire_count = 0
        while hire_count < need and hire_count < MAX_HIRE_SLOTS:
...
def _reset_plan(plan):
    """v9.5: full reset to a clean NONE -- no stale site/worker/timer.

    Site-clearing is safe (see file header): an already-built empty
    pasture is re-found by setup_action's site selection, which prefers
    view["empty_pastures"] globally. A stale UNBUILT site, left in
    place, would keep a plantable tile excluded via claimed_sites."""
    plan.update(stage="NONE", site=None, worker=None,
                stage_turn=None, retries=0)
...
def _fail_stage(plan, fallback_stage):
    ...
    if plan["retries"] > SETUP_MAX_RETRIES:
        ...
        if fallback_stage == "NONE":
            # v9.5: a plan back at NONE holds no purchase and no build --
            # clear site/worker so a dead retry candidate can't reserve
            # a crop tile or shadow a slot (retries intentionally kept:
            # this is the path that ladders to ABANDONED).
            plan["site"] = None
            plan["worker"] = None
    ...
    for plan in animal_plans:
        if plan["stage"] == "ORDERED":
            ...
            if shed.get(sp, 0) > claimed:
                ...
                _reset_plan(plan)  # v9.5: order truncated/unaffordable --
                                   # full reset, no stale site/worker
