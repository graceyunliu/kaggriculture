...
# =====================================================================
# MAIN V11.12 -- NO-ANIMAL PURE-CROP ECONOMY, ISOLATED on top of
# main_v10.6_radius3.py (champion). Structural capital-allocation test:
# removes the animal fleet entirely rather than tuning its gates, and
# redirects the freed capital/labor/land into crops.
#
# --- The change ---
# 1. Both animal-purchase gates, _animal_expansion_feasible() and
#    _sheep_expansion_feasible(), now return False unconditionally (a
#    single `return False` inserted at the top of each, ahead of all
#    their existing checks). No COW or SHEEP is ever bought; the
#    ANIMAL PIPELINE machinery (build slots, ORDERED/BOUGHT/CARRYING
#    confirmation) still exists in the file but never activates since
#    target_plan is never created.
# 2. Freed budget/seed targets: with no pastures ever built, the
#    EXPANSION_MIN_CROP_TILES_PER_QUAD crop-land reservation that used
#    to guard land for crops against animal-site claims is moot, but the
#    seed-buy targets themselves were still sized assuming a competing
#    fleet draw on cash and hands. Loosened (phase-0 / phase-1 pairs):
#      want["MELON"]:      6/12  -> 9/16
#      want["STRAWBERRY"]: 10/20 -> 13/24
#    (both still capped by `min(space, ...)`, so this only raises the
#    ceiling on how much of the freed land/cash actually gets used --
#    it does not force planting beyond available empty tiles.)
#    TOMATO/CARROT targets left unchanged (already small, uncapped by
#    phase) since the hypothesis under test is animal-vs-crop capital
#    allocation, not a general crop-mix reweight -- see v11.13 for a
#    dedicated per-tile-intensity test.
#
# --- Hypothesis ---
# Two costs of the animal fleet are on record in this project: (a) feed/
# service labor draws hands away from crop watering/harvesting, and
# (b) SHED_AT_CAP is the dominant loss tag (31% of losses, per project
# memory) -- animal products (MILK/WOOL/EGG) share the same 100-unit
# shed cap as crops, so a full pasture can crowd out crop-harvest
# capacity at exactly the moments harvest bursts hit. If (a) and/or (b)
# outweigh COW/SHEEP's own revenue, a pure-crop economy with all that
# capital/labor/shed-space redirected into more land and larger seed
# buys should out-earn the animal-inclusive champion.
# =====================================================================
#
# =====================================================================
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    ...
    """
    # v11.12: no-animal pure-crop economy -- COW purchases disabled
    # unconditionally, ahead of all other checks.
    return False
...
def _sheep_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    ...
    # v11.12: no-animal pure-crop economy -- SHEEP purchases disabled
    # unconditionally, ahead of all other checks.
    return False
    if n_animals >= EXPERIMENT_MAX_FLEET:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["MELON"] and budget > 700:
        melon_target = min(space, 9 if ph == 0 else 16)  # v11.12: 6/12 -> 9/16, no fleet to fund
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        want["STRAWBERRY"] = min(space, 13 if ph == 0 else 24)  # v11.12: 10/20 -> 13/24, no fleet to fund
