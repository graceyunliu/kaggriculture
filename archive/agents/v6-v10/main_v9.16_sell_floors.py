...
# =====================================================================
# MAIN V9.16 -- lower ordinary-paced-sell floors, on top of v9.12. Base:
# main_v9.12_melon_fert.py. See the change site (the "else" branch of
# the per-item sell loop) for the full rationale: v9.1-v9.15 all tested
# labor/fleet/crop-target volume; sell pacing was untested this session.
#
# RESULT -- TESTED, WASH both ways, same pattern as v9_oracle's earlier
# sell-timing-forecast finding. Vs v9.5 (5-seed smoke, both-seats):
# t=-0.13, mean -$364/game -- statistically zero. Vs opp_scenario_v14:
# -$46,863/game, essentially identical to v9.10/v9.12's -$46.7-46.9k --
# no additional compression, no regression either. Confirms the earlier
# v9_oracle finding (reactive sell-TIMING is worth "~nothing" given how
# weakly farms are price-coupled through the shared I0=10,000 buffer)
# extends to sell-FLOOR level too, not just reactive forecasting -- our
# own selling behavior, aggressive or conservative, doesn't move the
# needle much either way. NOT submitted; reverted mentally as a
# non-finding, kept only as the record of a real, cheap test in an
# untested category.
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        if is_attack and item in PREMIUM_CROPS:
            ...
            # v9.16: 0.85/0.70 -> 0.75/0.60 for ordinary paced sells (the
            # common day-to-day path, NOT the force_dump/capacity floors
            # above, which stay untouched). Demand-side economics memory
            # notes prices are set off a SHARED market inventory pool --
            # holding our own stock for a price target doesn't matter if
            # the opponent's own supply keeps the pool depressed
            # regardless, so a higher floor may just be forgoing volume
            # for a price recovery the shared pool won't necessarily
            # deliver. Untested category this session (v9.1-v9.15 all
            # touched labor/fleet/crop-target volume, never sell pacing).
            frac = 0.75 if item in PREMIUM_CROPS else 0.60
            if view["opp_imminent"].get(item, 0) >= 4:
                frac -= 0.25
