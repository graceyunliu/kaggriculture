...
# =====================================================================
# MAIN V7.13 -- per-unit target locking, ISOLATED. Base: v7.11,
# byte-identical except for unit_action()'s target selection and two
# small hookup changes in _agent(). Nothing else is touched: no economy
# change, no labor-count change, no crop-mix change.
#
# --- The bug it fixes (traced empirically, not assumed) ---
#
# Instrumented main_v7.11 with a before/after diff of the `tasks` dict on
# every unit_action() call (trace_v711_movement.py, one full 720-turn
# match vs "starter"). Overall movement/work/pass split came out 69.6% /
# 26.7% / 3.7% -- matches the previously-diagnosed ~66/32/2% figure this
# project already knew about.
#
# Of the 3,333 movement calls, only 886 (26.6%) were a unit continuing
# to walk toward the SAME tile it was already headed to the turn before.
# 918 (27.5% of all moves, 31.3% of "committed" moves) were RETARGETS:
# the unit had a target mid-walk and, without ever reaching it, switched
# to a different tile this turn. Root cause is structural: unit_action's
# "tasks" dict is rebuilt from scratch every turn from raw game state
# (perceive()), with zero memory of what any unit was walking toward.
# Each turn, every idle unit independently recomputes "nearest task of
# the highest-priority pending category from MY CURRENT position" -- so
# a unit one step into a long walk can get pulled onto a different,
# now-technically-closer tile of the same or higher-priority category
# the moment its position or the task set shifts even slightly (e.g. a
# new urgent_water tile appears, or another unit claims what used to be
# "nearest" and the ranking reshuffles). The walk toward the original
# target is abandoned with whatever partial progress was made simply
# discarded -- not necessarily catastrophic per event, but it happens on
# ~1 in 4 of ALL movement decisions in a real match.
#
# --- The fix ---
#
# Give each unit (farmer + every hand, identified by a stable id: farmer
# = "farmer", hand i = "handN") a persistent commitment, tracked in the
# new module-level `unit_targets` dict (mutated in place, same pattern as
# the existing `animal_plans`/`reserved_sites` globals). Once a unit picks
# a target tile, it keeps walking toward THAT tile every subsequent turn
# -- ignoring the fresh "nearest" recompute -- until it either arrives
# (task resolves normally) or the lock goes stale (the tile vanished from
# its task category, e.g. handled by whichever unit actually reached it).
# Locked-but-not-yet-claimed tiles are also excluded from OTHER units'
# fresh target selection each turn, so a different unit can't be assigned
# to chase a tile someone else is already most of the way to.
#
# Deliberately NOT touched: task_order/priority logic, weed thresholds,
# how many hands exist, where PLANT targets are chosen from view["empty"]
# -- this is scheduling-only, one mechanism, to keep the A/B attributable.
# =====================================================================
#
# =====================================================================
...
# v7.13: per-unit committed target, keyed by a stable unit id ("farmer",
# "hand0", "hand1", ...). See header comment for the traced mechanism this
# closes. Mutated in place like animal_plans/reserved_sites.
unit_targets = {}
...
def unit_action(pos, tasks, seeds, day, task_order, unit_id=None, locked_tiles=frozenset()):
    ...
    # v7.13: if this unit already has a live commitment from a previous
    # turn, keep walking it instead of recomputing "nearest" from scratch
    # -- that recompute is exactly what was causing ~28% of all movement
    # decisions to abandon an in-progress walk (see header comment).
            ...
            if ltile in tasks.get(lkey, ()):
                if (x, y) != ltile:
                    ...
                    if step:
                        ...
                        return [step]
                # else: arrived -- fall through to on-tile handling below,
                # which will find (x, y) in tasks[lkey] and clear the lock.
            else:
