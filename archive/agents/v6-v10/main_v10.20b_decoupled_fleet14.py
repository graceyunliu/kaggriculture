...
# gates run unbounded."
EXPERIMENT_MAX_FLEET = 14  # v9.1 iter C: 12+ animals saturate the hand pool (weeds 22 vs 2, straw tiles die late -- seed 3/15 blowouts); 10 is v8.11's validated service capacity
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans, inventories):
    budget = me["money"]
    ...
    # --- Animal fleet purchase (v10.20: DECOUPLED from worker-slot
    # pinning) ---
    # Prior architecture (v9.2-v9.22) pinned each purchased animal to one
    # specific worker slot (farmer or a specific hand index) for its
    # entire multi-turn walk-to-shed/pickup/walk-to-site/place journey,
    # capped at MAX_CONCURRENT_BUILDS simultaneous in-flight animals. This
    # session's gate-instrumentation work (see project memory) found that
    # wasn't actually the binding constraint most of the time -- but a
    # THIRD, independent test this round (v10.15, raising the fleet
    # ceiling on a smaller board) re-confirmed real ladder opponents
    # running 14-15-animal fleets (opp_scenario_v14/opp_frontier_v12,
    # 8 COW + 6 SHEEP) genuinely require a different placement mechanism
    # to service, not just a higher ceiling on the old one. Those clones
    # buy in bulk with purchase fully decoupled from placement: animals
    # sit as plain shed stock, and whichever worker is free that turn
    # claims one. This block replaces the pinned-slot purchase gate with
    # that model: no MAX_CONCURRENT_BUILDS limit, buy up to a few animals
    # per turn (bulk, gated only by the SAME feasibility checks as
    # before), keyed off a live fleet count read directly from game state
    # (active pastures + shed stock + units currently carrying + orders
    # awaiting confirmation) rather than a plan-object state machine.
    # Placement/pickup dispatch is rewritten separately below (search
    # "v10.20 dispatch").
    n_active_cow = sum(1 for _pos, t in view["my_pastures"] if t.get("animal") == "COW")
    n_active_sheep = sum(1 for _pos, t in view["my_pastures"] if t.get("animal") == "SHEEP")
    n_carry_cow = sum(1 for inv in inventories if inv.get("COW", 0) > 0)
    n_carry_sheep = sum(1 for inv in inventories if inv.get("SHEEP", 0) > 0)
    n_ordered_cow = sum(1 for p in animal_plans if p["species"] == "COW")
    n_ordered_sheep = sum(1 for p in animal_plans if p["species"] == "SHEEP")
    n_cow = n_active_cow + shed.get("COW", 0) + n_carry_cow + n_ordered_cow
    n_sheep = n_active_sheep + shed.get("SHEEP", 0) + n_carry_sheep + n_ordered_sheep
    n_animals = n_cow + n_sheep

    # Keep enough near-shed reserved tiles growing to match the fleet we
    # already have/expect, so pickup dispatch always has somewhere to walk
    # a newly-claimed animal toward.
    while len(reserved_sites) < min(int(n_animals), EXPERIMENT_MAX_FLEET):
        _grow_reserved_sites(view)

    pending = 0
    for _burst in range(3):   # cap same-turn purchase burst (bulk, not unlimited)
        if n_animals >= EXPERIMENT_MAX_FLEET:
            break
        # v8.10/11 composition guard, adapted to the live count above.
        composition_wants_sheep = (SHEEP_ENABLED and
            n_sheep < _min_sheep_for_fleet(n_animals + 1))
        species = None
        # v10.20 note: this file's _animal_expansion_feasible/_sheep_
        # expansion_feasible predate the `pending` kwarg (that's a v9.21+
        # addition, not present in this v9.3-lineage file) -- cumulative
        # same-turn load is still reflected adequately since `n_animals`
        # itself is updated after each purchase below and fed into the
        # next iteration's gate call.
        ...
        if not (spec["start_day"] <= day <= spec["last_day"] and budget > spec["min_money"]):
            break
        priced_orders.append((PRIORITY_MEDIUM, ["BUY_ANIMAL", species, 1]))
        animal_plans.append({"species": species, "stage": "ORDERED",
                              "stage_turn": day * 24 + hour, "retries": 0})
        budget -= spec["cost"]
        n_animals += 1
        pending += 1
        if species == "SHEEP":
...
def _animal_acquire_action(role_key, pos, carry, view, reserved_sites, shed, taken_sites, claimed_shed):
    """v10.20: decoupled replacement for animal_setup_action(). No `plan`
    object -- state is read live. If this unit is already carrying an
    unplaced COW/SHEEP, continue walking it to (and building/placing on)
    its assigned site, remembered in the module-level `carry_site` dict
    for the rest of the current day. Otherwise, if the shed holds animal
    stock nobody else has claimed THIS TURN yet (`claimed_shed`, a
    same-turn-only counter, not persistent state), walk over and pick one
    up -- any idle unit can do this, not just a pre-pinned one. Mutates
    `taken_sites` (set) and `claimed_shed` (dict) as a side effect so a
    second call later in the same turn doesn't double-claim the same site
    or the same unclaimed shed animal. Returns None if there's nothing to
    do right now (not carrying, no unclaimed stock, or no site free)."""
    species = "COW" if carry.get("COW", 0) > 0 else ("SHEEP" if carry.get("SHEEP", 0) > 0 else None)
    if species is not None:
        site = carry_site.get(role_key)
        site_open = site is not None and (
            site in view["empty"] or site in view["empty_pastures"]
            or any(p == site for p, _t in view["my_pastures"]))
        if not site_open:
            candidates = [t for t in (view["empty_pastures"] or view["empty"])
                          if t not in CENTER_TILES and t not in taken_sites]
            pool = [t for t in reserved_sites if t in candidates] or candidates
            if not pool:
                return None  # nowhere to go yet -- try again next turn
            site = min(pool, key=lambda p2: _dist(p2, _nearest_center(p2)))
            carry_site[role_key] = site
        taken_sites.add(site)
        already_pasture = any(p == site for p, _t in view["my_pastures"])
        if site not in view["empty_pastures"] and not already_pasture:
            if pos != site:
                return [_step_toward(pos, site)]
            return ["BUILD_PASTURE"]
        if pos != site:
            return [_step_toward(pos, site)]
        carry_site.pop(role_key, None)
        return ["PLACE", species]

    unclaimed_cow = shed.get("COW", 0) - claimed_shed.get("COW", 0)
    unclaimed_sheep = shed.get("SHEEP", 0) - claimed_shed.get("SHEEP", 0)
    pick = "COW" if unclaimed_cow > 0 else ("SHEEP" if unclaimed_sheep > 0 else None)
    if pick is None:
        return None
    candidates = [t for t in (view["empty_pastures"] or view["empty"])
                  if t not in CENTER_TILES and t not in taken_sites]
    if not candidates:
        return None  # no room to put another one yet -- don't pull it off the shelf
    claimed_shed[pick] = claimed_shed.get(pick, 0) + 1
    center = _nearest_center(pos)
    if pos not in CENTER_TILES:
        return [_step_toward(pos, center)]
    return ["PICKUP", pick, 1]
...
animal_plans = []    # v10.20: ORDERED-only now (see _agent's hygiene pass) --
                      # tracks purchases submitted but not yet confirmed in
                      # the shed, purely to avoid re-submitting redundant
                      # buys for a few turns. Placed/pending fleet state is
                      # read live from game state, not from this list.
...
carry_site = {}       # v10.20: role_key -> target site for a unit currently
                       # carrying an unplaced animal. Valid for the current
                       # day only (cleared on day change in _agent) since
                       # inventories and hand rosters both reset at midnight.
_last_day = [-1]       # mutable single-element box so _agent can update it
...
def _agent(obs):
    ...
    # --- v10.20: ORDERED-plan hygiene only. The old BOUGHT/CARRYING
    # stages, per-plan worker pinning, and animal_reconcile() are gone --
    # in the decoupled model, live game state (view["my_pastures"] for
    # placed animals, shed.get(species) for unclaimed stock, per-unit
    # carry for in-transit ones) IS the fleet state, always read fresh,
    # so there's nothing to reconcile a stale plan object against. The
    # ONLY bookkeeping left is: an ORDERED plan exists purely to stop the
    # purchase loop from re-submitting redundant buys in the few turns
    # between submitting a BUY_ANIMAL order and it actually landing in
    # the shed. Drop it after ORDER_CONFIRM_TIMEOUT turns regardless of
    # outcome -- if it landed, shed stock already reflects that with or
    # without this plan; if it didn't (truncated past the 10-order cap,
    # or unaffordable), there's nothing to revert.
    turn_now = day * 24 + hour
    animal_plans[:] = [p for p in animal_plans
                        if turn_now - p["stage_turn"] < ORDER_CONFIRM_TIMEOUT]

    if day != _last_day[0]:
        carry_site.clear()
        _last_day[0] = day

    market = economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans, inventories)
    ...
    n_animals_for_wheat = (len(view["my_pastures"]) +
                            ...
                            sum(1 for inv in inventories if inv.get("COW", 0) > 0 or inv.get("SHEEP", 0) > 0))
    ...
    # purchase actually fires. v10.20: site claims now live in carry_site
    # (per in-transit unit) instead of per-plan.
    excluded = set(carry_site.values()) | set(sites)
    if excluded:
    ...
    n_animals_for_service = n_animals_for_wheat  # same live count, computed above
    ...
    # An urgent feed need on an already-placed pasture preempts acquisition
    # work for the turn (matches old behavior), so onboarding animal N+1
    # can't starve animal N's feeding.
    urgent_existing_feed = any(
        not t.get("fed_today", True) and t.get("consecutive_unfed", 0) >= 1
        for _pos, t in view["my_pastures"]
    )
    # v10.20: per-turn claim tracking for the decoupled acquisition dispatch
    # -- which reserved sites and which unclaimed shed animals have
    # already been spoken for by an earlier unit THIS turn, so a later
    # unit in the loop doesn't walk toward the same site/animal.
    taken_sites = set()
    claimed_shed = {}
    ...
    if not urgent_existing_feed:
        farmer_op = _animal_acquire_action(0, farmer_pos, farmer_carry, view, sites, shed, taken_sites, claimed_shed)
    ...
    if farmer_op is None:
        ...
        # back to acquisition work rather than idling.
        farmer_op = _animal_acquire_action(0, farmer_pos, farmer_carry, view, sites, shed, taken_sites, claimed_shed)
    if farmer_op is None:
    ...
    for i, h in enumerate(me["hands"]):
        ...
        # v10.20: any hand not needed for maintenance/urgent-feed this turn
        # can pick up acquisition work -- no more pre-pinned worker slot.
        if not urgent_existing_feed:
            op = _animal_acquire_action(i + 1, hand_pos, hand_carry, view, sites, shed, taken_sites, claimed_shed)
            if op is not None:
                hand_ops[i] = op
                continue
