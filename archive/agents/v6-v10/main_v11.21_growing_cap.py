...
# =====================================================================
# v11.21 -- HARD CAP ON SIMULTANEOUS GROWING TILES (shot in the dark, on
# top of v10.6). Base chassis: main_v10.6_radius3.py, unmodified except
# for this one new cap. Round-7 "shots in the dark" theme.
#
# Today's aggregate replay data (129W/100L/4T) shows SHED_AT_CAP (shed
# capacity 100 fills, blocks storage/selling) as the dominant loss
# cause, 31/100 losses. This hypothesis adds a new global constant,
# MAX_SIMULTANEOUS_GROWING_TILES = 40, and bounds new PLANT-task
# issuance in the tasks["plant"] pool construction in _agent() so the
# total count of currently-growing PLANT tiles (kind=="PLANT" across the
# whole farm, regardless of crop or growth stage) never exceeds this
# cap -- once hit, no new PLANT tasks are issued that turn even if
# there's empty land and available seeds. This directly bounds the
# maximum possible simultaneous harvest volume that could ever land on
# the shed at once -- a blunt-force, flat-cap version of the
# production-throttle idea (a parallel agent in this round is testing a
# shed-occupancy-PROPORTIONAL throttle instead; this is a deliberately
# different, simpler mechanism: a fixed ceiling independent of current
# shed fill level).
# =====================================================================
...
MAX_SIMULTANEOUS_GROWING_TILES = 40  # v11.21: hard flat cap on concurrent PLANT-kind tiles, independent of land/hand count -- bounds max simultaneous harvest volume hitting the shed (see file header)
...
def _agent(obs):
    ...
    # v11.21: flat cap on simultaneously-growing PLANT tiles, independent
    # of land/hand count -- bounds the max possible simultaneous harvest
    # volume that could ever hit the shed at once. Counts every kind ==
    # "PLANT" tile on the farm (any crop, any growth stage) and only
    # trims how many NEW plant tasks are issued this turn; never touches
    # existing growing tiles.
    n_growing_tiles = sum(1 for row in me["tiles"] for t in row
                           if isinstance(t, dict) and t.get("kind") == "PLANT")
    plant_room = max(0, MAX_SIMULTANEOUS_GROWING_TILES - n_growing_tiles)
    n_plantable = min(n_plantable, plant_room)
