...
# =====================================================================
# MAIN V11.9 -- FULL PER-QUADRANT CROP SPECIALIZATION, isolated test on
# top of v10.6_radius3 (champion). Base: main_v10.6_radius3.py,
# unchanged except for one hard filter added to plantable_crops().
#
# Trigger: Grace's screenshot of a real opponent's board -- diagonal
# quadrants (NE/SW) both run strawberry, the other two run other single
# crops each. This is the most literal read of that pattern: each
# quadrant gets exactly ONE assigned crop, forever, no exceptions.
#
# Mechanism: QUADRANT_CROP = {"NW": "WHEAT", "NE": "STRAWBERRY",
# "SW": "STRAWBERRY", "SE": "MELON"}. plantable_crops() filters the
# candidate list for a given tile down to ONLY that tile's quadrant's
# assigned crop (if that crop isn't in the filtered list -- no seed
# stock, or past its CUTOFF day -- the tile simply doesn't get planted
# this turn; nothing else is substituted). Everything else (seed-buy
# targets, PLANT_PRIORITY order for the tile=None call sites, animal
# siting, land timing) is byte-identical to v10.6.
#
# KNOWN CONSEQUENCE, called out up front: this codebase has planted NO
# wheat since v9.1 (want["WHEAT"] was deleted -- all animal feed is
# market-bought, confirmed at line ~1247's "wheat planting removed
# entirely" comment; plant_priority_current's fleet-dependent WHEAT
# reinsertion is dead for planting purposes because seeds["WHEAT"] is
# always 0). Assigning WHEAT to NW under a strict "this quadrant may
# ONLY plant its assigned crop" rule therefore means NW -- roughly 25%
# of total board tiles -- goes UNPLANTED for the entire game. This is
# an intentional, literal implementation of the stated hypothesis (which
# explicitly allows an empty tile over a substituted crop), not a bug;
# flagged here because it is expected to dominate the result.
# =====================================================================
...
# v11.9: hard per-quadrant crop assignment (see file header).
QUADRANT_CROP = {"NW": "WHEAT", "NE": "STRAWBERRY", "SW": "STRAWBERRY", "SE": "MELON"}
...
def plantable_crops(seeds, day, tile=None):
    ...
    # v11.9: hard-lock the candidate list to this tile's quadrant's one
    # assigned crop -- if that crop isn't currently plantable (no seed
    # stock / past cutoff), the tile goes unplanted this turn; nothing
    # else is substituted.
    if tile is not None:
        assigned = QUADRANT_CROP.get(_quadrant_of(tile))
        base = [c for c in base if c == assigned]
