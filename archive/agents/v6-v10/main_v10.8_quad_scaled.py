...
NEAR_SHED_RADIUS_BY_QUADS = {1: 3, 2: 4, 3: 5, 4: 6}
# v10.8 hypothesis: v10.6's radius=3 was swept and validated on the
# ALREADY-COMMON case (most of the game is played with 2-4 quadrants
# unlocked, land bought days ~7-16), but a fixed radius means "near" is a
# shrinking FRACTION of the farm as more land unlocks (radius 3 covers a
# much bigger share of a single 5x5 quadrant than of the full 10x10
# board). Scale the boundary with unlocked quadrant count instead of
# holding it flat, so the near zone stays a roughly consistent fraction
# of the currently-available farm. Values chosen to grow roughly in step
# with the quadrant count (3/4/5/6); not independently swept this round
# (v10.6 already validated the shape of the underlying idea, this only
# tests whether making the boundary land-aware helps further).
...
def plantable_crops(seeds, day, tile=None, quads=1):
    """v10.5/v10.6/v10.8 siting hypothesis -- see project report for full
    ...
    shed) x (number of times it must be revisited). Prefer ongoing crops
    (STRAWBERRY/TOMATO, watered daily for their whole life) on tiles
    within a quadrant-scaled radius of the shed, one-time crops
    (WHEAT/CARROT/MELON, watered only during a short bonus window) beyond
    it. `tile=None` preserves the exact old (position-agnostic) behavior
    for call sites that don't have a specific tile in hand."""
    base = [c for c in plant_priority_current if seeds.get(c, 0) > 0 and day <= CUTOFF.get(c, -1)]
    if tile is None or len(base) <= 1:
        return base
    radius = NEAR_SHED_RADIUS_BY_QUADS.get(quads, 6)
    near = _dist_to_shed(tile) <= radius
    if near:
...
def assign_tasks(unit_positions, tasks, seeds, day, task_order, quads=1):
    ...
    for i, pos in enumerate(unit_positions):
        ...
        if pos == tile:
            if key == "plant":
                op = None
                for crop in plantable_crops(seeds, day, tile, quads):  # v10.8: quadrant-scaled siting
                    seeds[crop] -= 1
...
def _agent(obs):
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        ...
        crop_ops = assign_tasks(crop_unit_positions, tasks, seeds, day, task_order, quads)
        k = 0
