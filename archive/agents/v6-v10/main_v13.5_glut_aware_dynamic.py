...
# =====================================================================
# v13.5 (Aug 7) -- LIVE, REACTIVE glut-awareness for animal purchases, on
# top of v10.6 champion (siting only, no goose plumbing originally).
# Sibling build to v13.4 (which used a fixed, aggressive GOOSE
# composition floor); this build instead reads the market's live price
# for MILK/WOOL each turn and reacts directly, rather than assuming a
# glut in advance. Same root motivation as v13.4's header: v10.22's
# rejected goose-floor test (-$9,444/game) ran entirely in self-play vs
# v10.6, whose own fleet is capped at EXPERIMENT_MAX_FLEET=10, so
# milk/wool never actually glutted the shared market in that matchup --
# MARKET_PARAMS shows MILK (T=122, af="linear", at=1.60) and WOOL
# (T=105, af="sq", at=3.20) both crash to the $1 PRICE_FLOOR once
# COMBINED market oversupply crosses ~1x T past I0, something a real
# 15-animal opponent (Opponents/opp_scenario_v14.py) can trigger mostly
# on its own, while EGG (T=332, af="log", at=0.20) stays glut-proof.
# Mechanism: each turn, live_price(obs, "MILK") / live_price(obs,
# "WOOL") are compared against GLUT_PRICE_FRAC (0.30) of their
# MARKET_PARAMS base price. If MILK has crashed, COW's purchase gate is
# skipped this turn (new cow capital is redirected toward GOOSE
# instead); if WOOL has crashed, SHEEP's gate is skipped the same way.
# GOOSE (ported plumbing: perceive() extended for COOP tiles,
# ANIMAL_STRUCTURE map, structure-aware animal_setup_action,
# _goose_expansion_feasible gate) gets first look at the turn's purchase
# slot whenever EITHER glut condition is live, and keeps its normal
# last-resort fallback slot regardless. Unlike v13.4, there is NO fixed
# composition floor (GOOSE_FLOOR_BY_FLEET_SIZE is neutralized to 0/all
# sizes) -- goose allocation is driven entirely by the live price
# signal, so it should automatically look close to v10.6's original cow/
# sheep-heavy mix against a non-glutting opponent (self-play vs v10.6,
# where combined fleets stay small enough that milk/wool prices should
# stay healthy) while shifting toward goose specifically when facing a
# glutting opponent (opp_scenario_v14.py) -- that DIFFERENCE in behavior
# between the two opponents is the core thing being tested, stronger
# evidence for the underlying mechanism than either result alone. See
# run report for both sets of stats.
# =====================================================================
...
# SHEEP cost=500, both structure="PASTURE"; v13.4 adds GOOSE cost=300,
...
ANIMAL_SPECS = {
    ...
    # v13.4 (ported from v10.22/v10.21): GOOSE, engine cost=$300,
    # first_yield_day=4/interval=1 (much faster payback than COW/SHEEP) --
    # min_money set to ~1.3x cost, matching the COW/SHEEP ratio convention.
    # last_day=20 (later than either) since the fast interval still leaves
    # real production days even on a late purchase.
    "GOOSE": {"cost": 300, "min_money": 400, "start_day": 0,  "last_day": 20},
}
# v13.4: species -> structure it needs built before PLACE. COW/SHEEP share
# PASTURE; GOOSE needs a separate COOP structure (engine-verified,
# kaggriculture.py ANIMALS dict).
...
# v13.4 (ported from v10.21): GOOSE gets its own gate too
# (_goose_expansion_feasible), tried alongside sheep in the composition
# chain -- see header. Small headroom since it should compete for real
# purchase slots (this build's whole point), not just mop up leftover
# slack.
GOOSE_ENABLED = True
...
# v13.5: unlike v13.4's fixed floor, this build has NO standing goose
# composition requirement -- left empty so _min_goose_for_fleet always
# returns 0. Goose allocation here is driven entirely by the live-price
# glut check below (_milk_glutted/_wool_glutted), not by a pre-set
# target fraction of the fleet.
GOOSE_FLOOR_BY_FLEET_SIZE = {}

# v13.5: live glut-reaction thresholds. A product's live market price
# falling below this fraction of its own MARKET_PARAMS base price is
# treated as "already glutted" for purchase-gating purposes -- below
# this, MILK/WOOL's ADDITIONAL marginal revenue per service-day is
# assumed to be poor enough that new COW/SHEEP capital should be
# redirected to GOOSE instead (EGG's glut-proof shape means it rarely if
# ever hits this same trigger).
GLUT_PRICE_FRAC = 0.30
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    my_coops = []              # v13.4: same, for COOP tiles (GOOSE)
    empty_coops = []
    ...
    return {
        ...
        "my_coops": my_coops, "empty_coops": empty_coops,  # v13.4
        # v13.4: combined view used by every piece of maintenance/feed-
...
def _min_goose_for_fleet(next_animal_index):
    """v13.4: same composition-guard pattern as _min_sheep_for_fleet, for
    GOOSE_FLOOR_BY_FLEET_SIZE."""
    floor = 0
...
def _milk_glutted(obs):
    """v13.5: True once MILK's live price has already crashed below
    GLUT_PRICE_FRAC of its base ($160) -- the reactive trigger that
    redirects new COW capital toward GOOSE for the turn."""
    return live_price(obs, "MILK") < GLUT_PRICE_FRAC * MARKET_PARAMS["MILK"]["base"]


def _wool_glutted(obs):
    """v13.5: same check for WOOL (base $200) -- redirects new SHEEP
    capital toward GOOSE for the turn."""
    return live_price(obs, "WOOL") < GLUT_PRICE_FRAC * MARKET_PARAMS["WOOL"]["base"]
...
def _goose_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    """v13.4 (ported from v10.21/v10.22): GOOSE's own gate, tried alongside
    SHEEP's in economy()'s per-turn composition chain. Same real/safety
    checks as COW/SHEEP (survival across the combined my_animals view,
    wheat budget, crop neglect, crop space, shared EXPERIMENT_MAX_FLEET
    ceiling); hand-capacity projection gets a small headroom
    (GOOSE_HAND_HEADROOM). Mirrors sheep's wool-glut stop with an
    egg-glut stop instead (EGG's own MARKET_PARAMS T=332 pool, glutting
    it the same way would floor marginal-egg prices) -- though per this
    build's header, EGG's glut-proof shape (af="log", at=0.20) means this
    stop should rarely if ever actually trigger in practice."""
    if n_animals >= EXPERIMENT_MAX_FLEET:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is None:
            # v13.5: live glut check, read fresh every turn from the
            # market's own live prices -- see _milk_glutted/_wool_glutted
            # and GLUT_PRICE_FRAC above.
            milk_glutted = _milk_glutted(obs)
            wool_glutted = _wool_glutted(obs)
            ...
            # cow's own gate happens to fire first every turn. v13.5: this
            # composition push is suppressed while WOOL is already
            # glutted -- no point steering the fleet toward sheep when
            # marginal wool is worth near-nothing right now.
            n_sheep_owned = sum(1 for p in animal_plans
                                  if p["species"] == "SHEEP"
                                  and p["stage"] in ("ACTIVE", "ORDERED", "BOUGHT", "CARRYING"))
            composition_wants_sheep = (SHEEP_ENABLED and not wool_glutted and
                n_sheep_owned < _min_sheep_for_fleet(n_animals + 1))

            # v13.5: unlike v13.4's fixed floor (composition_wants_goose
            # driven by GOOSE_FLOOR_BY_FLEET_SIZE, which this build leaves
            # empty), goose gets first look at the turn's purchase slot
            # purely REACTIVELY -- whenever milk or wool has already
            # crashed below GLUT_PRICE_FRAC of base this turn. Against a
            # non-glutting opponent (self-play vs v10.6) this should
            # almost never fire, leaving the mix close to v10.6's
            # original cow/sheep composition; against a glutting opponent
            # (opp_scenario_v14.py) it should fire increasingly often as
            # the match wears on and milk/wool prices erode.
            composition_wants_goose = GOOSE_ENABLED and (milk_glutted or wool_glutted)
            ...
            # v13.5: COW's gate is skipped entirely while MILK is already
            # glutted -- new cow capital would be buying into a marginal
            # $/day close to the $1 floor, so it's redirected toward
            # GOOSE (via the composition_wants_goose branch above, or the
            # unconditional goose fallback below) instead.
            elif (not milk_glutted) and _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day):
                target_plan = {"species": "COW", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            # v13.5: same skip for SHEEP while WOOL is glutted.
            elif (not wool_glutted) and SHEEP_ENABLED and _sheep_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day):
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            # Goose fallback retained too, unconditionally (not gated on
            # glut), for turns where cow AND sheep both decline for a
            # non-glut reason (day-window closed, budget, etc.) but
            # goose's own window is still open.
...
# nearest tile in view["my_animals"] (v13.4: pastures + coops combined),
...
def animal_setup_action(pos, view, shed, farmer_carry, plan, day, hour, reserved_sites=(), exclude_sites=()):
    ...
    v13.4: structure-aware -- COW/SHEEP still build/use PASTURE, GOOSE
