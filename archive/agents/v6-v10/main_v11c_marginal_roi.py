# =====================================================================
# MAIN V11C -- MARGINAL ROI PLANNER
#
# Economy reset on the v10.6 execution chassis.
# Goal: replace fixed crop targets with a marginal-return allocator. Crop seed demand is
# derived from live prices, remaining production runway, and the current near/far empty
# tile mix. Animal expansion is additionally gated by a rough remaining-season ROI test.
# Empty land itself is not work.
# =====================================================================
...
#   4. Hard EXPERIMENT_MAX_FLEET = 14 cap. Unchanged from v8.10.
...
def _remaining_days(day):
    return max(0, 30 - day)

def _crop_contract_scores(obs, view, day):
    """Rough profit-per-service-weight scores; intentionally simple first-gen planner."""
    prices = {c: live_price(obs, c) for c in ("STRAWBERRY", "MELON", "TOMATO", "CARROT")}
    rem = _remaining_days(day)
    near_empty = sum(1 for t in view["empty"] if _dist_to_shed(t) <= NEAR_SHED_RADIUS)
    far_empty = max(0, len(view["empty"]) - near_empty)
    scores = {}
    # Revenue potential is clipped by remaining runway; denominator proxies recurring service.
    scores["STRAWBERRY"] = (prices["STRAWBERRY"] * min(4, max(0, rem // 2)) - CROPS["STRAWBERRY"]["seed"]) / 10.0
    scores["TOMATO"] = (prices["TOMATO"] * min(4, max(0, rem)) - CROPS["TOMATO"]["seed"]) / 8.0
    scores["MELON"] = (prices["MELON"] * (6 if rem >= 12 else 0) - CROPS["MELON"]["seed"]) / 4.0
    scores["CARROT"] = (prices["CARROT"] * (4 if rem >= 3 else 0) - CROPS["CARROT"]["seed"]) / 3.0
    return scores, near_empty, far_empty

def _animal_roi_score(obs, species, day):
    """Approximate remaining-season gross margin before labor, used only as a buy/no-buy gate."""
    rem = _remaining_days(day)
    if species == "COW":
        first, interval, product = 8, 2, "MILK"
    else:
        first, interval, product = 6, 3, "WOOL"
    events = 0 if rem < first else 1 + (rem - first) // interval
    gross = events * live_price(obs, product)
    feed = rem * 2 * live_price(obs, "WHEAT")
    return gross - feed - ANIMAL_SPECS[species]["cost"]
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion: only if the planner has nearly filled current capacity. ---
    if 1 <= day <= 18 and len(view["empty"]) < 5:
        land_score = PRIORITY_LAND_SCARCE
        if quads == 1 and budget > 1300:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2500 and day <= 15:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 4800 and day <= 12:
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is not None:
            ...
            animal_roi_ok = _animal_roi_score(obs, sp, day) > 0
    ...
    # v11C: dynamic production contracts from live marginal scores.
    want = {}
    crop_scores, near_empty, far_empty = _crop_contract_scores(obs, view, day)
    ranked = [c for c, s in sorted(crop_scores.items(), key=lambda kv: -kv[1])
              if s > 0 and day <= CUTOFF[c]]
    for c in ranked[:3]:
        if c in ONGOING_CROPS:
            cap = near_empty
        else:
            cap = far_empty + max(0, near_empty // 3)
        # Keep the first-gen planner conservative: enough inventory to fill
        # profitable current capacity, not the whole unlocked board.
        want[c] = min(space, max(0, min(cap, 14 if c == "STRAWBERRY" else 8)))
    ...
    if hour == 0 and day < 29:
        animal_work = _pending_animal_work(view)
        # v11 economy reset: empty land is not unfinished work. Hire against
        # committed production obligations only; idle tiles consume zero labor.
        plant_commitment = min(len(view["empty"]), sum(seeds.get(c, 0) for c in plantable_crops(seeds, day)))
