...
# =====================================================================
# v11.19 -- INVERTED SITING SANITY CHECK (shot in the dark, on top of
# v10.6). Base chassis: main_v10.6_radius3.py, unmodified except for
# this one flip. Round-7 "shots in the dark" theme.
#
# This is a deliberate sanity check, EXPECTED TO LIKELY LOSE. v10.5/v10.6
# validated (9/10 seeds, +$7,310/game at radius 3 alone, and +$9.4k-13.1k
# vs two independent baselines for the underlying siting mechanism) that
# ONGOING crops (STRAWBERRY/TOMATO -- watered every day of a ~12-17 day
# life) belong NEAR the shed (every worker respawns there each morning,
# so a far ongoing-crop tile pays its distance cost on every visit) and
# ONE-TIME crops (WHEAT/CARROT/MELON -- a handful of visits total) belong
# FAR, where distance barely matters. This variant flips plantable_crops()
# to the exact opposite: one-time crops preferred near the shed, ongoing
# crops pushed far. Worth re-testing (rather than just trusting the old
# finding) given how much else has changed in the codebase since v10.5
# (radius tuning, GOOSE/EGG exploration, etc.) and given the newly
# discovered SHED_AT_CAP dominant-loss-cause dynamic might interact with
# siting in a way the original test never saw. Report result honestly
# either way -- a LOSS here re-confirms the original finding under new
# conditions; anything else would be a genuinely new result.
# =====================================================================
...
def plantable_crops(seeds, day, tile=None):
    ...
    # v11.19 INVERTED: flipped from v10.5/v10.6's validated rule -- put
    # ONE-TIME crops near the shed and ONGOING crops far (see file header
    # sanity-check rationale).
    if near:
