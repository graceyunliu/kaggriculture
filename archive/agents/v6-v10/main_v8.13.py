...
# =====================================================================
# NAMED main_v8.13.py -- byte-identical copy of main_v8.11_wheat.py,
# rerun on Grace's explicit request to resolve its original 15-seed
# "statistical wash" (8/15, -$2,740/game, t=-1.50) with more seeds
# instead of tuning blind. RESULT (30-seed --both-seats vs main_v8.3.py,
# original 15 + 15 new): 16/30 wins, mean -$2,626/game, t=-2.29, 95% CI
# [-$4,879,-$374] -- now REJECTED, statistically real at n=30 (the wash
# was underpowered, not a coin flip; win rate ~53% but losing seeds lose
# bigger than winning seeds win). main_v8.3.py remains champion. See
# project memory for the full breakdown and 30-seed raw table.
# =====================================================================
#
# MAIN V8.11 (WHEAT variant) -- zero-diversion fertilize pipeline,
# ISOLATED. Base: main_v8.3.py, byte-identical except for a new
# "fertilize" task TIER folded directly into task_order/tasks/
# assign_tasks() (see PERCEPTION and CROP TASK ALLOCATION sections
# below), plus a FERTILIZER_RESERVE carve-out in economy()'s
# sell-everything-every-turn FERTILIZER order.
#
# --- Why this is a re-attempt, not a new idea ---
#
# main_v8.1.py already tried "FERTILIZE on WHEAT only" and lost 0/15,
# mean -$19,019/game (see kaggriculture-v8-architecture.md sec 4 /
# project memory). Root-caused by re-reading v8.1's own
# fertilize_action(): it ran BEFORE the farmer/hand was routed into
# assign_tasks()'s batched crop-duty pool, so it captured a unit
# UNCONDITIONALLY every turn an eligible WHEAT tile + spare reserve
# fertilizer existed -- not just the handful of turns FERTILIZE itself
# actually fired -- directly fighting v8's own validated global task-
# assignment mechanism (the thing that beat v7.11 by +$10,238/game in
# the first place). The postmortem's explicit recommendation: "do not
# re-attempt without a zero-diversion design (e.g., only fertilize a
# tile a unit is already passing through)."
#
# The WHEAT yield math itself was never in question -- confirmed by
# reading the engine directly (kaggriculture.py): WHEAT is
# `"ongoing": False`, gets an initial yield_units=1 at planting, and
# each WATER action inside the age-2..4 window adds 1 (unfertilized) or
# 2 (fertilized) more, capped at max_yield=6. Unfertilized real ceiling:
# min(6, 1+3) = 4. Fertilized: min(6, 1+6) = 6 -- a genuine 50% TOTAL
# yield increase on a single-harvest, non-ongoing crop (HARVEST clears
# the tile permanently for non-ongoing crops, so this is the crop's
# entire lifetime output, not a per-cycle number). WHEAT also has the
# largest town demand pool in the game (635 units/match). This is the
# crop the postmortem said was worth retrying.
#
# --- The fix: fertilize as a competing TIER, not a pre-emptive grab ---
#
# Instead of a standalone function checked ahead of assign_tasks() on
# every eligible turn, "fertilize" is now one more entry in
# _task_order()'s tier list (inserted right after "harvest", before
# ordinary "water") and tasks["fertilize"] is populated in perceive()
# exactly like urgent_water/harvest/water/weeds. It goes through the
# SAME Pass-1 (standing-claim) / Pass-2 (joint nearest-pair match)
# machinery in assign_tasks() as every other tier -- meaning a unit only
# ever gets routed to fertilize duty if, in that turn's joint
# competitive matching, urgent_water and harvest (processed first in
# task_order, consuming units before fertilize's turn) didn't already
# need it. Only the final ops-conversion step gets a fertilize-specific
# branch (mirroring how "plant" already needs dynamic op logic there):
# if the assigned unit is carrying FERTILIZER, walk to the tile and
# FERTILIZE; if not, detour toward the shed and PICKUP (only while
# shed stock exceeds FERTILIZER_RESERVE). No unit is ever sent hunting
# for fertilizer ahead of a real crop-tier need -- it only fires with
# whatever's left over after urgent_water/harvest are satisfied for the
# turn, and the reserve gate means the pipeline goes fully quiet (pool
# empty) whenever shed stock is at/under the reserve.
#
# --- Eligibility window (unchanged math from v8.1) ---
#
# A WHEAT tile qualifies for the "fertilize" tier while
# 0 <= age-since-planted <= FERT_MAX_AGE (2) and
# fertilized_until_day < day. Applying right at age 2 (the window's
# open day) covers all three bonus-window days (2,3,4) with one
# FERTILIZER unit -- the efficient case v8.1 already identified; this
# file does not change that part, only how (and how often) a unit gets
# sent to do it.
#
# --- Fertilizer supply: byproduct collection, not market purchase ---
#
# animal_maintenance_action() already calls COLLECT_FERTILIZER as the
# lowest-priority pasture task (v8.3, unchanged) -- every pasture visit
# for feed/care/harvest also has a chance to pick up a free FERTILIZER
# byproduct into the worker's own carry. Previously that unit walked it
# straight to the shed and it was 100%-sold every turn by economy()'s
# "Fertilizer straight to market" block. This file keeps that block but
# carves out FERTILIZER_RESERVE units first; the fertilize tier spends
# from that reserve via PICKUP, same withdraw-from-shed mechanic
# animal_maintenance_action already uses for wheat-as-feed.
#
# --- Instrumentation ---
#
# FERT_STATS tracks fertilizer_applied (successful FERTILIZE ops),
# fertilizer_sold (units actually sold, post-reserve),
# fert_worker_actions (every worker-turn spent inside the fertilize
# branch, including walks/PICKUPs/no-ops -- the real labor-cost side of
# the ledger), target_harvest_units (WHEAT yield_units actually
# harvested), and target_tile_days (WHEAT PLANT-tile count, sampled
# once/day) -- printed as one FERTTRACE line/turn from day>=29 onward
# (see fert_diag.py/fert_trace.py for how these get pulled out of
# env.logs).
#
# --- Test protocol ---
#   Seeded --both-seats vs main_v8.3.py, same 15-seed set used for
#   every v8.x round (1,7,42,99,123,202,303,555,2024,8080,17,2025,
#   3001,3002,3003).
# =====================================================================
#
# =====================================================================
...
# --- v8.11: zero-diversion fertilize pipeline ---
FERTILIZER_RESERVE = 4        # keep this many FERTILIZER in shed for the
                                # fertilize tier; the rest still sells every
                                # turn like v8.3
FERT_TARGET_CROPS = {"WHEAT"}
FERT_MAX_AGE = 2               # non-ongoing crops: eligible while
                                # 0 <= age-since-planted <= this (matches
                                # v8.1's WHEAT_FERT_MAX_AGE)
FERT_MAX_DAYS_SINCE_FIRST = 2  # ongoing crops: eligible while
                                # 0 <= days-since-first-yield-day <= this
                                # (unused by this variant, kept for the
                                # shared _fert_eligible() helper)

FERT_STATS = {
    "fertilize_ops": 0, "fert_units_sold": 0, "fert_worker_actions": 0,
    "target_harvest_units": 0, "target_tile_days": 0,
}


def _fert_eligible(t, crop, day):
    cd = CROPS[crop]
    planted = t.get("planted_day", day)
    if cd["ongoing"]:
        days_since_first = day - planted - cd["first"]
        if not (0 <= days_since_first <= FERT_MAX_DAYS_SINCE_FIRST):
            return False
    else:
        age = day - planted
        if not (0 <= age <= FERT_MAX_AGE):
            return False
    return t.get("fertilized_until_day", -1) < day


def _any_carry_fert(inventories):
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    fert_targets = []         # v8.11: target-crop tiles eligible for FERTILIZE
        ...
        for x, t in enumerate(row):
            ...
            if kind == "WEED":
                ...
                if c:
                    ...
                    if yu > 0 and ready:
                        harvest.append((x, y))
                # v8.11: fertilize-tier eligibility for the target crop set.
                crop = t.get("crop")
                if crop in FERT_TARGET_CROPS and _fert_eligible(t, crop, day):
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Fertilizer: v8.11 keeps FERTILIZER_RESERVE units in shed for the
    # fertilize tier (assign_tasks()'s "fertilize" key); sells the rest,
    # same as v8.3 sold 100%.
    ...
    # v8.11: mechanism trace, printed every turn from day 29 onward (the
    # engine's final input observation tops out at day=29 -- see
    # main_v8.1.py's identical FERTTRACE placement/rationale). Take the
    # LAST such stderr line per game for the true final counters.
    if day >= 29:
        print(
            f"FERTTRACE fertilizer_applied={FERT_STATS['fertilize_ops']} "
            f"fertilizer_sold={FERT_STATS['fert_units_sold']} "
            f"fert_worker_actions={FERT_STATS['fert_worker_actions']} "
            f"target_harvest_units={FERT_STATS['target_harvest_units']} "
            f"target_tile_days={FERT_STATS['target_tile_days']}",
            file=sys.stderr,
        )
...
def _task_order(n_weeds):
    ...
    v8.11: "fertilize" inserted right after "harvest", ahead of ordinary
    (non-urgent) water/plant/weeds -- it never outranks urgent_water or
    harvest (animal survival/harvest is handled entirely outside this
    tier list, before a unit ever reaches task_order), and a tile skipped
    for fertilize this turn is still watered on schedule by the
    unmodified "water"/"urgent_water" tiers -- same fallback-never-worse
    guarantee v8.1's header claimed but this file actually enforces via
    tier competition instead of a pre-emptive grab."""
    if n_weeds >= WEED_URGENT_THRESHOLD:
        return ["urgent_water", "harvest", "fertilize", "weeds", "water", "plant"]
    if n_weeds >= WEED_PRIORITY_THRESHOLD:
        return ["urgent_water", "harvest", "fertilize", "water", "weeds", "plant"]
    return ["urgent_water", "harvest", "fertilize", "water", "plant", "weeds"]
...
def assign_tasks(unit_positions, tasks, seeds, day, task_order, carries=None, shed=None, me=None):
    ...
    # Convert assignments to ops -- same action set as unit_action(), plus
    # v8.11's fertilize special-case (dynamic op, like "plant" already is).
    carries = carries or [{}] * n
    ops = [None] * n
    for i, pos in enumerate(unit_positions):
        ...
        if tile is None:
            ...
            continue
        if key == "fertilize":
            FERT_STATS["fert_worker_actions"] += 1
            carry = carries[i] if i < len(carries) else {}
            ...
            if carry_fert > 0:
                ...
                # Not carrying yet -- detour via the shed and PICKUP, but
                # only while stock exceeds the reserve (a unit already
                # mid-detour when the reserve dries up just idles in place
                # rather than emptying it below the floor).
                center = _nearest_center(pos)
                if pos in CENTER_TILES:
                    avail = (shed or {}).get("FERTILIZER", 0) - FERTILIZER_RESERVE
                    ops[i] = ["PICKUP", "FERTILIZER", 1] if avail > 0 else ["PASS"]
                else:
                    step = _step_toward(pos, center)
                    ops[i] = [step] if step else ["PASS"]
            continue
        if pos == tile:
            if key == "plant":
                ...
                # v8.11: instrument target-crop harvest units at the moment
                # the HARVEST op is actually issued (pre-clear yield_units).
                if me is not None:
                    row = me["tiles"][tile[1]] if 0 <= tile[1] < len(me["tiles"]) else []
                    t = row[tile[0]] if 0 <= tile[0] < len(row) else None
                    if isinstance(t, dict) and t.get("crop") in FERT_TARGET_CROPS:
                        FERT_STATS["target_harvest_units"] += t.get("yield_units", 0)
...
def _agent(obs):
    ...
    # v8.11: tile-days sample, once/day (hour==0) to avoid double-counting
    # across a day's 24 turns.
    if hour == 0:
        FERT_STATS["target_tile_days"] += sum(
            1 for row in me["tiles"] for t in row
            if isinstance(t, dict) and t.get("kind") == "PLANT" and t.get("crop") in FERT_TARGET_CROPS
        )
    ...
    tasks = {
        ...
        # v8.11: fertilize tier only goes live while shed stock exceeds the
        # reserve, or some unit is already mid-delivery carrying FERTILIZER
        # (lets an in-progress carry finish even if the reserve just dried
        # up this turn) -- see file header for the full zero-diversion
        # design this replaces (main_v8.1.py's pre-emptive grab).
        "fertilize": (list(view["fert_targets"])
                      if (shed.get("FERTILIZER", 0) > FERTILIZER_RESERVE or _any_carry_fert(inventories))
                      else []),
    }
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        ...
        crop_carries = ([farmer_carry] if farmer_needs_crop else []) + [
            (inventories[i + 1] if len(inventories) > i + 1 else {}) for i, _pos in hand_needs_crop
        ]
        crop_ops = assign_tasks(crop_unit_positions, tasks, seeds, day, task_order,
                                 carries=crop_carries, shed=shed, me=me)
        k = 0
