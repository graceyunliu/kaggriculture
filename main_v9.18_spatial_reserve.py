...
# =====================================================================
# MAIN V9.18 -- spatial (farthest-from-pasture-first) crop/animal hand
# reservation, on top of v9.12 (melon fertilize-in-window on v9.10's
# hire fix).
#
# Hypothesis (task-dispatch-efficiency category, v9.17's water-tiering
# having failed to move the gap): _reserved_crop_hand_count() computes
# HOW MANY hands get walled off from animal maintenance for crop-only
# duty, but the v7.6a call site picked WHICH ones by fixed roster index
# (i < reserved_count) -- position never factored in. A hand standing
# next to a pasture with a low index got walled off from care/feed it
# was already adjacent to, while a high-index hand clear across the map
# still ran animal_maintenance_action and walked the whole way to reach
# it (or found nothing and fell through to crop duty anyway, having
# wasted the attempt). This build reserves the `reserved_count` hands
# actually FARTHEST from any pasture instead, leaving pasture-adjacent
# hands free to answer maintenance on the spot. reserved_count's own
# formula (weed ratio / crop-pressure -> PROTECTED_FRACTION_LOW/HIGH)
# is untouched -- only which hands fill that same count changes.
#
# RESULT -- TESTED, REJECTED DECISIVELY. 5-seed smoke, both-seats, vs
# v9.5: 0/5 wins, mean -$21,384 both-seats-sum (-$10,692/game), t=-8.66,
# 95% CI [-$26,225,-$16,544] -- not noise, a real regression. Stopped
# before testing vs opp_scenario_v14 (no plausible recovery from a loss
# this clean vs the milder baseline). Likely cause: the reservation set
# is recomputed from scratch every turn from CURRENT hand position, so
# as hands walk toward whatever they were doing, "farthest from every
# pasture" reshuffles turn to turn -- a hand can be near-pasture (freed
# for maintenance) one turn, walk toward a distant crop tile because
# that's what it was mid-assignment on, and by arrival find itself now
# the farthest-away hand and get walled back into crop-only, permanently
# never able to hand off maintenance to whoever's actually closest. The
# old fixed-roster-index reservation was unintentionally more stable:
# indices don't reshuffle turn to turn, so the same physical hands
# tended to keep the same role and finish what they started. Spatial
# reassignment without hysteresis just thrashes. NOT submitted.
# =====================================================================
...
def _agent(obs):
    ...
    # v9.18: WHICH hands are walled off for crop work is now spatial
    # (farthest-from-any-pasture first) instead of fixed roster order.
    # v7.6a always reserved indices [0, reserved_count) regardless of
    # position -- a hand standing next to a pasture with a low roster
    # index got walled off from maintenance it was already adjacent to,
    # while a high-index hand across the map still tried to answer
    # animal_maintenance_action and walked the whole way. Reserving the
    # hands actually farthest from every pasture aligns "who's walled
    # off from animal duty" with "who was going to have a long walk to
    # do it anyway," on the theory this reduces wasted travel without
    # changing reserved_count's existing proportional-to-neglect logic
    # (only WHICH hands fill the same count changes, not how many).
    pasture_positions = [pos for pos, _t in view["my_pastures"]]
    if pasture_positions and reserved_count > 0:
        dist_to_pasture = [
            (min(_dist(tuple(h), pp) for pp in pasture_positions), i)
            for i, h in enumerate(me["hands"])
        ]
        dist_to_pasture.sort(key=lambda p: (-p[0], p[1]))  # farthest first, stable
        reserved_idx = {i for _d, i in dist_to_pasture[:reserved_count]}
    else:
        reserved_idx = set(range(min(reserved_count, len(me["hands"]))))

    # v7.6a: `reserved_count` hands are walled off for crop work -- they
    # may still answer a feed-critical animal need (critical_only still
    # checks fed_today), but never the optional maintenance (CARE/
    # HARVEST/COLLECT_FERTILIZER/product deposit) that was crowding out
    # watering/weeding in the real loss replays. Remaining hands keep
