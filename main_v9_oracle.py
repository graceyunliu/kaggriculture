...
# =====================================================================
# MAIN V9 ORACLE -- opponent-aware SELL TIMING overlay on main_v8.3.py.
# Byte-identical to v8.3 except: (a) a market-forecast module (town-drain
# schedule + live opponent-production estimate read off obs.farms[1-p]),
# and (b) four lines in economy()'s sell loop that consume it.
#
# Hypothesis under test (v9 "semi-adaptive" direction, Aug 6): countering
# the opponent's observable supply schedule beats optimizing our own
# fixed strategy. This is the ORACLE step -- no strategy classification
# yet; the forecast reads the opponent's live farm state directly, which
# works against any opponent and is exactly what a rules-engine detector
# would feed. If this can't beat v8.3, detection isn't worth building.
#
# Mechanism: for each product, project price FORECAST_H days out from
#   inv_future = inv_now + opp_supply/day * H - town_drain/day * H
# where town drain is EXACT (engine's shop/center schedule is public via
# obs.town.unlocked_shops) and opp supply is estimated from their tiles:
# ongoing-crop production events remaining, standing unharvested
# yield_units (their future dumps), and animal yield intervals.
#   projected/current <= 0.95  -> FRONT-RUN: their supply is about to
#       crash this price; sell now at a lowered floor, and ignore the
#       timing-engine holding phase for this item.
#   projected/current >= 1.08  -> HOLD: the pool is draining faster than
#       anyone refills it (prices INFLATE over a match per v8.2 analysis);
#       skip ordinary paced sells. Shed pressure, force_dump and
#       LIQUIDATE_DAY overrides all still apply unchanged.
# Rationale vs the meta-clone (93.2% of ladder field, see
# kaggriculture_opponent_strategy_identifiers.md): its strawberry supply
# ramps 10->50 units/day over days 17-28 (kaggriculture.db sell_events,
# 258 clone seats) -- exactly when v8.3's timing engine HOLDS premium
# crops (days 17-19). WOOL/MILK glut sides are linear/sq (brutal), WHEAT
# glut side is log (dump-safe): timing, not volume, is where the money is.
#
# =====================================================================
...
# =====================================================================
# v9 ORACLE: opponent-aware market forecast (see file header)
# =====================================================================
...
SHOP_FIRES_PER_DAY = 6      # engine: step % townShopSellInterval(4), 24 turns/day
CENTER_FIRES_PER_DAY = 2    # engine: step % townCenterSellInterval(12)

FORECAST_H = 2.0            # projection horizon, days
FORECAST_SELL_EDGE = 0.95   # projected/current at/below this -> front-run
FORECAST_HOLD_EDGE = 1.08   # at/above this -> hold paced sells
HOLD_LAST_DAY = 24          # never hold at/after this day
HOLD_SHED_HEADROOM = 25     # don't hold when shed within this of cap
FRONTRUN_FRAC_DROP = 0.20   # sell-floor drop for about-to-crash items

ANIMAL_PRODUCT_OF = {"COW": "MILK", "SHEEP": "WOOL", "GOOSE": "EGG"}
ANIMAL_YIELD_SPECS = {"COW": (8, 2), "SHEEP": (6, 3), "GOOSE": (4, 1)}  # (first, interval)


def _town_drain_per_day(town, day):
    """Exact per-item town consumption per day from the engine's public
    schedule (unlocked shops fire 6x/day, center 2x/day at the day-10/20
    demand multipliers)."""
    drain = {}
    for shop in (town or {}).get("unlocked_shops", []):
        products = TOWN_SHOPS.get(shop, [])
        mult = 2 if len(products) == 1 else 1
        for item in products:
            drain[item] = drain.get(item, 0.0) + mult * SHOP_FIRES_PER_DAY
    center_mult = 4 if day >= 20 else (2 if day >= 10 else 1)
    for item in MARKET_PARAMS:
        if item != "FERTILIZER":  # fertilizer has no town demand at all
            drain[item] = drain.get(item, 0.0) + center_mult * CENTER_FIRES_PER_DAY
    return drain


def _opp_supply_per_day(opp, day):
    """Units/day the opponent is positioned to push to market near-term,
    read off their public farm state: remaining ongoing-crop production
    events, one-shot crops near ripeness, animal yield intervals, plus
    standing unharvested yield_units smeared over the horizon (their
    future dump). Deliberately coarse -- direction is what matters."""
    supply = {}

    def add(item, rate):
        supply[item] = supply.get(item, 0.0) + rate
    ...
    for row in opp["tiles"]:
        for t in row:
            ...
            if t.get("kind") == "PLANT":
                ...
                stock = t.get("yield_units", 0)
                if stock > 0:
                    add(crop, stock / FORECAST_H)
                if c["ongoing"]:
                    step = max(1, c["interval"])
                    events = 0 if age < c["first"] else (age - c["first"]) // step + 1
                    if events < c["max_yield"] and age >= c["first"] - 2:
                        add(crop, 1.0 / step)
                elif c["first"] - 2 <= age <= c["max_day"]:
                    add(crop, c["max_yield"] / 3.0)
            elif t.get("animal"):
                item = ANIMAL_PRODUCT_OF.get(t.get("animal"))
                spec = ANIMAL_YIELD_SPECS.get(t.get("animal"))
                if not item or not spec:
                    continue
                first, interval = spec
                stock = t.get("yield_units", 0)
                if stock > 0:
                    add(item, stock / FORECAST_H)
                if day - t.get("placed_day", day) >= first - 2:
                    add(item, 1.3 / interval)  # +30% for typical care bonuses
    return supply


def market_forecast(obs, opp, day):
    """Per-item ratio of projected price FORECAST_H days out to current
    price, on the path where WE hold: inventory moves by opponent supply
    minus town drain. <=FORECAST_SELL_EDGE means their supply is about
    to crash it; >=FORECAST_HOLD_EDGE means the pool is draining and
    holding earns more."""
    inv = obs["market"]["inventory"]
    drain = _town_drain_per_day(obs.get("town"), day + 1)
    opp_sup = _opp_supply_per_day(opp, day)
    ratios = {}
    for item in MARKET_PARAMS:
        inv_now = inv.get(item, I0)
        net = opp_sup.get(item, 0.0) - drain.get(item, 0.0)
        p_now = market_price(item, inv_now)
        p_fut = market_price(item, inv_now + net * FORECAST_H)
        ratios[item] = (p_fut / p_now) if p_now > 0 else 1.0
    return ratios
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    forecast = market_forecast(obs, opp, day)  # v9
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        fc = forecast.get(item, 1.0)                       # v9
        falling = fc <= FORECAST_SELL_EDGE                 # v9
        rising = (fc >= FORECAST_HOLD_EDGE and day < HOLD_LAST_DAY
                  and shed_load < SHED_CAP - HOLD_SHED_HEADROOM)  # v9
        reason = "paced"
        if is_attack and item in PREMIUM_CROPS:
            qty = max(1, int(n * timing_engine.dump_ratio))
            reason = "attack"
        elif is_holding and item in PREMIUM_CROPS and not force_dump and not falling:
            continue
        elif day >= LIQUIDATE_DAY:
            qty = int(n)
            reason = "final"
        elif falling and not force_dump:
            # v9 FRONT-RUN: opponent supply (or a fading town-demand pool)
            # is about to push this price down -- sell ahead of it at a
            # lowered floor rather than holding into the glut.
            frac = (0.85 if item in PREMIUM_CROPS else 0.70) - FRONTRUN_FRAC_DROP
            if view["opp_imminent"].get(item, 0) >= 4:
                frac -= 0.05
            floor = max(0.30, frac) * base
            relax = max(0.0, 1.0 - max(0, shed_load - 40) / 50.0)
            qty = units_sellable_above(item, inv.get(item, I0), floor * relax, int(n))
        elif rising and not force_dump:
            # v9 HOLD: pool draining faster than anyone refills it --
            # holding earns more. Shed pressure (force_dump), capacity and
            # LIQUIDATE_DAY overrides all still fire via their own branches.
            continue
