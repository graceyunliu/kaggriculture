...
| 6 | [REPORT_PHASE6_LAND_ALLOCATION.md](REPORT_PHASE6_LAND_ALLOCATION.md) | Attempted causal test of Phase 5's land-allocation hypothesis; the mandatory baseline-representativeness check FAILED (current champion candidates V3_12/V3_15 reach 100% land-expansion in simulation vs Phase 5's measured 59.3% real-play figure, which turns out to be a mixture statistic across Grace's whole play history, not a current-policy property) -- no intervention variant was built. Status: NO_ROBUST_SIGNAL. |
| 7 | [REPORT_PHASE7_MELON_CANDIDATE.md](REPORT_PHASE7_MELON_CANDIDATE.md) | Retested Phase 4B's melon-sell-threshold hypothesis with a verified baseline (current champion V3_15's own measured ~43-54% opportunity-conditioned sell rate, not C1's unrepresentative near-unconditional selling). Manipulation worked as intended (rate -> 100%) but score effect was inconsistent in sign/significance across fresh seeds and 3 opponent cohorts -- strengthens, does not reopen, Phase 4B's null result. Status: NO_ROBUST_SIGNAL. |
| 8 | [REPORT_PHASE8_LABOR_HEADCOUNT.md](REPORT_PHASE8_LABOR_HEADCOUNT.md) | Mining campaign Lead 1: opponent-vs-opponent natural experiment found max_hands correlates with score (r=0.187, t=3.79, n=400); causal test (force +3 hands from same budget) found no score effect across matched+fresh seeds/3 opponents -- confound (richer play affords both) not ruled out in headcount's favor. Status: FALSIFIED_AS_CAUSAL. |
| 9 | [REPORT_PHASE9_CHORE_PRIORITY.md](REPORT_PHASE9_CHORE_PRIORITY.md) | Mining campaign Lead 4: chore-type priority ordering (weeds ahead of harvest). Initial 4-test external-opponent design showed a promising positive signal (pooled t=1.97, n=160), but did NOT replicate under a scaled, more direct head-to-head confirmation test (200 games, t=-1.53, negative-leaning). Status: FAILED_CONFIRMATION. |
| 10 | [REPORT_PHASE10_FERTILIZER_TIMING.md](REPORT_PHASE10_FERTILIZER_TIMING.md) | Mining campaign Lead 5: fertilizer target-tile eligibility timing (opened 2 days earlier per tile in _fert_eligible). Tested with the direct head-to-head harness as the PRIMARY design from the start (the fix for Lead 4's false positive). Replicated across 2 independent 200-game batches (t=2.45, t=2.97, pooled t=3.84, n=400) and corroborated vs an external opponent. RNG_INDEPENDENT. Status: CONFIRMED (diagnostic only, not promoted). |
