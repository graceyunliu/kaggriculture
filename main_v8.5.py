...
# =====================================================================
# MAIN V8.5 -- relax SHEEP's crop-space check too, ISOLATED. Base:
# main_v8.3.py (last_day stays 22 -- a follow-up build, main_v8.4.py,
# tried lowering it to 17 from a breakeven calc and was REJECTED, see
# below). Byte-identical except for one relaxed threshold in
# _sheep_expansion_feasible.
#
# --- Why (diagnosis, not a guess) ---
#
# Instrumented _sheep_expansion_feasible's five checks turn-by-turn
# across all 15 standard seeds. Finding: crop-space (check 5,
# `available_for_crops < EXPANSION_MIN_CROP_TILES_PER_QUAD * quads`) is
# the dominant blocker almost everywhere -- 267-315 occurrences per
# match out of ~600 checked turns, MORE than hand-capacity (85-140),
# and this holds even in v8.3's own WINNING seeds (1, 555, 3003): sheep
# spent most of each match blocked on crop-space, with only a brief
# multi-turn window where every check happened to align and let a
# purchase through. main_v8.3.py only relaxed hand-capacity
# (SHEEP_HAND_HEADROOM) -- crop-space was never touched, so it's still
# the tighter constraint in most games. This directly explains why the
# aligned-PASS window lands so late (day 27-28) or never in v8.3's 5
# tied seeds: crop-space, not hand-capacity, is what's actually keeping
# the gate shut for most of the match.
#
# --- Why EXPANSION_MIN_CROP_TILES_PER_QUAD's floor is the wrong bar for
#     a single incremental sheep in the first place ---
#
# The reserve (8 tiles/quadrant) exists to stop the fleet from crowding
# out crop land wholesale -- sized for a general "don't let animals eat
# the farm" safety margin, not calibrated for "is there room for ONE
# more pasture tile," which is all a single additional sheep actually
# needs (same one-tile footprint as a cow). Applying the full per-quad
# floor to a marginal single-animal decision is stricter than the
# decision it's gating.
#
# --- The change ---
#
# SHEEP_CROP_SPACE_HEADROOM = 10: _sheep_expansion_feasible's crop-space
# check now compares against `EXPANSION_MIN_CROP_TILES_PER_QUAD * quads
# - SHEEP_CROP_SPACE_HEADROOM` instead of the raw per-quad floor -- same
# structural idea as v8.3's SHEEP_HAND_HEADROOM (relax the one check
# that's specifically gating a marginal addition, leave the other four
# -- survival, wheat-budget, crop-neglect -- exactly as strict as
# COW's). COW's own gate and crop-space check are completely untouched.
#
# --- REJECTED sibling build: main_v8.4.py (SHEEP last_day 22 -> 17) ---
#
# Computed from real engine numbers (SHEEP: cost 500, first_yield_day 6,
# interval 3, no lifespan cap) that a sheep needs >= 3 production ticks
# to safely cover its $500 cost even at the unrealized $200 base price,
# which requires placement by day 17. Tested seeded --both-seats vs
# main_v8.3.py, 15 seeds: 0/15 wins, 3/15 losses (seeds 303, 17, 3003 --
# all three of which had opened at day 18 in v8.3 and WON), 12/15 ties,
# mean -$631/game, t=-1.80. The conservative floor-price breakeven calc
# was wrong in practice -- v8.3's actual 2-tick (day 18-20) purchases
# were realizing enough value to be real wins, most likely because this
# project's own real ladder data shows WOOL realizing $239-291/unit, not
# the $200 base the conservative calc used, plus uncounted fertilizer
# byproduct and possible care-bonus ticks. Lesson: the correctness
# argument for decoupling was right (SHEEP not sharing COW's number is
# still correct), but the specific replacement value was wrong --
# caught by testing, not assumed. main_v8.4.py is kept in the folder as
# a documented rejected experiment; SHEEP's last_day here stays 22.
#
# --- Test protocol ---
#   Seeded --both-seats vs main_v8.3.py, same 15-seed set as every prior
#   v8.x round. Smoke tests: starter/pass 720-turn, starter 48-turn
#   truncated, all must stay DONE/DONE.
# =====================================================================
#
# =====================================================================
...
# v8.5: crop-space (not hand-capacity) is the dominant real blocker of
# sheep's gate in most games, including v8.3's own winning ones -- see
# file header. Relax the per-quad crop-tile floor by this many tiles for
# sheep's check specifically; a single incremental sheep only needs room
# for one more pasture tile, not the full general-purpose reserve.
SHEEP_CROP_SPACE_HEADROOM = 10
...
def _sheep_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads):
    ...
    # v8.5: relaxed crop-space floor -- see file header. Checks 1, 2, 4
    # above are identical/unrelaxed vs _animal_expansion_feasible(); this
    # is the second (alongside hand-capacity) of the two checks this
    # build treats as scaled for a marginal single-animal addition
    # rather than a whole-quadrant safety margin.
    available_for_crops = len(view["empty"]) - len(reserved_sites) - 1
    if available_for_crops < EXPANSION_MIN_CROP_TILES_PER_QUAD * quads - SHEEP_CROP_SPACE_HEADROOM:
        return False
