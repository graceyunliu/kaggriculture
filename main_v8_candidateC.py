...
# =====================================================================
# MAIN V8 CANDIDATE C -- v7.11's already-validated cost-of-delay order
# sort (+$1,279/game over 40 seeded games historically; re-measured here
# at mean +$5,143/11 seeds, win 8/11, vs v7.9 in the current test set --
# see project report) PLUS Candidate B's WHEAT/STRAWBERRY demand-
# headroom reweight (want["WHEAT"]: 8 -> min(space,25); want["STRAWBERRY"]:
# 6/10 -> 8/14). Both changes are in the two `want[...]` lines below;
# everything else is byte-identical to v7.11 (which is itself
# byte-identical to v7.9 except for the order-priority scoring). Does
# NOT include Candidate A's fertilize pipeline -- that was empirically
# rejected (decisive loss in 3 tested variants, see project report).
# =====================================================================
#
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        # Candidate C = v7.11 (cost-of-delay order priority) + Candidate
        # B's crop reweight (raised further from v7.9's 6/10 -- still
        # well under the 534-unit town demand pool headroom). B itself
        # measured as a near-wash in isolation (+$5,384 mean/11 seeds,
        # not significant) but C tests whether it compounds usefully
        # with the order-priority fix, since v7.11's seed-purchase
        # priority now rises toward each crop's cutoff instead of
        # competing in a flat fixed bucket -- a bigger want[] target
        # might realize more of its value once seed orders can no longer
        # get starved by bucket contention.
    ...
    if day <= CUTOFF["CARROT"]:
        want["CARROT"] = 4
    if day <= CUTOFF["WHEAT"]:
        # Candidate C: v7.9/v7.11's flat 8 was tiny against a 635-unit
        # pool (the largest in the game); WHEAT is last in PLANT_PRIORITY
        # so this only fills tiles the premium crops don't claim first.
