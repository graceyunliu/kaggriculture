...
- Animal production audited, NO DISCREPANCY (Sep 11): tools/animal_yield.py (wraps the engine's _daily_refresh_animals, both farms, same games) shows the policy's RATE (COW 1.5 / SHEEP 1.33 u/animal-day) is the perfect-care steady-state ceiling and WE REACH IT -- fed at 100% of production events, entitled 3.35/4.31 u/event vs the 3.00/4.00 steady state, 0% lost to the max_held=6 clamp. Tapes run 86-95% fed, 1.09-1.15 (cow) and 1.08-1.12 (sheep) u/animal-day, and lose 1% of cow / 19% of goose production to the cap. Animal service is a STRENGTH, not a gap: do not propose feeding/care/collection improvements or RATE/FIRST_YIELD corrections. New engine fact: the FIRST production event banks base + the bonus accrued across the entire pre-yield ramp, clamped at 6 -- pre-yield care days are banked, not wasted, and that clamp is why GOOSE truncates. Writeup: experiments/ANIMAL_YIELD_AUDIT_20260911/REPORT.md.
- DURABLE (Sep 11): a protected economic window may tolerate EDGE TRIMMING while being highly sensitive to INTERIOR INTERRUPTION. Diagnostic for any future commitment mechanism: test trimming the boundary and punching a hole in the middle as two different questions -- they have opposite signs on the melon convoy (h7 mildly beats h8; urgency-shaped holes lose on all four tapes).
- DURABLE (Sep 11): the h<=8 melon convoy is a protected economic COMMITMENT, not spare dispatch capacity. The orchestrator can correctly identify an urgent obligation and still be economically wrong to take capacity from that commitment -- the O18 lesson one level up (generic local timing has no value; protecting a persistent economic cycle does). Nuance worth keeping: h7 mildly BEATS h8 on the cutoff sweep while punching urgency-shaped holes in the window loses on all four tapes, so the convoy's value is in being CONTIGUOUS, not in its total length -- ending it slightly earlier is cheap, interrupting it is not. Check any future candidate that spends capacity mid-window against this before testing it.
- Urgent-water exemption for the melon convoy (Sep 11): REJECTED. O32_MELON_URGENT_EXEMPT (renamed from O30 to avoid the collision with the other session's O30_MELON_URGENT_YIELD; melon morning yields a unit back to normal dispatch when the orchestrator assigned it an URGENT crop-water task) and O32B (pre-production tiles only) both lose vs O26 on the fixed-shops panel: pooled n=480 over 3 fresh seed sets, O32 margin -592 (t-4.3) / own -733 (t-5.9), O32B -594 / -626, negative on all four tapes. straw_life shows why: early deaths 2/155 -> 1/153 while watered-on-production-night falls 65% -> 59% -- the exemption relocates displacement instead of removing it, because `urgent` is a common task kind and the held-back unit is the nearest one. CORRECTION for the whole strawberry-death family: the 17% early-death figure was a seeds-11-14 artifact; O26 vs tape_bahaenes dies before first production at 12%/2%/0%/1%/1%/8% across seed windows 11-14, 15-18, 21-24, 151-154, 171-174, 191-194 (~4% pooled). The tapes' 0% is real, so the gap exists, but size the prize on >=20 fresh seeds and stop quoting 17%. Untested cheaper shapes: service urgency at the window handoff, or exempt only when the urgent tile is already on the unit's path. Writeup: experiments/MELON_URGENT_20260911/REPORT.md; generator evolve/gen_melon_urgent.py.
- Adaptive melon-window termination (Sep 11): CLOSED. 2,240-game cutoff sweep on O26 (MELON_MORNING_LAST_HOUR h6-h10, fixed shops, 4 tapes, both seats, 4 fresh seed sets) shows the optimal cutoff does NOT vary with day-9 farm state. Segment argmax does not replicate across seed sets, and within a set the low and high segments of the same variable pick the SAME cutoff (animals low=h10 and high=h10 in set A; h7/h7 in set B) -- a whole-set shift, not a state-dependent boundary. Tested: animal count, hands, plantings, cash, strawberry tiles, weeds; melon tiles and land are degenerate (10 tiles / 2 quads in nearly every game). Do not build a marginal-conversion-vs-displaced-work score over ENTRY-state features. CAVEAT: the join key had to be cutoff-invariant, so only day-9 h0 state could be segmented; the convoy's real cost falls on strawberries planted INSIDE the window, so the per-turn urgency exemption (consult the urgent crop-water pool before claiming a routeless unit for melon duty, or reserve 1 unit days 9-14) is NOT closed by this and remains the best candidate in the family. Any state-dependent timing proposal must first show its segment argmax replicates out of sample -- ~2 min/800 games. Incidental, NOT promoted: h7 beats h8 by +295 margin/+242 own pooled (t3.3/3.2, n=640) but fails 1 of 4 seed sets and wins only ~51% of games; read it as h8 having been tuned on the O22 chassis, not as a promotable change. Note O22's window already excludes animal-route units (i not in S["routes"]), so the displaced work at the boundary is crop/service work. Full writeup: experiments/MELON_CUTOFF_20260911/REPORT.md.
...
## Sep 11: melon_commitment_recoverability -> ABANDON (tools/melon_trace.py)

The "deferred melon pickup becomes a lost sale" premise does not hold. Per-tile trace of O26 vs O32 on 10 bahaen seeds:
every melon that reaches 6 units is harvested and sold in both; melon units == 6 x tiles in every game; the 8-11-unit
delta is a second-wave (d10-11) melon REPLANT-count difference that goes either way by seed (O32 +14 units on s151/153,
-12 on s155/191). Melon weed-deaths 1 each in 10 games. Money deltas do not track melon units (s171: O32 +12 units,
-9.4k). The O32 loss is diffuse allocation, as the relocation finding said. Melon-side fixes are closed with the family.

## Sep 11 (late): melon family closed on every axis; durable rule; champion record

- Closed: entry-state adaptive cutoff (2,240 games), urgent-exemption triggers (O30/O31/O32/O32B, 1,440 games, negative on
  all four tapes), deferred-pickup recoverability (tools/melon_trace.py). All three are in `db.REJECTED_MECHANISMS` and
  `evolve/directions.yaml`. Kept as local observations, not reopeners: one O32 melon weed-death (the exemption starving a
  melon's own urgent water), one unreplicated sale-lost anomaly (s171, 102 harvested / 91 sold).
- Durable rule: a protected block tolerates EDGE TRIMMING (h7 vs h8, +$250 lead) but not INTERIOR INTERRUPTION.
- `hire_gate_output_loss` is reframed as an accounting bridge ($804 wages -> service degradation -> ~$504 output): the
  fertilizer-bonus piece is closed from engine code (+1.2 strawberry units, 15-18%); the weed piece needs
