...
class Loop:
    ...
    def consume_queue(self):
        ...
        files = sorted(p for p in self.queue_dir.glob("*.json"))
        for f in files:
            if self.out_of_budget():
                return
            try:
                ...
                shutil.move(str(f), str(self.queue_dir / "done" / (f.name + ".bad")))
                continue
            kind = item.get("kind", "candidate")
            origin = item.get("origin", "queue")
            island = item.get("island", "queue")
            parent = item.get("parent")
            parents = item.get("parents") or ([parent] if parent else [])
            ...
            if kind == "candidate":
                ...
                blocks = blocks_mod.resolve_sources(item.get("blocks"), ROOT)
                self.evaluate(params, blocks, parents, f"{origin}:{f.stem}", island, item.get("note"))
                n = 1
            elif kind == "factorial":
                axes = {k: v for k, v in (item.get("axes") or {}).items() if k in space.SPACE}
                bopts = {b: [blocks_mod.resolve_source(v, ROOT) for v in values]
                         for b, values in (item.get("block_options") or {}).items()}
                ...
                for combo in itertools.product(*levels):
                    ...
                    self.evaluate(params, blocks or None, parents, f"{origin}:{f.stem}", island,
    ...
    def generate_one(self, island):
        ...
        elif self.rng.random() < cfg.get("paired", 0.0):
            if self.rng.random() < cfg.get("block_pair", 0.0):
                child, cblocks = operators.block_pair_mutate(pparams, pblocks, rng=self.rng)
                self.evaluate(child, cblocks, [parent["key"]], "block_pair", island)
            else:
                child = operators.paired_mutate(pparams, rng=self.rng, sigma_frac=cfg["sigma"])
                self.evaluate(child, pblocks, [parent["key"]], "paired", island)
        else:
        ...
        try:
            ...
            while not self.out_of_budget():
                ...
                if self.out_of_budget():
                    break
                self.gen += 1
                if self.args.crossover_interval and self.gen % self.args.crossover_interval == 0:
                    paths = crossover_mod.queue_top_crossovers(
                        self.db, self.queue_dir, self.gen, self.rng,
                        top_k=self.args.crossover_top_k, count=self.args.crossover_count,
                        k_sha=self.k_sha, frontier=self.args.frontier)
                    if paths:
...
def main():
    ...
    ap.add_argument("--crossover-top-k", type=int, default=10)
    ap.add_argument("--crossover-count", type=int, default=2)
    ap.add_argument("--paired-rate", type=float, default=0.0,
                     help="fraction of non-crossover children on the c1/wide islands generated by "
                          "operators.paired_mutate (econ+exec) instead of space.mutate")
    ap.add_argument("--block-pair-rate", type=float, default=0.0,
                     help="fraction of paired children (see --paired-rate) that swap an execution "
