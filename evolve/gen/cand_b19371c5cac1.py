...
KNOBS = {'melon_floor': 150, 'harvest_min': 1, 'opening': 'frontier', 'wheat_tiles': 0, 'wheat_stock': 0, 'min_hands': 4, 'load_per_hand': 20, 'geese': 0, 'open_melons': 10, 'open_wheat': 7, 'open_cows': 2, 'open_sheep': 2, 'early_hire_days': 5, 'feed_spare_poor': 0, 'fert_keep': 0, 'fert_buy': 3, 'fert_carry': 2, 'demand_share': 0.55, 'max_animals': 17, 'wheat_per_animal': 0.0, 'wheat_cap': 22, 'wheat_water_tier': 0, 'wheat_sell_price': 30, 'wheat_hold_days': 0, 'sell_hourly': 0, 'drop_min': 0, 'drop_radius': 0, 'capital_hour2': -1, 'melon_rush': 0, 'straw_delay': 0, 'hands_early': 0, 'setup_capital_share': 0.25, 'labor_reserve_buffer': 134}
...
def _load_model(v, seeds_planned, n_animals_total, n_setup, day):
    ...
    _lph = KNOBS["load_per_hand"]
    if 8 <= day <= 15:
        _lph = max(6, _lph * 0.75)
    else:
        _lph = _lph * 1.08
    tgt = int(math.ceil(load / _lph))
