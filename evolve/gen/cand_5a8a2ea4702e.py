...
KNOBS = {'melon_floor': 0, 'harvest_min': 1, 'opening': 'frontier', 'wheat_tiles': 0, 'wheat_stock': 0, 'min_hands': 3, 'load_per_hand': 20, 'geese': 0, 'open_melons': 10, 'open_wheat': 7, 'open_cows': 2, 'open_sheep': 2, 'early_hire_days': 3, 'feed_spare_poor': 0, 'fert_keep': 0, 'fert_buy': 3, 'fert_carry': 2, 'demand_share': 0.55, 'max_animals': 17, 'wheat_per_animal': 0.0, 'wheat_cap': 22, 'wheat_water_tier': 0, 'wheat_sell_price': 25, 'wheat_hold_days': 0, 'sell_hourly': 0, 'drop_min': 0, 'drop_radius': 0, 'capital_hour2': -1, 'melon_rush': 0, 'straw_delay': 0, 'hands_early': 0, 'setup_capital_share': 0.3, 'labor_reserve_buffer': 92}
...
def economy(obs, v, pending_drop=None):
    ...
    # Stateful capital checkpoint: accumulate cash changes since the last
    # capital evaluation instead of comparing with only the previous turn.
    if S.get("capital_event_day") != day:
        S["capital_event_day"] = day
        S["capital_events"] = 0
        S["capital_anchor_cash"] = cash
    anchor = S.get("capital_anchor_cash", cash)
    event = (2 <= hour <= 12 and 1 <= day <= 20 and
             cash >= anchor + 100 and S.get("capital_events", 0) < 3)
    if event:
        S["capital_events"] = S.get("capital_events", 0) + 1
        S["capital_anchor_cash"] = cash
    orders = []
...
ORCH_ON = 0# 0 = no orchestration (exactly O15), 1 = global crop-task assignment (exactly O16)
