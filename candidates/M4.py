...
SWITCH_DAY = 30
# M4: turn-by-turn ablation search (Sep 8) over every one of M2's 247 copied market actions --
# each one individually removed and evaluated against a 6-opponent panel that M2 (=H32 exactly
# on these opponents, see below) loses to. Three of them turned out to be net-harmful: day-4
# BUY_PRODUCT WHEAT 1 micro-purchases at steps 97/101/103 (three separate 1-unit top-ups a few
# hours apart), each buying a single unit at an elevated per-unit market price for negligible
# benefit. Skipping just these three (SKIP_STEPS) gains +$5.2k/game across the 6 opponents that
# were previously identical to H32, and +$2.0k/game across the full 29-opponent replay panel
# (+38,314 vs M2's +36,316, 69-18 vs 68-19 wins) with no opponent regressing by more than ~$400.
# IMPORTANT: combining removals is NOT additive -- removing several of these together (even ones
# that each individually help) pushes the live animal/hand counts far enough from the recorded
# _SIG signature to trip _diverged() and fall back to the much weaker general dispatcher for the
# rest of the game, which is catastrophic (see SWITCH_DAY sweep: forcing an earlier handoff to
# _agent() alone, with no tape at all, loses -$195k vs these opponents rather than -$23k). So this
# short list was chosen by testing single-turn removals in isolation, confirming no divergence is
# triggered, and validating the specific surviving combination together -- not by removing every
# individually-positive turn.
SKIP_MARKET_STEPS = {97, 101, 103}
MONEY_TOL = 800
COUNT_TOL = 1
...
def agent(obs, configuration=None):
    ...
    if not _H["switched"] and step < SWITCH_DAY * 24 and step < len(_TAPE):
        ...
        if why is None:
            ...
            if step in SKIP_MARKET_STEPS:
                act = _json.loads(_json.dumps(_TAPE[step]))
                act["market"] = []
                return act
            return _TAPE[step]
        _H["switched"] = True
