...
def _fert_eligible(t, day):
    ...
    if not c["ongoing"]:
        if t.get("crop") != "MELON":
            return False
        # melon: fertilize late in the yield window (age 7-8) so the remaining waterings
...
def _crop_step(i, pos, v, day, hour, carry, pools, seeds_left):
    ...
    while sweep:
        ...
        if kind in ("harvest","mharv") and c and not c["ongoing"] and not t.get("watered_today") and (c["max_day"]+1)//2<=age<=c["max_day"] and t.get("yield_units",0)<c["max_yield"]:
            return ["WATER"]
        sweep.pop(0)
        if kind in ("urgent", "water", "slack", "wwater"):
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if day >= 28 and prod_carried >= (3 if day == 29 else 6) and i not in S["routes"]:
        ...
        if day == 29 or hour >= 18:
            return [_step(pos, _nearest(pos, unlocked_shed))]
    # 2a. melon return: a full tile of melons on the first melon days goes straight to the shed
    if 10 <= day <= 11 and carry.get("MELON", 0) >= 6 and carry.get("WHEAT", 0) == 0 and i not in S["routes"]:
        if pos in unlocked_shed:
            return ["DROP"]
        return [_step(pos, _nearest(pos, unlocked_shed))]
