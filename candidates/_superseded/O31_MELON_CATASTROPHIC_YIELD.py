...
def _catastrophic_urgent(v, day):
    # O31: of the urgent (cu>=1) crop-water pool, only the pre-first-production tiles of an ONGOING
    # crop (STRAWBERRY/TOMATO) are catastrophic: one more missed day converts them to WEED at 0 units
    # ever produced (straw_trace.py). A one-shot crop (WHEAT/CARROT/MELON) or a post-production ongoing
    # tile losing a watering only costs partial yield, not the whole planting -- not worth surrendering
    # melon-morning timing for. O30 gated on ANY urgent task and came out a wash (mixed +-, ~$0 net);
    # this narrows the trigger to the actual catastrophic-loss case.
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if MELON_MORNING and 9 <= day <= 14 and hour <= MELON_MORNING_LAST_HOUR and i not in S["routes"]:
        ...
        if mc.get("day") != day:
            mc.clear(); mc["day"] = day
        carrying_melon = carry.get("MELON", 0) >= 4 or (carry.get("MELON", 0) > 0 and not any(
                _t.get("crop") == "MELON" and (_harvest_ready(_t, day) or _t.get("yield_units", 0) >= MELON_MORNING_MIN_YIELD)
                for q, _t in v["crops"]))
        if carrying_melon:
            # already committed cargo: always finish the delivery regardless of urgent crop work.
            if pos in unlocked_shed:
                return ["DROP"]
            return [_step(pos, _nearest(pos, unlocked_shed))]
        # O30: a strawberry (or other crop) that has already missed one watering day is one more miss
        # from permanent weed-conversion with zero yield ever produced (straw_trace.py / straw_displace.py:
        # this unit-selection loop ran 48/48 times on a dying tile's fatal day without ever seeing it,
        # because MELON_MORNING claimed every routeless unit before the crop dispatcher was ever consulted).
        # If urgent crop-water work is still unclaimed this turn, let this unit fall through to the normal
        # dispatcher instead of picking up a new melon target -- but a unit already walking toward/holding
        # a specific melon tile (mc[i] still valid) keeps going, so in-progress melon trips aren't wasted.
        tp = mc.get(i)
        if tp is not None:
            tt = v["tiles"][tp[1]][tp[0]]
            if not (isinstance(tt, dict) and tt.get("crop") == "MELON" and tt.get("yield_units", 0) > 0):
                mc.pop(i, None); tp = None
        if tp is None and not _catastrophic_urgent(v, day):
