...
def _build_sweep(i, pos, v, day, hour, carry, pools, seeds_left):
    tiers = ["urgent", "wwater", "harvest", "water"] if hour >= 10 else ["urgent", "wwater", "harvest"]
    if carry.get("FERTILIZER", 0) > 0:
        tiers = ["urgent", "fert", "wwater", "harvest", "water"] if hour >= 10 else ["urgent", "fert", "wwater", "harvest"]
    if hour < 22:
        tiers.append("plant")
    tiers.append("weeds")
    if hour >= 14:
