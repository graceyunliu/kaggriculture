...
S = {"day": -1, "routes": {}, "crop": {}, "sweep": {}, "setup": {}, "claimed_sites": set(), "animal_claim": set(),
     "hires_target": 0, "wheat_budget": 0, "early_bundle_deferral_used": False}
...
def economy(obs, v):
    ...
    # V3.24: trajectory-derived one-shot early circuit breaker. When a farm
    # with <=4 active hands proposes >=4 animals by day 8, preserve only
    # sales/feed this turn and replan all investments from the next state.
    animal_units = sum(int(o[2]) for o in orders if o and o[0] == "BUY_ANIMAL")
    if (not S["early_bundle_deferral_used"] and day <= 8
            and n_hands <= 4 and animal_units >= 4):
        S["early_bundle_deferral_used"] = True
        orders = [o for o in orders if o and o[0] in ("SELL", "BUY_PRODUCT")]
    return orders[:MAX_ORDERS]
