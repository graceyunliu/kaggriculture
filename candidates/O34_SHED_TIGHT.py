# O34_SHED_TIGHT: O26_CARROT_SIZING with the shed_load sell-trigger margins tightened,
# melon shed_load>75 -> >65, wheat surplus shed_load>80 -> >70. Tests shed_capacity_margin_calibration's
# would_do criterion (evolve/directions.yaml, corrected Sep 12): O26 mean max shed_load runs 91-94,
# touching 100 in 38-75% of games with zero measured discard/blocked-buy loss so far -- this checks
# whether pulling the sell triggers in further reduces ceiling contact without costing own money.
# Sep 11 governance note: this is a Stage-1 isolation candidate (sell-side only; no buy-side shed
# gate exists in the chassis to isolate separately) per docs/research-governance.md AGE-363.
...
def economy(obs, v, pending_drop=None):
    ...
    if True:
        ...
        n = shed.get("MELON", 0)
        if n > 0 and (prices.get("MELON", 0) >= KNOBS["melon_floor"] or day >= 27 or shed_load > 65):
