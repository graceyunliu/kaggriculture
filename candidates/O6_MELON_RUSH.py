...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if shed_animals and (i in S["animal_claim"] or len(S["animal_claim"]) < 2) and hour >= 1 and day < 27:
        if pos in unlocked_shed:
...
def _agent(obs):
    ...
    # Same-turn drop->sell: the engine applies unit actions (incl. DROP) before
    # processing market SELL orders within one turn, so goods a unit drops THIS
    # turn are already sellable this turn -- but our SELL quantities were decided
    # by economy() before we knew what would be dropped. Top up already-decided
    # SELL orders (never add a new one) with this turn's anticipated drop amount,
    # so the policy doesn't wait an extra hour to sell what it's about to shelve.
    same_turn_drop = {}
    for i, op in enumerate(ops):
        if op and op[0] == "DROP" and positions[i] in unlocked_shed:
            carry_i = inv[i] if i < len(inv) else {}
            for item, qty in carry_i.items():
                if item in PRODUCTS and qty > 0:
                    same_turn_drop[item] = same_turn_drop.get(item, 0) + qty
    if same_turn_drop:
        for order in market:
            if order and len(order) >= 3 and order[0] == "SELL" and order[1] in same_turn_drop:
                order[2] = int(order[2]) + same_turn_drop.pop(order[1])
