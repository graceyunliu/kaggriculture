...
CASH_BUFFER = 40.0              # RETUNE v4 (DIAGNOSIS_V3.md): 120 -> 40. instrument_v3_selltiming.py showed P8v2
                                 # carries ~2.5x P6/C1's early-game cash (mean $1,288 vs $480-503, days 1-10) and
                                 # money_by_day never approaches the 120 floor after day 1 -- CASH_BUFFER was not the
                                 # binding constraint, but this frees a little more cash per turn for herd/crop buys
                                 # on the (untested-until-now) chance the buffer was still shaving a purchase here
                                 # or there at the margin.
...
MELON_FLOOR_FRAC = 0.55         # RETUNE v4 (DIAGNOSIS_V3.md): 0.72 -> 0.55. instrument_v3_selltiming.py showed
                                 # P8v2's realized MELON price (mean $216/unit) already beats P6 ($178) and C1
                                 # ($190) -- the floor was not costing $/unit -- but MELON shed inventory still
                                 # peaks at 42 units (vs P6 48 briefly, but P8v2 sells far fewer total: 42 vs
                                 # 63-72 units/game) and shed level stays elevated (mean 1.35, comparable) while
                                 # volume lags; lowering the floor lets a real (not reactive-only) price dip still
                                 # clear and sell, testing whether floor-driven delay was silently trimming volume
                                 # on top of the (out-of-scope, R4) production-mix gap DIAGNOSIS_V3.md documents.
