...
S = {"day": -1, "routes": {}, "crop": {}, "sweep": {}, "setup": {}, "claimed_sites": set(), "animal_claim": set(),
     "hires_target": 0, "wheat_budget": 0, "animal_positions": set()}
...
# ===== EVOLVE-BLOCK: siting =====
def _site_key(s_):
    # O41_PASTURE_ADJACENCY: same shed-distance-first priority as O36, plus a second-priority
    # adjacency term (distance to nearest already-claimed animal/pasture site) before the raw
    # (x, y) tiebreak. Isolated siting-only change -- nothing else touched.
    animals = S["animal_positions"]
    adj = min((_dist(s_, c) for c in animals), default=0)
    return (_shed_dist(s_), adj, s_)
...
def _setup_step(i, pos, v, carry):
    ...
    if site is None:
        if sp == "GOOSE":
            c = [s_ for s_ in v["empty"] if s_ not in S["claimed_sites"] and s_ not in SHED_TILES]
            site = min(c, key=_site_key) if c else None
        else:
            site = _pick_site(v)
        if site is None:
            return ["PASS"]
