...
KNOBS = {"melon_floor": 150, "harvest_min": 2, "opening": "frontier", "wheat_tiles": 0, "wheat_stock": 0, "min_hands": 3, "load_per_hand": 20, "geese": 0, "open_melons": 8, "open_wheat": 7, "open_cows": 2, "open_sheep": 2, "early_hire_days": 5, "feed_spare_poor": 0}
...
def economy(obs, v):
    ...
    if day == 0:
        ...
        if KNOBS["wheat_tiles"] > 0 and day <= 24:
            deficit = KNOBS["wheat_tiles"] - wheat_tiles_now - seeds.get("WHEAT", 0)
...
def _build_sweep(i, pos, v, day, hour, carry, pools, seeds_left):
    ...
    if carry.get("FERTILIZER", 0) > 0:
        tiers = ["urgent", "fert", "harvest", "water"]
    if hour < 22:
        tiers.append("plant")
    tiers.append("weeds")
    if hour >= 14:
...
def agent(obs, configuration=None):
    ...
    if step < SWITCH_DAY * 24 and step < len(_TAPE):
        return _TAPE[step]
    try:
