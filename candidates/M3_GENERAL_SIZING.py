...
# M3: turn-1 sizing is derived only from the visible wheat curve and our own
# balance sheet/action plan.  These are the documented master-engine defaults;
# custom market parameters are used directly when the observation exposes them.
_WHEAT_MARKET = {"base": 25, "I0": 10000, "T": 400,
                 "below_func": "sqrt", "below_target": 0.80,
                 "above_func": "log", "above_target": 0.20}


def _market_shape(kind, x):
    x = max(0.0, float(x))
...
def _wheat_price(inventory, market):
    """Mirror the engine's documented inventory-to-price function."""
    params = (market.get("params", {}) or {}).get("WHEAT", _WHEAT_MARKET)
    base, i0, scale = params["base"], params["I0"], params["T"]
    if inventory < i0:
        kind, target, distance = params["below_func"], params["below_target"], i0 - inventory
        price = base + target * base * _market_shape(kind, distance) / _market_shape(kind, scale)
    else:
        kind, target, distance = params["above_func"], params["above_target"], inventory - i0
        price = base - target * base * _market_shape(kind, distance) / _market_shape(kind, scale)
    return max(1, int(round(price)))


def _planned_turn1_cost(obs, action):
    """Cost of our own non-sale orders, using only public rules/live state."""
    ...
    for order in action.get("market", []):
        ...
        if op == "BUY_ANIMAL":
            cost += ANIMALS[order[1]] * qty
        elif op == "BUY_SEED":
            cost += CROPS[order[1]]["seed"] * qty
        elif op == "BUY_PRODUCT":
            cost += obs["market"]["prices"].get(order[1], 1) * qty
        elif op == "HIRE":
            cost += _fib(hires)
            hires += 1
    return cost
...
def _general_opening_sell(obs, action):
    """Smallest curve-priced sale that funds our plan while retaining feed.

    Revenue is integrated one unit at a time because every filled SELL increases
    inventory and therefore changes the next unit's quote.  The reserve is one
    day's feed for animals already owned or bought in this action, plus the same
    one-unit operational buffer used by the chassis opening.
    """
    private = obs.get("private", {}) or {}
    inventories = private.get("inventories", []) or []
    wheat = int((private.get("shed", {}) or {}).get("WHEAT", 0))
    wheat += sum(int(unit.get("WHEAT", 0)) for unit in inventories)

    farm = obs["farms"][obs["player"]]
    owned = sum(1 for row in farm["tiles"] for tile in row
                if isinstance(tile, dict) and tile.get("animal"))
    owned += sum(int((private.get("shed", {}) or {}).get(a, 0)) for a in ANIMALS)
    owned += sum(int(unit.get(a, 0)) for unit in inventories for a in ANIMALS)
    planned = sum(int(order[2]) for order in action.get("market", [])
                  if order[0] == "BUY_ANIMAL")
    sell_cap = max(0, wheat - (owned + planned + 1))

    cash_gap = max(0, _planned_turn1_cost(obs, action) - int(farm["money"]))
    inventory = int(obs["market"]["inventory"]["WHEAT"])
    revenue = 0
    for qty in range(sell_cap + 1):
        if revenue >= cash_gap:
            return qty
        revenue += _wheat_price(inventory + qty, obs["market"])
    return sell_cap


def _m3_turn1(obs):
    act = _json.loads(_json.dumps(_TAPE[1]))
    act["market"][0] = ["SELL", "WHEAT", _general_opening_sell(obs, act)]
    return act
