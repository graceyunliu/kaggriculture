"""P8v3 — land/herd retune attempt on top of P8v2 (see DIAGNOSIS_V2.md).

P8v3 = P8v2 + ONE changed constant (HERD_RATIO_CAP: 1.1 -> 0.85). Three candidate
retunes in the land/herd domain were tested via compare_paired() against P8v2/P6/C1
(OCC_HIGH 0.65->0.80; HERD_RATIO_CAP 1.1->1.6; HERD_RATIO_CAP 1.1->1.25;
HERD_RATIO_CAP 1.1->0.85) -- ALL FOUR made the paired margin vs P6/C1 worse than
P8v2's own -$12.6k/-$16.4k baseline; this file ships the least-bad of the four
(HERD_RATIO_CAP=0.85) so the shipped artifact matches the actually-tested numbers in
DIAGNOSIS_V2.md, not because it is a win. See DIAGNOSIS_V2.md for the honest verdict:
the land-expansion/herd-purchase-ratio hypothesis for the remaining P8v2-vs-P6/C1 gap
is NOT supported by this instrumentation -- P8v2 already out-paces P6/C1 on land
(2nd quad day 6.6 vs 8, final land 3.95 vs 3.1 quads) and herd-ratio changes only
made results worse in both directions tested, so the residual gap likely lives
elsewhere (R5 sell timing, wage/cash-buffer sizing, or R4 crop-mix threshold --
outside this task's land/herd-only scope).

=====================================================================================
Original P8/P8v2 docstring below (executor/allocation split description, unchanged):
=====================================================================================
...
LOAD_PER_HAND = 20.0            # RETUNE (untested proposal, see DIAGNOSIS.md $ 4): matches P6/C1 KNOBS["load_per_hand"]
CASH_BUFFER = 120.0             # $ kept unspent as a safety margin on every purchase decision
OCC_HIGH = 0.65                 # reverted to P8v2 value; 0.80 tested worse (-$21,956/game vs P6), see DIAGNOSIS_V2.md
HERD_RATIO_CAP = 0.85           # RETUNE v3 (was 1.1). All three tested directions on land/herd knobs (OCC_HIGH up,
                                 # HERD_RATIO_CAP up to 1.6, up to 1.25) lost decisively/borderline to P8v2; this
                                 # (down to 0.85) was the least-bad of four tested variants (t=-1.81 vs P8v2, still
                                 # a net loss) -- see DIAGNOSIS_V2.md. Kept as the reported result, not a win.
