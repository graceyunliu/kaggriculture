...
def economy(obs, v):
    ...
    # RETUNE (P8v5, see DIAGNOSIS_V4.md): P8v2 has no hour gate on R4, so this
    # greedy loop re-runs every hour of every day (~6x/day on average per
    # instrument_v4_cropmix.py, vs P6's capital_hour gate which runs its
    # equivalent loop once/day). WHEAT's cheap seed cost (10) and short cycle
    # (5 days -> fast tile turnover) let it win the very frequent small
    # `space`-refill windows this produces far more often than the once-daily
    # cadence P6 uses, even though instrument_v4's own eligibility data shows
    # STRAWBERRY's greedy `val` is on average HIGHER than WHEAT's when both are
    # eligible (46.96 vs 41.05) -- the tile-occupancy skew is a call-frequency
    # artifact, not a ranking-formula preference for WHEAT. Restricting R4 to
    # one decision per day (capital_hour, same value P6 uses) removes that
    # frequency artifact without touching the ranking formula itself.
