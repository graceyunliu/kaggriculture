# O6_SHED4: O5_ENDGAME + route shed trips to ANY of the four shed-access tiles (EG all_shed_tiles=1).
#   The engine resolves DROP/PICKUP before the LOCKED-tile guard and lets units walk onto LOCKED tiles,
#   and hands spawn on all four access tiles (NWSE, least-occupied) even while three are LOCKED. O4/O5
#   only routed to unlocked access tiles, so a hand spawning on (5,4)/(4,5)/(5,5) with one quadrant
#   owned walked to (4,4) before every PICKUP/DROP. Only difference from O5 is the knob below.
...
EG = {"same_turn_sell": 2,   # 2 = day 29 only (default), 1 = all game (tested: wash, see header), 0 = off (O4 behaviour)
      ...
      "all_shed_tiles": 1,   # 1 = route shed trips to any of the 4 access tiles, not just unlocked ones
...
def _agent(obs):
    ...
    pending = {}
    if EG["same_turn_sell"] == 1 or (EG["same_turn_sell"] == 2 and day == 29):
        for i, pos in enumerate(positions):
