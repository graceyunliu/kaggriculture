# O21_MELON_CONVOY: O20_MELON_FIRST + ready melons get top orchestrator priority (day<=14) + melon_rush=1 (carry >=4 melons -> straight to the shed). Goal: harvest and sell the whole melon crop on day 10 before the price pool is drained (tapes: 60 units harvested by h9, sold h9-h15 at $217-260).
...
ORCH_COMMIT = 0.75
MELON_CONVOY_PRIO = 4.0   # O21      # bonus for keeping the task a unit is already heading to (prevents swap oscillation)
ORCH_SLACK_HOUR = 14
    ...
    for j in free:
        ...
        for ti, (tp, kind) in enumerate(tasks):
            ...
            if kind == "harvest":
                ...
                if isinstance(tt, dict) and tt.get("crop") == "MELON" and day <= 14:
                    cost -= MELON_CONVOY_PRIO   # O21: ready melons are the day's first job -- the melon price pool is emptied by whoever sells first
