# O23_FERT_COURIER: O22_MELON_MORNING + O19 phase rule (fertilize ongoing crops on production days only) + fert courier (a free unit fetches fertilizer from the shed when eligible tiles are waiting; before, fertilizer only left the shed with hands spawning there at h1). Target: the tapes' 3.5 fertilized production nights per strawberry planting (O16: 1.9).
...
def _fert_eligible(t, day):
    ...
    if not (done < c["max_yield"] and age >= c["first"] - 1 and t.get("fertilized_until_day", -1) < day):
        return False
    # O19 phase rule: fertilizer lasts 3 days; fertilizing on a production day covers two production nights, on an
...
ORCH_PRIO = {"urgent": 0.0, "harvest": 0.5, "wwater": 0.5, "fert": -3.0, "plant": 1.0, "water": 1.0, "weeds": 1.5, "slack": 6.0}
ORCH_COMMIT = 0.75
FERT_RUNNERS = 3            # O23: units allowed to walk to the shed for fertilizer at once
FERT_RUN_LAST_HOUR = 14     # O23
MELON_MORNING = 1            # O22
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if day <= 26 and v["fert"]:
        prod_carried -= carry.get("FERTILIZER", 0)   # O23: carried fertilizer is an input on its way to a tile, not cargo to deposit
    ...
    if (KNOBS["fert_keep"] > 0 or KNOBS["fert_buy"] > 0) and day <= 26 and hour >= 1 and pos in unlocked_shed \
            and (hour >= 2 or len(v["animals"]) == 0) \
            and carry.get("FERTILIZER", 0) == 0 and v["fert"] and shed.get("FERTILIZER", 0) > 0 \
        ...
        if n > 0:
            ...
            S.get("fert_runner", set()).discard(i)
            return ["PICKUP", "FERTILIZER", int(n)]
    # 4c. O23 fert courier: fertilizer only left the shed with hands that happened to spawn there at h1, so on most days
    # 2-17 eligible tiles went unfertilized while fertilizer sat in the shed (measured). If fertilizer is in the shed and
    # eligible tiles are unclaimed, the nearest free unit (at most FERT_RUNNERS at a time) goes to fetch it.
    if FERT_RUNNERS and day <= 26 and 1 <= hour <= FERT_RUN_LAST_HOUR and i not in S["routes"] \
            and carry.get("FERTILIZER", 0) == 0 and carry.get("WHEAT", 0) == 0 and not any(carry.get(a, 0) for a in ANIMALS) \
            and shed.get("FERTILIZER", 0) > 0 and len(v["fert"]) - S.get("fert_taken", 0) >= 1:
        runners = S.setdefault("fert_runner", set())
        runners &= set(range(len(v["positions"])))
        if pos in unlocked_shed:
            runners.discard(i)      # arrived but could not pick up (nothing left to take): release the slot
        if i in runners or len(runners) < FERT_RUNNERS:
            if _shed_dist(pos) + 1 <= 24 - hour:
                runners.add(i); S["fert_runs"] = S.get("fert_runs", 0) + 1
                return [_step(pos, _nearest(pos, unlocked_shed))]
...
def _agent(obs):
    ...
    day, hour = obs["day"], obs["hour"]
    if S["day"] != day:
        S.update({"day": day, "routes": {}, "crop": {}, "sweep": {}, "setup": {}, "claimed_sites": set(), "animal_claim": set(), "fert_taken": 0, "fert_runner": set()})
    v = perceive(obs)
