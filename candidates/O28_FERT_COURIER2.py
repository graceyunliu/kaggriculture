...
CROP_SWEEP_RADIUS = 5
FERT_RADIUS = 3
FERT_RUNNERS = 3            # O23/O28: units allowed to walk to the shed for fertilizer at once
FERT_RUN_LAST_HOUR = 14     # O23/O28
SPREAD_W = 1.25
SPREAD_CAP = 3
STRAW_CUTOFF = 19
...
def _fert_eligible(t, day):
    ...
    if not (done < c["max_yield"] and age >= c["first"] - 1 and t.get("fertilized_until_day", -1) < day):
        return False
    # O19/O23 phase rule: fertilizer lasts 3 days; fertilizing on a production day covers two production
    # nights instead of ~1.3. Strawberry: ages 9, 11, 13, 15 (the tapes fertilize at exactly 9 and 13).
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if (KNOBS["fert_keep"] > 0 or KNOBS["fert_buy"] > 0) and day <= 26 and hour >= 1 and pos in unlocked_shed \
        ...
        if n > 0:
            ...
            S.setdefault("fert_runner", set()).discard(i)
            return ["PICKUP", "FERTILIZER", int(n)]
    # O23/O28 fert courier: fertilizer only left the shed with hands that happened to spawn there at h1, so on
    # most days eligible tiles went unfertilized while fertilizer sat in the shed (measured, allocation_matrix.py).
    # If fertilizer is in the shed and eligible tiles are unclaimed, the nearest free unit (at most FERT_RUNNERS
    # at a time) goes to fetch it.
