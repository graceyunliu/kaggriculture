# O19_FERT_PHASE_FB8: O19_FERT_PHASE with fert_buy 8 (was 3). Margin panel vs O16 +1.5k (t6.8) but OWN money +287 (t1.6) / +59 (t0.4 fresh) -> share-shift via strawberry/fert prices, fails the own-money gate. Not a candidate.
# O19_FERT_PHASE: O16_ORCH_ON_O15 + phase-aware ongoing-crop fertilizing (only on production days, so each $35 fertilizer covers 2 production nights instead of ~1.3). From the allocation matrix (tools/allocation_matrix.py, tools/straw_life.py): tapes get 7.5 strawberry units/planting with 3.5 fertilized production nights, O16 5.5 with 1.9.
...
def _fert_eligible(t, day):
    ...
    if not (done < c["max_yield"] and age >= c["first"] - 1 and t.get("fertilized_until_day", -1) < day):
        return False
    # O19: fertilizer lasts 3 days (day..day+2) and an ongoing crop produces on the night of day d when
    # (d + 1 - first) % interval == 0. Fertilizing on a production day covers TWO production nights (tonight and
    # day+2); fertilizing on an off day covers only one. Only fertilize when tonight is a production night.
