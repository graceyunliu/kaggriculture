...
def economy(obs, v):
    ...
    if day == 0:
        ...
        if wheat_target > 0 and day <= 24:
            ...
            k = min(deficit, max(space, 0) + 4, int(free // 10))   # wheat may take pending-animal slack; labor model not consulted
            if k > 0:
                seed_orders["WHEAT"] = k
                free -= 10 * k
                seeds_on_hand += k
