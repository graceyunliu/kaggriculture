...
def economy(obs, v):
    ...
    revenue_est = 0.0
    fert_want = min(3 + 1, len(v["fert"])) if day <= 26 else 0
    if hour == 0 or day >= 28:
    ...
    if hour != capital_hour:
        if hour == 0:
            ...
            if day <= 26 and len(v["fert"]) > 0:
                fp = prices.get("FERTILIZER", 100)
                have = shed.get("FERTILIZER", 0)
                k = min(1, max(0, len(v["fert"]) - have), int(max(0.0, cash - 50) // max(1, fp)))
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if day <= 26 and hour >= 1 and pos in unlocked_shed and carry.get("FERTILIZER", 0) == 0 \
            and pools["fert"] and shed.get("FERTILIZER", 0) > 0 and S.get("fert_taken", 0) < len(v["fert"]):
        n = min(3, shed.get("FERTILIZER", 0), len(v["fert"]) - S.get("fert_taken", 0))
        if n > 0:
