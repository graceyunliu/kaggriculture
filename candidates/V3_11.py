"""V3_9_LABOR_MODEL — from-scratch Kaggriculture agent.

Design (see experiments/V3_9_LABOR_MODEL/contract.json):
  * labor: per-unit persistent pasture routes with a single wheat pickup, no product deposit
    trips (nightly auto-drop), fertilize from carried inventory, yield-linked watering,
    persistent crop targets, hiring spread over hours 0-2 from an explicit load model;
  * capital: animal-first opening, fertilizer bridge, reserves before discretionary spend,
    herd waves from free cash, land at saturation, strawberry waves, melon wave selling.
"""
import math
import sys
...
MELON_PRICE_CUSHION = 100     # units above I0 before MELON drops from $250 toward $150 (sq curve)
...
def _harvest_ready(t, day):
    ...
    return yu >= c["max_yield"] or age >= c["max_day"]
...
def _daily_demand(obs, item, day, sell_start=None):
    """Expected town consumption of `item` per day over the selling window [sell_start, 29]:
    unlocked shops count fully; each still-locked shop counts with the probability it is unlocked
    by the window midpoint (one random shop unlocks every townShopUnlockInterval days)."""
    ...
    n_unlocked = len(shops)
    locked = [n for n in SHOPS if n not in shops]
    slots = max(0.0, mid // 3 - n_unlocked)
    p_unlock = min(1.0, slots / max(1, len(locked))) if locked else 0.0
    d = 0.0
    for name, prods in SHOPS.items():
        if item not in prods:
            continue
        w = 1.0 if name in shops else p_unlock
        d += w * per_day * (2 if len(prods) == 1 else 1)
    center_mult = 4 if mid >= 20 else (2 if mid >= 10 else 1)
    d += center_mult * CFG["turns_per_day"] / max(1, CFG["center_interval"])
    return d
...
def economy(obs, v):
    ...
    if hour != capital_hour:
        if hour == 0:
            # hour 0 (day >= 1): sell, buy today's feed, and hire FIRST (labor is the obligation)
            ...
            seeds_on_hand = sum(seeds.get(c, 0) for c in ("STRAWBERRY", "MELON"))
        ...
        for sp in ("SHEEP", "COW"):
            ...
            if sp == "SHEEP":
                if prices.get("WOOL", 0) >= 1.05 * 200 and day >= 8:
                    room = max(room, 2)          # observed scarcity overrides the shop-unlock forecast
    ...
    # ---- seeds
    if day == 0:
        orders.append(["BUY_SEED", "MELON", 14])
        free -= 1120
    else:
        n_melon_tiles = sum(1 for _pos, t in v["crops"] if t.get("crop") == "MELON")
        if any(a <= day <= b for a, b in MELON_WAVE_DAYS) and n_melon_tiles + seeds.get("MELON", 0) < MELON_MAX_TILES:
            room = _melon_room(obs, day, n_melon_tiles, seeds.get("MELON", 0))
            k = min(room, MELON_MAX_TILES - n_melon_tiles - seeds.get("MELON", 0), int(free // 80), max(0, empty_count - pending_place - 2))
            if k > 0:
                orders.append(["BUY_SEED", "MELON", int(k)])
                free -= 80 * k
                empty_count -= k
                seeds_on_hand += k
        if day <= STRAW_CUTOFF:
            opp = obs["farms"][1 - p]
            opp_straw = sum(1 for row in opp["tiles"] for t in row if isinstance(t, dict) and t.get("crop") == "STRAWBERRY")
            my_straw = sum(1 for _pos, t in v["crops"] if t.get("crop") == "STRAWBERRY")
            demand_room = _strawberry_room(obs, v, day, my_straw, seeds.get("STRAWBERRY", 0), opp_straw)
            space = empty_count - pending_place - seeds.get("STRAWBERRY", 0) - seeds.get("MELON", 0)
            if demand_room > space + 4:
                land_wanted = demand_room - space
            k = min(20, space, demand_room, int(free // 100))
            # labor coupling: do not add crop obligations the hand cap cannot service
            while k > 0 and _load_model(v, seeds_on_hand + k, n_total, pending_place, day) >= MAX_HANDS:
                k -= 1
            if k > 0:
                orders.append(["BUY_SEED", "STRAWBERRY", int(k)])
                free -= 100 * k
                seeds_on_hand += k
...
def _crop_pools(v, seeds_left, day):
    n_seeds = sum(seeds_left.get(c, 0) for c in ("STRAWBERRY", "MELON"))
...
def _plant_choice(pos, seeds_left):
    ...
    order = ["STRAWBERRY", "MELON"] if near else ["MELON", "STRAWBERRY"]
    for c in order:
...
def _build_sweep(i, pos, v, day, hour, carry, pools, seeds_left):
    ...
    tiers = ["urgent", "harvest", "water"]
    if carry.get("FERTILIZER", 0) > 0:
        tiers = ["urgent", "fert", "harvest", "water"]     # a carried unit is worth more on a crop than sold
    if hour < 22:
        tiers.append("plant")
    tiers.append("weeds")
    if hour >= 14:
