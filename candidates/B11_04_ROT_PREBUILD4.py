# O9_MELON_LATEFERT: O9_O8_ENDGAME + B4_01 melon late-fertilization (age 7-8, MELON only) + one-shot harvest age gate.
...
def _crop_step(i, pos, v, day, hour, carry, pools, seeds_left):
    ...
    while sweep:
        ...
        if kind in ("harvest","rharv") and c and not c["ongoing"] and not t.get("watered_today") and (c["max_day"]+1)//2<=age<=c["max_day"] and t.get("yield_units",0)<c["max_yield"]:
            return ["WATER"]
        sweep.pop(0)
        if kind in ("urgent", "water", "slack", "wwater"):
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    if day <= 8 and hour < 20 and len(v["empty_pastures"]) < 4:
