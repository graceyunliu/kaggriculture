# O13_HOLD: O12_EVENING_DEPOSIT + late-game HOLD of selected products, dumped on day 29 (EG hold_*).
# ---- O10 header follows ----
# O12_EVENING_DEPOSIT: O9 + evening deposit of idle units' cargo (EG evening_deposit / same_turn_sell=3).
# ---- O9 header follows ----
...
EG = {"same_turn_sell": 3,   # 2 = day 29 only (default), 1 = all game (tested: wash, see header), 0 = off (O4 behaviour)
      ...
      "hold_items": ("MILK", "WOOL"),  # O11: consumable products to HOLD in the shed late game and dump at the end
      "hold_day": 20,        # start holding from this day
...
def economy(obs, v, pending_drop=None):
    ...
    if True:
        for item in ("MILK", "WOOL", "STRAWBERRY", "FERTILIZER", "TOMATO", "CARROT", "EGG", "MELON"):
            n = shed.get(item, 0)
