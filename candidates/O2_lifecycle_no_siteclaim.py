# O2: original state-based resource-ownership candidate.
# Own frozen chassis + own P6 executor; no replay actions or opponent identity rules.
# Correct setup-slot admission, reserve livestock sites/stock before crop work,
# reserve crop tiles across chore types, and complete related chores locally.
# Research candidate: promotion depends on the frozen fresh-seed evaluation.
...
def _care_useful(t,day):
    ...
    if True and due-day>cap-1:return False
    return True
...
def _pick_site(v, species=None):
    if v["empty_pastures"]:
        c = [s_ for s_ in v["empty_pastures"]]
        if c:
            return min(c, key=_shed_dist)
    c = [s_ for s_ in v["empty"] if s_ not in SHED_TILES]
    if not c:
        return None
...
def _crop_pools(v, seeds_left, day):
    ...
    plant = [q for q in sorted(v["empty"], key=lambda q: (_shed_dist(q), q))][:n_seeds]
...
def _agent(obs):
    ...
    for i, pos in enumerate(positions):
        ...
        ops.append(op)
        if op and op[0]=="PICKUP" and op[1] in ANIMALS and pos in unlocked_shed:
