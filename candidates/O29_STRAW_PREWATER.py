...
def _water_needed(t, day):
    ...
    if c["ongoing"]:
        ...
        if dsf < 0:
            # O29: pre-production growth window (age < first-yield day). The engine's own
            # consecutive_unwatered>=2 -> WEED rule (kaggriculture.py _daily_refresh_plants) runs
            # every day regardless of production status, but this policy previously only queued
            # watering here reactively at cu>=1 ("urgent"), leaving zero slack: any single day the
            # unit servicing an "urgent" tile is late or displaced, the tile dies with 0 units ever
            # produced. tools/straw_trace.py showed this exact alternating cu 0/1/0/1... pattern on
            # every tile that died. Water proactively during growth so cu stays at 0, giving one full
            # day of slack instead of none. Tapes show 0% early deaths vs our 17% (straw_life.py).
            return "water"
        if dsf % step_i != 0:
            return None
