...
CROP_SPECS = {
    ...
    "WHEAT":      {"seed": 10,  "units": 5.0, "first": 2,  "cycle": 5,  "cutoff": 26, "base": 25,  "min_val": 12},
    "CARROT":     {"seed": 20,  "units": 4.0, "first": 2,  "cycle": 4,  "cutoff": 26, "base": 35,  "min_val": 12},
    ...
    if day == 0:
        ...
        while space > 0 and n_seed_orders < 4:
            ...
            for c, sp_ in CROP_SPECS.items():
                ...
                if val < sp_["min_val"] and not (c == "WHEAT" and day >= 20):
                    continue
            ...
            while k > 0 and _load_model(v, seeds_on_hand + k, n_total, pending_place, day) >= MAX_HANDS and not (c == "WHEAT" and day >= 20):
                k -= 1
            if c == "WHEAT" and day >= 20:
                k = min(space, int(free // 10), 30)   # late wheat filler: every empty tile, labor model bypassed
            if k <= 0:
                excluded.add(c)
                continue
            excluded.add(c)
