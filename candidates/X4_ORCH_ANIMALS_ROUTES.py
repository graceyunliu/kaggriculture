...
def _orchestrate(v, pools, positions, day, hour, inv, seeds_left, busy):
    ...
    # animal service tasks: any free unit may take one pending animal (1-stop route); wheat detour priced in
    if ORCH_ANIMALS and hour >= 1 and day < 29:
        claimed = set()
        for r in S["routes"].values():
            claimed.update(r["stops"])
        for p2, t in v["animals"]:
            if p2 not in claimed and _animal_pending(t, day):
                tasks.append((p2, "animal"))
    ...
    for j in free:
        ...
        for ti, (tp, kind) in enumerate(tasks):
            ...
            if kind == "animal":
                t = v["tiles"][tp[1]][tp[0]]
                needs_wheat = day < 29 and _feed_useful(t, day) and carry.get("WHEAT", 0) == 0
                if needs_wheat and S["wheat_budget"] <= 0 and hour <= 2:
                    d = _dist(pj, tp) + 2   # wheat lands at end of h1; a unit at the shed waits
                elif needs_wheat:
                    sh = _nearest(pj, S.get("shed_tiles") or SHED_TILES)
                    d = _dist(pj, sh) + _dist(sh, tp)
                else:
                    d = _dist(pj, tp)
            else:
                d = _dist(pj, tp)
            ...
            if kind == "animal":
                ...
                if t.get("consecutive_unfed", 0) >= 1:
                    cost -= 4.0          # escapes tomorrow if unfed today
                elif hour >= 10:
                    cost -= 2.0
...
def _unit_action(i, pos, carry, obs, v, pools, seeds_left, shed, unlocked_shed):
    ...
    r = None if (hour == 0 or ORCH_ANIMALS) else _build_route(i, pos, v, day, shed, carry, unlocked_shed, hour)
    if r is not None:
        S["routes"][i] = r
        S["sweep"].pop(i, None)
        op = _route_step(i, pos, v, day, hour, shed, carry, unlocked_shed)
...
def _agent(obs):
    ...
    for j, (tp, kind) in S["assign"].items():
        if kind == "animal":
            ...
            carry = inv[j] if j < len(inv) else {}
            claimed = set()
            for r_ in S["routes"].values():
                claimed.update(r_["stops"])
            others = [p2 for p2, t2 in v["animals"] if p2 != tp and p2 not in claimed and _animal_pending(t2, day) and _dist(tp, p2) <= ORCH_ANIMAL_R]
            stops = [tp]; cur = tp
            while others and len(stops) < ROUTE_LEN:
                nxt = _nearest(cur, others); stops.append(nxt); others.remove(nxt); cur = nxt
            unfed = sum(1 for s_ in stops if day < 29 and _feed_useful(v["tiles"][s_[1]][s_[0]], day))
            need = max(0, unfed - carry.get("WHEAT", 0))
            pickup = min(need, max(0, S["wheat_budget"])) if unlocked_shed else 0
            if need and pickup == 0 and hour <= 2:
                pickup = need   # order lands at end of h1; route waits at the shed
            S["wheat_budget"] = max(0, S["wheat_budget"] - pickup)
            S["routes"][j] = {"stops": stops, "pickup": pickup}
            S["sweep"].pop(j, None)
            continue
        sw = S["sweep"].get(j)
    ...
    market = economy(obs, v, pending or None)
