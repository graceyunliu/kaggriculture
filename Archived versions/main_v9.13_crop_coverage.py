...
# =====================================================================
# MAIN V9.13 -- raise STRAWBERRY/MELON seed-buy targets, on top of
# v9.12. Base: main_v9.12_melon_fert.py.
#
# want["MELON"]=6/12 and want["STRAWBERRY"]=10/20 (ph0/ph1) were tuned
# under the old effective hand ceiling (~9-10/day, from the hour==0
# hiring bug now fixed in v9.10). With real hand capacity genuinely
# higher (~12/day, v9.10), those crop-coverage targets may now be
# leaving usable labor on the table rather than land/labor actually
# being the binding constraint. Isolated change: MELON 6/12 -> 8/16,
# STRAWBERRY 10/20 -> 12/26 -- moderate increases, not a redesign;
# `space = len(view["empty"])+2` still caps both to whatever land is
# actually open, so this only matters on days where land, not the
# target number, was the loose constraint.
#
# RESULT -- TESTED, REJECTED, decisively and worse on BOTH baselines.
# 5-seed smoke, both-seats: v9.5 wins 5/5, +$14,160/game (paired-sum/2),
# t=-8.61. Vs opp_scenario_v14: -$60,611/game -- WORSE than v9.5's own
# -$51.7k baseline, and much worse than v9.10/v9.12's -$46.7-46.9k. So
# land/target volume was never the loose constraint at any hand-count
# level tested so far -- pushing more seeds into an already-large empty-
# tile backlog (our traces show 40-55 unplanted "empty" tiles most days,
# even under v9.5) just spreads the same labor thinner and increases
# neglect. Confirms the remaining gap is a watering/servicing-THROUGHPUT
# problem on already-available land, not a seed-buying-volume problem.
# NOT submitted, and no further "raise the target number" variants of
# this idea are worth testing.
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["MELON"] and budget > 700:
        melon_target = min(space, 8 if ph == 0 else 16)  # v9.13: was 6/12 -- v9.10's
                                                            # hire fix gives genuinely more
                                                            # usable hands (~12/day vs the
                                                            # old ~9-10 effective ceiling);
                                                            # these targets were tuned
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        # v9.13: 10/20 -> 12/26, same rationale as MELON above.
        want["STRAWBERRY"] = min(space, 12 if ph == 0 else 26)
