...
# =====================================================================
# MAIN V12.4 -- UNIFIED CROSS-CATEGORY EV ALLOCATOR, on top of v10.6
# (champion, undefeated across 41 hypotheses). Base: main_v10.6_radius3.py,
# unchanged except for WHICH ORDER land/animal/hire get first claim on
# each turn's shared budget.
#
# --- Why ---
# v10.6 runs land, then animals, then seeds, then selling, then hiring, in
# that FIXED order every turn, decrementing a shared local `budget`
# variable as it goes -- so if land's gate happens to fire, it claims cash
# before hiring or the next animal ever gets a look this turn, regardless
# of which one is actually worth more today. This hypothesis computes a
# genuine EV score for each of the three categories every turn and runs
# them in EV-descending order instead -- each category's own existing
# feasibility/safety gate (survival checks, budget checks, composition
# guard, etc.) still decides whether an order actually fires; only the
# ORDER of attempts, and therefore who gets first claim on the budget,
# changes. Seeds/feed/fertilizer/selling are not part of this hypothesis
# and keep v10.6's exact fixed position (after all three EV categories).
#
# --- Mechanism (all three scores expressed in the same $/remaining-day
# units, per the task's cross-category comparability requirement) ---
#   land:   EV = (days_remaining - LAND_RAMP_DAYS) x LAND_PER_TILE_PROFIT_
#           PER_DAY x TILES_PER_QUAD x LAND_TILE_UTILIZATION - quad_cost,
#           / days_remaining. Same constants as the independent v12.3
#           hypothesis (LAND_PER_TILE_PROFIT_PER_DAY=20, blended from
#           STRAWBERRY's MARKET_PARAMS base=$120 anchor, discounted for
#           the project's own documented ~58-60% movement-time-share and
#           the cheaper WHEAT/CARROT tiles in the mix; UTILIZATION=0.6 for
#           non-productive tile share; RAMP_DAYS=6 for planting lag), but
#           built independently in this file per the task brief. NOTE:
#           v12.3's LAND_EV_HURDLE_MULT margin check is NOT reused here as
#           a go/no-go gate -- v10.6's own quads==N/budget/day gate is the
#           real precondition, unchanged; the score exists only to rank
#           land against animal/hire.
#   animal: EV = (n_yield_events x product_price - feed_cost) - animal_
#           cost, / days_remaining, where n_yield_events = (days_remaining
#           - first_yield_offset) / interval. cost from ANIMAL_SPECS,
#           product price from MARKET_PARAMS (MILK base $160, WOOL base
#           $200); first_yield_offset/interval taken directly from this
#           file's own v9.2-header comments ("cow placed day 14 -> first
#           milk day 22" = +8d; "sheep: first wool +6d, 3d interval").
#           Feed cost uses live_price(obs,"WHEAT") x 2/day, matching the
#           JIT feed pipeline's own n_animals*2 formula. Scored for both
#           COW and SHEEP; the category score is the better of the two
#           (which species actually gets attempted first is still decided
#           by the unchanged composition-guard/gate logic inside
#           _run_animal).
#   hire:   EV = (CROP_NEGLECT_TASKS_PER_HAND x HIRE_VALUE_PER_TASK_PER_DAY
#           x effective_days) - hire_cost, / days_remaining. hire_cost is
#           the engine's REAL formula (verified against kaggriculture.py:
#           cost = FARM_HAND_COST_MULT x fib(hires_already_today), reset
#           daily) for the very next hire this turn, i.e. fib(0)=1.
#           CROP_NEGLECT_TASKS_PER_HAND (5) is the existing hiring
#           formula's own work/5 scaling; HIRE_VALUE_PER_TASK_PER_DAY=15
#           sits in the same order of magnitude as LAND_PER_TILE_PROFIT_
#           PER_DAY since a "task" and a "tile-day" are the same unit of
#           labor (water/harvest/plant one tile). Only computed at
#           hour==0/day<29/need>0 (hire's own real gate), else -inf so it
#           never wins the ranking off-schedule.
#   HONEST CAVEAT (documented, not hidden): because the engine resets the
#   Fibonacci hire counter every day, the very first hire of the day is
#   always ~$1 -- so hire's EV score will almost always dominate land's
#   and animal's AT HOUR 0 specifically, by construction of the real cost
#   formula, not by a modeling choice on this file's part. The
#   land-vs-animal ranking (the more genuinely contested comparison) is
#   what applies at every other hour, since hire's score is -inf then.
# =====================================================================
...
# =====================================================================
# v12.4: EV SCORING for the three re-ordered categories (land / animal /
# hire). See file header for the full formula-by-formula rationale.
# =====================================================================
LAND_PER_TILE_PROFIT_PER_DAY = 20
LAND_TILE_UTILIZATION = 0.6
LAND_RAMP_DAYS = 6
LAND_QUAD_COSTS = {1: 1000, 2: 2000, 3: 4000}


def _land_ev_score(day, quads):
    """$/remaining-day EV score for the next quadrant purchase -- used
    only to rank land against animal/hire. v10.6's own quads==N/budget/
    day gate (unchanged, called inside _run_land) still decides whether
    BUY_LAND actually fires."""
    if quads not in LAND_QUAD_COSTS:
        return float("-inf")
    cost = LAND_QUAD_COSTS[quads]
    days_remaining = max(1, 30 - day)
    effective_days = max(0, days_remaining - LAND_RAMP_DAYS)
    rate = LAND_PER_TILE_PROFIT_PER_DAY * TILES_PER_QUAD * LAND_TILE_UTILIZATION
    ev = rate * effective_days - cost
    return ev / days_remaining


ANIMAL_ECON = {
    # first_yield_offset/interval are this file's own v9.2-header figures
    # ("cow placed day 14 -> first milk day 22" = +8d; "sheep: first wool
    # +6d, 3d interval"). cost comes from ANIMAL_SPECS, product price from
    # MARKET_PARAMS -- neither re-estimated here.
    "COW":   {"first_yield_offset": 8, "interval": 2, "product": "MILK"},
    "SHEEP": {"first_yield_offset": 6, "interval": 3, "product": "WOOL"},
}
ANIMAL_YIELD_PER_EVENT = 1   # conservative: one product unit per yield
                               # event, consistent with PRODUCT_DEPOSIT_
                               # AT=3's carry-then-place cadence
ANIMAL_FEED_PER_DAY = 2      # matches the JIT feed pipeline's own
                               # n_animals*2 target_wheat formula


def _animal_ev_score(obs, day, species):
    """$/remaining-day EV score for the next animal-of-this-species
    purchase -- used only to rank animal against land/hire. The real
    _animal_expansion_feasible()/_sheep_expansion_feasible() gates
    (unchanged, called inside _run_animal) still decide whether
    BUY_ANIMAL actually fires."""
    econ = ANIMAL_ECON[species]
    cost = ANIMAL_SPECS[species]["cost"]
    price = MARKET_PARAMS[econ["product"]]["base"]
    days_remaining = max(1, 30 - day)
    effective_days = max(0, days_remaining - econ["first_yield_offset"])
    n_events = effective_days / max(1, econ["interval"])
    revenue = n_events * ANIMAL_YIELD_PER_EVENT * price
    feed_cost = ANIMAL_FEED_PER_DAY * live_price(obs, "WHEAT") * effective_days
    ev = revenue - feed_cost - cost
    return ev / days_remaining


HIRE_RAMP_DAYS = 1                # a hired hand works the very next hour
                                    # -- no build/travel delay like land
                                    # or animals have
HIRE_VALUE_PER_TASK_PER_DAY = 15  # $ value per backlog task relieved
                                    # (a task IS a tile-turn: water/
                                    # harvest/plant); same order of
                                    # magnitude as LAND_PER_TILE_PROFIT_
                                    # PER_DAY since both represent one
                                    # tile's daily service value


def _hire_ev_score(day, hour, need):
    """$/remaining-day EV score for the next hire this turn -- used only
    to rank hire against land/animal. Uses the engine's REAL daily-reset
    Fibonacci cost (kaggriculture.py: cost = FARM_HAND_COST_MULT x
    fib(hires_already_today); verified against the engine source) for the
    very next hire this turn, i.e. fib(0). Only scored at hour==0/day<29/
    need>0 -- hire's own real gate -- else -inf so it never wins the
    ranking off-schedule."""
    if not (hour == 0 and day < 29 and need > 0):
        return float("-inf")
    cost = FARM_HAND_COST_MULT * _fib(0)
    days_remaining = max(1, 30 - day)
    effective_days = max(0, days_remaining - HIRE_RAMP_DAYS)
    ev = CROP_NEGLECT_TASKS_PER_HAND * HIRE_VALUE_PER_TASK_PER_DAY * effective_days - cost
    return ev / days_remaining


def _run_land(priced_orders, budget, view, quads, day):
    """v10.6's land-purchase block, unchanged, extracted so it can run in
    EV-ranked order relative to animal/hire."""
    if 1 <= day <= 22:
...
def _run_animal(priced_orders, budget, obs, me, view, shed, quads, day, hour, animal_plans):
    """v10.6's animal-purchase block, unchanged, extracted so it can run
    in EV-ranked order relative to land/hire. Returns (budget, n_animals)
    -- n_animals is still needed by the (unchanged, fixed-position) feed/
    sell blocks later in economy()."""
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is not None:
            ...
            if (spec["start_day"] <= day <= spec["last_day"]
                ...
                budget -= spec["cost"]
    return budget, n_animals


def _run_hire(priced_orders, budget, me, view, day, hour):
    """v10.6's hiring block, unchanged, extracted so it can run in
    EV-ranked order relative to land/animal (was previously fixed to run
    last, after seeds/feed/sell)."""
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- v12.4: unified cross-category EV allocator -- land / animal /
    # hire run in EV-descending order every turn instead of v10.6's fixed
    # land-then-animal(-then-later-hire) order, so whichever category is
    # genuinely worth the most today gets first claim on this turn's
    # budget. See file header for the exact formulas. Each category's own
    # existing feasibility/safety gate (unchanged, inside _run_land/
    # _run_animal/_run_hire) is still the real go/no-go -- only the
    # ORDER of attempts changes.
    n_animals_precount = (len(view["my_pastures"]) +
                           sum(1 for p in animal_plans if p["stage"] in ("ORDERED", "BOUGHT", "CARRYING")))
    land_score_ev = _land_ev_score(day, quads)
    animal_score_ev = max(
        _animal_ev_score(obs, day, "COW"),
        _animal_ev_score(obs, day, "SHEEP") if SHEEP_ENABLED else float("-inf"))
    hire_need_est = 0
    if hour == 0 and day < 29:
        animal_work_est = _pending_animal_work(view)
        work_est = (len(view["water"]) + len(view["urgent_water"]) +
                    len(view["harvest"]) + len(view["empty"]) +
                    animal_work_est * ANIMAL_WORK_WEIGHT)
        target_est = max(HAND_TARGET_MIN, min(HAND_TARGET_MAX, math.ceil(work_est / 5)))
        hire_need_est = max(0, target_est - len(me["hands"]))
    hire_score_ev = _hire_ev_score(day, hour, hire_need_est)

    ev_scores = {"land": land_score_ev, "animal": animal_score_ev, "hire": hire_score_ev}
    order_categories = sorted(("land", "animal", "hire"), key=lambda c: -ev_scores[c])

    n_animals = n_animals_precount
    for cat in order_categories:
        if cat == "land":
            budget = _run_land(priced_orders, budget, view, quads, day)
        elif cat == "animal":
            budget, n_animals = _run_animal(priced_orders, budget, obs, me, view, shed, quads, day, hour, animal_plans)
        elif cat == "hire":
            budget = _run_hire(priced_orders, budget, me, view, day, hour)
    ...
    # v12.4: hiring already ran above, in EV-ranked order alongside land
    # and animal (see _run_hire) -- no separate fixed-position hiring
    # block here anymore.
