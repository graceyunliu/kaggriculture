...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        # Candidate B2 (tuned down from B's 8/14 after B measured as a
        # statistical wash, mean +$5,384 favoring v7.9, t=1.52, not
        # significant -- mechanism trace showed weeds climbing to 13-16
        # by day 27-28 vs a controlled baseline, suggesting labor, not
        # seed budget, is the real ceiling; B's targets may have been
        # overshooting it).
        want["STRAWBERRY"] = min(space, 7 if ph == 0 else 12)
    if day <= CUTOFF["TOMATO"] and budget > 600:
        want["TOMATO"] = 3   # ongoing crop, same small-standing-patch approach
    if day <= CUTOFF["CARROT"]:
        want["CARROT"] = 4
    if day <= CUTOFF["WHEAT"]:
        # Candidate B2: tuned down from B's 25 to 16.
