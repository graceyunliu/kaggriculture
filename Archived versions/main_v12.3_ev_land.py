...
# =====================================================================
# MAIN V12.3 -- EV-BASED QUADRANT EXPANSION TIMING, on top of v10.6
# (champion, undefeated across 41 hypotheses). Base: main_v10.6_radius3.py,
# unchanged except for the land-purchase decision below.
#
# --- Why ---
# v10.6's land-purchase gate is three independent hardcoded rules keyed on
# how many quadrants are already unlocked: quads==1 (buy NE) needs
# budget>1000, no deadline; quads==2 (buy SW) needs budget>2000, day<=18;
# quads==3 (buy SE) needs budget>4000, day<=16. None of these numbers is a
# calculation -- they're tuned thresholds, not an estimate of what the
# marginal quadrant is actually worth on the day it's considered. This
# hypothesis replaces all three gates with one formula: buy only once the
# projected profit from the tiles being unlocked, over the days actually
# left to use them, clearly exceeds the purchase price.
#
# --- Mechanism (mathematical framing) ---
#   EV(quadrant) = (days_remaining - RAMP_DAYS) x per_tile_profit/day x
#                  tiles_per_quad x utilization  -  cost
#   BUY_LAND fires only when EV > LAND_EV_HURDLE_MULT x cost (a genuine
#   margin, not a bare breakeven, given the constants below are estimates).
#
# Constants and their justification (all traceable to this file's own
# CROPS/MARKET_PARAMS/TILES_PER_QUAD, or explicitly flagged as a rough
# blended estimate per the task brief's suggested $10-30/tile/day range):
#   LAND_PER_TILE_PROFIT_PER_DAY = 20 -- blended $/tile/day once a tile is
#     ramped and being serviced normally. Upper anchor: STRAWBERRY
#     (MARKET_PARAMS base=$120) with v9.3's documented fertilized-event
#     yield of up to 2 units per 2-day interval implies ~$120/tile/day on a
#     PERFECTLY tended premium tile -- but most of a quadrant's 25 tiles
#     run cheaper crops (WHEAT base $25, CARROT $35) or sit imperfectly
#     serviced (the project's own census puts ~58-60% of every unit-turn
#     in movement, not service), so 20 sits well below that ceiling, in
#     the middle of the brief's own $10-30/tile/day working range.
#   LAND_TILE_UTILIZATION = 0.6 -- a quadrant's 25 tiles are never 100%
#     productive crop/pasture land (paths, animal sites, partial-season
#     coverage); 0.6 is a conservative haircut, not a measured figure.
#   LAND_RAMP_DAYS = 6 -- days between unlocking a quadrant and it being
#     fully planted/serviced (travel + planting + first-harvest lag);
#     matches this file's own STRAWBERRY "first": 10 / TOMATO "first": 8
#     day-to-first-yield figures discounted for the fact land, unlike a
#     single seed, needs several planting passes to fill.
#   LAND_EV_HURDLE_MULT = 1.2 -- requires EV to clear cost by a 20% margin
#     before firing, given the estimates above are not engine-measured.
# TILES_PER_QUAD (25) and the three quadrant costs (1000/2000/4000) are
# read directly from v10.6's own code, not re-estimated.
#
# This is a genuine formula substitution, not a coarse reordering -- land
# still runs in the same pipeline position as v10.6 (before animals/seeds),
# only the buy/no-buy decision within that slot changed.
# =====================================================================
...
# --- v12.3: EV-based land-purchase timing (see file header for the full
# rationale and constant-by-constant justification). Replaces v10.6's flat
# quads==N / budget>threshold / day<=deadline gates with a genuine
# expected-value calculation.
LAND_PER_TILE_PROFIT_PER_DAY = 20   # blended $/tile/day once ramped
LAND_TILE_UTILIZATION = 0.6         # fraction of a quadrant's tiles that
                                      # end up productively planted/serviced
LAND_RAMP_DAYS = 6                  # days before a freshly unlocked
                                      # quadrant is fully planted/serviced
LAND_EV_HURDLE_MULT = 1.2           # EV must clear cost by this margin
LAND_QUAD_COSTS = {1: 1000, 2: 2000, 3: 4000}   # cost to go from
                                                   # quads -> quads+1,
                                                   # read directly from
                                                   # v10.6's own gates


def _land_ev(day, quads):
    """EV (in $, over the rest of the game) of buying the next quadrant
    right now. Returns (ev, cost); (None, None) if quads isn't a valid
    expansion step (already at 4, or below 1)."""
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion (v12.3: EV-driven, replaces v10.6's flat
    # quads==N/budget>threshold/day<=deadline gates -- see file header) ---
    if 1 <= day <= 22:
        land_score = _land_priority(view)
        ev, cost = _land_ev(day, quads)
        if ev is not None and budget > cost and ev > LAND_EV_HURDLE_MULT * cost:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= cost
