...
# =====================================================================
# MAIN V9.17 -- value-tiered water dispatch, on top of v9.12 (melon
# fertilize-in-window on top of v9.10's hire fix).
#
# Hypothesis (task-dispatch-efficiency category, per user direction
# after the v9.7-v9.16 volume-knob sweep converged on "the gap is
# labor-throughput efficiency, not volume"): assign_tasks()'s "water"
# tier is a single flat pool matched by nearest-distance only -- a
# worker one tile from a $35 CARROT and three tiles from a $250 MELON
# grabs the CARROT every time. Split "water" into water_hi (STRAWBERRY/
# MELON, PREMIUM_CROPS) serviced strictly before water_lo (everything
# else) in task_order, keeping both in the same tier slot the flat
# "water" list held (after harvest, before plant/weeds). "water" itself
# is left in view/perceive() unchanged (still used by neglect_work/
# crop_pressure/hire-target work estimates elsewhere) -- only the
# tasks-dict tier actually consumed by assign_tasks()/unit_action() is
# split. urgent_water intentionally NOT split: a day-late tile risks
# withering regardless of crop value, so splitting it would trade a
# real penalty for a value gain that may not materialize before the
# penalty hits.
#
# RESULT -- TESTED, REJECTED (no gap compression). 5-seed smoke, both-
# seats: vs v9.5, mean -$3,840 both-seats-sum (-$1,920/game), t=-0.57,
# 95% CI [-$17,150,+$9,470] -- statistically zero, slightly negative
# point estimate. Vs opp_scenario_v14: mean -$92,593 both-seats-sum
# (-$46,296/game), t=-12.32 -- essentially identical to v9.12's own
# -$46,860/game (all 5/5 losses either way, +$564/game "improvement" is
# noise). Reprioritizing WITHIN the water tier by crop value doesn't
# touch the real constraint: the herd clone's advantage isn't which
# tile gets watered next, it's that its whole fleet completes ~2 days
# earlier on our same labor. Not worth a 15-seed confirmation -- both
# readings are far from any decision boundary. NOT submitted.
# =====================================================================
...
def perceive(me, opp, day):
    ...
    water_hi, water_lo = [], []  # v9.17: value-split "water" tier --
    # premium (STRAWBERRY/MELON) tiles serviced before ordinary crops
    # instead of pure nearest-distance matching within one flat tier.
    ...
    return {
        ...
        "water_hi": water_hi, "water_lo": water_lo,  # v9.17
...
TASK_ACTION = {
    ...
    "harvest": ["HARVEST"],
    "water": ["WATER"],
    "water_hi": ["WATER"],  # v9.17
    "water_lo": ["WATER"],  # v9.17
    "weeds": ["DIG"],
}
...
def _task_order(n_weeds):
    ...
    # v9.17: "water" split into water_hi (STRAWBERRY/MELON) serviced
    # before water_lo (other crops), still occupying the same slot the
    # flat "water" tier held relative to harvest/weeds/plant.
    if n_weeds >= WEED_URGENT_THRESHOLD:
        return ["urgent_water", "harvest", "weeds", "water_hi", "water_lo", "plant"]
    if n_weeds >= WEED_PRIORITY_THRESHOLD:
        return ["urgent_water", "harvest", "water_hi", "water_lo", "weeds", "plant"]
    return ["urgent_water", "harvest", "water_hi", "water_lo", "plant", "weeds"]
...
def _agent(obs):
    ...
    tasks = {
        ...
        "water_hi": list(view["water_hi"]),  # v9.17
        "water_lo": list(view["water_lo"]),  # v9.17
