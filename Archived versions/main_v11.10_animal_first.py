...
# =====================================================================
# MAIN V11.10 -- ANIMAL-FLEET-FIRST ECONOMY, ISOLATED on top of
# main_v10.6_radius3.py (champion). Structural capital-allocation test,
# not a parameter tweak: every prior v10.x round tuned constants WITHIN
# the existing "land runs first, animals compete for whatever budget/
# hands/crop-space is left over" ordering. This build inverts that
# ordering instead.
#
# --- The change ---
# economy() computes the current animal-fleet count (n_animals, same
# definition already used lower down for the purchase gate) BEFORE the
# land-expansion block instead of after it, and gates the entire land
# block on fleet completeness:
#   fleet_complete = n_animals >= EXPERIMENT_MAX_FLEET
#                     or day > max(COW.last_day, SHEEP.last_day)  # == 15
# No BUY_LAND order (NE/SW/SE, in the existing fixed unlock order) is
# submitted at all until fleet_complete is True. Once the fleet is either
# full (10 animals, the validated ceiling) or its purchase windows have
# both closed (day > 15, so no more animals will ever be bought), land
# purchasing resumes exactly as before -- same day<=22 window, same
# per-quad cost/day gates, same _land_priority() scoring.
# Nothing else changes: the animal purchase gates themselves
# (_animal_expansion_feasible / _sheep_expansion_feasible), the seed
# pipeline, hiring, and selling are all untouched.
#
# --- Hypothesis ---
# Land bought early sits mostly idle relative to hand/animal-service
# capacity for most of this project's history (EXPANSION_MIN_CROP_
# TILES_PER_QUAD reservations exist specifically because there's usually
# more open land than hands to work it). If early capital is the truly
# scarce resource, funding the fleet to completion first -- before any
# of it is diverted into land whose crop-tiles won't be worked for many
# turns anyway -- should get all 10 animals earning revenue sooner,
# at the cost of delaying land (and the crop revenue land unlocks).
# =====================================================================
#
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- v11.10: animal-fleet-first gate ---
    # Computed here (same definition the animal-purchase block below uses)
    # so the land block can be gated on it. Land purchasing is withheld
    # entirely until the fleet is either complete (hit the validated
    # 10-animal ceiling) or can no longer grow (both species' purchase
    # windows have closed) -- inverts v10.6's land-runs-first ordering.
    _n_animals_now = (len(view["my_pastures"]) +
                       sum(1 for p in animal_plans if p["stage"] in ("ORDERED", "BOUGHT", "CARRYING")))
    _fleet_complete = (_n_animals_now >= EXPERIMENT_MAX_FLEET or
                        day > max(ANIMAL_SPECS["COW"]["last_day"], ANIMAL_SPECS["SHEEP"]["last_day"]))

    # --- Land expansion ---
    if _fleet_complete and 1 <= day <= 22:
