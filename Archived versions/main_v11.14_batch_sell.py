...
# =====================================================================
# V11.14 -- BIG-BATCH SELLING on top of v10.6 (round-7 selling-rhythm
# theme). Aggregate replay data flagged SHED_AT_CAP as the dominant loss
# cause (31/100 losses): shed fills up and blocks production/selling.
# Hypothesis: the champion's continuous paced selling (re-evaluates and
# sells a fraction of every held item's stock nearly every turn once it
# clears a price floor) may be leaving the shed chronically half-full of
# small dribs, without ever building the kind of big accumulated batch
# that would justify a genuinely different sell rhythm. This variant
# tests the OPPOSITE temporal pattern: hold each item until it reaches a
# substantial quantity (BATCH_MIN_QTY=20) or a periodic re-evaluation day
# (day % BATCH_INTERVAL == 0), then sell it -- rather than nibbling every
# turn. force_dump / final-day liquidation / attack-window selling remain
# untouched as safety overrides so the shed can never actually hit
# SHED_CAP purely because a batch gate is still closed. Mechanism: a new
# gate right before the per-item sell branch in economy() skips adding a
# SELL order entirely unless the batch condition holds.
# =====================================================================
...
I0 = 10_000
SHED_CAP = 100
# v11.14: big-batch selling thresholds -- see file header.
BATCH_MIN_QTY = 20
BATCH_INTERVAL = 5
LIQUIDATE_DAY = 28
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        reason = "paced"
        # v11.14: big-batch gate -- skip this item entirely unless it has
        # accumulated to a substantial quantity, today is a periodic
        # re-evaluation day, or a safety override (force_dump / final
        # liquidation / attack window) still applies. Everything below
        # this point is unchanged from v10.6.
        batch_ok = (n >= BATCH_MIN_QTY or day % BATCH_INTERVAL == 0 or
                    force_dump or day >= LIQUIDATE_DAY or
                    (is_attack and item in PREMIUM_CROPS))
        if not batch_ok:
            continue
