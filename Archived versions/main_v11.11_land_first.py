...
# =====================================================================
# MAIN V11.11 -- LAND-FIRST MAXIMAL ECONOMY, ISOLATED on top of
# main_v10.6_radius3.py (champion). Structural capital-allocation test,
# the opposite bet from v11.10 (animal-fleet-first). Every prior v10.x
# round tuned constants WITHIN the existing balanced/animal-priority-
# score ordering; this build instead maximizes crop-land footprint as
# early and aggressively as budget allows, and deliberately delays
# animal purchases so land buys never have to compete with them for
# early cash.
#
# --- The change ---
# 1. Land: the day<=18 / day<=16 deadline gates on the SW (quads==2) and
#    SE (quads==3) BUY_LAND branches are removed. Land now buys as soon
#    as budget allows regardless of day, so a full 3-quadrant unlock can
#    in principle land turn 1-2 if the (unrealistic) cash were there --
#    in practice budget is the only remaining constraint, same $1000/
#    $2000/$4000 costs and same fixed NE/SW/SE order.
# 2. Animals: the entire animal-purchase block is now gated on
#    `_land_maxed_or_late = (quads >= 4) or (day >= 8)` -- no COW/SHEEP
#    purchase is attempted until either all four quadrants are unlocked
#    or day 8 arrives, whichever comes first. Before that point 100% of
#    early capital is available to land (and land alone competes with
#    the seed pipeline, which is unchanged).
# Nothing else changes: the animal purchase gates themselves, seed
# targets, hiring, and selling are all untouched.
#
# --- Hypothesis ---
# The current balanced/priority-score ordering caps land unlocks at
# day 16-18 deadlines that were tuned for a build where animals get
# first claim on cash. If crop-land footprint (not animal count) is
# the real long-run revenue driver -- more tiles growing STRAWBERRY/
# MELON/etc for more of the match -- buying out the full map first and
# funding animals later from crop revenue should out-earn the current
# ordering, at the cost of a later/smaller animal fleet.
# =====================================================================
#
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion ---
    # v11.11: SW (quads==2)/SE (quads==3) day-deadline gates removed --
    # land now buys purely on budget availability, as early and
    # aggressively as cash allows.
    if 1 <= day <= 22:
        land_score = _land_priority(view)
        if quads == 1 and budget > 1000:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2000:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 4000:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 4000

    # v11.11: deprioritize animals -- no purchase attempted until land is
    # maxed out (all 4 quadrants unlocked) or day 8 arrives, whichever
    # comes first. Before that point all early capital is reserved for
    # land (and the unchanged seed pipeline).
    _land_maxed_or_late = (quads >= 4) or (day >= 8)
    ...
    if _land_maxed_or_late and n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1 / v11.11: land-first gate
