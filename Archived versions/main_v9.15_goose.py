...
# =====================================================================
# MAIN V9.15 -- GOOSE/EGG pipeline, on top of v9.12. Base:
# main_v9.12_melon_fert.py.
#
# Never implemented before this build: ANIMAL_SPECS only had COW/SHEEP.
# The engine's third animal, GOOSE (cost $300, cheapest; structure COOP,
# not PASTURE; first_yield_day=4, interval=1 -- fastest cycle of the
# three), has sat completely unused. Read the engine directly to confirm
# FEED/CARE/COLLECT_FERTILIZER/HARVEST are all structure-agnostic (only
# check for an "animal" key, kaggriculture.py ~481-506) -- so the
# existing maintenance state machine works for geese once COOP tiles are
# tracked, no new maintenance logic needed. The one real hazard: PLACE
# requires an exact structure-kind match and BUILD_* requires a truly
# empty tile, so PASTURE and COOP site pools must never be mixed (a COW
# plan picking an existing empty COOP, or vice versa, would silently
# stall forever) -- animal_setup_action is now structure-aware via
# ANIMAL_STRUCTURE, using view["empty_coops"]/BUILD_COOP for GOOSE and
# view["empty_pastures"]/BUILD_PASTURE for COW/SHEEP, never crossed.
#
# GOOSE gets its own small, separate cap (GOOSE_MAX_FLEET=3) and gate
# (_goose_expansion_feasible), tried only after COW's AND SHEEP's gates
# both already say no this turn -- deliberately a free add-on, not a
# second fleet-scaling experiment (EXPERIMENT_MAX_FLEET's own n_animals
# count is now explicitly filtered to species in COW/SHEEP, so geese
# never count against the ceiling v9.6/v9.11 both found to be a hard
# constraint independent of labor). Maintenance-facing counts (feed/
# wheat target, animal-service hand reservation, survival/urgent-feed
# checks, _pending_animal_work) all now read a combined view["my_animals"]
# (my_pastures + my_coops) instead of my_pastures alone, since geese
# generate the same real feed/care/harvest work COW/SHEEP do.
#
# RESULT -- TESTED, REJECTED. Mechanically works (confirmed via
# instrumentation: 3/3 geese purchased and active, EGG flowing through
# the shed and selling) but net harmful. 5-seed smoke, both-seats: v9.5
# wins 4/5, +$6,349/game (paired-sum/2), t=-2.07. Vs opp_scenario_v14:
# -$62,862/game -- WORSE than v9.5's own -$51.7k baseline and much
# worse than v9.10/v9.12's -$46.7-46.9k. Two implementation notes worth
# keeping for any future animal-species work: (1) the shared crop-space
# gate formula (5*quads) was tuned for COW/SHEEP's footprint and blocked
# GOOSE on ~80% of eligible turns even with mostly-idle land -- needed a
# much smaller floor (>=2) to be reachable at all; (2) GOOSE's purchase
# window only gets a real look once COW's AND SHEEP's own gates already
# fail that turn (by design, so it never competes with them), which in
# practice doesn't happen until ~day 10+ -- an early last_day cutoff
# (10) left almost no real window, widened to 20.
#
# But the RESULT itself confirms the deeper pattern from v9.11/v9.13/
# v9.14: this project has now tested four different "add more" ideas on
# top of v9.10 (bigger fleet, more seed targets, more hands, a whole
# third animal species) and all four made things worse. The remaining
# gap looks like a labor-THROUGHPUT-efficiency problem -- how well the
# existing ~10-12 hands are routed to the work already in front of them
# -- not a volume/scale problem solvable by giving them more of
# anything. NOT submitted.
# =====================================================================

# =====================================================================
# MAIN V9.12 -- melon fertilize-in-window, on top of v9.10. Base:
# main_v9.10_hire_calibrated.py.
#
# v9.3's fertilize-cycle-compression header flagged "melon fertilize-
# in-window (they do it, we don't)" as the next small lever, without
# implementing it -- v9.3's mechanism (fert_targets -> dispatch pass)
# only covers "ongoing" crops (STRAWBERRY/TOMATO), which accumulate
# yield via a completely different real-engine code path
# (_daily_refresh_plants) than MELON does. Read the engine's WATER
# action handler directly (kaggriculture.py ~374-386) to find MELON's
# actual mechanic: a non-ongoing crop gets a yield_units bonus each time
# it's watered during the back half of its growth window (age in
# [(max_day+1)//2, max_day] -- for MELON, days 6-12 post-planting) --
# +1 normally, +2 if fertilized that day, capped at max_yield=6. This
# build extends fert_targets to include MELON tiles inside that window
# (below the yield cap, no active fertilizer), reusing the existing
# dispatch/shed-reserve machinery unchanged -- isolated to the target
# set, not the mechanism (v9.4's separate tuning of that mechanism --
# harvest-first guards, value-ranking, dynamic reserve -- was a proven
# wash, so none of it is touched here).
#
# RESULT -- TESTED, WASH both ways. Vs v9.5 (5-seed smoke, both-seats):
# t=-0.03, mean -$95/game -- statistically zero. Vs opp_scenario_v14:
# -$46,667/game, essentially identical to v9.10's own -$46,860 (no
# additional compression, within noise). Not worth a 15-seed
# confirmation -- neither reading is close to a decision boundary.
# Melon's fertilize-in-window bonus apparently isn't a meaningful lever
# at our current melon planting volume/coverage, or the fertilize-
# dispatch's existing shed reserve/worker pool is already saturated by
# STRAWBERRY/TOMATO before MELON gets a turn. Kept as a free, harmless
# addition (same precedent as the v9 oracle sell-timing module) but
# does not move the top-tier gap. NOT submitted.
# =====================================================================
...
ANIMAL_SPECS = {
    ...
    # v9.15: GOOSE -- cheapest animal ($300), COOP structure (not
    # PASTURE), fastest cycle (first_yield_day=4, interval=1 day,
    # engine ANIMALS table). Never implemented before this build: no
    # purchase/setup/maintenance code existed for it at all. Its own
    # small independent cap (GOOSE_MAX_FLEET) and gate keep it out of
    # EXPERIMENT_MAX_FLEET's COW/SHEEP count entirely (see that
    # counting site, which now filters view["my_pastures"] to
    # kind=="PASTURE" specifically) -- geese don't compete with the
    # fleet-size ceiling that v9.6/v9.11 both found to be a hard
    # constraint independent of labor.
    "GOOSE": {"cost": 300, "min_money": 400, "start_day": 0,  "last_day": 20},  # v9.15:
    # was last_day=10 -- GFAIL instrumentation showed the goose gate is
    # mostly only REACHED (COW's and SHEEP's own gates both already
    # failing) from day ~10 onward, so a day-10 cutoff left almost no
    # real window. Widened to 20: interval=1 means even a day-15 goose
    # still nets ~14 days of eggs before the day-29 endgame.
}
ANIMAL_STRUCTURE = {"COW": "PASTURE", "SHEEP": "PASTURE", "GOOSE": "COOP"}  # v9.15
GOOSE_MAX_FLEET = 3   # v9.15: deliberately small -- this is meant to be a
                       # free addition on top of the existing crop/COW/SHEEP
                       # economy, not a second fleet-scaling experiment.
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    my_coops = []              # v9.15: (pos, tile) for coops WITH a goose --
    empty_coops = []           # kept SEPARATE from my_pastures/empty_pastures
                                # (site selection and BUILD_* must not mix
                                # PASTURE and COOP tiles -- PLACE requires an
                                # exact structure-kind match per animal, and
                                # BUILD_* requires the tile be truly empty,
                                # so a COW plan must never treat an existing
                                # empty COOP as a buildable/placeable site).
    ...
    return {
        ...
        "my_coops": my_coops, "empty_coops": empty_coops,  # v9.15
        "my_animals": my_pastures + my_coops,  # v9.15: for maintenance/feed/
                                                 # service-hand logic, which is
                                                 # already structure-agnostic
                                                 # in the real engine (FEED/
                                                 # CARE/COLLECT_FERTILIZER/
                                                 # HARVEST only check for an
                                                 # "animal" key, not kind).
                                                 # EXPERIMENT_MAX_FLEET's own
                                                 # count stays on my_pastures
                                                 # alone (filtered to
                                                 # kind=="PASTURE") so geese
...
def _pending_animal_work(view):
    """Count of pastures/coops with an actual pending need this turn (unfed,
    ready to harvest, needs care, or has fertilizer waiting). v9.15: was
    my_pastures only -- geese (my_coops) generate the same kinds of work."""
    return sum(1 for _pos, t in view["my_animals"]
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    ...
    if any(not t.get("fed_today", True) and t.get("consecutive_unfed", 0) >= 1
           for _pos, t in view["my_animals"]):  # v9.15: was my_pastures -- include geese
        return False
...
def _goose_expansion_feasible(obs, view, budget, shed, animal_plans, n_hands, quads, day=99):
    """v9.15: GOOSE's own gate, tried only after COW's AND SHEEP's gates
    have both already said no this turn (see the purchase-selection
    block in economy()) -- a free add-on for an otherwise-empty purchase
    slot, not a competitor for it. Deliberately small and simple: real
    safety checks (survival across ALL animals including existing geese,
    wheat budget, hand-capacity headroom, crop-space) still apply, but
    the fleet-size ceiling is GOOSE_MAX_FLEET (3), completely separate
    from EXPERIMENT_MAX_FLEET -- geese never compete with COW/SHEEP for
    that ceiling (see n_animals's species filter in economy())."""
    n_goose = sum(1 for p in animal_plans if p["species"] == "GOOSE"
                  and p["stage"] in ("ACTIVE", "ORDERED", "BOUGHT", "CARRYING"))
    if n_goose >= GOOSE_MAX_FLEET:
        return False

    if any(not t.get("fed_today", True) and t.get("consecutive_unfed", 0) >= 1
           for _pos, t in view["my_animals"]):
        return False

    n_support_now = len(view["my_pastures"]) + len(view["my_coops"])
    next_target_wheat = (n_support_now + 1) * 2 + FEED_CARRY_TARGET
    have = shed.get("WHEAT", 0)
    shortfall = max(0, next_target_wheat - have)
    shortfall_cost = shortfall * live_price(obs, "WHEAT")
    if shortfall_cost > EXPANSION_WHEAT_BUDGET_FRAC * max(budget, 1):
    ...
    # v9.15: a much smaller crop-space floor than COW/SHEEP's (5*quads) --
    # one goose coop is a single tile, not a slot in a 10-animal fleet, and
    # the shared per-quad formula was found (via GFAIL instrumentation, not
    # kept in this file) to block GOOSE on ~80% of otherwise-eligible turns
    # even with a whole empty quadrant of idle land, because it was tuned
    # for COW/SHEEP's much bigger footprint. Still a real floor (not zero),
    # just proportional to what a single coop actually costs.
    available_for_crops = len(view["empty"]) - len(reserved_sites) - 1
    if available_for_crops < 2:
        return False
...
def _feed_priority(view):
    ...
    growth."""
    if any(not t.get("fed_today", True) and t.get("consecutive_unfed", 0) >= 1
           for _pos, t in view["my_animals"]):  # v9.15: was my_pastures -- geese need feed too
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v9.15: n_animals/composition gates stay COW+SHEEP-only (pasture-based
    # fleet), filtered explicitly by species -- GOOSE plans must NOT count
    # toward EXPERIMENT_MAX_FLEET or the sheep-floor composition guard.
    # n_support (feed/wheat sizing) DOES include geese -- they eat wheat
    # too (engine's FEED handler is animal-agnostic).
    n_animals = (len(view["my_pastures"]) +
                 sum(1 for p in animal_plans if p["species"] in ("COW", "SHEEP")
                     and p["stage"] in ("ORDERED", "BOUGHT", "CARRYING")))
    n_support = (len(view["my_pastures"]) + len(view["my_coops"]) +
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is None:
            ...
            # its own separate (relaxed-ceiling) gate -- see file header.
                ...
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            # v9.15: GOOSE only gets a look once COW's AND SHEEP's gates
            # have both already said no this turn -- it's meant to be a
            # free add-on for a turn that would otherwise buy nothing,
            # not a competitor for the same purchase slot.
            elif _goose_expansion_feasible(obs, view, budget, shed, animal_plans, len(me["hands"]), quads, day):
                target_plan = {"species": "GOOSE", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
        if target_plan is not None:
            ...
            if (spec["start_day"] <= day <= spec["last_day"]
                ...
                # v9.5: clear stale site/worker in case this is a reused
                # NONE plan from an earlier failed cycle -- a fresh order
                # owns nothing until confirmation.
...
# nearest tile in view["my_animals"] (v9.15: my_pastures + my_coops),
# which naturally includes every ACTIVE plan's site regardless of
# species or how many are active.
# =====================================================================
...
def animal_reconcile(plan, view, shed, farmer_carry, day, hour):
    ...
    site_occupied = site is not None and any(pos == site for pos, _tile in view["my_animals"])  # v9.15: was my_pastures -- GOOSE sites are COOP tiles
    if site_occupied:
        if stage != "ACTIVE":
...
def animal_setup_action(pos, view, shed, farmer_carry, plan, day, hour, reserved_sites=(), exclude_sites=()):
    ...
    # v9.15: structure-aware -- GOOSE needs a COOP, COW/SHEEP need a
    # PASTURE. Must not mix these pools: PLACE requires an exact
    # structure-kind match, and BUILD_* requires the tile be truly
    # empty, so a COW plan picking an existing empty COOP (or vice
    # versa) would silently stall forever (BUILD_PASTURE no-ops on a
    # non-None tile, PLACE no-ops on the wrong structure kind).
    structure = ANIMAL_STRUCTURE.get(species, "PASTURE")
    empty_structs = view["empty_coops"] if structure == "COOP" else view["empty_pastures"]
    build_op = "BUILD_COOP" if structure == "COOP" else "BUILD_PASTURE"

    site = plan.get("site")
    if site is None or (site not in view["empty"] and site not in empty_structs):
        candidates = [t for t in (empty_structs or view["empty"])
    ...
    # 1. Build the structure first (animal waits safely in the shed).
    if site not in empty_structs:
        if pos != site:
...
def animal_maintenance_action(pos, view, shed, carry, day, hour, exclude=(), critical_only=False):
    ...
    next worker."""
    candidates = [(p, t) for p, t in view["my_animals"] if p not in exclude]  # v9.15: was my_pastures -- include geese
    if critical_only:
...
def _agent(obs):
    ...
    n_animals_for_wheat = (len(view["my_pastures"]) + len(view["my_coops"]) +  # v9.15: include geese
                            sum(1 for p in animal_plans if p["stage"] in ("BOUGHT", "CARRYING")))
    ...
    n_animals_for_service = (len(view["my_pastures"]) + len(view["my_coops"]) +  # v9.15: include geese
                              sum(1 for p in animal_plans if p["stage"] in ("BOUGHT", "CARRYING")))
