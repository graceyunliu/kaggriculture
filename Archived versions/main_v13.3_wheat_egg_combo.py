...
# =====================================================================
# v13.3 (Aug 7) -- Round 9 glut-resistance hypothesis, built on top of
# v10.22's already-tested GOOSE/EGG composition-floor mechanism (below,
# unchanged in its plumbing) plus v13.1's wheat-cash-crop reintroduction.
# Prior finding (project memory): v10.22's GOOSE_FLOOR_BY_FLEET_SIZE =
# {3:1, 6:2, 9:3} (~1/3 goose share) DECISIVELY LOST self-play vs v10.6,
# -$9,444/game, because EGG's low base price ($50) gave lower revenue-
# per-service-day (~$50/day) than the cow/sheep (~$80/$67/day at BASE
# price) it displaced. But that test only ever ran against v10.6-derived
# opponents, which don't glut the market -- so cow/sheep's MILK/WOOL
# revenue was measured at something close to full sticker price. Against
# a real high-volume opponent (Opponents/opp_scenario_v14.py, confirmed
# via its own code to target a 15-animal fleet), MILK (T=122,
# above_target=1.60, linear) and WOOL (T=105, above_target=3.20, sq) are
# both tiny-T, steep-shape goods that plausibly sit near the $1 floor
# most of the game once the OPPONENT's own 15-animal production is also
# drawing down the same shared market-inventory number -- meaning their
# $80/day and $66.67/day "at base price" numbers, used throughout this
# project's history to prioritize cow/sheep over goose, may never
# actually be realized against a real opponent. EGG (T=332,
# above_target=0.20, log) stays glut-resistant regardless. This
# hypothesis bumps the goose floor further (GOOSE_FLOOR_BY_FLEET_SIZE
# below, ~30-40% of fleet slots by the time the fleet is full) and
# retests BOTH against v10.6 self-play (expect it to replicate v10.22's
# loss, since v10.6 doesn't glut the market either) AND against
# opp_scenario_v14 (the real test: does goose's glut-resistance flip the
# result once MILK/WOOL are actually being suppressed by real combined
# volume?). Combined with v13.1's WHEAT cash-crop reintroduction (see
# that file's header) since both changes exploit the same glut-
# resistance property (WHEAT: T=400, above_target=0.20, log -- same
# shape family as EGG) and are additive (crop-side vs animal-side), on
# the theory that a real opponent is where BOTH should show a bigger win
# margin than they do in self-play vs v10.6.
# =====================================================================

# =====================================================================
# v10.22 (Aug 7) -- on top of v10.21's GOOSE/EGG plumbing (perceive/
# setup/maintenance/reconcile all generalized to COOP, see below): v10.21
# tried goose ONLY as a last-resort fallback after cow's and sheep's
# gates declined, sharing the same EXPERIMENT_MAX_FLEET=10 ceiling.
# Result: 10/10 seeds identical to the dollar vs v10.6 -- goose never
# fired once, because cow+sheep reliably fill all 10 slots before either
# gate's day-window (12/15) closes, so goose's shared-ceiling gate was
# always already blocked too. v10.22 adds a real composition guard
# (GOOSE_FLOOR_BY_FLEET_SIZE, same pattern as the existing SHEEP one) so
# goose genuinely displaces some cow/sheep slots within the SAME
# validated 10-slot ceiling -- testing whether goose's lower cost ($300
# vs 400/500) and faster payback (first_yield_day=4/interval=1 vs cow's
# 8/2 or sheep's 6/3) earns more per service-slot than the animal it
# replaces.
# =====================================================================

# =====================================================================
# v10.21 (Aug 7) -- GOOSE/EGG third animal species, built on v10.6's
# champion siting mechanism. Standing next-step from the fresh-rethink
# report: 18 hypotheses on top of v10.6 (siting refinements, land/hand
# tuning, fleet-ceiling changes, a full animal-purchase architecture
# rewrite) all either lost or were conditional/marginal -- every one of
# them was a reallocation within the SAME cow/sheep/land/crop trade-off
# space already explored since v8. GOOSE is a genuinely separate, never-
# attempted revenue category: engine-verified (kaggriculture.py ANIMALS
# dict) as cost=$300 (cheapest of the three), structure=COOP (not
# PASTURE -- a distinct buildable), first_yield_day=4 /interval=1 (much
# faster payback than COW's day8/interval2 or SHEEP's day6/interval3),
# max_held=4, product=EGG (MARKET_PARAMS T=332, base=$50 -- a real,
# separate demand pool real opponents barely touch per project memory).
# Mechanically FEED/CARE/HARVEST/COLLECT_FERTILIZER/escape rules are
# byte-identical between COOP and PASTURE animals (verified in engine's
# _daily_refresh_animals/apply_action) -- only the structure tile kind
# and BUILD_* op differ. Implementation below generalizes perceive()'s
# pasture scan to also track COOP tiles (my_coops/empty_coops, plus a
# combined my_animals used everywhere maintenance/feed-target/service-
# target code was species-agnostic already), adds a species->structure
# map, and threads a GOOSE branch through the SAME state machine (no
# rewrite -- the v10.20 decoupled-purchase rewrite already lost
# decisively, see project memory). Purchase gate: GOOSE is tried LAST in
# economy()'s per-turn elif chain, only on turns where COW's gate and
# SHEEP's gate (both unchanged) already declined -- the most
# conservative, additive framing: does GOOSE profitably use purchase
# capacity/labor slack the existing cow/sheep composition wasn't
# otherwise claiming? Shares the SAME already-validated
# EXPERIMENT_MAX_FLEET=10 ceiling (n_animals now counts all three
# species) rather than adding a new/higher cap -- raising the ceiling
# itself has failed 3 independent times this project.
# =====================================================================
...
# SHEEP cost=500, both structure="PASTURE"; v10.21 adds GOOSE cost=300,
# structure="COOP" -- see ANIMAL_STRUCTURE below for the site-type map
# this now needs).
ANIMAL_SPECS = {
    ...
    # v10.21: GOOSE, engine cost=$300, first_yield_day=4/interval=1 (much
    # faster payback than COW/SHEEP) -- min_money set to ~1.3x cost,
    # matching the COW/SHEEP ratio convention. last_day=20 (later than
    # either) since the fast interval still leaves real production days
    # even on a late purchase, and since goose is tried LAST in the
    # per-turn gate chain (see header) it needs a wide window to fire at
    # all.
    "GOOSE": {"cost": 300, "min_money": 400, "start_day": 0,  "last_day": 20},
}
# v10.21: species -> structure it needs built before PLACE. COW/SHEEP
# share PASTURE (unchanged from v8.3-era comment below); GOOSE needs a
# separate COOP structure (engine-verified, kaggriculture.py ANIMALS
# dict).
...
# v10.21: GOOSE gets its own gate too (_goose_expansion_feasible), tried
# last in the chain -- see header. Small headroom since it's meant to
# use slack, not compete for the hand budget cow/sheep already claim.
GOOSE_ENABLED = True
GOOSE_HAND_HEADROOM = 2
...
# v10.22: v10.21's "try goose only after cow/sheep decline" design never
# fired a single purchase across 10 seeds (identical-to-the-dollar vs
# v10.6) -- cow+sheep reliably fill EXPERIMENT_MAX_FLEET=10 well before
# either gate's day-window (12/15) closes, so goose's shared-ceiling
# last-resort gate was always already blocked by n_animals>=10 too. This
# genuinely reallocates: like SHEEP_FLOOR_BY_FLEET_SIZE, a minimum goose
# count required once the fleet reaches a given size -- goose is
# cheapest ($300 vs 400/500) and pays back fastest (first_yield_day=4,
# interval=1 vs cow's 8/2, sheep's 6/3), so testing whether it's worth
# more $/service-slot than an equivalent cow/sheep slot within the SAME
# already-validated 10-slot ceiling (not a new/higher cap).
# v13.3: raised from v10.22's {3:1, 6:2, 9:3} (~30% share at full fleet)
# to reach roughly 35-40% of fleet slots by the time the fleet fills the
# validated EXPERIMENT_MAX_FLEET=10 ceiling -- a meaningfully larger
# goose allocation, per this file's header rationale.
GOOSE_FLOOR_BY_FLEET_SIZE = {3: 1, 5: 2, 8: 3, 10: 4}
...
def perceive(me, opp, day):
    ...
    empty_pastures = []
    my_coops = []              # v10.21: same, for COOP tiles (GOOSE)
    empty_coops = []
    ...
    return {
        ...
        "my_coops": my_coops, "empty_coops": empty_coops,  # v10.21
        # v10.21: combined view used by every piece of maintenance/feed-
        # target/service-target/survival-check code that was already
        # species-agnostic (FEED/CARE/HARVEST/COLLECT_FERTILIZER/escape
        # rules are byte-identical between PASTURE and COOP animals --
        # verified in the engine). Only site-BUILD logic (animal_setup_
        # action) needs to distinguish structure type; everything else
        # just needs "every active animal tile, any species."
        "my_animals": my_pastures + my_coops,
...
def _min_goose_for_fleet(next_animal_index):
    """v10.22: same composition-guard pattern as _min_sheep_for_fleet, for
    GOOSE_FLOOR_BY_FLEET_SIZE."""
    floor = 0
    for size, req in GOOSE_FLOOR_BY_FLEET_SIZE.items():
        if next_animal_index >= size:
            floor = max(floor, req)
    return floor
...
def _pending_animal_work(view):
    """Count of animal structures (pasture or coop) with an actual pending
    need this turn (unfed, ready to harvest, needs care, or has fertilizer
    waiting)."""
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    ...
    # v10.21: was view["my_pastures"] -- now checks every active animal
    # structure (pasture or coop), so a goose already missing a feeding
    # blocks new cow/sheep purchases too, and vice versa.
...
def _goose_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    """v10.21: GOOSE's own gate, tried LAST in economy()'s per-turn chain --
    only reached on turns where COW's gate AND SHEEP's gate (both
    completely unchanged) have already declined. Same real/safety checks
    as COW/SHEEP (survival across the combined my_animals view, wheat
    budget, crop neglect, crop space, shared EXPERIMENT_MAX_FLEET
    ceiling); hand-capacity projection gets a small headroom
    (GOOSE_HAND_HEADROOM) since this purchase is meant to use otherwise-
    idle capacity, not compete for the hand budget cow/sheep already
    claim. Mirrors sheep's wool-glut stop with an egg-glut stop instead
    (EGG's own MARKET_PARAMS T=332 pool, glutting it the same way would
    floor marginal-egg prices)."""
    if n_animals >= EXPERIMENT_MAX_FLEET:
        return False

    if obs["market"]["inventory"].get("EGG", I0) >= I0 + 30:
        return False
    ...
    crop_work = neglect_work + len(view["empty"])
    projected_work = crop_work + (animal_work + 1) * ANIMAL_WORK_WEIGHT
    if math.ceil(projected_work / 5) > HAND_TARGET_MAX + GOOSE_HAND_HEADROOM:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v10.21: was view["my_pastures"] -- now counts geese too, so the
    # already-validated EXPERIMENT_MAX_FLEET=10 ceiling covers all three
    # species jointly rather than adding a new/separate cap.
    n_animals = (len(view["my_animals"]) +
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is None:
            ...
            # v10.22: same pattern for GOOSE -- tried BEFORE the sheep
            # composition check (goose is cheapest and pays back fastest,
            # so it should get first claim on its guaranteed floor slots),
            # genuinely reallocating some of the fixed 10-slot ceiling
            # away from cow/sheep instead of only ever firing as a
            # last-resort fallback (v10.21's design, which never fired --
            # see header).
            n_goose_owned = sum(1 for p in animal_plans
                                  if p["species"] == "GOOSE"
                                  and p["stage"] in ("ACTIVE", "ORDERED", "BOUGHT", "CARRYING"))
            composition_wants_goose = (GOOSE_ENABLED and
                n_goose_owned < _min_goose_for_fleet(n_animals + 1))

            if (composition_wants_goose and
                    _goose_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day)):
                target_plan = {"species": "GOOSE", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            elif (composition_wants_sheep and
            ...
            # its own separate (relaxed-ceiling) gate -- see file header.
                ...
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            # v10.22: goose fallback retained too, for turns where cow AND
            # sheep both decline for a non-ceiling reason (day-window
            # closed, budget, etc.) but goose's window is still open.
            elif GOOSE_ENABLED and _goose_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads, day):
                target_plan = {"species": "GOOSE", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
    ...
    # v13.3: reintroduced as a glut-resistant cash crop, same
    # target/reasoning as v13.1 -- combined with the raised GOOSE floor
    # above, both exploit the same glut-resistance property.
...
def animal_reconcile(plan, view, shed, farmer_carry, day, hour):
    ...
    # v10.21: was view["my_pastures"] -- now checks the combined
    # pasture+coop view so this works for GOOSE/COOP plans too.
    site_occupied = site is not None and any(pos == site for pos, _tile in view["my_animals"])
    if site_occupied:
        if stage != "ACTIVE":
...
def animal_setup_action(pos, view, shed, farmer_carry, plan, day, hour, reserved_sites=(), exclude_sites=()):
    ...
    v10.21: structure-aware -- COW/SHEEP still build/use PASTURE, GOOSE
    builds/uses COOP (ANIMAL_STRUCTURE map). Everything else (pickup at
    shed, walk-and-place) is identical across species."""
    species = plan["species"]
    stage = plan["stage"]
    ...
    empty_structures = view["empty_coops"] if structure == "COOP" else view["empty_pastures"]

    site = plan.get("site")
    if site is None or (site not in view["empty"] and site not in empty_structures):
        candidates = [t for t in (empty_structures or view["empty"])
    ...
    if site not in empty_structures:
        if pos != site:
    ...
    # 3. Walk it to the structure and place it. VERIFIED format: no qty arg.
    if pos != site:
        return [_step_toward(pos, site)]
...
def animal_maintenance_action(pos, view, shed, carry, day, hour, exclude=(), critical_only=False):
    ...
    next worker."""
    # v10.21: was view["my_pastures"] -- FEED/CARE/HARVEST/COLLECT_
    # FERTILIZER/deposit logic below is species-agnostic (verified
    # identical in the engine for COOP vs PASTURE animals), so this now
    # covers the whole fleet including geese.
    candidates = [(p, t) for p, t in view["my_animals"] if p not in exclude]
    if critical_only:
...
def _agent(obs):
    ...
    n_animals_for_wheat = (len(view["my_animals"]) +
    ...
    n_animals_for_service = (len(view["my_animals"]) +
                              sum(1 for p in animal_plans if p["stage"] in ("BOUGHT", "CARRYING")))
