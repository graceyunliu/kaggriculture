...
# v11.4 (Aug 7) -- PRODUCTION THROTTLE by shed occupancy, shed-capacity
# round. Base: main_v10.6_radius3.py, one isolated change, nothing else
# touched.
...
# Mechanism: v10.6 only throttles SELLING under shed pressure (force_dump
# / paced-selling "relax" term) plus one narrow MELON-specific seed cap.
# This hypothesis throttles PRODUCTION itself -- every seed-buy target in
# the `want` dict (MELON/STRAWBERRY/TOMATO/CARROT) is scaled down by a
# multiplier keyed to live shed_load/SHED_CAP: 1.0 below 50% occupancy,
# 0.6 at 50-70%, 0.3 at 70-90%, 0.0 (new seed buys pause) above 90%. This
# stacks on top of (applies after) the existing MELON-specific
# SHED_CAP-30 throttle. Goal: never grow more than the shed/market can
# realistically clear.
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v11.4 (shed-capacity round): PRODUCTION THROTTLE. Scale every seed
    # target DOWN by a multiplier based on live shed occupancy, so the
    # farm never keeps growing more than it can realistically sell
    # through -- this throttles production itself, not just selling
    # (which v10.6 already does via force_dump/paced selling). Tiers:
    # 1.0 below 50% shed occupancy, 0.6 at 50-70%, 0.3 at 70-90%, 0.0
    # above 90% (new seed buys pause entirely; existing stock still
    # sells normally). This stacks with the existing MELON-specific
    # SHED_CAP-30 throttle above -- that one still applies first, then
    # this multiplier scales the (possibly already-reduced) result.
    shed_frac = shed_load / SHED_CAP
    if shed_frac >= 0.9:
        production_mult = 0.0
    elif shed_frac >= 0.7:
        production_mult = 0.3
    ...
    if production_mult < 1.0:
        want = {crop: int(round(tgt * production_mult)) for crop, tgt in want.items()}
