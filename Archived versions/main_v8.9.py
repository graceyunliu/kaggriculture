...
# =====================================================================
# MAIN V8.9 -- marginal-return COW/SHEEP competition, ISOLATED. Base:
# main_v8.3.py (the validated champion -- NOT main_v8.8.py's rejected
# crop-share controller, see graveyard). Byte-identical except for HOW
# the next animal purchase picks a species.
#
# --- Why (Grace's request after v8.8's rejection) ---
#
# v8.3's species selection (still in this file, unchanged, below) tries
# COW's own gate (_animal_expansion_feasible) first every turn, and only
# even LOOKS at SHEEP via a separate elif once COW's gate has already
# said no. That means whenever both species would be affordable/
# feasible, COW wins automatically -- SHEEP is never actually compared
# against COW on which one is the better buy right now, it only gets a
# turn when COW is already blocked. Grace asked for this to change: make
# the two species compete on marginal return, not sequence order.
#
# --- The change ---
#
# Both _animal_expansion_feasible() (COW) and _sheep_expansion_feasible()
# (SHEEP) are called UNCHANGED, in the same place, with the same five
# checks each (survival/wheat-budget/hand-capacity/crop-neglect/crop-
# space) -- total fleet feasibility is untouched, per Grace's
# requirement. What changes is what happens when BOTH pass: instead of
# COW winning by default, _animal_marginal_score() scores each species
# independently and whichever scores higher is bought. If only one
# passes its gate, that one is bought (same as before). If neither
# passes, no purchase (same as before). Crop scheduling, PLANT_PRIORITY,
# task_order, and every other v8.3 system are completely untouched.
#
# The score is an expected $/tile/day return, built entirely from real
# engine constants (ANIMALS dict in vendor/kaggle_environments_engine/
# kaggriculture.py: COW cost=400/first_yield_day=8/interval=2/
# max_held=6/product=MILK; SHEEP cost=500/first_yield_day=6/interval=3/
# max_held=6/product=WOOL) and live observation (obs market prices, this
# farm's own currently-observed care-completion rate) -- see
# _animal_marginal_score()'s docstring below for the six factors Grace
# asked for and exactly how each is grounded. This replaces the old
# header's static "COW nets ~$80/day vs SHEEP's ~$66.7/day" estimate,
# which baked in one fixed, unverified care-completion assumption and
# never adapted to how well THIS game's hand pool is actually keeping up
# with maintenance.
#
# --- What this deliberately does NOT do ---
#
# No change to either gate function, no crop-share/PLANT_PRIORITY
# changes (that's v8.8, already rejected -- see graveyard), no GOOSE, no
# fertilizer-application action. Single variable: which species wins
...
# v8.9: real per-species facts, from vendor/kaggle_environments_engine/
...
def _animal_marginal_score(species, obs, view, day, n_hands, n_animals):
    """v8.9: expected $/tile/day return of buying `species` right now --
    the shared yardstick _agent() uses to let COW and SHEEP compete on
    marginal return instead of COW always winning by being checked
    first. Six factors, each grounded in either a real engine constant or
    a live observation:

      1. remaining production ticks -- _remaining_ticks(), real engine
         cadence (first_yield_day + interval from ANIMAL_ECON).
      2. market price -- live_price(obs, product), this turn's actual
         market price, not a fixed base assumption.
      ...
      4. maintenance burden -- `hand_pressure`: how much thinner one more
         ...
         n_animals+1). This is the real, verifiable labor-cost lever in
         this codebase (feed/care visit cost is otherwise IDENTICAL
         between species -- both need exactly one FEED + one CARE visit
         per day, confirmed by reading the engine's FEED handler, which
         takes 1 WHEAT regardless of species). Expressed as a discount on
         expected care completion, not an invented flat dollar penalty.
      5. feed cost -- remaining_days * live wheat price, real and
         identical per species (see above).
      6. pasture tile value -- the function's return value itself: net
         ...
         since each occupies exactly one pasture tile.

    Caveat, stated plainly: `hand_pressure` and the 0.6 fallback in
    _observed_care_rate() are reasonable proxies, not a full forward
    simulation of future care completion -- nothing in this codebase
    simulates future turns before committing to a purchase. This is
    intentionally simpler than that, using only numbers this turn
    already knows to be real.
    """
    econ = ANIMAL_ECON[species]
    ...
    price = live_price(obs, econ["product"])
    ...
    feed_cost = remaining_days * live_price(obs, "WHEAT")   # 1 WHEAT/day/animal,
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if not in_progress:
        ...
        if target_plan is None:
            # v8.9: both species' own gates are checked, UNCHANGED from
            # v8.3 (total fleet feasibility untouched, per Grace's
            # requirement). What changes is what happens when both pass:
            # instead of COW winning automatically by being checked
            # first, _animal_marginal_score() picks whichever species has
            # the better expected $/tile/day return right now. If only
            # one gate passes, that species is bought (same as v8.3). If
            # neither passes, no purchase (same as v8.3).
