...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    want = {}
    # v7.9b: STRAWBERRY inserted BEFORE melon. The loop below walks
    # want.items() in insertion order and stops once budget runs out, so
    # insertion order is seed-budget priority. v7.9 promoted strawberry in
    # PLANT_PRIORITY (tile priority) but left melon first here, so on
    # tight-budget turns melon still claimed the seed cash first --
    # inconsistent with the planting order it now competes under.
