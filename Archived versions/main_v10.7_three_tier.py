...
NEAR_SHED_RADIUS = 3
MID_SHED_RADIUS = 6   # v10.7: second boundary, near/mid vs mid/far

# v10.7 hypothesis: v10.5/v10.6's near/far split treats all one-time crops
# alike, but they aren't -- WHEAT (bonus window ages 2-4, ~3 possible
# waterings) and CARROT (ages 2-3, ~2 waterings) mature fast and need very
# few visits, while MELON's bonus window is ages 6-12 (up to 7 waterings
# across a 12-day pre-harvest span) -- real revisit count much closer to
# an ongoing crop's than to wheat/carrot's, even though it's still a
# one-time harvest. Generalize the binary near/far split into 3 tiers by
# expected revisit frequency (0=highest/ongoing, 1=melon, 2=wheat/carrot),
# and site each tile with whichever available crop's tier is CLOSEST to
# that tile's distance zone (graceful degradation via absolute tier
# difference, not a hard cutoff -- if the ideal-tier crop is out of seed
# stock, the next-closest tier still wins over an arbitrary fallback).
CROP_TIER = {"STRAWBERRY": 0, "TOMATO": 0, "MELON": 1, "WHEAT": 2, "CARROT": 2}
...
def _tile_tier(tile):
    d = _dist_to_shed(tile)
    if d <= NEAR_SHED_RADIUS:
        return 0
    if d <= MID_SHED_RADIUS:
        return 1
    return 2


def plantable_crops(seeds, day, tile=None):
    """v10.5/v10.6/v10.7 siting hypothesis -- see project report for full
    history. Every unit respawns at the shed each morning (engine
    mechanic), so a tile's real season-long labor cost is (distance to
    shed) x (number of times it must be revisited). v10.7 ranks crops by a
    3-tier expected-revisit-frequency (CROP_TIER above) instead of v10.5's
    binary ongoing/one-time split, and sites each tile with whichever
    plantable crop's tier is closest to that tile's own distance zone.
    `tile=None` preserves the exact old (position-agnostic) behavior for
    call sites that don't have a specific tile in hand (e.g. the
    ...
    if tile is None or len(base) <= 1:
        return base
    target = _tile_tier(tile)
    return sorted(base, key=lambda c: abs(CROP_TIER.get(c, 2) - target))
