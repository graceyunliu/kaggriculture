...
# --- v7.11 experiment: explicit ECONOMIC cap on the hand pool ---
# The engine prices hire k of the day at _fib(k), so a day at N hands costs
# fib(N+1)-1 -- exponential in N. v7.9 asks for HAND_TARGET_MAX=18 but only
# ~7.4 land, rationed accidentally by order-slot contention. This constant
# caps the HIRE LOOP ONLY; HAND_TARGET_MAX is left at 18 so the animal
# expansion gate (_animal_expansion_feasible) is not confounded.
HIRE_HAND_CAP = 5
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if hour == 0 and day < 29:
        ...
        target = max(HAND_TARGET_MIN, min(HIRE_HAND_CAP, math.ceil(work / 5)))
