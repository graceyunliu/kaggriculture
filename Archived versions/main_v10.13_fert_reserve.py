...
FERT_SHED_RESERVE = 12      # v10.13: doubled from v9.3's 6. FERTILIZER has
                             # literally zero town demand (in no shop, not
                             # in TOWN_CENTER_PRODUCTS per project memory)
                             # so every unit sold permanently depresses a
                             # market nobody structurally buys from -- selling
                             # it is nearly worthless, while FERTILIZE gives
                             # ~+$900/strawberry-plant. Under v10.6's siting,
                             # fertilizer carriers (originating from near-shed
                             # pastures) now route to near-shed STRAWBERRY/
                             # TOMATO tiles that are themselves clustered
                             # nearby, so a bigger working stock should be
                             # cheaper to actually deploy than it was pre-v10.5.
