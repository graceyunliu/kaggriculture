...
# =====================================================================
# MAIN V11.13 -- TWO-QUADRANT INTENSIVE ECONOMY, ISOLATED on top of
# main_v10.6_radius3.py (champion). Structural capital-allocation test:
# hard-caps total land at 2 quadrants (NW starting quadrant + NE, the
# first BUY_LAND unlock) instead of the champion's normal progression up
# to 4, and reallocates the capital/labor that would have gone to SW/SE
# land into a denser, shorter-average-travel-distance economy on the
# smaller footprint.
#
# --- The change ---
# 1. Land: the SW (quads==2 -> 3) and SE (quads==3 -> 4) BUY_LAND
#    branches are removed entirely from the land-expansion block. Only
#    the quads==1 -> 2 (NE) branch remains, so `quads` never exceeds 2
#    for the rest of the match. (The $2000/$4000 in capital that would
#    have gone to SW/SE land stays in budget for animals/seeds/hiring
#    instead.)
# 2. EXPANSION_MIN_CROP_TILES_PER_QUAD: 5 -> 3. This constant scales the
#    animal-purchase crop-space reservation gate (`available_for_crops <
#    EXPANSION_MIN_CROP_TILES_PER_QUAD * quads`) -- capping quads at 2
#    already roughly halves that reserve on its own (vs. reaching quads
#    ==4), but the per-quad coefficient is tuned for a build where land
#    is comparatively abundant. Lowering it further ensures the fleet
#    can still reach EXPERIMENT_MAX_FLEET (10, unchanged) despite the
#    much smaller total map, since this build's whole bet is that a full
#    animal fleet plus denser cropping beats a bigger, sparser map.
# 3. Seed-buy ceilings raised modestly to use the smaller footprint more
#    intensively per tile (phase-0 / phase-1 pairs, both still capped by
#    `min(space, ...)` so this only raises the usable ceiling, it
#    doesn't force overplanting): want["MELON"] 6/12 -> 8/16,
#    want["STRAWBERRY"] 10/20 -> 12/24.
# Nothing else changes: animal purchase gates, hiring, and selling are
# untouched. NEAR_SHED_RADIUS (3, from the v10.6 siting mechanism) is
# also untouched -- on a 2-quadrant map every tile is inherently closer
# to the shed than on the full 4-quadrant board, so this build tests
# whether that structural closeness (not a radius retune) is itself
# valuable.
#
# --- Hypothesis ---
# Every unit respawns at the shed each morning, so real per-tile labor
# cost scales with distance to shed x revisit count (the same insight
# behind v10.5's siting mechanism, applied here at the map-footprint
# level instead of the per-tile level). A 2-quadrant map keeps the
# farthest tile roughly half as far as the full 4-quadrant board's
# farthest tile, which should cut aggregate movement time-share (~58-
# 60% of every unit-turn, the one identified-but-unclaimed structural
# lever per project memory) even after accounting for less total land.
# =====================================================================
#
# =====================================================================
...
EXPANSION_MIN_CROP_TILES_PER_QUAD = 3   # v11.13: 5 -> 3, land hard-capped at
                                          # quads==2 so this reserve must
                                          # loosen further to let the fleet
                                          # still reach EXPERIMENT_MAX_FLEET
                                          # on a much smaller total map.
                                          # crop-land reserve, scales with
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion ---
    # v11.13: hard-capped at 2 quadrants (NW + NE) -- the SW (quads==2->3)
    # and SE (quads==3->4) branches are removed entirely, so quads never
    # exceeds 2 for the rest of the match.
    if 1 <= day <= 22:
    ...
    if day <= CUTOFF["MELON"] and budget > 700:
        melon_target = min(space, 8 if ph == 0 else 16)  # v11.13: 6/12 -> 8/16, denser 2-quad footprint
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        want["STRAWBERRY"] = min(space, 12 if ph == 0 else 24)  # v11.13: 10/20 -> 12/24, denser 2-quad footprint
