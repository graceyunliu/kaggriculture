...
# =====================================================================
# MAIN V9.9 -- hire block gating hour==0 -> hour<=2, on top of v9.5.
# Base: main_v9.5_statehygiene.py.
#
# Direct mechanism finding (v9.8's postmortem left off at "the true
# bottleneck is downstream of budget entirely"). Instrumented the hire
# block itself (HIREDEBUG probe, not kept in this file) against
# Opponents/opp_scenario_v14.py, seed 1: `len(me["hands"])` is 0 EVERY
# SINGLE call at hour==0 -- confirmed against engine source
# (`_end_of_day`: `farm["hands"] = []` unconditionally every midnight).
# So the hire formula's "need" is really "target" every morning, and by
# day 13+ target pins at HAND_TARGET_MAX=18 -- the loop tries to
# request up to 18 HIRE orders, but `priced_orders` is truncated to
# MAX_MARKET_ORDERS=10 per turn (real engine default, confirmed in
# vendor/kaggle_environments_engine/kaggriculture.json), and HIRE's own
# priority score (700) already outranks every seed/land/feed order, so
# on high-workload mornings all 10 surviving slots go to HIRE and nothing
# else gets ordered that hour. Net effect: hand count is hard-capped at
# ~10/day, no matter how high the formula's target climbs or how much
# cash is available -- this, not capital, is what pinned our fleet at
# 9 animals in the v9.6/v9.7 traces. Checked Opponents/opp_scenario_v14.py
# directly for how it reaches 12 hands despite the same engine default:
# its own hire dispatch runs at `hour <= 2` (`_market_actions`), i.e. up
# to 3 separate turns, each with its own fresh 10-order allotment, to
# close the same gap ours tries to close in one shot. Isolated change:
# match that exactly, hour==0 -> hour<=2, nothing else touched. Also
# fixed a real bug this build's first draft introduced: the Fibonacci
# cost index must start from the REAL me["hires_today"] (persists across
# hours, only resets at end of day), not a fresh local 0, or hour 1/2's
# cost estimate is wildly wrong.
#
# RESULT -- TESTED, REJECTED, and it's an important negative result:
# the hire mechanism now works exactly as intended (fleet reaches the
# formula's target -- HAND_TARGET_MAX=18 hands/day by day 19, confirmed
# via HIREDEBUG instrumentation, not kept in this file) but the
# strategy is catastrophically worse: vs v9.5 (5-seed smoke, both-seats)
# -$114,708/game, t=-24.70; vs opp_scenario_v14, -$205,776/game
# (baseline -$51.7k) -- WORSE than v9.5 vs the same opponent, not
# better. Root cause: hands reset to empty every engine midnight, so
# reaching HAND_TARGET_MAX=18 costs the FULL Fibonacci ladder sum(fib(0
# ..17))=$6,764 fresh EVERY SINGLE DAY (verified: sum(fib(0..9))=$143,
# sum(fib(0..11))=$376, sum(fib(0..13))=$986, sum(fib(0..15))=$2,583,
# sum(fib(0..17))=$6,764 -- costs explode well before 18). The old
# single-hour cap (v9.5, v9.8) accidentally acted as a cost control by
# making 18 unreachable in practice (~10/day was the realized ceiling);
# removing that ceiling exposed that HAND_TARGET_MAX=18 was NEVER
# economically calibrated against the real daily-reset cost structure --
# it was tuned back when it could never actually bind. Opponents/
# opp_scenario_v14.py's own MAX_HANDS=12 lines up closely with the cheap
# end of this curve ($376/day) -- almost certainly not a coincidence.
# See v9.10: same hour<=2 infrastructure, but HAND_TARGET_MAX lowered
# to 12 to match the actually-affordable target instead of the
# previously-unreachable one. NOT submitted.
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # alongside crops so the hand pool scales with the fleet too. v9.9:
    # hour==0 -> hour<=2 -- see file header. Hands reset to empty every
    # engine midnight (_end_of_day, farm["hands"]=[]), so a single-hour
    # hire block is hard-capped at MAX_MARKET_ORDERS=10 new hires/day
    # regardless of the formula's target (up to HAND_TARGET_MAX=18) --
    # every other order category (land/seed/feed/sell) also competes for
    # that same turn's 10 slots, but HIRE's priority (700) already wins
    # ties against all of them, so on high-workload mornings ALL 10
    # slots go to HIRE and the other categories get zero that hour.
    # Recomputing "need" fresh each call means later hours in the window
    # naturally see a smaller (or zero) remaining need once earlier
    # hours' hires have landed -- this isn't 3x the hiring budget, it's
    # up to 2 more chances to close whatever gap the first hour's
    # 10-slot cap left open, so it should also relieve MORE of the
    # crowd-out on other categories, not just raise the hand ceiling.
    if hour <= 2 and day < 29:
        ...
        # v9.9: start the Fibonacci cost index from the REAL cumulative
        # hires-today count (me["hires_today"], exposed directly in the
        # observation), not a fresh local 0 -- the engine's real
        # _hire_cost(n_already_today) is keyed off the whole day's hire
        # count, and hires_today does NOT reset between hour==0 and
        # hour==1/2 (only end-of-day resets it). Assuming n=0 at hour 1/2
        # after hour 0 already hired ~9-10 hands massively underestimated
        # the true cost of continuing (fib climbs fast: fib(9)=55,
        # fib(12)=233, fib(15)=987...), so the first version of this
        # build over-requested hires the engine then actually charged
        # the real (much higher) price for, draining most of that day's
        # cash on hands we didn't need. Reading the real counter fixes
        # the estimate to match what the engine will actually charge.
