...
# =====================================================================
# MAIN V9.14 -- HAND_TARGET_MAX 12 -> 14, on top of v9.12. Base:
# main_v9.12_melon_fert.py. EXPERIMENT_MAX_FLEET untouched (stays 10,
# unlike v9.11 which raised both at once and was rejected -- this
# isolates whether more crop/animal-servicing capacity alone helps
# without also trying to grow the fleet).
#
# RESULT -- TESTED, REJECTED. 5-seed smoke, both-seats: v9.5 wins 4/5,
# +$5,197/game (paired-sum/2), t=-1.64 -- trending negative, and vs
# opp_scenario_v14, -$51,314/game -- WORSE than v9.10/v9.12's -$46.7-
# 46.9k, back down near v9.5's own -$51.7k baseline (no compression).
# So even with the fleet cap untouched, 14 hands/day is already past
# the sweet spot -- 12 (v9.10) sits closer to the actual optimum on
# this cost curve than 14, confirming the gain is a genuinely narrow
# window, not "more hands always helps a bit more, just slower." NOT
# submitted. v9.10/v9.12's HAND_TARGET_MAX=12 remains the best hand-
# capacity calibration found this session.
# =====================================================================
...
HAND_TARGET_MIN = 4
HAND_TARGET_MAX = 14  # v9.14: was 12 (v9.10). Hands reset to empty every
                       # engine midnight, so reaching a daily target costs
                       # sum(fib(0..target-1)) fresh EVERY day: 10->$143,
                       # 12->$376, 14->$986, 16->$2,583, 18->$6,764. v9.9
                       # proved 18 is reachable once hiring isn't capped to a
                       # single hour, but ruinously expensive; v9.10 landed
                       # on 12 (matches Opponents/opp_scenario_v14.py's own
                       # MAX_HANDS=12) and got a real but partial gap
                       # compression. Testing whether 14 -- still cheap
                       # ($986/day) -- buys more crop/animal-servicing
                       # throughput without re-touching EXPERIMENT_MAX_FLEET
                       # (stays at 10, unlike v9.11's conflated test).
