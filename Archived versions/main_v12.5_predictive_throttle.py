...
# =====================================================================
# MAIN V12.5 -- PREDICTIVE SHED THROTTLE, on top of v10.6 (radius 3).
# Base: main_v10.6_radius3.py, unchanged except for the additions tagged
# "v12.5" below. Round-8 hypothesis (shed-integrated combos): the prior
# round's 5 REACTIVE shed fixes (trigger selling / throttle buying once
# LIVE shed_load crosses a threshold) all washed or no-op'd, because
# shed_load measured live rarely crosses those thresholds before the
# emergency window against this project's test opponent -- the reaction
# was catching the SYMPTOM (goods already sitting in the shed) too late
# in the causal chain. This build moves the trigger one step earlier:
# forecast tomorrow's/this week's shed INFLOW from what is already
# growing/placed right now (before it lands), and throttle NEW capital
# commitments (seeds, land, animals) off of that forecast, not off the
# live shed_load number.
#
# --- Forecast formula (exact) ---
#
# For every tile currently in the PLANT state with an ONGOING crop
# (STRAWBERRY/TOMATO) that has already reached its first production age
# (age >= CROPS[crop]["first"]), the engine's own accrual rule
# (_daily_refresh_plants: yield_units += 1 or 2 every `interval` days) is
# converted into an EXPECTED units/day rate of 1.0 / interval -- i.e. the
# unfertilized (base) rate; fertilizer's 2x bonus is deliberately NOT
# assumed, so this is a conservative (under-, not over-) forecast of
# crop-side inflow. STRAWBERRY (interval=2) contributes 0.5 units/day per
# growing tile; TOMATO (interval=1) contributes 1.0 units/day per tile.
#
# For every currently-ACTIVE pasture in view["my_pastures"] (tile carries
# "animal"), the engine's own accrual rule (_daily_refresh_animals: base=1
# unit every ANIMALS[species]["interval"] days, verified against the
# engine source at kaggle_environments/envs/kaggriculture/kaggriculture.py)
# is converted the same way: COW (interval=2) -> 0.5 units/day, SHEEP
# (interval=3) -> 0.333 units/day. The occasional +1 care bonus is also
# deliberately NOT assumed, same conservative-forecast reasoning.
#
# projected_daily_inflow = sum of the above over every currently-growing
# ongoing-crop tile and every currently-active animal. This is a rate,
# not a one-shot estimate -- it is EXACTLY what continues landing in the
# shed every day this fleet/crop footprint stays as-is, before any NEW
# purchase this turn is even considered.
#
# A rough daily sell-through capacity is estimated as SELL_CAPACITY_
# PER_DAY = 18 units/day. Rationale: MAX_MARKET_ORDERS=10 order slots
# exist per turn, but (a) a variable number of those slots are claimed by
# higher cost-of-delay categories (hire/feed/seed-near-cutoff/animal) per
# the v7.11 priority sort, typically leaving roughly half free for SELL
# most turns, and (b) each SELL order this file emits normally clears the
# ENTIRE current shed quantity of one item in one order (see economy()'s
# `qty = int(n)` paths), so the binding constraint is "how many distinct
# sellable items can be cleared today" rather than a fixed per-order unit
# cap. 18 units/day is a deliberately conservative flat estimate (roughly
# one mid-value item's several-day accumulation) -- the forecast is
# designed to trigger on the case reactive throttling missed: sustained
# NET inflow (inflow that exceeds even a generous sell-through estimate)
# compounding over the days it takes new purchases to mature, not a
# transient one-day spike.
#
# net_daily_inflow = max(0, projected_daily_inflow - SELL_CAPACITY_PER_DAY)
# projected_shed_load_in_H_days = current shed_load
#                                  + net_daily_inflow * FORECAST_HORIZON_DAYS
# (FORECAST_HORIZON_DAYS = 5, matching the "next ~4-5 days" spec.)
#
# THROTTLE fires when projected_shed_load_in_H_days exceeds
# THROTTLE_TRIGGER_FRAC (0.85) of SHED_CAP -- i.e. before the live number
# would itself cross any of the existing reactive thresholds (which key
# off shed_load > SHED_CAP-15/-20/-30 directly), catching the buildup on
# its way up rather than after it arrives.
#
# --- Mechanism changed when THROTTLE fires ---
#
#   1. Seed targets: every want[crop] target computed in economy() is
#      scaled down by SEED_THROTTLE_SCALE (0.4) before the buy-quantity
#      math runs -- fewer NEW ongoing-crop plantings get started, which
#      is exactly the lever that determines next week's inflow rate.
#   2. Animal purchases: the entire BUY_ANIMAL block (COW/SHEEP gates,
#      both species) is skipped for the turn -- a new animal is a
#      multi-day-maturing NEW inflow source, the same class of commitment
#      as a new planting.
#   3. Land purchases: BUY_LAND is skipped for the turn -- more land
#      under an already-throttled footprint would only let MORE new
#      plantings start once the throttle lifts, working against the
#      point of throttling in the first place.
# Nothing else changes: crop scheduling, dispatch, selling logic, hiring,
# and NEAR_SHED_RADIUS siting are all byte-identical to v10.6.
#
# --- What could make this fail ---
#
# The forecast is a rate-based projection off currently-growing/-active
# production, deliberately conservative (no fertilizer/care bonus
# assumed) -- if SELL_CAPACITY_PER_DAY is too generous a floor, the
# throttle may rarely fire (a near-no-op, like several round-7 reactive
# variants); if the seed-scale/animal-skip response is too aggressive
# once it does fire, this could cost real revenue by under-planting
# during a stretch that the existing selling logic would have cleared
# just fine anyway. Both are informative outcomes for this round's
# question (does moving the trigger earlier in the causal chain matter).
# =====================================================================
#
# =====================================================================
...
# --- v12.5: predictive shed-inflow forecast (see file header for the
# exact formula and rationale). ---
ANIMAL_INTERVAL = {"COW": 2, "SHEEP": 3}   # verified vs engine ANIMALS dict
FORECAST_HORIZON_DAYS = 5
SELL_CAPACITY_PER_DAY = 18   # rough, deliberately conservative estimate
                              # -- see header
THROTTLE_TRIGGER_FRAC = 0.85
SEED_THROTTLE_SCALE = 0.4


def _projected_daily_inflow(me, day, view):
    """Sum of expected units/day from every currently-growing ongoing-crop
    tile (STRAWBERRY/TOMATO past their first production age) plus every
    currently-active pasture, using the engine's own per-species/per-crop
    interval as the accrual rate (base rate only -- no fertilizer/care
    bonus assumed, a conservative under-estimate). See file header."""
    inflow = 0.0
    for row in me["tiles"]:
        for t in row:
            if not (isinstance(t, dict) and t.get("kind") == "PLANT"):
...
def _shed_throttle_active(me, day, view, shed_load):
    """True when the FORECAST (not the live shed_load) shows this
    footprint's own inflow, net of a rough sell-through estimate,
    compounding past THROTTLE_TRIGGER_FRAC of SHED_CAP within
    FORECAST_HORIZON_DAYS -- catches the buildup before it arrives,
    unlike a live-shed_load reactive trigger."""
    inflow = _projected_daily_inflow(me, day, view)
    net_daily = max(0.0, inflow - SELL_CAPACITY_PER_DAY)
    projected = shed_load + net_daily * FORECAST_HORIZON_DAYS
    return projected > THROTTLE_TRIGGER_FRAC * SHED_CAP
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v12.5: predictive throttle -- computed once per turn, off the
    # FORECAST of what's already growing/placed, not the live shed_load
    # number. See file header for the exact formula.
    throttle = _shed_throttle_active(me, day, view, shed_load)
    ...
    # --- Land expansion ---
    # v12.5: skip entirely while throttled -- more land under an already-
    # overproducing footprint just lets more new plantings start once the
    # throttle lifts, working against the point of throttling.
    if 1 <= day <= 22 and not throttle:
    ...
    # v12.5: skip starting any NEW animal purchase while throttled -- a new
    # animal is a multi-day-maturing new inflow source, same class of
    # commitment as a new planting. In-flight builds already ORDERED/
    # BOUGHT/CARRYING still complete normally (not touched here).
    if n_in_progress < MAX_CONCURRENT_BUILDS and not throttle:  # v9.2: was serialized to 1
    ...
    # v12.5: throttled -- scale every seed target down. This is the lever
    # that determines next week's inflow rate, so it's the primary
    # response (land/animals above are secondary, one-shot commitments).
    if throttle:
        want = {crop: max(0, int(tgt * SEED_THROTTLE_SCALE)) for crop, tgt in want.items()}
