...
# v11.2 (Aug 7) -- REMOVE PREMIUM-CROP HOLDING PHASE, shed-capacity round.
...
# Mechanism: v10.6 skips selling PREMIUM_CROPS (STRAWBERRY/MELON, plus
# MILK/WOOL by extension of the same item set) entirely while
# timing_engine.is_holding_phase() is true and force_dump hasn't fired,
# waiting for a price spike (see `elif is_holding and item in
# PREMIUM_CROPS and not force_dump: continue` in the selling loop below).
# That deliberately lets premium goods pile up in the shed. This
# hypothesis disables that gate (dead `False and ...` branch, kept for
# diff-visibility) so premium crops fall through to the same normal
# paced-selling logic as everything else -- trading peak-price capture
# for consistently lower shed occupancy.
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        if is_attack and item in PREMIUM_CROPS:
            ...
            reason = "attack"
        elif False and is_holding and item in PREMIUM_CROPS and not force_dump:
            # v11.2: holding-phase gate REMOVED (shed-capacity round).
            # Was: `elif is_holding and item in PREMIUM_CROPS and not
            # force_dump: continue` -- this skipped selling premium crops
            # (STRAWBERRY/MELON/MILK/WOOL) entirely while
            # timing_engine.is_holding_phase() was true, waiting for a
            # price spike. Kept as a dead (`False and ...`) branch for
            # diff-visibility rather than deleted outright; the branch can
            # never execute, so premium crops now fall through to the
            # exact same normal paced-selling logic as everything else
            # (price-floor + shed-pressure "relax" term below), trading
            # peak-price capture for consistently lower shed occupancy.
            continue
