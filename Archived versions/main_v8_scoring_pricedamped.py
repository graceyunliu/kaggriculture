...
# =====================================================================
# MAIN V8-SCORING-PRICEDAMPED -- v8.9's marginal-return COW/SHEEP scorer,
# PLUS price-elasticity dampening, ISOLATED. Base: main_v8.3.py (the
# validated champion). Ported v8.9's scoring mechanism verbatim; see
# graveyard for v8.9's own standalone result.
#
# NAMING NOTE: this build was first saved as main_v8.11.py, but a
# parallel session working on this SAME project folder independently
# used the identical "next available number" convention for a totally
# different experiment (a 10-animal capacity bundle) and overwrote it
# on disk within the same second this file was originally written
# (confirmed via `stat` timestamps -- both writes landed at
# 01:30:10 local time). This is the second such collision tonight (the
# first clobbered a six-piece main_v8.10.py from that same parallel
# session). Saved under a non-numeric name this time specifically to
# stop colliding with whichever session reaches a given number next --
# Grace, if you're running two sessions on this project concurrently,
# it's worth telling each one a distinct naming prefix.
#
# --- Why (Grace's request after v8.9's close-but-rejected result and
#     v8.10's decisive rejection of the alternative gate-tuning recipe) ---
#
# v8.9 (marginal $/tile/day scoring for COW vs SHEEP, gates untouched)
# lost a real but small -$7,060/game to v8.3 (3/15 wins, 5 exact ties,
# t=-3.22) -- an order of magnitude smaller than v8.8's or v8.10's
# rejections, and one seed (303) showed the mechanism working exactly as
# intended: +1 SHEEP, same total animals, same crop tiles, a genuine
# win. Diagnosed cause of the net loss: the score used
# `live_price(obs, product)` at face value, an instantaneous snapshot,
# for a purchase decision that has to be lived with for many remaining
# days -- and WOOL's market is thinner than MILK's (MARKET_PARAMS:
# WOOL T=105 vs MILK T=122, and `market_price()`'s own amplitude formula
# means a SMALLER T produces BIGGER price swings per unit of inventory
# deviation, confirmed by reading the formula, not assumed), so a
# transient WOOL spike could tip the score toward SHEEP for a
# commitment that then didn't pay off over the rest of the match. Grace
# asked for exactly this follow-up: keep v8.9's scoring mechanism
# untouched, gate constants untouched (no repeat of v8.10's failed
# recipe), and only fix the diagnosed price-trust weakness.
#
# --- The change vs v8.9 ---
#
# One new function, `_dampened_price()`, replaces every `live_price()`
# call inside `_animal_marginal_score()` (both the product price and the
# WHEAT feed cost). It blends the live price toward the item's own
# MARKET_PARAMS `base` price, weighted by how much this codebase should
# trust a live reading of that specific item: deeper markets (bigger T)
# are trusted close to 100%, thinner ones are pulled harder toward their
# stable base. WHEAT's T=400 is far above the reference scale used
# below, so `min(1.0, ...)` clamps its trust to 1.0 -- the feed-cost
# term is therefore UNCHANGED from v8.9 in practice; only the MILK/WOOL
# product-price term is actually dampened, and WOOL (T=105) more than
# MILK (T=122), which is exactly the asymmetry the diagnosis called for.
# Everything else -- ANIMAL_ECON, _remaining_ticks(), _observed_care_rate(),
# the both-gates-then-score selection logic, both feasibility gates,
# every crop/task-order system -- is byte-identical to v8.9/v8.3.
#
# --- RESULT (15-seed --both-seats vs main_v8.3.py, standard seed set) ---
#
# 8/15 wins, 7/15 losses, 0 ties. Mean margin -$240/game, min -$10,548,
# max +$9,394, t=-0.13, 95% CI [-$3,771,+$3,290] -- a statistical WASH,
# not a win and not a loss (CI straddles zero). This closed essentially
# the entire gap from v8.9's real -$7,060/game loss (t=-3.22). Smoke
# matrix clean, 3/3, and absolute score vs starter/pass (~$85-95k) was
# back in v8.3's healthy range (v8.10's broken recipe scored only ~$60k
# on the same matchup, a useful early red flag that build direction had
# gone wrong before the paired test even confirmed it).
#
# Fleet diagnostic (vs starter, spot checks only, not the paired-seed
# states): seed 303 ends with 7 cow + 3 sheep (10 total, 31 crop tiles)
# vs v8.3's 6 cow + 2 sheep (8 total, 36 crop tiles); seed 1 ends 7+3=10/
# 30 vs v8.3's 6+1=7/37. Reward was higher for this build in both spot
# checks (100,560 vs 94,310; 85,057 vs 83,622). This build trends toward
# a bigger animal fleet at a small crop-tile cost, not a clean "same
# crop tiles, more sheep" swap the way v8.9's one clean win (seed 303)
# showed -- but net positive in these particular checks.
#
# --- Verdict ---
#
# NOT PROMOTABLE as a champion replacement (this project's bar is a
# decisive win, e.g. v8.2's +$4,482/game at t=+4.86, all-positive 95%
# CI) -- but also not a rejection. This is the strongest, most
# defensible result in the whole v8.8/v8.9/v8.10/this-build animal-
# composition experiment thread: real engine constants, a live-observed
# care signal, and now price dampening grounded in the market's own real
# elasticity parameter, landing at a coin-flip outcome against a
# champion that's already been through several rounds of validated
# tuning. If pushed further: widen the seed set for more statistical
# power (CI is narrow but still straddles zero at n=15), or revisit the
# `hand_pressure`/`_observed_care_rate` proxies now that price is no
# longer the dominant noise source.
#
...
# real per-species facts, from vendor/kaggle_environments_engine/
# kaggriculture.py's ANIMALS dict -- ground truth, not estimates. Costs
# stay in ANIMAL_SPECS above (unchanged); this only adds the production
# cadence needed to score a purchase.
ANIMAL_ECON = {
    "COW":   {"first_yield_day": 8, "interval": 2, "product": "MILK"},
    "SHEEP": {"first_yield_day": 6, "interval": 3, "product": "WOOL"},
}
MATCH_END_DAY = 29   # 720 episodeSteps / 24 turnsPerDay - 1, confirmed by
                       # smoke-test final_day across every version in this
                       # project's history.

# reference market-depth scale for price dampening. Picked from an
# existing MARKET_PARAMS T value already in this table (TOMATO/
# FERTILIZER's T=200) rather than an invented number -- both MILK
# (T=122) and WOOL (T=105) sit below it, so both get pulled toward
# their base price to some degree, WOOL more than MILK, while WHEAT
# (T=400) clamps to full trust (min(1.0, ...) below) and is effectively
# untouched.
PRICE_TRUST_T_REF = 200.0


def _dampened_price(obs, item):
    """Don't trust an instantaneous live price at face value for a
    decision (which animal to buy) that has to be lived with for many
    remaining days. Blends live_price() toward the item's own
    MARKET_PARAMS base price, weighted by that item's real elasticity
    constant T -- a thinner market (smaller T means BIGGER price swings
    per unit of inventory deviation, confirmed by reading
    market_price()'s amplitude formula: amp = bt/at * base /
    _shape(bf/af, T), so smaller T inflates amp) gets pulled harder
    toward its stable base, since its live reading is a noisier signal
    of what price will actually hold over this animal's productive
    lifetime. This directly targets the diagnosed weakness in a prior
    version: a transient WOOL spike tipping the SHEEP score for a
    purchase that then didn't pay off."""
    base = MARKET_PARAMS[item]["base"]
    T = MARKET_PARAMS[item]["T"]
    trust = min(1.0, T / PRICE_TRUST_T_REF)
    return trust * live_price(obs, item) + (1 - trust) * base


def _remaining_ticks(species, day):
    """How many production ticks a `species` bought TODAY could still fire
    before the match ends. Real engine cadence (_produce_animals): first
    yield at day + first_yield_day, then every `interval` days after
    that."""
    econ = ANIMAL_ECON[species]
    first = day + econ["first_yield_day"]
    if first > MATCH_END_DAY:
        return 0
    return (MATCH_END_DAY - first) // econ["interval"] + 1


def _observed_care_rate(view):
    """Live proxy for 'expected care completion' -- the fraction of the
    CURRENT fleet that is both fed and cared for today, right now. This is
    a real, live signal (not a forecast): this project's testing
    discipline treats forward animal-care simulation as unreliable (see
    main_v8.1.py's rejected FERTILIZE build, which assumed an idealized
    execution pattern that didn't hold up against the real dispatch
    loop). With no pastures yet to observe, falls back to a neutral 0.6
    rather than assuming either extreme."""
    pastures = view["my_pastures"]
    if not pastures:
        return 0.6
    n_cared = sum(1 for _pos, t in pastures if t.get("cared_today") and t.get("fed_today"))
    return n_cared / len(pastures)


def _animal_marginal_score(species, obs, view, day, n_hands, n_animals):
    """Expected $/tile/day return of buying `species` right now -- the
    shared yardstick _agent() uses to let COW and SHEEP compete on
    marginal return instead of COW always winning by being checked
    first.

      1. remaining production ticks -- _remaining_ticks(), real engine
         cadence (first_yield_day + interval from ANIMAL_ECON).
      2. market price -- _dampened_price(obs, product).
      3. expected care completion -- _observed_care_rate(view), this
         farm's OWN currently-observed fed+cared rate across its real
         fleet, discounted by...
      4. maintenance burden -- `hand_pressure`: how much thinner one more
         animal would stretch the existing hand pool (n_hands vs
         n_animals+1), expressed as a discount on expected care
         completion rather than an invented flat dollar penalty (feed/
         care visit cost is otherwise IDENTICAL between species -- both
         need exactly one FEED + one CARE visit per day, confirmed by
         reading the engine's FEED handler, which takes 1 WHEAT
         regardless of species).
      5. feed cost -- remaining_days * _dampened_price(obs, "WHEAT")
         (in practice WHEAT's T=400 clamps trust to 1.0, so this term is
         effectively the raw live price).
      6. pasture tile value -- the function's return value itself: net
         expected value divided by the animal's own remaining productive
         lifetime, i.e. $/tile/day, directly comparable between species.
    """
    econ = ANIMAL_ECON[species]
    ticks = _remaining_ticks(species, day)
    if ticks <= 0:
        return float("-inf")

    remaining_days = max(1, MATCH_END_DAY - day - econ["first_yield_day"] + econ["interval"])

    care_rate_now = _observed_care_rate(view)
    hand_pressure = min(1.0, n_hands / max(1, n_animals + 1))
    expected_care_completion = care_rate_now * hand_pressure

    price = _dampened_price(obs, econ["product"])
    expected_bonus = econ["interval"] * expected_care_completion
    units_per_tick = min(6, 1 + expected_bonus)   # max_held=6, real engine cap
    gross = ticks * units_per_tick * price

    feed_cost = remaining_days * _dampened_price(obs, "WHEAT")   # 1 WHEAT/day/animal,
                                                                   # identical both species

    return (gross - feed_cost) / remaining_days
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if not in_progress:
        ...
        if target_plan is None:
            # Both species' own gates are checked, UNCHANGED from v8.3.
            # What changes is what happens when both pass: instead of
            # COW winning automatically by being checked first,
            # _animal_marginal_score() (with dampened prices) picks
            # whichever species has the better expected $/tile/day
            # return right now. If only one gate passes, that species is
            # bought (same as v8.3). If neither passes, no purchase
            # (same as v8.3).
            cow_ok = _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads)
            sheep_ok = SHEEP_ENABLED and _sheep_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads)
            species = None
            if cow_ok and sheep_ok:
                cow_score = _animal_marginal_score("COW", obs, view, day, len(me["hands"]), n_animals)
                sheep_score = _animal_marginal_score("SHEEP", obs, view, day, len(me["hands"]), n_animals)
                species = "SHEEP" if sheep_score > cow_score else "COW"
            elif cow_ok:
                species = "COW"
            elif sheep_ok:
                species = "SHEEP"
