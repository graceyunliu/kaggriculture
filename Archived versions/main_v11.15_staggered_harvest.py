...
# =====================================================================
# V11.15 -- STAGGERED HARVEST TIMING on top of v10.6 (round-7 selling-
# rhythm theme). Aggregate replay data flagged SHED_AT_CAP as the
# dominant loss cause (31/100 losses). Hypothesis: ongoing crops
# (STRAWBERRY/TOMATO) planted across all currently-unlocked quadrants at
# once means their production events -- and therefore shed arrivals --
# tend to phase-align, producing periodic shed-fill spikes rather than a
# smooth trickle. This variant staggers the START of ongoing-crop
# planting per quadrant: a quadrant may not receive new STRAWBERRY/
# TOMATO plantings until STAGGER_DAYS days after that quadrant was
# unlocked (one-time crops WHEAT/CARROT/MELON are unaffected and may
# still be planted immediately). NW (the starting quadrant) is treated
# as unlocked on day 1, so its ongoing-crop behavior is unchanged from
# v10.6; only later-unlocked quadrants (NE/SW/SE, per LAND_ORDER) get
# deliberately offset harvest cycles. Mechanism: plantable_crops() now
# also takes the tile's quadrant unlock day into account and filters
# ONGOING_CROPS out of the candidate list (falling back to one-time
# crops) during that quadrant's stagger window; a small module-level
# dict records each quadrant's first-seen-unlocked day, updated once per
# turn in economy().
# =====================================================================
...
# v11.15: staggered harvest timing. STAGGER_DAYS = days after a
# quadrant's first-seen-unlocked day during which ongoing-crop
# (STRAWBERRY/TOMATO) planting is suppressed in that quadrant, forcing a
# deliberate offset between quadrants' harvest cycles instead of letting
# them all phase-align. _quad_unlock_day is populated once per turn in
# economy() (NW defaults to day 1, the starting quadrant).
STAGGER_DAYS = 3
_quad_unlock_day = {"NW": 1}


def _quadrant_of(pos):
    """Mirrors the engine's _quadrant_of (kaggriculture.py): board is
    10x10, quadrant = N/S (y<5 / y>=5) + W/E (x<5 / x>=5)."""
    x, y = pos
    half = 5
...
def plantable_crops(seeds, day, tile=None):
    ...
    if tile is None or len(base) <= 1:
        return base
    # v11.15: suppress ongoing-crop planting in a quadrant during its
    # stagger window (falls back to one-time crops if any remain
    # eligible; if none do, ongoing crops stay available rather than
    # forcing an idle turn).
    quad = _quadrant_of(tile)
    unlock_day = _quad_unlock_day.get(quad, 1)
    if day - unlock_day < STAGGER_DAYS:
        staggered = [c for c in base if c not in ONGOING_CROPS]
        if staggered:
            base = staggered
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v11.15: record each quadrant's first-seen-unlocked day, once.
    for _q in me["unlocked_quadrants"]:
        if _q not in _quad_unlock_day:
            _quad_unlock_day[_q] = day
