...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        # v10.12 hypothesis: v10.4/v10.10 both showed that ADDING more
        # ongoing-crop plants loses (labor ceiling: movement is still
        # ~58% of unit-turns even under v10.6's better-targeted siting --
        # siting improved OUTPUT per unit of existing labor, it didn't
        # free up spare labor capacity). This tests a REALLOCATION
        # instead of an addition: shift a few units of the SAME total
        # ongoing-crop target from STRAWBERRY to TOMATO (13 total at
        # ph==0 either way: 10+3 -> 8+5), so total planted-tile count and
        # total labor demand stay flat, only the STRAWBERRY:TOMATO ratio
        # changes. TOMATO's market absorbs volume much more gently
        # (above_target 0.60 vs STRAWBERRY's 1.60) and is half the seed
        # cost, so worth testing whether the current 10:3 split is
        # actually revenue-optimal or just an inherited default.
        want["STRAWBERRY"] = min(space, 8 if ph == 0 else 17)
    if day <= CUTOFF["TOMATO"] and budget > 600:
        want["TOMATO"] = 5   # v10.12: reallocated from STRAWBERRY, see above
