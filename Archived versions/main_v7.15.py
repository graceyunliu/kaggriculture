...
# =====================================================================
# MAIN V7.15 -- priority-preemptible per-unit target locking, ISOLATED.
# Base: v7.11. Round 2 of the labor-efficiency investigation; round 1
# (main_v7.13.py, kept in the folder as a documented rejected experiment)
# tried UNCONDITIONAL locking and lost cleanly to v7.11 (5/20 seeds,
# mean margin -$13,492, t=-3.42, --both-seats, 20 seeds, 720 turns).
#
# --- Round 1 recap: the bug and why the naive fix lost ---
#
# Instrumented v7.11: of 3,333 movement calls in a 720-turn match, only
# 26.6% continued toward the same target as the prior turn; 27.5% of all
# moves were RETARGETS -- a unit mid-walk toward tile A got pulled onto
# a different tile B before ever reaching A. Root cause: `tasks` is
# rebuilt from raw perception every turn with zero memory of what any
# unit was walking toward, so every idle unit independently recomputes
# "nearest task of the highest-priority pending category from my current
# position" each turn.
#
# v7.13 fixed this with UNCONDITIONAL locking: once a unit picks a
# target it keeps walking toward it every turn, ignoring the fresh
# recompute, until arrival or the lock goes stale. This worked as
# designed (retarget rate 32.2% -> 12.9%, continuation ~24% -> 48%) but
# LOST vs v7.11 (5/20, -$13,492 mean). Diagnosis: unconditional locking
# removes priority-preemption. The unlocked baseline re-evaluates the
# highest-priority pending category fresh every turn, so a newly-appeared
# urgent_water tile immediately pulls in whichever unit is closest --
# even one mid-walk toward something lower-priority. Once locked, a unit
# ignores task_order entirely until its current (possibly low-priority)
# walk resolves, so urgent work goes unaddressed while units finish
# already-committed low-priority walks. The walking-efficiency gain was
# real but smaller than the cost of lost responsiveness.
#
# --- v7.15's fix: priority-preemptible locking ---
#
# Same unit_targets locking mechanism as v7.13 (module-level dict, keyed
# by stable unit id "farmer"/"handN", mutated in place). The difference
# is a preemption check inserted before the "keep walking the lock"
# continuation: each turn, for a locked unit, compute the lock's tier
# index in task_order (lower index = higher priority). If ANY task in a
# STRICTLY higher-priority tier is currently pending in `tasks` and not
# already covered by another unit's lock (i.e. not in this turn's
# locked_tiles snapshot), drop this unit's lock and fall through to the
# normal fresh-recompute path (same as the v7.11 baseline, and same as
# v7.13's own fresh-recompute branch, which already excludes
# locked_tiles so it can't collide with another unit's in-progress walk).
# If nothing strictly higher is pending, keep walking the existing lock
# and skip the recompute entirely -- this is what eliminates the
# same-or-lower-priority churn that made up the bulk of v7.11's 27.5%
# retargets, without touching the responsiveness path for genuinely
# urgent new work.
#
# Note: feed-critical animal work (fed_today / consecutive_unfed) is
# handled entirely OUTSIDE unit_action, via animal_maintenance_action
# called before unit_action every turn for every unit regardless of any
# crop-task lock -- so it was never actually at risk from v7.13's crop
# locking. The preemption gate implemented here is about crop task_order
# tiers only (urgent_water/harvest/water/plant/weeds), which is where the
# real risk of "locked into low-priority, urgent crop work waits" lived.
#
# Deliberately NOT touched: task_order/priority logic itself, weed
# thresholds, how many hands exist, where PLANT targets are chosen from
# view["empty"] -- this is scheduling-only, one mechanism, to keep the
# A/B attributable.
# =====================================================================
#
# =====================================================================
...
# v7.13/v7.15: per-unit committed target, keyed by a stable unit id
# ("farmer", "hand0", "hand1", ...). See header comment for the traced
# mechanism this closes. Mutated in place like animal_plans/reserved_sites.
unit_targets = {}


def unit_action(pos, tasks, seeds, day, task_order, unit_id=None, locked_tiles=frozenset()):
    x, y = pos

    # v7.15: if this unit already has a live commitment from a previous
    # turn, keep walking it instead of recomputing "nearest" from scratch
    # -- UNLESS a strictly higher-priority task (per task_order) is
    # pending and not already claimed by another unit's lock, in which
    # case drop the lock and fall through to the normal recompute so the
    # urgent work can pull in whichever unit gets there first (same as
    # the unlocked v7.11 baseline). Same-or-lower-priority pending work
    # never preempts -- that's the churn this version eliminates.
    if unit_id is not None:
        lock = unit_targets.get(unit_id)
        if lock is not None:
            lkey, ltile = lock
            if ltile in tasks.get(lkey, ()):
                preempt = False
                lock_tier = task_order.index(lkey)
                for higher_key in task_order[:lock_tier]:
                    if any(t not in locked_tiles for t in tasks.get(higher_key, ())):
                        preempt = True
                        break
                if not preempt:
                    if (x, y) != ltile:
                        step = _step_toward((x, y), ltile)
                        if step:
                            tasks[lkey].remove(ltile)
                            return [step]
                    # else: arrived -- fall through to on-tile handling
                    # below, which will find (x, y) in tasks[lkey] and
                    # clear the lock.
                else:
                    unit_targets.pop(unit_id, None)  # preempted
            else:
                unit_targets.pop(unit_id, None)  # stale: resolved/vanished

    for key in task_order:
        if (x, y) in tasks[key]:
            tasks[key].remove((x, y))
            if unit_id is not None:
                unit_targets.pop(unit_id, None)
            if key == "plant":
    ...
    for key in task_order:
        candidates = [t for t in tasks[key] if t not in locked_tiles]
        tgt = _nearest((x, y), candidates)
        if tgt:
            step = _step_toward((x, y), tgt)
            if step:
                tasks[key].remove(tgt)
                if unit_id is not None:
                    unit_targets[unit_id] = (key, tgt)
                return [step]
...
def _agent(obs):
    ...
    # v7.13: prune stale target locks (task already resolved/vanished since
    # last turn -- e.g. a different unit reached it) against THIS turn's
    # fresh tasks, then snapshot the still-valid locked tiles once so no
    # other unit's fresh pick this turn can target a tile someone else is
    # already committed to and mid-walk toward.
    for _uid in list(unit_targets.keys()):
        _lkey, _ltile = unit_targets[_uid]
        if _ltile not in tasks.get(_lkey, ()):
            del unit_targets[_uid]
    locked_tiles = {t for _k, t in unit_targets.values()}
    ...
    if farmer_op is None:
        farmer_op = unit_action(farmer_pos, tasks, seeds, day, task_order,
                                 unit_id="farmer", locked_tiles=locked_tiles)
    ...
    for i, h in enumerate(me["hands"]):
        ...
        if op is not None:
            ...
            hand_ops.append(unit_action(hand_pos, tasks, seeds, day, task_order,
                                         unit_id=f"hand{i}", locked_tiles=locked_tiles))
