...
# =====================================================================
# MAIN V12.7 -- SELF-SUFFICIENT PER-QUADRANT MINI-ECONOMY, on top of
# v10.6 (radius 3). Base: main_v10.6_radius3.py, unchanged except for the
# additions tagged "v12.7" below. Round-8 hypothesis (shed-integrated
# combos): rather than one GLOBAL predictive throttle (v12.5) gating
# ALL new commitments off ONE forecast, this build decentralizes -- each
# currently-unlocked quadrant gets its own fixed crew (farmer/hands
# pinned by ROSTER SLOT, same pinning technique the existing animal-build
# pipeline already uses -- see ANIMAL PIPELINE header), whose crop task
# dispatch is filtered to ONLY that quadrant's own tiles, and whose
# PLANTING decisions are throttled independently off a forecast of that
# quadrant's OWN production versus an EVEN SHARE of shed capacity
# (SHED_CAP / number of unlocked quadrants). Tests whether a genuinely
# decentralized, self-regulating economy (each zone balancing its own
# production against its own capacity share) beats a single global
# throttle or a single global joint-nearest-pair dispatch.
#
# --- Crew partition (v12.7 Part A) ---
#
# Board is BOARD x BOARD (10x10); each quadrant is a fixed 5x5 region
# (TILES_PER_QUAD=25), identified the same way the engine itself does
# (_quadrant_of: N/S by y<half, W/E by x<half) -- rebuilt here as
# _tile_quadrant(), self-contained.
#
# Worker roster SLOT k (0=farmer, k>=1=hands[k-1]) is assigned to
# unlocked_quadrants[k % n_unlocked] every turn -- a fixed, deterministic
# round-robin by POSITION, not by persistent hand identity (hands reset
# nightly in this engine; the existing animal-build pipeline already pins
# in-flight builds to slots on exactly this basis, for exactly this
# reason -- see the ANIMAL PIPELINE section header inherited from v9.2).
# This produces a STABLE crew-to-quadrant mapping turn over turn (each
# slot always serves the same quadrant) without needing any persistent
# per-hand bookkeeping across days.
#
# --- Filtered dispatch (v12.7 Part B) ---
#
# The existing global task lists (urgent_water/water/harvest/weeds/plant,
# built once per turn from live perception exactly as in v10.6 -- no
# change to perceive() itself) are partitioned per quadrant by tile
# position. assign_tasks() -- UNCHANGED, still the same joint-nearest-
# pair matcher -- is then called ONCE PER QUADRANT, with only that
# quadrant's own crew and that quadrant's own task subset, instead of one
# global call across the whole roster and the whole board. A unit can
# never be assigned to work outside its own quadrant.
#
# --- Per-quadrant predictive throttle (v12.7 Part C) ---
#
# Same forecast MATH as main_v12.5_predictive_throttle.py (rebuilt self-
# contained here, not assumed to exist in any other file), applied per
# quadrant instead of globally:
#   quad_inflow = sum of expected units/day (1.0/interval, base rate, no
#                 fertilizer/care bonus -- conservative) from THAT
#                 quadrant's own currently-growing ongoing-crop tiles
#                 (STRAWBERRY/TOMATO past first production age) plus any
#                 currently-active pasture SITED within that quadrant
#                 (pastures are sited near the shed by _grow_reserved_
#                 sites, which can land in any of the 4 quadrants since
#                 the 4 CENTER_TILES straddle all of them -- confirmed by
#                 inspection of _quadrant_of's half-board split).
#   quad_budget = SHED_CAP / n_unlocked_quadrants   (an even ACCOUNTING
#                 share of the one real, physically shared shed -- not a
#                 physical partition; the shed itself is not split, this
#                 is each crew's own self-imposed capacity budget)
#   quad_share_of_load = current shed_load / n_unlocked_quadrants  (same
#                 even-share assumption applied to what's already there)
#   quad_sell_share = SELL_CAPACITY_PER_DAY / n_unlocked_quadrants  (same
#                 even split of the global rough sell-through estimate)
#   net_daily = max(0, quad_inflow - quad_sell_share)
#   projected = quad_share_of_load + net_daily * FORECAST_HORIZON_DAYS
#   throttled = projected > THROTTLE_TRIGGER_FRAC * quad_budget
# When a quadrant is throttled, its crew's "plant" task list for THIS
# TURN is emptied before dispatch (Part B) -- that quadrant's crew still
# does water/harvest/weed duty on its own tiles, just doesn't start new
# plantings there this turn. Other, non-throttled quadrants are
# unaffected -- this is the genuinely decentralized part: one zone's
# overproduction doesn't stop another zone's crew from planting.
#
# --- Deliberately OUT of scope (documented, not silently dropped) ---
#
# Animal purchases/siting are LEFT GLOBAL, unchanged from v10.6 -- not
# partitioned per quadrant. Two reasons: (1) pastures site near the
# shared shed via the existing reserved-sites mechanism, which can land
# in any quadrant depending on availability -- there is no clean, stable
# "home quadrant" for a NEW animal purchase the way there is for a crop
# tile that already sits in exactly one quadrant; (2) this project's own
# history (see project memory, round 7) already tested a full animal-
# purchase architecture rewrite and it lost decisively (-$16.8k to
# -$27.8k/game) even running crash-free -- reworking that pipeline again
# here, under a genuinely different structural change (quadrant crews),
# would confound this test's result and risk repeating that failure mode
# for reasons unrelated to the question this build actually asks (does
# decentralizing CROP dispatch/throttling help). Seed-buying quantities
# (the market BUY_SEED orders in economy()) also stay global/unchanged --
# seeds are a shared shed resource in this engine, not sited anywhere
# until planted, so there is no natural per-quadrant seed budget either;
# decentralization here operates at the DISPATCH layer (which tiles get
# worked, and whether NEW planting starts on them), not the purchase
# layer, for exactly the crop-vs-animal distinction just described.
#
# --- What could make this fail ---
#
# This trades one accurate global forecast (v12.5) for four smaller,
# noisier per-quadrant forecasts built on the SAME even-split assumption
# for shed_load and sell-through capacity that may not reflect reality
# (a quadrant with more ongoing-crop tiles legitimately deserves a bigger
# share of both, which this build does not model). It could also fragment
# labor unhelpfully if one quadrant's crew sits idle (no tasks left in
# its own zone) while another quadrant's crew is overloaded, since
# assign_tasks() is no longer optimizing across the WHOLE roster -- the
# global joint-nearest-pair dispatch v10.6 already uses could out-perform
# four smaller, non-cooperating pools for this reason alone, independent
# of whether the per-quadrant throttle itself helps.
# =====================================================================
#
# =====================================================================
...
# =====================================================================
# v12.7: per-quadrant crew partition + per-quadrant predictive throttle.
# See file header for full rationale/formula.
# =====================================================================
ANIMAL_INTERVAL = {"COW": 2, "SHEEP": 3}   # verified vs engine ANIMALS dict
FORECAST_HORIZON_DAYS = 5
SELL_CAPACITY_PER_DAY = 18
THROTTLE_TRIGGER_FRAC = 0.85


def _tile_quadrant(pos):
    """Mirrors the engine's own _quadrant_of (half-board split), rebuilt
    self-contained here -- not imported from the engine."""
    x, y = pos
    half = BOARD // 2
    return ("N" if y < half else "S") + ("W" if x < half else "E")


def _crew_quadrant_map(n_hands, unlocked_quadrants):
    """Fixed round-robin: roster SLOT k (0=farmer, k>=1=hands[k-1]) is
    assigned to unlocked_quadrants[k % n_unlocked] every turn -- a
    deterministic mapping by POSITION, not persistent hand identity (see
    file header). Returns a list indexed by slot."""
    n_q = max(1, len(unlocked_quadrants))
    n_slots = 1 + n_hands
    return [unlocked_quadrants[i % n_q] for i in range(n_slots)]


def _partition_tasks_by_quadrant(tasks, quadrant):
    """Copy of `tasks`, filtered to only the tiles inside `quadrant`."""
    return {key: [t for t in lst if _tile_quadrant(t) == quadrant]
            for key, lst in tasks.items()}


def _quad_ongoing_crop_inflow(me, day, quadrant):
    inflow = 0.0
    for y, row in enumerate(me["tiles"]):
        for x, t in enumerate(row):
            if _tile_quadrant((x, y)) != quadrant:
                continue
            if not (isinstance(t, dict) and t.get("kind") == "PLANT"):
                continue
            c = CROPS.get(t.get("crop"))
            ...
            age = day - t.get("planted_day", day)
            if age >= c["first"]:
                inflow += 1.0 / max(1, c["interval"])
    return inflow


def _quad_animal_inflow(view, quadrant):
    inflow = 0.0
    for pos, t in view["my_pastures"]:
        if _tile_quadrant(pos) == quadrant:
            iv = ANIMAL_INTERVAL.get(t.get("animal"))
            if iv:
                inflow += 1.0 / iv
    return inflow


def _quad_throttled(me, day, view, quadrant, n_quads, shed_load):
    """v12.7 Part C: per-quadrant forecast vs an even ACCOUNTING share of
    the one real shed's capacity/load/sell-through. See file header."""
    inflow = _quad_ongoing_crop_inflow(me, day, quadrant) + _quad_animal_inflow(view, quadrant)
    quad_budget = SHED_CAP / max(1, n_quads)
    quad_share_of_load = shed_load / max(1, n_quads)
    quad_sell_share = SELL_CAPACITY_PER_DAY / max(1, n_quads)
    net_daily = max(0.0, inflow - quad_sell_share)
    projected = quad_share_of_load + net_daily * FORECAST_HORIZON_DAYS
    return projected > THROTTLE_TRIGGER_FRAC * quad_budget
...
def _agent(obs):
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        # v12.7: decentralized per-quadrant dispatch, replacing v10.6's
        # single global assign_tasks() call. See file header Parts A-C.
        quads_list = me["unlocked_quadrants"]
        n_quads = max(1, len(quads_list))
        crew_map = _crew_quadrant_map(len(me["hands"]), quads_list)

        # Part C: per-quadrant throttle -- drop "plant" tasks in any
        # quadrant whose OWN forecast trips its OWN capacity share, before
        # partitioning/dispatch. Other quadrants are unaffected.
        shed_load_now = sum(n for n in shed.values() if isinstance(n, (int, float)) and n > 0)
        throttled_quads = {q for q in quads_list
                            if _quad_throttled(me, day, view, q, n_quads, shed_load_now)}
        if throttled_quads:
            tasks["plant"] = [t for t in tasks["plant"] if _tile_quadrant(t) not in throttled_quads]

        # Part A+B: group this turn's crop-duty units by their crew's
        # fixed quadrant, then run assign_tasks() ONCE PER QUADRANT with
        # only that quadrant's own crew and own (filtered) task subset.
        groups = {}  # quadrant -> list of (kind, idx_or_None, pos)
        if farmer_needs_crop:
            q0 = crew_map[0]
            groups.setdefault(q0, []).append(("farmer", None, farmer_pos))
        for (i, pos) in hand_needs_crop:
            slot = i + 1
            q = crew_map[slot] if slot < len(crew_map) else quads_list[0]
            groups.setdefault(q, []).append(("hand", i, pos))

        for q, members in groups.items():
            q_tasks = _partition_tasks_by_quadrant(tasks, q)
            q_tasks_before = {k: list(v) for k, v in q_tasks.items()}
            positions = [m[2] for m in members]
            crop_ops = assign_tasks(positions, q_tasks, seeds, day, task_order)
            # Reconcile claimed tiles back into the global tasks dict.
            for key, before_list in q_tasks_before.items():
                after_list = q_tasks.get(key, [])
                claimed = set(before_list) - set(after_list)
                if claimed:
                    tasks[key] = [t for t in tasks[key] if t not in claimed]
            for (kind, idx, _pos), op in zip(members, crop_ops):
                if kind == "farmer":
                    farmer_op = op
                else:
                    hand_ops[idx] = op
