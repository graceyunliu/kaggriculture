...
# =====================================================================
# MAIN V11.7 -- QUADRANT-GATED SEQUENTIAL EXPANSION, isolated test on top
# of v10.6_radius3 (champion). Base: main_v10.6_radius3.py, unchanged
# except for the land-expansion gate in economy().
#
# Trigger: Grace's screenshot of a real opponent's board -- that opponent
# appears to fully complete one quadrant (crops + animals) before
# expanding into the next, rather than buying land on a pure
# budget/day schedule while the current quadrant is still half-empty.
# v10.6's land-expansion block (`if quads==1 and budget>1000: BUY_LAND`,
# etc.) fires purely on cash + day thresholds -- it has no notion of
# whether the quadrant it already owns is actually being used.
#
# Mechanism: each BUY_LAND branch keeps its existing budget/day gate
# UNCHANGED, but ANDs in a new completeness check on the CURRENT highest-
# unlocked quadrant (me["unlocked_quadrants"][-1]):
#   _quadrant_complete(me, view, qname) is True only when
#     (a) >=70% of that quadrant's 25 tiles are non-empty (planted,
#         weeded, pasture, or coop -- anything but a bare empty tile),
#         AND
#     (b) >=2 animals (my_pastures) are sited within that quadrant.
# If not yet complete, the next quadrant's BUY_LAND is simply skipped
# this turn (tried again next turn) -- budget that would have gone to
# land stays free for other priced_orders instead. This tests "fully
# develop one area before expanding" as a genuinely different pacing
# model, not a threshold tweak on the existing gates.
# =====================================================================
...
# v11.7: quadrant-gated sequential expansion helpers (see file header).
def _quadrant_of_v117(tile):
    x, y = tile
    return ("N" if y < 5 else "S") + ("W" if x < 5 else "E")


def _quadrant_tile_coords_v117(qname):
    xs = range(0, 5) if "W" in qname else range(5, 10)
    ys = range(0, 5) if "N" in qname else range(5, 10)
    return [(x, y) for y in ys for x in xs]


def _quadrant_complete(me, view, qname, min_frac=0.70, min_animals=2):
    """v11.7: True once the given quadrant is >=min_frac non-empty AND has
    at least min_animals of our own pastures sited within it."""
    coords = _quadrant_tile_coords_v117(qname)
    total = len(coords)
    non_empty = sum(1 for (x, y) in coords
                     if me["tiles"][y][x] not in (None, "LOCKED"))
    if non_empty < min_frac * total:
        return False
    n_animals_in_quad = sum(1 for pos, _t in view["my_pastures"]
                              if _quadrant_of_v117(pos) == qname)
    return n_animals_in_quad >= min_animals
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion ---
    # v11.7: each branch's existing budget/day gate is unchanged; ANDed
    # with a completeness check on the current highest-unlocked quadrant
    # (see _quadrant_complete / file header). Land purchase skips (not
    # aborts) while that quadrant isn't "done" yet.
    if 1 <= day <= 22:
        land_score = _land_priority(view)
        current_quad = me["unlocked_quadrants"][-1]
        quad_ready = _quadrant_complete(me, view, current_quad)
        if quads == 1 and budget > 1000 and quad_ready:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2000 and day <= 18 and quad_ready:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 4000 and day <= 16 and quad_ready:
