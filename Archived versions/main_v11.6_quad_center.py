...
# =====================================================================
# MAIN V11.6 -- QUADRANT-CENTER pasture/coop SITING, isolated test on top
# of v10.6_radius3 (champion). Base: main_v10.6_radius3.py, unchanged
# except for animal-structure SITE selection.
#
# Trigger: Grace's screenshot of a real opponent's board -- that opponent
# sites pastures near the CENTER OF EACH INDIVIDUAL QUADRANT, not near
# the single shared board-center shed. v10.5/v10.6's validated
# near-shed-radius mechanism sites BOTH crops and animals relative to
# CENTER_TILES (the shared shed every worker respawns at each morning) --
# that's a real, measured, movement-time-share-driven win for ONGOING
# CROPS (daily revisits all season). This hypothesis asks whether the
# same "near-shed" rule is actually the right one for ANIMAL STRUCTURES
# specifically, or whether siting animals near the currently-developing
# quadrant's own center is better.
#
# Mechanism (isolated to animal pasture/coop SITE CHOICE only -- crop
# siting via plantable_crops()/NEAR_SHED_RADIUS is untouched, and the
# "pick the animal up at the shed" step in animal_setup_action is also
# untouched since that's fixed by the engine: BUY_ANIMAL always delivers
# stock to a CENTER_TILES shed tile, not our choice):
#   - _current_dev_quadrant(view): the quadrant currently being
#     developed = the highest-index unlocked quadrant (NW < NE < SW < SE,
#     matching the engine's own unlock order) that still has empty land
#     in view["empty"] (locked tiles never appear there, so this needs no
#     me["unlocked_quadrants"] lookup). Falls back to NW once the whole
#     board is full.
#   - QUADRANT_CENTERS: each quadrant's own geometric center point
#     ((2,2) NW, (7,2) NE, (2,7) SW, (7,7) SE on this 10x10 board).
#   - _grow_reserved_sites() and animal_setup_action()'s site-selection
#     fallback now sort/pick candidate pasture tiles by distance to the
#     CURRENT quadrant's own center instead of distance to the shared
#     shed (_nearest_center). This is a genuinely different spatial rule,
#     not a NEAR_SHED_RADIUS tweak -- its own function, its own distance
#     origin, and it moves as the frontier of development advances
#     through the quadrants over the game.
# =====================================================================
...
# v11.6: quadrant geometry for animal-structure siting (see file header).
QUADRANT_ORDER = ["NW", "NE", "SW", "SE"]
QUADRANT_CENTERS = {"NW": (2, 2), "NE": (7, 2), "SW": (2, 7), "SE": (7, 7)}


def _quadrant_of(tile):
    x, y = tile
    return ("N" if y < 5 else "S") + ("W" if x < 5 else "E")


def _current_dev_quadrant(view):
    """v11.6: the quadrant currently being developed = the highest-index
    unlocked quadrant (NW<NE<SW<SE, matching the engine's own unlock
    order LAND_ORDER=[NE,SW,SE]) that still has empty land. Locked tiles
    never appear in view["empty"], so this needs no explicit
    me["unlocked_quadrants"] lookup. Falls back to NW if the whole board
    is already full."""
    empty_quads = {_quadrant_of(t) for t in view["empty"]}
    for q in reversed(QUADRANT_ORDER):
        if q in empty_quads:
            return q
    return "NW"
...
def _grow_reserved_sites(view):
    """v11.6: claim exactly one more tile for the fleet, called only when
    a new animal purchase actually fires. Picks the still-unclaimed empty
    tile nearest the CURRENT quadrant's own center (was: nearest the
    shared shed)."""
    candidates = [t for t in view["empty"]
                  if t not in CENTER_TILES and t not in reserved_sites]
    if not candidates:
        return
    dq_center = QUADRANT_CENTERS[_current_dev_quadrant(view)]
    candidates.sort(key=lambda t: _dist(t, dq_center))
...
def animal_setup_action(pos, view, shed, farmer_carry, plan, day, hour, reserved_sites=(), exclude_sites=()):
    ...
    if site is None or (site not in view["empty"] and site not in view["empty_pastures"]):
        ...
        if not candidates:
            return None
        # Prefer a still-unclaimed reserved slot over the nearest fallback,
        # so daily walks stay short even for late purchases. v11.6: the
        # fallback distance origin is the CURRENT quadrant's own center,
        # not the shared shed (see _current_dev_quadrant / file header).
        pool = [t for t in reserved_sites if t in candidates] or candidates
        dq_center = QUADRANT_CENTERS[_current_dev_quadrant(view)]
        site = min(pool, key=lambda p2: _dist(p2, dq_center))
        plan["site"] = site
