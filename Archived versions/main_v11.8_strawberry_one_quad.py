...
# =====================================================================
# MAIN V11.8 -- STRAWBERRY-IN-ONE-QUADRANT CONCENTRATION, isolated test
# on top of v10.6_radius3 (champion). Base: main_v10.6_radius3.py,
# unchanged except for one exclusion rule added to plantable_crops().
#
# Trigger: Grace's screenshot of a real opponent's board -- that
# opponent concentrates STRAWBERRY into just one or two quadrants
# (diagonally opposite ones) rather than spreading it board-wide.
# v10.6's siting rule only controls WHICH crop type wins a given empty
# tile (near-shed tiles prefer ONGOING crops STRAWBERRY/TOMATO, far
# tiles prefer one-time crops) -- it never restricts a crop to a
# specific quadrant, so STRAWBERRY still ends up planted all over the
# board wherever a near-shed tile happens to be empty.
#
# Mechanism: STRAWBERRY_QUADRANT = "NE" (the first quadrant unlocked
# after NW, i.e. the first real land expansion). plantable_crops() now
# drops STRAWBERRY from the candidate list entirely for any tile whose
# quadrant isn't NE, BEFORE the existing near/far ongoing-vs-one-time
# sort runs -- so strawberry can only ever be chosen for NE tiles, and
# every other quadrant (NW/SW/SE) falls back to its next-best crop by
# the unchanged v10.6 priority/siting order. Everything else (TOMATO's
# near/far treatment, one-time crop siting, seed-buy targets) is
# byte-identical to v10.6.
# =====================================================================
...
# v11.8: quadrant restriction for STRAWBERRY only (see file header).
STRAWBERRY_QUADRANT = "NE"
...
def plantable_crops(seeds, day, tile=None):
    ...
    # v11.8: strawberry may only be chosen for a tile inside its
    # designated quadrant -- applied before the near/far sort so it never
    # wins a tile elsewhere regardless of distance to shed.
    if tile is not None and "STRAWBERRY" in base and _quadrant_of(tile) != STRAWBERRY_QUADRANT:
        base = [c for c in base if c != "STRAWBERRY"]
