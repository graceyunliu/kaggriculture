...
def economy(obs, me, opp, view, seeds, day, hour):
    market_orders = []
    # land before seeds: the seed pipeline otherwise outspends the land gate (D31 A/B)
    quads = len(me["unlocked_quadrants"])
    if day >= 1 and day <= 20:
        if quads == 1 and me["money"] > 1100:
            market_orders.append(["BUY_LAND"])
        elif quads == 2 and me["money"] > 4500 and day <= 18:
            market_orders.append(["BUY_LAND"])
    money = me["money"]
    ...
    for item, n in sorted(sellable, key=lambda kv: -market_price(kv[0], inv.get(kv[0], I0))):
        ...
        if force_dump or day >= LIQUIDATE_DAY:
            ...
            qty = n                      # scarcity: the top belongs to the first mover (D36)
        else:
            floor = sell_threshold(item, day, view["opp_imminent"], behind) * base
            # crowded shed lowers standards smoothly, so the guard never panic-dumps at $1
    ...
    if hour == 0 and day < 29:
        ...
        target = max(4, min(8, work // 5))
        for _ in range(target):
            market_orders.append(["HIRE"])

    # --- seed pipeline by phase FSM (D19, D22 portfolio) ---
    ph = phase(day)
    space = len(view["empty"]) + 2   # tiles free now plus imminent harvest frees
    want = {}
    if day <= CUTOFF["MELON"] and money > 700:
        want["MELON"] = min(space, 4 if ph == 0 else 6)
        ...
        if (x, y) in tasks[key]:
            ...
            if key == "plant":
                for crop in plantable_crops(seeds, day):
                    seeds[crop] -= 1
                    tasks["urgent_water"].append((x, y))   # same-day water or it dies tonight
                    return ["PLANT", crop]
...
def _agent(obs):
    ...
    tasks = {
        ...
        "water": view["water"],
        "plant": (view["empty"][:n_plantable] if crops_now and hour < 21 else []),  # D17 + no doomed late plants
...
def agent(obs):
    try:
        ...
        print(f"GUARD swallowed: {e!r}", file=sys.stderr)
        traceback.print_exc(file=sys.stderr)
        return {"farmer": ["PASS"], "hands": [], "market": []}   # never crash, but never silently
