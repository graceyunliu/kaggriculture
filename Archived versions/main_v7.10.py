...
# =====================================================================
# MAIN V7.10 -- SPREAD THE DAILY HIRE BURST, ISOLATED.
# Base: v7.9, byte-identical except for the hire block in economy() and
# the two constants HIRE_HOURS / HIRE_SLOTS_PER_TURN (see comment there).
# Nothing else is touched: no selling change, no animal change, no crop
# change, no scheduler change. v7.9's own header follows unchanged.
#
# =====================================================================
...
# --- v7.10: SPREAD THE DAILY HIRE BURST (the only change vs v7.9) ---
#
# D69: `work` climbs to 101-112, so target pins to HAND_TARGET_MAX=18 on 17
# of 29 days and v7.9 dutifully queues up to 18 HIRE orders -- all inside the
# single hour==0 turn. But the engine caps market orders at 10 per turn
# (MAX_MARKET_ORDERS), and the merge loop at the bottom of economy() lets at
# most ONE hire displace a sell once those 10 are full. So on any turn where
# selling already fills the slate, 17 of the 18 hire orders are silently
# dropped. Measured effect: hands actually held (sampled hour 12) are only
# 5-9 and DECLINING late, while frontier -- which splits hiring over two
# turns per day -- lands 324 hires to our 207 and holds 12 hands all midgame.
# Hands wipe to zero at every day boundary, so this shortfall is re-paid
# daily and gates every downstream throughput number (water, weed, harvest,
# wheat output).
#
# Fix: issue the day's hires over hours 0/1/2 with a per-turn cap, so hire
# orders compete with sells three times for a few slots instead of once for
# all ten. `need` is recomputed from live len(me["hands"]) each hour, so the
# spread is self-correcting: if hour 0's orders all land, hours 1-2 ask for
# little or nothing.
HIRE_HOURS = (0, 1, 2)
HIRE_SLOTS_PER_TURN = 5   # leaves >=5 of the 10 order slots for sells
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if hour in HIRE_HOURS and day < 29:
        animal_work = _pending_animal_work(view)
        ...
        # The engine prices hire k of the day at _fib(farm["hires_today"]), and
        # hands only ever leave at the day boundary, so hires_today == held.
        # v7.9 restarted the fib count at 0 every turn, which was correct only
        # because it fired once a day at hour 0 (held == 0). Spreading across
        # hours makes `held` the right base -- otherwise hours 1-2 would
        # under-price their hires and overdraw the virtual budget.
        n = 0
        while n < need and len(hire_orders) < HIRE_SLOTS_PER_TURN:
            cost = FARM_HAND_COST_MULT * _fib(held + n)
            if budget < cost:
                break
