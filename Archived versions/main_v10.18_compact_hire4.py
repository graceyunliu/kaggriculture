...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Land expansion ---
    # v10.14 hypothesis: Grace observed that real ladder players scoring
    # in the 800s never expand to a 4th (SE) quadrant by game end. An
    # older test of this exact idea exists (main_v7.10c_no_se.py, back in
    # the v7 lineage) and came back NEUTRAL (-$234/game, t=-0.11) -- but
    # that was before v8's task-dispatch rewrite, v9's whole animal/hire
    # rebuild, and v10.5/v10.6's siting logic, so it's worth a fresh,
    # independent re-test on the current champion rather than trusting a
    # 20-versions-old result. Mechanism: SE is the farthest quadrant from
    # the shed (each new quadrant's own near-shed corner is progressively
    # farther out than the previous one), so under v10.6's siting most of
    # it would be planted in one-time crops anyway (WHEAT/CARROT/MELON) --
    # its $4,000 cost + the labor time to reach and service it may not
    # pay back against just running the existing 3 quadrants harder.
    if 1 <= day <= 22:
        ...
        # v10.14: SE (4th quadrant) purchase removed entirely.
    ...
    # v10.16 hypothesis: the real fixed-tape clone opponents in this
    # project (opp_scenario_v14/opp_frontier_v12, extracted from actual
    # top-of-ladder replays) run WHEAT/STRAWBERRY/MELON only -- no
    # CARROT, no TOMATO -- per confirmed project memory (Savko/Subin An
    # replay analysis). That's the same playbook that never buys the 4th
    # quadrant either. Testing the crop-mix half of that pattern on top
    # of v10.14's land half: drop CARROT and TOMATO entirely, so their
    # seed budget and planting labor concentrate into STRAWBERRY/MELON
    # instead of being spread across 4 crop types on a now-smaller farm.
    # TOMATO/CARROT seed lines removed (want["TOMATO"], want["CARROT"]).
    ...
    if hour == 0 and day < 29:
        ...
        # v10.18: divisor 5->4 (hire modestly more aggressively per unit
        # of pending work). A near-identical change (divisor 5->3) was
        # tested on the OLD full-board, spread-crop-mix architecture and
        # lost catastrophically (main_v7.10a, -$33,243/game) -- but that
        # was a much bigger step (5->3) on a much bigger farm with no
        # siting logic. Re-testing a smaller step on the smaller,
        # concentrated-crop v10.16 base, where each hand's average walk
        # is shorter, on the theory that extra hands pay back faster here
        # than they did in the context v7.10a was tested in.
        target = max(HAND_TARGET_MIN, min(HAND_TARGET_MAX, math.ceil(work / 4)))
