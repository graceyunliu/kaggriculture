...
# =====================================================================
# MAIN V12.1 -- FREQUENCY x DISTANCE CONCENTRIC RANKING (Aug 7, round 8
# on top of the undefeated main_v10.6_radius3.py champion).
#
# HYPOTHESIS: a tile's real seasonal labor cost is approximately
# visit_frequency x active_lifetime x distance (every worker respawns at
# the shed each morning, an engine mechanic -- see CENTER_TILES). v10.5/
# v10.6 already exploit this for CROPS ALONE (ongoing STRAWBERRY/TOMATO
# near, one-time WHEAT/CARROT/MELON far, NEAR_SHED_RADIUS=3) but treat
# animals and ongoing crops as competing for the SAME undifferentiated
# near-shed tile pool with no ranking between them. Animals (COW/SHEEP)
# have visit_frequency ~= 1 (feeding is mandatory daily or the animal
# escapes after 2 consecutive missed days) for their ENTIRE placed
# lifetime, often 20+ days once purchased early -- likely the single
# highest f x lifetime object in the game, higher than even STRAWBERRY's
# ~12-17 day ongoing life. Yet main_v10.6's `_grow_reserved_sites()` only
# claims a near-shed tile for an animal REACTIVELY, after that animal's
# purchase has already been confirmed in the shed -- by then an ongoing
# crop may have already been planted on the innermost, highest-priority
# tile, since `plantable_crops()` has no notion of a pending animal claim
# and will happily seed the nearest empty tile with STRAWBERRY the turn
# before a pasture purchase confirms.
#
# SCORES USED (documents the ranking that motivates the mechanism below;
# not literally recomputed every turn -- they justify the ordering, the
# code enforces it structurally):
#   ANIMAL:        f=1.0  x lifetime~20+ days -> score ~20  (highest)
#   ONGOING CROP:   f=1.0  x lifetime~12-17 days -> score ~15
#   ONE-TIME CROP:  f~0.15 x active-window~6 days -> score ~0.9 (lowest;
#     watered only during the bonus window, harvested once -- a handful
#     of total visits, so distance barely matters, matching v10.5's
#     original rationale for sending these tiles far).
#
# MECHANISM CHANGED: reserved_sites growth is made PROACTIVE instead of
# purely reactive. `_grow_reserved_sites_to()` (new) tops the fleet's
# near-shed reservation up to min(EXPERIMENT_MAX_FLEET, current-and-
# pending fleet size + ANIMAL_LOOKAHEAD_BUFFER) EVERY turn, not just when
# a purchase actually confirms. Those reserved tiles are already excluded
# from `tasks["plant"]` by existing v9.2 logic (`excluded = claimed_sites
# | set(sites)`) -- so this single change is enough to implement the
# ranked/ordered claim: the highest-score category (animals) locks in its
# near-shed tiles 1-2 purchases AHEAD of need, so a pending animal
# purchase structurally bumps a would-be strawberry planting out of the
# innermost ring, exactly as the hypothesis requires. plantable_crops()'s
# existing NEAR_SHED_RADIUS ongoing-vs-one-time preference is untouched
# and simply operates on whatever ring is left once the animal
# reservation has been carved out -- ongoing crops still get first pick
# of THAT remainder, one-time crops still get what's left after that,
# preserving the score order ANIMAL > ONGOING > ONE-TIME.
#
# ANIMAL_LOOKAHEAD_BUFFER=2 (not a flat reserve-all-10-from-day-1 policy):
# the v10.6 radius sweep already showed over-reserving near-shed land
# hurts (NEAR_SHED_RADIUS=6 lost decisively to 4, -$12,693/game, 10/10)
# -- reserving all EXPERIMENT_MAX_FLEET=10 slots before the fleet is
# anywhere near that size would strand early crop-land the same way. A
# buffer of 2 keeps the next purchase or two pre-claimed without
# starving day-1..10 crop planting.
# =====================================================================
...
# v12.1: how many purchases AHEAD of the current+pending fleet size to
# keep the near-shed reservation grown to (see file header for the score
# rationale: ANIMAL score ~20 beats ONGOING CROP's ~15, so the fleet's
# claim on innermost tiles should be staked before, not after, purchase
# confirmation). Capped at EXPERIMENT_MAX_FLEET regardless.
ANIMAL_LOOKAHEAD_BUFFER = 2
...
def _grow_reserved_sites_to(view, target_count):
    """v12.1: PROACTIVE counterpart to `_grow_reserved_sites()` above.
    v10.6 only ever called that function reactively, once a purchase had
    already confirmed -- this tops the reservation up to `target_count`
    (already capped by the caller at EXPERIMENT_MAX_FLEET) every turn, so
    the fleet's highest-priority claim (see file header score rationale)
    on near-shed tiles is staked BEFORE the purchase that needs it, not
    after. Stops early and silently if the board runs out of candidate
    tiles (matches `_grow_reserved_sites()`'s own no-op-when-empty
    behavior)."""
    while len(reserved_sites) < target_count:
        before = len(reserved_sites)
        _grow_reserved_sites(view)
        if len(reserved_sites) == before:
            break  # no more candidate tiles this turn
...
def _agent(obs):
    ...
    # v12.1: proactively grow the fleet's near-shed reservation AHEAD of
    # confirmed purchases (see file header) -- must run before `excluded`
    # is built below so this turn's crop planting already respects it.
    _grow_reserved_sites_to(view, min(EXPERIMENT_MAX_FLEET,
                                       n_animals_for_wheat + ANIMAL_LOOKAHEAD_BUFFER))
