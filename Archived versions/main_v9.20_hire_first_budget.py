...
# =====================================================================
# MAIN V9.20 -- hire block moved to the FRONT of economy() (before land/
# animal/seed/feed/fertilizer-sell), on top of v9.12 (melon fertilize-
# in-window on v9.10's hire fix).
#
# Hypothesis (capital-timing category, second attempt after v9.19's
# land/animal reserve found no compression): economy()'s `budget` local
# variable is decremented in fixed CODE order as each block appends its
# order, but the final order list is chosen by SCORE (sort + truncate
# to MAX_MARKET_ORDERS) at the very end. HIRE (PRIORITY_HIGH_HIRE=700)
# outranks every other budget-competing block under normal conditions
# (land 15-320, animal 320, seed up to 690, feed 340) -- but the hire
# block was written LAST in economy(), so on the hours it actually runs
# (hour<=2, matching v9.10's fix) it only sees whatever budget survived
# land+animal+seed+feed+fertilizer-sell's earlier claims, even though
# it would out-rank all of them once sorted. This build simply
# relocates the hire block (logic byte-identical, only its position in
# the function changed) to run FIRST, so it claims capital in the same
# order its own priority score would rank it -- most direct possible
# test of "hiring capital is being crowded out by lower-priority same-
# turn spending" as a cause of the day8-20 stall.
#
# RESULT -- TESTED, REJECTED (no gap compression). 5-seed smoke, both-
# seats: vs v9.5, mean -$7,400 both-seats-sum (-$3,700/game), t=-1.33,
# 95% CI [-$18,272,+$3,472] -- crosses zero but a consistently negative
# point estimate (1/5 wins). Vs opp_scenario_v14: mean -$92,695 both-
# seats-sum (-$46,347/game), t=-13.86 -- statistically the same as
# v9.10/v9.12's own -$46,860/game baseline (the +$513/game "gain" is
# noise given stderr $6,688). Reordering WHICH block gets first claim on
# `budget` doesn't move the gap either way -- most turns during the
# capital-constrained window apparently don't have enough total order
# volume for the land/animal/seed/feed blocks to actually crowd hire
# out before it runs, so the code-order-vs-priority-order mismatch this
# targeted is real (confirmed by reading the code, same finding as
# v9.19) but not the behavioral driver of the day8-20 stall in
# practice. Second and final capital-timing attempt this round -- both
# closed the category out empty-handed, same pattern as volume/scale
# (v9.6-v9.16) and task-dispatch-efficiency (v9.17-v9.18) before it. Not
# worth a 15-seed confirmation -- neither reading is close to a decision
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- Hiring: MOVED to the front of economy() (v9.20) -- real
    # Fibonacci cost, counts animal maintenance workload alongside crops
    # so the hand pool scales with the fleet too. v9.10: hour==0 ->
    # hour<=2 (matches opp_scenario_v14's own gating; hands reset to
    # empty every engine midnight and a single hour is capped at
    # MAX_MARKET_ORDERS=10 new hires, so one hour alone can never reach
    # a target above 10) + Fibonacci cost index starts from the real
    # me["hires_today"] (persists across hours, only resets at end of
    # day -- assuming a fresh-0 index at hour 1/2 wildly underprices
    # continuing to hire). See file header and HAND_TARGET_MAX below.
    #
    # v9.20: this block used to run dead LAST (after land/animal/seed/
    # feed/fertilizer-sell all had first claim on `budget`), even though
    # HIRE's own priority score (PRIORITY_HIGH_HIRE=700) outranks every
    # one of those blocks' own scores under normal conditions (land
    # 15-320, animal 320, seed up to 690, feed 340) -- `budget` is
    # decremented in fixed CODE order, not by each order's eventual
    # sorted priority, so the highest-priority block was actually first
    # in line for TRUNCATION but LAST in line for the underlying cash.
    # Moved here so it claims budget in the order that already matches
    # its own priority ranking, instead of getting whatever's left over.
