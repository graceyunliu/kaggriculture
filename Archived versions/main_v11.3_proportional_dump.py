...
# v11.3 (Aug 7) -- CONTINUOUS PROPORTIONAL DUMP, shed-capacity round.
# Base: main_v10.6_radius3.py, one isolated change, nothing else touched.
# Diagnosis motivating this round: aggregate replay data (129W/100L/4T)
# shows SHED_AT_CAP as the single dominant loss cause (31/100 losses) --
# the shed (SHED_CAP=100) fills and blocks further storage/selling. Prior
# hypothesis rounds targeted siting/quadrants/animal composition, never
# shed throughput itself.
# Mechanism: an ADDITIONAL selling mechanism layered alongside (not
# replacing) v10.6's existing price-based paced selling and force_dump.
# Every turn, once shed_load > 0.5*SHED_CAP, every sellable product
# above a 5-unit buffer sheds 15% of its excess -- regardless of price
# -- as a new priced_order. Placement: priority PRIORITY_PROPORTIONAL_DUMP
# = 450, deliberately below force_dump's emergency relief (760, so real
# overflow emergencies still win) but above ordinary paced selling (tops
# out at 400) -- a proactive, price-blind, continuous release valve that
# should out-rank an item just sitting at a so-so price. The dumped
# quantity is debited from what the paced-selling loop sees for that
# item so the two mechanisms can never jointly oversell a product past
# what the shed actually holds.
...
# v11.3: proportional dump sits below the real emergency (force_dump,
# 760) but ABOVE ordinary paced selling (which tops out at
# PRIORITY_MEDIUM_RISING_LO=400 below) -- it is a proactive, price-blind
# release valve that should out-rank an item just sitting at a so-so
# price, but must never outrank genuine overflow relief.
PRIORITY_PROPORTIONAL_DUMP = 450
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # v11.3 (shed-capacity round): continuous PROPORTIONAL DUMP, an
    # ADDITIONAL selling mechanism layered alongside (not replacing) the
    # existing price-based paced selling below. Every turn, once total
    # shed load exceeds half of SHED_CAP, every sellable product above a
    # small 5-unit buffer sheds 15% of its excess, regardless of price --
    # a continuous pressure-release valve instead of only reacting at
    # force_dump's near-full emergency threshold. Priced as a real
    # priced_order at PRIORITY_PROPORTIONAL_DUMP (see constant comment
    # above): below force_dump's emergency relief, above ordinary paced
    # selling. We debit the dumped amount from the quantity the paced
    # loop below sees for that item (via `proportional_dumped`), so the
    # two mechanisms can never jointly request more units than the shed
    # actually holds for a given product.
    proportional_dumped = {}
    if shed_load > 0.5 * SHED_CAP:
        for pitem, pn in shed.items():
            if not (isinstance(pn, (int, float)) and pn > 0):
                continue
            if pitem not in MARKET_PARAMS or pitem == "FERTILIZER" or pitem in ("COW", "SHEEP", "GOOSE"):
                continue
            peff = pn - feed_reserve if pitem == "WHEAT" else pn
            dump_qty = int(max(0, peff - 5) * 0.15)
            if dump_qty > 0:
                priced_orders.append((PRIORITY_PROPORTIONAL_DUMP, ["SELL", pitem, dump_qty]))
                proportional_dumped[pitem] = dump_qty
    ...
    for i, n in shed.items():
        ...
        if i not in MARKET_PARAMS or i == "FERTILIZER" or i in ("COW", "SHEEP", "GOOSE"):
            continue
        n = n - proportional_dumped.get(i, 0)
        if i == "WHEAT":
            n = n - feed_reserve
            if n <= 0:
                continue
