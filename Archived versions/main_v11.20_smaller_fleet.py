...
# =====================================================================
# v11.20 -- SMALLER FLEET FOR SHED RELIEF (shot in the dark, on top of
# v10.6). Base chassis: main_v10.6_radius3.py, unmodified except for
# this one constant. Round-7 "shots in the dark" theme.
#
# Today's aggregate replay data (129W/100L/4T) shows SHED_AT_CAP (shed
# capacity 100 fills up and blocks storage/selling) as the single
# dominant loss cause, 31/100 losses. Animal products (MILK/WOOL/EGG/
# FERTILIZER) are a big contributor to steady shed inflow -- roughly
# once-per-interval production from every animal in the fleet, all
# funneling into the same 100-slot shed as crop harvests. This
# hypothesis lowers EXPERIMENT_MAX_FLEET from 10 to 6: a smaller fleet
# produces less constant animal-product volume needing storage, freeing
# shed headroom for higher-value crop products, which might net out
# ahead given SHED_AT_CAP's now-documented dominance as a loss cause.
#
# Note this is explicitly NOT a retest of "raise the fleet ceiling"
# (which has failed 3 independent times in this project due to
# labor/movement saturation at >10 animals) -- it is the opposite
# direction, for a different reason (shed relief, not labor relief),
# and trades away confirmed animal revenue to test it.
# =====================================================================
...
# gates run unbounded."
EXPERIMENT_MAX_FLEET = 6  # v11.20: lowered from 10 -- shed-relief bet, not labor relief (see file header). v9.1 iter C baseline note (10 vs 12+): 12+ animals saturate the hand pool (weeds 22 vs 2, straw tiles die late -- seed 3/15 blowouts); 10 was v8.11's validated service capacity ceiling.
