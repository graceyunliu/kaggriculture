...
# v11.5 (Aug 7) -- LOWER PACED-SELLING PRICE FLOORS, shed-capacity round.
...
# Mechanism: v10.6's normal (non-force_dump) paced selling only sells
# once price clears `frac * base` where frac = 0.85 (PREMIUM_CROPS) /
# 0.70 (everything else) -- both in the force_dump fall-through branch
# and the ordinary branch. This hypothesis lowers frac to 0.65 / 0.45 in
# BOTH places, so ordinary paced selling clears inventory faster at a
# lower average price at all times, instead of holding for peak prices --
# keeping shed occupancy chronically lower rather than only reacting once
# an emergency (force_dump) or holding-phase gate fires.
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    for item, n in sorted(sellable, key=lambda kv: -live_price(obs, kv[0])):
        ...
        if is_attack and item in PREMIUM_CROPS:
            ...
            # v11.5 (shed-capacity round): floors lowered from
            # 0.85/0.70 to 0.65/0.45 so ordinary paced selling clears
            # inventory faster at a lower average price at all times,
            # instead of holding for peak prices -- keeps shed occupancy
            # chronically lower rather than only reacting at emergencies.
            frac = 0.65 if item in PREMIUM_CROPS else 0.45
            ...
            # v11.5: same floor reduction as the force_dump branch above
            # (0.85/0.70 -> 0.65/0.45), applied to the normal (non-dump)
            # paced-selling path.
            frac = 0.65 if item in PREMIUM_CROPS else 0.45
            if view["opp_imminent"].get(item, 0) >= 4:
                frac -= 0.25
