...
# =====================================================================
# MAIN V9.19 -- land-purchase capital reserve during the day8-20
# cash-constrained window, on top of v9.12 (melon fertilize-in-window
# on v9.10's hire fix).
#
# Hypothesis (capital-timing category, per Grace's selection after the
# volume-scale AND task-dispatch-efficiency categories both closed out
# empty-handed across v9.6-v9.18): v9.6's original diagnosis found cash
# sitting at $100-$5,000 through days 13-20, too thin to fund both
# animal purchases and the Fibonacci hire ladder -- flagged but never
# revisited. Reading economy()'s actual code found a real mismatch: the
# `budget` variable used to gate every purchase check is decremented in
# FIXED CODE ORDER (land, then animal, then seed, then feed...), not by
# each order's priority score -- the score-based sort/truncate to
# MAX_MARKET_ORDERS only happens at the very end. So a LOW-priority
# (PRIORITY_LAND_IDLE=15, fired whenever idle tiles aren't actually
# scarce -- the common case per project memory's demand-side analysis)
# land buy, costing $1,000-$4,000 in one lump, can still eat cash out
# from under the animal-purchase check (PRIORITY_MEDIUM=320) that runs
# immediately after it in the same turn, even though the final sorted
# order list would have ranked animal above it. This build adds a
# `LAND_CAPITAL_RESERVE=3000` headroom requirement, but ONLY for
# IDLE-priority land buys during days 8-20 -- a SCARCE-priority buy
# (land actually blocking crop throughput, idle tiles < 15) is never
# deferred, so this can't reintroduce v8's land-starvation failure mode
# on a fleet-size or crop-count basis, only reorders WHEN a
# non-urgent land buy is allowed to compete for the same thin cash.
#
# RESULT -- TESTED, REJECTED (no gap compression). 5-seed smoke, both-
# seats: vs v9.5, mean -$4,220 both-seats-sum (-$2,110/game), t=-0.59,
# 95% CI [-$18,246,+$9,805] -- statistically zero, slightly negative
# point estimate. Vs opp_scenario_v14: mean -$95,056 both-seats-sum
# (-$47,528/game), t=-11.52 -- marginally WORSE than v9.12's own
# -$46,860/game baseline (difference is noise given stderr $8,251, but
# certainly no compression). The land/animal budget-ordering mismatch
# is real (confirmed by reading the code), but apparently not the
# actual behavioral driver of the day8-20 stall at our current land-
# buy frequency/timing -- land purchases in that window are likely too
# small/rare relative to the Fibonacci hire-cost curve itself (already
# tuned in v9.10) to matter. Not worth a 15-seed confirmation -- neither
# reading is close to a decision boundary. NOT submitted.
# =====================================================================
...
LAND_CAPITAL_RESERVE = 3000     # v9.19: headroom an IDLE-priority land
                                  # buy must leave untouched during the
                                  # day8-20 capital-constrained window
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if 1 <= day <= 22:
        ...
        # v9.19: during the capital-constrained day8-20 window (v9.6's
        # diagnosis: cash sits at $100-$5,000 through days 13-20, too
        # thin to fund animal purchases AND the Fibonacci hire ladder
        # simultaneously), require extra headroom before an IDLE-
        # priority land buy (tiles aren't actually scarce here --
        # land_score == PRIORITY_LAND_IDLE) is allowed to eat into that
        # same cash. This targets a real mismatch: `budget` below is
        # decremented in fixed CODE order (land, then animal, then
        # seed...), not by the priced_orders' eventual sorted-and-
        # truncated priority -- so a low-priority (15) land buy can
        # still starve the animal-purchase check that runs right after
        # it, even though the final order list would rank animal (320)
        # above it. A SCARCE-priority buy (land_score ==
        # PRIORITY_LAND_SCARCE, idle tiles < 15) is never deferred --
        # land genuinely blocking crop throughput is a real cost this
        # window shouldn't get to postpone.
        land_reserve = (LAND_CAPITAL_RESERVE
                         if 8 <= day <= 20 and land_score == PRIORITY_LAND_IDLE
                         else 0)
        if quads == 1 and budget > 1000 + land_reserve:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 1000
        elif quads == 2 and budget > 2000 + land_reserve and day <= 18:
            priced_orders.append((land_score, ["BUY_LAND"])); budget -= 2000
        elif quads == 3 and budget > 4000 + land_reserve and day <= 16:
