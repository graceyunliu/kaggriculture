...
# =====================================================================
# v13.4 (Aug 7) -- GOOSE re-test with GLUT-ADJUSTED $/day math, on top of
# v10.6 champion (siting only, no goose plumbing). Round-8-era v10.22
# tested a goose composition floor and LOST decisively (-$9,444/game)
# with the stated mechanism "EGG's low base price ($50) means lower
# revenue-per-service-day than the cow/sheep it displaces" -- but that
# test ran ENTIRELY in self-play vs v10.6, whose OWN fleet is capped at
# EXPERIMENT_MAX_FLEET=10, so milk/wool never actually glut the shared
# market in that matchup. New analysis of MARKET_PARAMS' price-decay
# shape parameters shows a hard dichotomy: EGG (T=332, af="log",
# at=0.20) is nearly glut-proof -- price only falls from $50 to ~$39 at
# DOUBLE T oversupply. MILK (T=122, af="linear", at=1.60) and WOOL
# (T=105, af="sq", at=3.20) both crash to the $1 PRICE_FLOOR once
# combined (both players') market oversupply crosses roughly 1x T past
# I0 -- a real opponent running a bigger fleet (e.g. opp_scenario_v14.py,
# which targets 15 animals -- 50% past this file's own ceiling) can push
# milk/wool into glut territory basically on its own. Against such an
# opponent, cow's "$80/day" and sheep's "$66.67/day" sticker prices may
# never be realized, while EGG (a demand pool real opponents barely
# touch per project memory) realizes close to its full $50/day the whole
# game. This build ports the GOOSE/COOP plumbing (perceive() extended
# for COOP tiles, ANIMAL_STRUCTURE species->structure map, structure-
# aware animal_setup_action, GOOSE added to ANIMAL_SPECS, a
# _goose_expansion_feasible gate mirroring SHEEP's) from the (rejected)
# v10.22 build, but re-sizes GOOSE_FLOOR_BY_FLEET_SIZE far more
# aggressively -- roughly HALF the 10-slot ceiling reserved for geese by
# the time the fleet is full (vs v10.22's ~30-40%), reflecting the
# revised math that GOOSE may be the single best per-slot animal once
# realistic glut-adjusted $/day is used, not just an additive
# afterthought squeezed in after cow/sheep's own gates decline. Tested
# both against v10.6 (self-play control -- fleets stay <=10/side, so
# glut should be mild/absent) AND against Opponents/opp_scenario_v14.py
# (the real 15-animal glutting opponent) to see whether goose's
# performance differs between the two -- that difference is the actual
# test of the glut-resistance hypothesis, not the self-play number
# alone. See run report for both sets of stats.
# =====================================================================
...
# v13.4: goose composition floor, same pattern as SHEEP_FLOOR_BY_FLEET_SIZE
# but sized far more aggressively -- v10.22's rejected build used
# {3: 1, 6: 2, 9: 3} (~1/3 of the fleet). This build's revised glut-
# adjusted math says GOOSE's realistic $/day may beat cow/sheep's once
# milk/wool glut is accounted for, so the floor now targets roughly HALF
# the validated 10-slot ceiling by the time the fleet is full (5 of 10),
# ramping up from the very first purchases rather than only kicking in
# late.
GOOSE_FLOOR_BY_FLEET_SIZE = {2: 1, 4: 2, 6: 3, 8: 4, 10: 5}
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if n_in_progress < MAX_CONCURRENT_BUILDS:  # v9.2: was serialized to 1
        ...
        if target_plan is None:
            ...
            # v13.4: same pattern for GOOSE -- tried BEFORE the sheep
            # composition check (goose's revised glut-adjusted $/day may
            # beat cow/sheep's, so it should get first claim on its
            # guaranteed floor slots), genuinely reallocating some of the
            # fixed 10-slot ceiling away from cow/sheep instead of only
            # ever firing as a last-resort fallback (v10.21's original
            # design, which never fired even once -- see project memory).
            ...
            # its own separate (relaxed-ceiling) gate -- see file header.
                ...
                target_plan = {"species": "SHEEP", "stage": "NONE", "site": None,
                                "stage_turn": None, "retries": 0}
            # v13.4: goose fallback retained too, for turns where cow AND
