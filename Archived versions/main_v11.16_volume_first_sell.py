...
# =====================================================================
# V11.16 -- VOLUME-FIRST SELL PRIORITY on top of v10.6 (round-7
# selling-rhythm theme). Aggregate replay data flagged SHED_AT_CAP as
# the dominant loss cause (31/100 losses). Hypothesis: the champion's
# sellable-item sort key (-live_price, highest per-unit price first)
# optimizes each turn's few sell-order slots for value, not for shed
# relief -- a turn can spend its limited SELL orders on a high-price/
# low-quantity item while a large low-price pile keeps growing toward
# the cap. This variant changes the sort key to -quantity-held (highest
# volume first), so whichever product would free the most shed space
# gets evaluated/sold first when order slots are limited, on the theory
# that relieving volume pressure proactively matters more than always
# grabbing the single highest per-unit price first. All the existing
# per-item quantity/floor/force-dump logic is unchanged -- only the
# iteration order of the `sellable` loop changes.
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    for item, n in sorted(sellable, key=lambda kv: -kv[1]):  # v11.16: volume-first, was -live_price(obs, kv[0])
