...
ANIMAL_SPECS = {
    "COW":   {"cost": 400, "min_money": 700, "start_day": 3,  "last_day": 22},
...
FLEET_TARGET = {"COW": 4, "SHEEP": 0}     # SCALING KNOB — validated safe ceiling for a single farmer,
...
def animal_maintenance_action(pos, view, shed, farmer_carry, day, hour):
    ...
    if not view["my_pastures"]:
        return None
    (ppos, tile) = min(view["my_pastures"], key=lambda pt: _pasture_priority(pt[0], pos, pt[1]))
