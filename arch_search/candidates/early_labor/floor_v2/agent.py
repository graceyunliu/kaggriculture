"""arch_search family=early_labor id=floor_v2 -- TDAS Phase 1 hand-built candidate.

Same hypothesis and base as early_labor/floor_v1 (see that file's docstring), but with a
MORE AGGRESSIVE floor, front-loaded to day 0-2 instead of ramping through day 6. This is
the family's second, competing instantiation of the SAME structural hypothesis (spec 2.6:
"generates 2-4 competing architectural hypotheses" -- here, 2 competing strengths of the
same mechanism) used to test whether the panel's earlier LABOR_SCALE needs to be gradual
(floor_v1) or can be a single up-front burst (floor_v2) to produce the same downstream
DENSIFICATION/EXPANSION/THROUGHPUT_SCALE shift.
"""
import math


def _early_labor_floor(day):
    """arch_search override (v2, aggressive): front-load hiring to near-max in days 0-2,
    tapering by day 5. Still not copied from any single opponent's exact hire count."""
    if day == 0:
        return 8
    if day <= 2:
        return 9
