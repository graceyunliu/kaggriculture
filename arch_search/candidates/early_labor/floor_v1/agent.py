"""arch_search family=early_labor id=floor_v1 -- TDAS Phase 1 hand-built candidate.

Base: an unmodified copy of candidates/V3_12.py (the champion this repo does not edit in
place -- this is a separate file). ONLY CHANGE: _load_model()'s output is floored by
_early_labor_floor(day) below, so the hiring target is pulled ahead of V3_12's own
demand-coupled schedule during days 1-6. This instantiates the structural hypothesis in
arch_search/reports/PHASE_0_STATE_MACHINE_AUDIT.md section D: the champion's DENSIFICATION
state (see arch_search/state_machine.py) fires ~3 days later than every reference-panel
trajectory (day 6 vs day 3), and LABOR_SCALE (a hiring burst) fires ~6-7 days later (day 12
vs day 5-7 across the panel). The claim under test is that this labor lag is UPSTREAM of
the later densification/expansion/throughput states, not merely correlated with them --
see the Hypothesis object and test_causal_prediction() call in
arch_search/tests/test_predict_check.py and the Phase 1 findings in
arch_search/reports/PHASE_0_STATE_MACHINE_AUDIT.md section (Phase 1 addendum).

This is NOT a port of any observed opponent's exact actions (the clone-replica dead end
recorded in repo memory) -- it is a floor on ONE internal knob (_load_model's target),
independent of any specific opponent's fingerprint.
...
def _early_labor_floor(day):
    """arch_search override: force the hiring target ahead of organic demand for the first
    week. Chosen to roughly match the panel's own LABOR_SCALE-burst days (5-7) rather than
    champion's day 12 -- NOT copied from any single opponent's exact hire count."""
    # day 0..6 -> 4,5,6,7,8,9,9 ; day 7+ -> no floor (let _load_model take back over)
    return min(9, 4 + day) if day <= 6 else 0
import sys
...
def economy(obs, v):
    ...
    if hour != capital_hour:
        if hour == 0:
            ...
            target = max(_load_model(v, seeds_on_hand, n_total, shed_animals + carried_animals, day),
                 _early_labor_floor(day))
    ...
    target = max(_load_model(v, seeds_on_hand, n_total, pending_place, day), _early_labor_floor(day))
    S["hires_target"] = target
