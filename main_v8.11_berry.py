...
# =====================================================================
# MAIN V8.11 (STRAWBERRY/TOMATO variant) -- zero-diversion fertilize
# pipeline, ISOLATED. Base: main_v8.3.py. Structurally IDENTICAL
# mechanism to main_v8.11_wheat.py (see that file's header for the full
# zero-diversion design writeup -- fertilize folded into
# task_order/tasks/assign_tasks() as a competing tier instead of a
# pre-emptive grab ahead of the global task-assignment mechanism). Only
# the target-crop set and eligibility window differ. This file exists
# because the user asked to test the disciplined dispatch design on
# STRAWBERRY/TOMATO specifically, alongside the WHEAT variant, despite
# a documented prior rejection -- see the flag below before trusting a
# win here.
#
# --- Known prior result for THIS crop target (read before trusting a
#     win here) ---
#
# main_v8_candidateA.py already tested STRAWBERRY/TOMATO fertilize (pre-
# v8, different base) and lost decisively across three iterations: v1
# -$50,711/11 seeds, v2 -$32,137/11 seeds (see
# kaggriculture-v8-architecture.md sec 4 / project memory). Re-derived
# independently by reading the engine directly (kaggriculture.py,
# _daily_refresh_plants): STRAWBERRY/TOMATO are `"ongoing": True`, and
# their yield accrual is gated by
# `production_count = days_since_first // interval + 1; if
# production_count > max_yield: continue` -- purely a function of
# calendar days since first_yield_day, monotonically increasing forever.
# Once a tile's production_count passes max_yield, _daily_refresh_plants
# permanently stops adding yield_units to it, regardless of whether it's
# been harvested. Since HARVEST never clears an ongoing-crop tile (only
# non-ongoing crops get removed), a STRAWBERRY/TOMATO tile's LIFETIME
# total extractable yield is exactly max_yield units (4 for both),
# harvested in however many separate HARVEST calls you like, REGARDLESS
# of fertilizer. Fertilizing an ongoing crop only compresses that same
# fixed total into fewer calendar days (front-loaded revenue timing) --
# it can never raise the ceiling. Candidate A's loss is exactly what
# that structural fact predicts: paying FERTILIZER + labor for zero net
# extra yield.
#
# This file is being run anyway (a) to close the loop empirically with
# the new zero-diversion dispatch (previously the dispatch mechanism
# was untested/confounded with the fundamental cap, so "did it lose
# because of the labor-diversion bug or because of the structural cap"
# was never cleanly separated), and (b) as a same-mechanism control
# against main_v8.11_wheat.py's result. Expectation going in, per the
# math above: still a loss, but a smaller/cleaner one than Candidate A
# if the zero-diversion design meaningfully cuts the labor cost side.
#
# --- Eligibility window ---
#
# A STRAWBERRY or TOMATO tile qualifies for the "fertilize" tier while
# 0 <= days-since-first-yield-day <= FERT_MAX_DAYS_SINCE_FIRST (2) and
# fertilized_until_day < day -- applying as early as possible after the
# tile becomes eligible maximizes how many of its (fixed-total) early
# production ticks land inside the 3-day fertilized window.
#
# --- Test protocol ---
...
FERTILIZER_RESERVE = 4        # keep this many FERTILIZER in shed for the
                                ...
                                # turn like v8.3
FERT_TARGET_CROPS = {"STRAWBERRY", "TOMATO"}
