...
# =====================================================================
# MAIN V9.7 -- hand-capacity work divisor 5 -> 7 (HAND_WORK_DIVISOR), on
# top of v9.5. Base: main_v9.5_statehygiene.py.
#
# Hypothesis (v9.6 postmortem): the fleet-scale gap vs the herd clone
# traces to a capital constraint, not a hard animal-count ceiling. The
# HAND_CAPACITY gate (projected_work/5 > HAND_TARGET_MAX=18) was found
# binding from day 13 on at projected_work ~111-121 (threshold 90), and
# even where the formula's own hire target implied 18 hands, actual
# hires plateaued at 9-10 because cash was too thin to fund them.
# Opponents/opp_scenario_v14.py's own _target_hands() divides its
# backlog by 7, not 5 -- it tolerates more pending work per hand before
# asking for another hire. Applying /7 here does two things at once:
# loosens the HAND_CAPACITY feasibility gate (ceiling 90->126) so more
# animal purchases become eligible, AND reduces the number of hands the
# hire formula itself requests for the same backlog, which should leave
# more cash free (fewer Fibonacci hire-cost hands paid for). Risk: fewer
# hands per unit of crop backlog could mean more neglect/weeds/dead
# tiles -- exactly the failure mode iter C and v9.6 hit, just from a
# different angle. Isolated change: HAND_WORK_DIVISOR 5->7, applied to
# both expansion gates and the hire-target formula (one lever, all three
# call sites use the same constant now instead of a repeated literal).
#
# RESULT -- TESTED, REJECTED. Gate vs v9.5, both-seats, 15 seeds (same
# set as v9.6): v9.5 wins 9/15, +$3,354/game (paired-sum/2), t=+2.22,
# 95% CI [+$396,+$6,311] -- entirely against this build. Vs
# opp_scenario_v14 (5-seed smoke, both-seats): -$51,188/game,
# statistically identical to v9.5's own -$51.7k baseline -- ZERO gap
# compression, not worth a full 15-seed confirmation. So the predicted
# capital-savings mechanism (fewer hands hired per unit backlog -> more
# free cash -> gate loosens further downstream) either didn't
# materialize or was outweighed by thinner crop coverage per hand; net
# effect is negative on both baselines. NOT submitted.
# =====================================================================
...
HAND_TARGET_MIN = 4
HAND_TARGET_MAX = 18
HAND_WORK_DIVISOR = 7  # v9.7: was 5. Opponents/opp_scenario_v14.py's own
                        # _target_hands() divides its due_jobs/risk_load
                        # backlog by 7, not 5 -- meaning it tolerates more
                        # backlog per hand before asking for another hire.
                        # Applied here to both the hand-capacity feasibility
                        # gate (_animal_expansion_feasible/_sheep_expansion_
                        # feasible) and the hire-target formula itself, so
                        # this is one lever, not two: fewer hands requested
                        # per unit of crop/animal backlog, which (a) loosens
                        # the HAND_CAPACITY gate that v9.6's diagnosis found
                        # binding from day 13 on (projected_work ~111-121 >
                        # 5*18=90 -- under /7 the ceiling is 7*18=126), and
                        # (b) should leave more cash for the SAME backlog
                        # since the daily Fibonacci hire cost is paid on
                        # fewer hands. Trade-off being tested: does the
                        # extra headroom exceed the cost of thinner crop
                        # coverage per hand.
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    ...
    if math.ceil(projected_work / HAND_WORK_DIVISOR) > HAND_TARGET_MAX:
        return False
...
def _sheep_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads, day=99):
    ...
    crop_work = neglect_work + len(view["empty"])
    projected_work = crop_work + (animal_work + 1) * ANIMAL_WORK_WEIGHT
    if math.ceil(projected_work / HAND_WORK_DIVISOR) > HAND_TARGET_MAX + SHEEP_HAND_HEADROOM:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if hour == 0 and day < 29:
        ...
        target = max(HAND_TARGET_MIN, min(HAND_TARGET_MAX, math.ceil(work / HAND_WORK_DIVISOR)))
