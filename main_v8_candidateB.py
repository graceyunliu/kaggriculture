...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["CARROT"]:
        want["CARROT"] = 4
    if day <= CUTOFF["WHEAT"]:
        # Candidate B: v7.9's flat 8 was tiny against a 635-unit pool (the
        # largest in the game) and WHEAT is last in PLANT_PRIORITY, so
        # this only fills tiles the premium crops don't claim first.
        want["WHEAT"] = min(space, 25)
