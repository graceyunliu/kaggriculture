...
# =====================================================================
# MAIN V9.8 -- move the hiring block ahead of the seed pipeline in
# economy(), on top of v9.5. Base: main_v9.5_statehygiene.py.
#
# Root cause found by reading economy() end to end (v9.6/v9.7's
# postmortems both pointed at "hiring is capital-constrained" without
# saying why): the local `budget` variable is decremented SEQUENTIALLY
# in append order -- LAND, then ANIMAL PURCHASE, then every SEED
# category (MELON/STRAWBERRY/TOMATO/CARROT), then FEED WHEAT -- and the
# HIRE block ran dead last, right before the final priority sort/
# truncate. That means every seed order ahead of it in the function had
# already decremented `budget`, even on turns where those seed orders
# don't survive the MAX_MARKET_ORDERS truncation -- hiring was gated on
# an artificially depleted number, not the real remaining cash. This is
# the same class of fix v9.2 iter D already made for animal purchases
# (moved ahead of the seed pipeline for the same reason, "herd capital
# gets first claim on early cash" -- see that header). Isolated change:
# cut the hiring block from its old location (after all sells) and
# paste it immediately after the animal-purchase block, before the seed
# pipeline. No other logic changed -- same gating (hour==0, day<29),
# same target formula, same Fibonacci cost loop.
#
# RESULT -- TESTED, PROVEN NO-OP. Byte-identical to v9.5 on all 5 smoke
# seeds (1,7,42,99,123), both-seats, every margin exactly 0 -- not
# worth a 15-seed confirmation. Vs opp_scenario_v14: -$51,182/game,
# statistically identical to v9.5's own baseline. Root cause of the
# no-op, found via instrumenting the hire block directly (HIREDEBUG
# probe, not kept in this file): `budget` never actually gated the
# hire loop in a way this reorder could change, because HIRE's own
# priority score (PRIORITY_HIGH_HIRE=700) already outranks every seed/
# land/feed order (max ~690) regardless of append order -- Python's
# sort-by-score is order-independent except for exact ties, and none
# occurred. So the true bottleneck is downstream of `budget` entirely:
# see v9.9 for the real mechanism (order-slot truncation, not capital).
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # alongside crops so the hand pool scales with the fleet too. v9.8:
    # moved here (right after animal purchase, before the seed pipeline)
    # -- see file header. Was previously the LAST block before the final
    # sort/truncate, meaning every seed category below (MELON/STRAWBERRY/
    # TOMATO/CARROT) and the feed-wheat buy had already decremented the
    # local `budget` tracker even on turns where those seed orders don't
    # survive the MAX_MARKET_ORDERS truncation -- hiring was seeing an
    # artificially depleted budget, not real remaining cash.
    ...
    # v9.8: hiring block moved up (right after animal purchase) -- see
    # file header and the block above, right after the animal-purchase
    # section.
