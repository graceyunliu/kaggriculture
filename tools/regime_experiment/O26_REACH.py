...
MAX_ANIMALS = 20
MAX_SHEEP = 14  # legacy constant kept for reference; behavior now driven by KNOBS['max_sheep']
BINDING_LOG = []  # (day, species, reason, values) -- reachability instrumentation only

def _log_binding(day, species, reason, **vals):
    BINDING_LOG.append({"day": day, "species": species, "reason": reason, **vals})
...
def economy(obs, v, pending_drop=None):
    ...
    if day == 0 and n_total == 0:
        ...
        if KNOBS["geese"] > 0 and my_counts["GOOSE"] < KNOBS["geese"]:
            if day > KNOBS["geese_day_limit"]:
                _log_binding(day, "GOOSE", "day_limit", geese_day_limit=KNOBS["geese_day_limit"])
            else:
                k_cash = int(free // ANIMALS["GOOSE"])
                k = min(KNOBS["geese"] - my_counts["GOOSE"], k_cash)
                if k <= 0:
                    _log_binding(day, "GOOSE", "cash", free=free, k_cash=k_cash)
                if k > 0:
        ...
        for sp in ("SHEEP", "COW"):
            ...
            if sp == "SHEEP":
                ...
                if yarn == 0:
                    ...
                    room = max(room, 2)
                room = min(room, KNOBS["max_sheep"] - my_counts[sp])
            else:
                if _instances(obs, "MILK") == 0 and day >= 9:
                    room = min(room, 4 - my_counts[sp])
                room = min(room, KNOBS["max_cow"] - my_counts[sp])
            land_room = empty_count - KNOBS["land_reserve"]
            cash_room = int(free // ANIMALS[sp])
            cap_room = KNOBS["max_animals"] - n_total
            terms = {"hardcode_6": 6, "demand_room": room, "max_animals": cap_room,
                     "cash": cash_room, "land_reserve": land_room}
            k = min(terms.values())
            if k <= 0:
                binder = min(terms, key=lambda t: terms[t])
                _log_binding(day, sp, "purchase_gate:" + binder, **terms)
            while k > 0 and _load_model(v, seeds_on_hand, n_total + k, pending_place + k, day) > MAX_HANDS:
                k -= 1
                if k <= 0:
                    _log_binding(day, sp, "labor_load_model")
            k_before_cashflow = k
            while k > 0:
                ...
                if cash_after >= need_tomorrow:
                    break
                k -= 1
            if k <= 0 and k_before_cashflow > 0:
                _log_binding(day, sp, "tomorrow_cashflow")
            if k > 0:
