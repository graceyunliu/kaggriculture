...
def economy(obs, v, pending_drop=None):
    ...
    if True:
        ...
        elif prices.get("WHEAT", 0) >= (KNOBS["wheat_sell_price"] if day < 27 else 30) or \
                (KNOBS["wheat_sell_override"] and shed_load >= 90):
    ...
    if hour != capital_hour and not (KNOBS["capital_hour2"] >= 0 and hour == KNOBS["capital_hour2"] and day >= 1):
        if hour == 0:
            ...
            if KNOBS["wheat_buy_gate"]:
                feed_need = min(feed_need, max(0, 90 - shed_load))
