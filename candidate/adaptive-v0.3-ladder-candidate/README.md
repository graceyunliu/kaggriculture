# adaptive-v0.3-ladder-candidate

Adaptive v0.3: one minimal, evidence-motivated controller change on top of
the audited/certified v0.2 mechanism. v0.2's frozen learner state has several
regimes where the B_ONE_PER_ORDER arm's empirical mean rests on 1-4 real games;
v0.2's frozen (ladder) selection is plain greedy on those noisy means, so it can
confidently deviate from O42 in states where the evidence does not actually
support that deviation. v0.3 adds MIN_N_TO_TRUST_B=10: frozen-mode selection only
picks B when that regime has at least 10 B observations in the frozen learner
state, otherwise it falls back to A_FULL_O42 (O42's own behavior). The learner
state itself, reward window, regime partition, and training-time UCB1 behavior
are all byte-identical/unchanged from v0.2. See provenance_manifest.json's
`v0_3_change` block for the full before/after per-regime comparison. v0.2 is
left intact at candidate/adaptive-v0.2-ladder-candidate (repackaged for reference
under this project's candidate/ directory during this iteration; hash-verified
identical to the version documented in
artifacts/ladder_adaptive_v0.2_diagnostic_2026-09-13/).

## What is in this package

| File | Role | SHA256 |
|---|---|---|
| `controller/adaptive_slice_v0.py` | v0.3 controller: v0.2 controller + MIN_N_TO_TRUST_B guard | `1e7393a52664558ff5bc132121f30f39fceb7a41401ad40097e0f72874afbe4f` |
