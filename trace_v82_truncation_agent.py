# =====================================================================
# DIAGNOSTIC-ONLY instrumented copy of main_v8.2.py, built for the
# architecture-doc section 5.4 stress test ("stress-test v7.11's own
# untested HIRE-priority risk"). Adds a TRUNC stderr print whenever a
# turn's priced_orders list exceeds MAX_MARKET_ORDERS=10 (i.e. truncation
# actually fires), reporting how many HIRE vs SELL orders got dropped and
# an estimated face value (qty * base price) of dropped SELL orders. Zero
# behavior change otherwise -- see trace_v82_truncation.py for the runner.
# =====================================================================
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # --- TRUNC TRACE instrumentation (not in the real champion file) ---
    if len(priced_orders) > MAX_MARKET_ORDERS:
        kept = priced_orders[:MAX_MARKET_ORDERS]
        dropped = priced_orders[MAX_MARKET_ORDERS:]
        hire_kept = sum(1 for sc, o in kept if o[0] == "HIRE")
        hire_dropped = sum(1 for sc, o in dropped if o[0] == "HIRE")
        dropped_by_type = {}
        dropped_sell_qty_value = 0.0
        for sc, o in dropped:
            dropped_by_type[o[0]] = dropped_by_type.get(o[0], 0) + 1
            if o[0] == "SELL":
                item, qty = o[1], o[2]
                base = MARKET_PARAMS.get(item, {}).get("base", 0)
                dropped_sell_qty_value += qty * base
        print(
            f"TRUNC day={day} hour={hour} total={len(priced_orders)} "
            f"dropped={len(dropped)} hire_kept={hire_kept} hire_dropped={hire_dropped} "
            f"dropped_by_type={dropped_by_type} "
            f"dropped_sell_est_value={dropped_sell_qty_value:.0f} "
            f"min_kept_score={kept[-1][0]:.0f} max_dropped_score={dropped[0][0]:.0f}",
            file=sys.stderr,
        )
