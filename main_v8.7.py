...
# =====================================================================
# MAIN V8.7 -- joint cross-tier OPTIMAL task assignment, ISOLATED. Base:
# main_v8.3.py (byte-identical when OPTIMAL_TASK_ASSIGNMENT=False).
#
# --- The gap this targets ---
#
# main_v8.py's assign_tasks() (still present here, unchanged, and still
# what runs when this build's flag is off) fixed the movement/work ratio
# a lot (15/15 vs v7.11, +$10,238/game) by matching a whole tier's idle
# units to that tier's tasks jointly, instead of one unit at a time in
# roster order. But it still processes tiers strictly in priority order,
# one at a time: it fully finalizes who does urgent_water, THEN looks at
# who's left for harvest, THEN water, etc. A unit already assigned to an
# early tier is never reconsidered, even if a small trade would leave
# the WHOLE turn better off -- e.g. unit A (near both an urgent task and
# a harvest task) gets sent to the urgent task because that tier goes
# first, even when unit B (a little farther from the urgent task, but
# standing right on the harvest tile) could have covered the urgent task
# almost as well, freeing A for a walk-free harvest instead. This is a
# known, named limitation, not a new theory -- kaggriculture-v8-
# architecture.md and project memory flagged it explicitly: "tier-by-
# tier consumption of unassigned units could itself leave a later tier's
# matching worse... not modeled here."
#
# --- The fix ---
#
# Same inputs/outputs as assign_tasks(), same Pass 1 (units already
# standing exactly on a pending task claim it for free, tier by tier --
# zero-cost regardless of priority, nothing to trade off there). Pass 2
# is replaced: instead of matching each tier's leftover units greedily
# and moving on, ALL remaining tiers' tasks go into ONE combined pool,
# each tagged with a cost of (tier's rank in task_order) * TIER_OFFSET
# + Manhattan distance, and a single true minimum-total-cost matching
# (Hungarian algorithm / shortest augmenting paths with potentials,
# O(n^2 * m) -- pure Python, no new dependency) is solved across every
# unit and every task at once.
#
# TIER_OFFSET (100) exceeds the largest possible on-board distance (a
# 10x10 board's max Manhattan distance is 18), so the optimal solution
# NEVER trades away coverage of a higher-tier task to save distance
# elsewhere -- task_order's priority guarantee is preserved exactly,
# proven by construction, not just tested for. What changes is that,
# subject to that guarantee, the matching now finds the true minimum
# total walking distance jointly across every tier, which is a strict
# superset of what tier-by-tier greedy can find (greedy is one specific,
# usually-suboptimal solution within the same feasible region).
#
# --- Correctness validation (done BEFORE wiring into the agent) ---
#
# The Hungarian implementation (_min_cost_assignment()) was checked
# against brute-force-optimal on 2000 random small matrices (sizes up to
# 5 workers x 8 jobs) -- 2000/2000 exact matches on total cost, plus
# edge cases (0 workers, 1 worker) and a realistic-scale timing check
# (13 workers x 100 jobs, <1ms). Not shipped on faith in a from-memory
# algorithm -- verified independently first, since a silent bug here
# would be much harder to notice than an obviously-wrong build.
#
# --- Why padding is needed ---
#
# The algorithm requires at least as many columns (jobs) as rows
# (workers). Whenever there are fewer real tasks left than unassigned
# units (common -- most turns don't have a task for every idle unit),
# each unit gets its own dummy "idle" column appended, all priced far
# above any real task (IDLE_COST=10**6, versus a max real cost around
# 400+18) so idling is always dominated by any real task and only
# chosen when there genuinely isn't one left to assign.
#
# --- What this does NOT change ---
#
# Pass 1, task_order()'s tier ordering and weed-priority escalation, the
# reserved-crop-hand split, the animal pipeline, economy() -- all
# untouched. Only Pass 2's algorithm changes, and only when
# OPTIMAL_TASK_ASSIGNMENT=True; assign_tasks() itself is left completely
# intact in this file for the flag-off no-op path.
#
# --- Test protocol ---
#   1. No-op (REQUIRED FIRST): OPTIMAL_TASK_ASSIGNMENT=False vs
#      main_v8.3.py, --both-seats, must be exactly 0.0 on every seed.
#   2. Real test: True vs main_v8.3.py, --both-seats, same 15-seed set
#      as every prior v8.x round.
#   3. Mechanism check: movement/work unit-turn ratio, to confirm this
#      is actually a further improvement on the same axis v8's
#      assign_tasks() already validated, not a different effect.
#   4. Smoke tests: starter/pass 720-turn, starter 48-turn truncated,
#      all DONE/DONE.
# =====================================================================
#
# =====================================================================
...
# v8.7: only meaningful when GLOBAL_TASK_ASSIGNMENT=True. False keeps
# assign_tasks()'s tier-by-tier greedy Pass 2 (required no-op check vs
# main_v8.3.py); True routes through assign_tasks_optimal() instead --
# see file header.
OPTIMAL_TASK_ASSIGNMENT = True
TASK_ASSIGNMENT_TIER_OFFSET = 100   # > any possible on-board Manhattan
                                      # distance (10x10 board, max 18) --
                                      # guarantees tier priority always
                                      # dominates raw distance.
TASK_ASSIGNMENT_IDLE_COST = 10 ** 6  # cost of a dummy "stay idle" column,
                                       # far above any real tier+distance
                                       # combination (max ~400+18).
...
def _min_cost_assignment(cost):
    """v8.7: minimum-cost matching of every row (worker) to some column
    (job), via the Hungarian algorithm (shortest augmenting paths with
    potentials, O(n^2 * m)). Requires len(cost) <= len(cost[0]) (callers
    pad with dummy columns first). Returns a list `match` of length
    len(cost) where match[i] is the assigned column index for row i.
    Validated against brute-force-optimal on 2000 random small cases
    before use here -- see file header."""
    n = len(cost)
    if n == 0:
        return []
    m = len(cost[0])
...
def assign_tasks_optimal(unit_positions, tasks, seeds, day, task_order):
    """v8.7: same inputs/outputs/Pass-1 as assign_tasks() -- see that
    function's docstring for Pass 1, unchanged here. Pass 2 replaces
    tier-by-tier greedy matching with one true minimum-total-cost
    bipartite matching across every remaining tier's tasks and every
    still-unassigned unit at once (Hungarian algorithm). Each task's
    cost is (tier's rank in task_order) * TASK_ASSIGNMENT_TIER_OFFSET +
    Manhattan distance -- the huge tier offset guarantees the optimal
    solution never leaves a higher-tier task uncovered to save distance
    on a lower one (task_order's priority guarantee is preserved by
    construction), while still finding genuine cross-tier trades that
    tier-by-tier greedy structurally cannot see. See file header."""
    n = len(unit_positions)
    assigned_tile = [None] * n
    assigned_key = [None] * n
    unassigned = set(range(n))

    # Pass 1: identical to assign_tasks() -- standing-on-task claims,
    # tier by tier, free (zero distance) regardless of priority, so
    # there's no cross-tier trade to consider for these.
    for key in task_order:
        if not unassigned:
            break
        pool = tasks.get(key) or []
    ...
    # Pass 2: v8.7 -- one joint optimal matching across every remaining
    # tier's tasks, instead of tier-by-tier greedy.
    if unassigned:
        rows = sorted(unassigned)
        combined = []   # (tile, key) for every remaining task, any tier
        offsets = []    # matching tier_offset per combined entry
        for rank, key in enumerate(task_order):
            for t in (tasks.get(key) or []):
                combined.append((t, key))
                offsets.append(rank * TASK_ASSIGNMENT_TIER_OFFSET)
        if combined:
            cost = []
            for i in rows:
                pos = unit_positions[i]
                cost.append([offsets[c] + _dist(pos, combined[c][0]) for c in range(len(combined))])
            # Pad with one dummy "idle" column per unit -- guarantees a
            # feasible complete matching even when fewer real tasks
            # remain than unassigned units (the common case).
            for row in cost:
                row.extend([TASK_ASSIGNMENT_IDLE_COST] * len(rows))
            match = _min_cost_assignment(cost)
            used_tiles = {}
            for row_idx, col in zip(rows, match):
                if col < len(combined):
                    tile, key = combined[col]
                    assigned_tile[row_idx] = tile
                    assigned_key[row_idx] = key
                    used_tiles.setdefault(key, set()).add(tile)
            for key, tiles in used_tiles.items():
                tasks[key] = [t for t in tasks[key] if t not in tiles]

    # Convert assignments to ops -- identical to assign_tasks().
    ops = [None] * n
    for i, pos in enumerate(unit_positions):
        tile, key = assigned_tile[i], assigned_key[i]
        if tile is None:
...
def _agent(obs):
    ...
    if GLOBAL_TASK_ASSIGNMENT and (farmer_needs_crop or hand_needs_crop):
        ...
        # v8.7: OPTIMAL_TASK_ASSIGNMENT=False must call assign_tasks()
        # unchanged (required no-op check vs main_v8.3.py) -- see file header.
        assigner = assign_tasks_optimal if OPTIMAL_TASK_ASSIGNMENT else assign_tasks
        crop_ops = assigner(crop_unit_positions, tasks, seeds, day, task_order)
        k = 0
