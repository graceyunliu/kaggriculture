...
# =====================================================================
# MAIN V8.10 -- ChatGPT/GLM's full 3-part "rebalance the sheep gate"
# recipe, bundled and tested together, ISOLATED. Base: main_v8.3.py (the
# validated champion -- NOT main_v8.8.py's rejected crop-share
# controller or main_v8.9.py's rejected marginal-return scorer, see
# graveyard).
#
# --- Why (Grace's explicit request, after v8.9's close-but-rejected
#     result) ---
#
# The external ChatGPT/GLM analysis's "Fix 4: rebalance the sheep gate"
# proposed three changes together: (1) lower
# EXPANSION_MIN_CROP_TILES_PER_QUAD from 8 to 4-5, (2) let SHEEP compete
# with COW via alternation instead of only firing as a fallback, (3)
# move SHEEP's last_day from 22 to 18. Grace asked whether v8.9 already
# tested this -- it didn't: v8.9 only addressed (2), and via a different
# mechanism (marginal $/tile/day scoring, not alternation). Components
# (1) and (3) have each ALREADY been tried in isolation and rejected:
# main_v8.5.py (added a SHEEP-only crop-space headroom relaxation, a
# narrower version of (1)) lost -$1,358/game with high variance;
# main_v8.4.py (moved SHEEP's last_day to 17 from real breakeven math,
# closely matching (3)) was also rejected. This build tests the literal
# three-part recipe AS A BUNDLE, since testing all three together is a
# genuinely different experiment from any of the isolated pieces --
# interaction effects (e.g. an earlier cutoff mattering less once
# alternation buys sheep earlier and more often; a looser crop-space
# floor mattering less once alternation caps how often sheep is tried)
# were never tested.
#
# --- The three changes ---
#
# 1. EXPANSION_MIN_CROP_TILES_PER_QUAD: 8 -> 5 (within ChatGPT's
#    suggested 4-5 range; picked the less aggressive end to reduce the
#    crop-starvation risk v8.8's graveyard entry already surfaced with a
#    much bigger land reallocation). This is a GLOBAL constant used by
#    BOTH _animal_expansion_feasible() (COW) and
#    _sheep_expansion_feasible() (SHEEP) -- unlike v8.5's SHEEP-only
#    headroom offset, this loosens the crop-land reserve for both
#    species equally, exactly as ChatGPT specified.
# 2. Species selection: replaced the "COW first, SHEEP only as a
#    fallback" if/elif with literal alternation -- "buy sheep if the
#    last purchase was a cow (and vice versa)" (ChatGPT's own more
#    concrete second option, chosen over a 50/50 coin flip for
#    determinism/reproducibility). Tracks the most recently APPENDED
#    species in animal_plans (chronological purchase order) as
#    "last_species_bought". If both gates pass and the last purchase was
#    COW (or there is no purchase yet), SHEEP is preferred (falling back
#    to COW if SHEEP's own gate fails); if the last purchase was SHEEP,
#    COW is preferred (falling back to SHEEP if COW's gate fails). If
#    only one gate passes, that species is bought regardless of
#    alternation (same safety behavior as v8.3/v8.9).
# 3. SHEEP's last_day: 22 -> 18, exactly as specified (line reference in
#    the original recipe assumed v8.3's own line numbers).
#
# --- What this deliberately does NOT do ---
#
# No marginal-return scoring (that was v8.9, already tested separately).
# No crop-share/PLANT_PRIORITY changes (that was v8.8, already
# rejected). No GOOSE, no fertilizer-application action.
#
...
ANIMAL_SPECS = {
    ...
    # v8.10: last_day 22 -> 18 (ChatGPT recipe part 3) -- a sheep placed
    # day 18 gets first_yield_day=6/interval=3 real production ticks at
    # days 24/27/30 vs a day-22 purchase's much tighter 1-2 tick window.
    "SHEEP": {"cost": 500, "min_money": 800, "start_day": 3,  "last_day": 18},
}
...
EXPANSION_MIN_CROP_TILES_PER_QUAD = 5   # v8.10: 8 -> 5 (ChatGPT recipe part
                                          # 1, less-aggressive end of their
                                          # suggested 4-5 range) -- crop-land
                                          # reserve, scales with
                                          # unlocked quadrants
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if not in_progress:
        ...
        if target_plan is None:
            # v8.10: ChatGPT recipe part 2 -- alternation instead of a
            # cow-first fallback. Both gates are checked unchanged; when
            # BOTH pass, prefer whichever species was NOT the last one
            # bought (falling back to the other species if the preferred
            # one's own gate fails this turn -- alternation never
            # overrides a hard feasibility no). When only one gate
            # passes, that species is bought regardless of alternation
            # (same safety behavior as v8.3).
            ...
            if cow_ok and sheep_ok:
                last_species = animal_plans[-1]["species"] if animal_plans else None
                species = "COW" if last_species == "SHEEP" else "SHEEP"
            elif cow_ok:
                species = "COW"
            elif sheep_ok:
                species = "SHEEP"
