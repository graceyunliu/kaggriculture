# TURN EFFICIENCY AGENT 02: melon_batch_farming
# Base: main_v8.3.py
# Objective: Concentrate labor into infrequent, high-value batch harvests.
# Isolated policy changes: Melon-first portfolio; sharply reduce ongoing and short-cycle crops.
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if day <= CUTOFF["MELON"] and budget > 700:
        melon_target = min(space, 14 if ph == 0 else 24)
    ...
    if day <= CUTOFF["STRAWBERRY"] and budget > 900:
        ...
        want["STRAWBERRY"] = min(space, 4 if ph == 0 else 7)
