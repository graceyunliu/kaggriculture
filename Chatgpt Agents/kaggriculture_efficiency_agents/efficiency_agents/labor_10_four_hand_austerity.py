# LABOR EFFICIENCY AGENT 10: four_hand_austerity
# Base: main_v8.3.py
# Objective: Measure the extreme output-per-worker frontier with a four-hand ceiling.
# Isolated policy changes: Hand target 2–4, no sheep, no care, no fertilizer collection, delayed land.
...
ANIMAL_SPECS = {
    ...
    "COW":   {"cost": 400, "min_money": 700, "start_day": 0,  "last_day": 10},
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if hour == 0 and day < 29:
        ...
        target = max(HAND_TARGET_MIN, min(HAND_TARGET_MAX, math.ceil(work / 9)))
