# BLENDED EFFICIENCY AGENT 20: eight_austerity_balanced
# Objective: jointly improve turn, land, and labor efficiency.
# Policy: 8 hands, strict compactness, reduced optional work.
# Base: main_v8.3.py
...
ANIMAL_SPECS = {
    ...
    "COW":   {"cost": 400, "min_money": 700, "start_day": 0,  "last_day": 12},
    "SHEEP": {"cost": 500, "min_money": 800, "start_day": 3,  "last_day": 12},
}
...
SHEEP_ENABLED = True
SHEEP_HAND_HEADROOM = 1   # extra hand-target ceiling allowed specifically
...
ANIMAL_WORK_WEIGHT = 1               # pasture visit weight vs. one crop tile
...
PROTECTED_FRACTION_LOW = 0.58
PROTECTED_FRACTION_HIGH = 0.72
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    if hour == 0 and day < 29:
        ...
        target = max(HAND_TARGET_MIN, min(HAND_TARGET_MAX, math.ceil(work / 8)))
...
def animal_maintenance_action(pos, view, shed, carry, day, hour, exclude=(), critical_only=False):
    ...
    needs_care = (not critical_only) and (day % 2 == 0) and (not tile.get("cared_today", True))
