# strategy_18_delayed_sheep_carrot.py
# Strategy #18: Delayed-start sheep (carrot bootstrap) — $169.00/tile/day
# Days 0–3 and 3–6: two carrot cycles for early cash.
# Day 6: BUILD_PASTURE, BUY_ANIMAL SHEEP, PLACE; feed + care every day from then on.
#
# This agent:
# - runs two quick carrot cycles early,
# - then commits to a single sheep from day 6 onward,
# - focuses on daily FEED + CARE for the sheep, minimal other activity.
...
ANIMAL_SPECS = {
    ...
    "SHEEP": {"cost": 500, "min_money": 800, "start_day": 6, "last_day": 22},  # Sheep starts day 6
...
PLANT_PRIORITY = ["CARROT", "MELON", "STRAWBERRY", "TOMATO", "WHEAT"]  # Carrot first for bootstrap
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands, quads):
    # Allow exactly one sheep, starting day 6.
    if n_animals >= 1:
        return False
...
def economy(obs, me, opp, view, seeds, day, hour, timing_engine, animal_plans):
    ...
    # Early carrot bootstrap: prioritize carrot seeds days 0–5.
    ph = phase(day)
    space = len(view["empty"]) + 2
    want = {}
    if day <= 5:
        want["CARROT"] = min(space, 6)
    ...
    # Animal fleet: one sheep starting day 6.
    ...
    if not in_progress:
        ...
        if target_plan is None:
            if day >= 6 and _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"]), quads):
