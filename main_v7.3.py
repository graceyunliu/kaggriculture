...
EXPANSION_MIN_CROP_TILES = 8        # always leave at least this many tiles
                                     # unreserved for crops (melon is the
                                     # dominant profit driver) after
                                     # reserving a tile for one more pasture
...
PLANT_PRIORITY = ["MELON", "STRAWBERRY", "WHEAT", "CARROT"]
...
def _animal_expansion_feasible(obs, view, budget, shed, n_animals, n_hands):
    ...
    fixed FLEET_TARGET count (see file header). All five must hold:
      ...
      5. Crop space: reserving one more near-shed tile for a pasture must
         ...
         other mechanism from the original 6-animal collapse.
    """
    ...
    if available_for_crops < EXPANSION_MIN_CROP_TILES:
        return False
        ...
        if target_plan is None:
            for species in ANIMAL_SPECIES_ORDER:
                if species == "SHEEP" and not SHEEP_ENABLED:
                    continue
                if _animal_expansion_feasible(obs, view, budget, shed, n_animals, len(me["hands"])):
