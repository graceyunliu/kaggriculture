...
# =====================================================================
# MAIN V8.4 -- SHEEP's own last_day, set from real engine breakeven math,
# ISOLATED. Base: main_v8.3.py, byte-identical except for one field in
# ANIMAL_SPECS["SHEEP"].
#
# --- Why COW's last_day=22 was never actually validated for SHEEP ---
#
# main_v8.3.py's SHEEP entry inherited last_day=22 from COW's own entry
# unchanged -- it was never derived from SHEEP's own economics. Real
# engine numbers (vendor/kaggle_environments_engine/kaggriculture.py,
# ANIMALS["SHEEP"]): cost=500, first_yield_day=6, interval=3, no
# lifespan cap (animals yield every `interval` days indefinitely once
# placed and fed, unlike ongoing/non-ongoing crops -- confirmed reading
# _daily_refresh_animals, ln 782-810).
#
# A sheep placed on day P produces its Nth unit on day P + 6 + 3*(N-1).
# The match's last playable day is 29 (720 steps / 24h = 30 days,
# 0-indexed). Ticks by placement day (assuming ~0 setup lag, which is
# generous -- reserved sites are pre-claimed near the shed):
#   ticks(P) = floor((23 - P) / 3) + 1   for P <= 23, else 0
#
#   P <= 17: 3+ ticks -> >= 3 * $200 (engine base price) = $600+,
#            covers the $500 cost even at the unrealized base price.
#   P 18-20: 2 ticks -> ~$400-490 at base-to-observed-real prices
#            (project's own real ladder data: WOOL realizes $239-291/
#            unit most games) -- marginal, roughly break-even, can go
#            either way on price alone.
#   P 21-23: 1 tick -> ~$200-290, a real loss against the $500 cost.
#   P >= 24: 0 ticks -- guaranteed pure -$500 loss, no revenue at all.
#
# --- The change ---
#
# ANIMAL_SPECS["SHEEP"]["last_day"]: 22 -> 17, the last day that still
# guarantees 3 ticks (safely profitable even at the unrealized base
# price, not just the marginal 2-tick case). COW's own entry, and every
# other field, is untouched.
#
# --- What this does and doesn't fix ---
#
# main_v8.3.py's own diagnosis (project memory) found the sheep gate's
# real bottleneck in most games -- including WINNING ones -- is crop-
# space (check 5, EXPANSION_MIN_CROP_TILES_PER_QUAD), not the day
# window: even seed 1 and seed 555 (day-12 openers) spent most of the
# match blocked on crop-space, only briefly passing all five checks at
# once. Three of v8.3's five ties (seeds 99/2024/2025) don't open the
# gate until day 27-28 -- past even the OLD last_day=22, so this change
# cannot rescue them; nothing about a later cutoff helps a gate that
# opens too late to begin with. This change is a real-numbers
# correctness fix (removing an unvalidated shared default, and
# specifically avoiding shipping a purchase in the 21-23 window that the
# math above says is a likely loser) -- not the fix for "buy earlier",
# which is a separate, not-yet-built experiment (relax crop-space for
# SHEEP's gate too, the same way v8.3 relaxed hand-capacity).
#
# --- Real risk, flagged not assumed ---
#
# Lowering the cutoff from 22 to 17 could remove some of v8.3's own
# wins if their real gate-open day falls in 18-23 (e.g. seed 3003 opened
# day 18 in v8.3 and won) -- the breakeven math is a floor-price
# estimate, not a guarantee those purchases were actually bad. This
# needs its own seeded --both-seats test against main_v8.3.py, not an
...
ANIMAL_SPECS = {
    ...
    # v8.4: decoupled from COW's 22 -- set to the last day that still
    # guarantees 3 production ticks (first_yield_day=6, interval=3, no
    # lifespan cap) before day-29 match end, i.e. safely profitable even
    # at the engine's unrealized $200 base WOOL price. See file header.
    "SHEEP": {"cost": 500, "min_money": 800, "start_day": 3,  "last_day": 17},
}
